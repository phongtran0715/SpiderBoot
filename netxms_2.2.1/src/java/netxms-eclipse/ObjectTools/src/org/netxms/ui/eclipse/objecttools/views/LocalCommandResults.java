...
public class LocalCommandResults extends AbstractCommandResults {
	...
	protected void createActions() {
		...
		actionTerminate = new Action(
				Messages.get().LocalCommandResults_Terminate,
				SharedIcons.TERMINATE) {
			@Override
			public void run() {
				synchronized (mutex) {
					if (running) {
		...
		actionTerminate
				.setActionDefinitionId("org.netxms.ui.eclipse.objecttools.commands.terminate_process"); //$NON-NLS-1$
		handlerService.activateHandler(actionTerminate.getActionDefinitionId(),
				new ActionHandler(actionTerminate));
	...
	public void runCommand(final String command) {
		...
		ConsoleJob job = new ConsoleJob(
				Messages.get().LocalCommandResults_JobTitle, this,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				try {
					...
					boolean isWindows = Platform.getOS().equals(
							Platform.OS_WIN32);
					while (true) {
						...
						// (like ping, tracert, etc.) generates output with
						// lines
					...
					Activator.logError(
							"Exception while running local command", e); //$NON-NLS-1$
				} finally {
					in.close();
					out.close();
				}
