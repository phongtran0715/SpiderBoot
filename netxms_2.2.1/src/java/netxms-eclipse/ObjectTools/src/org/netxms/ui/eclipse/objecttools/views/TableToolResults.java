...
public class TableToolResults extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		tool = ObjectToolsCache.getInstance()
				.findTool(Long.parseLong(parts[0]));
		if (tool == null)
			throw new PartInitException(
					Messages.get().TableToolResults_InvalidToolID);
		...
		if ((object == null)
			...
			throw new PartInitException(
					Messages.get().TableToolResults_InvalidObjectID);
	...
	public void refreshTable() {
		...
		new ConsoleJob(String.format(Messages.get().TableToolResults_JobTitle,
				tool.getName()), this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final Table table = session.executeTableTool(tool.getId(),
						nodeId);
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().TableToolResults_JobError,
						tool.getName());
			}
		}.start();
	}
	...
	private void updateViewer(final Table table) {
		if (!viewer.isInitialized()) {
			...
			WidgetHelper.restoreTableViewerSettings(viewer, Activator
					...
					"TableToolResults." + Long.toString(tool.getId())); //$NON-NLS-1$
