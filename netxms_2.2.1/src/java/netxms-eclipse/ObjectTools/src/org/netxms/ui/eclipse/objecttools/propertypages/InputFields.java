...
public class InputFields extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		viewer.setComparator(new ViewerComparator() {
			...
			public int compare(Viewer viewer, Object e1, Object e2) {
				return ((InputField) e1).getSequence()
						- ((InputField) e2).getSequence();
			}
		});
		setupTableColumns();
	...
	private void setupTableColumns() {
		...
		WidgetHelper.restoreColumnSettings(viewer.getTable(), Activator
				.getDefault().getDialogSettings(), "InputFieldsPropertyPage"); //$NON-NLS-1$
	}
	...
	private void addField() {
		InputField f = new InputField(
				"Field" + Integer.toString(fields.size() + 1)); //$NON-NLS-1$
	...
	private void editField() {
		...
		EditInputFieldDialog dlg = new EditInputFieldDialog(getShell(), false,
				(InputField) selection.getFirstElement());
		if (dlg.open() == Window.OK) {
