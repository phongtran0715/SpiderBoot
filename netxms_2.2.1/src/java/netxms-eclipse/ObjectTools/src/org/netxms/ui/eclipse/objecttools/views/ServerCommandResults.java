 ...
 */
public class ServerCommandResults extends AbstractCommandResults implements
		TextOutputListener, ISaveablePart2 {
	...
	protected void createActions() {
		...
		actionRestart = new Action(Messages.get().LocalCommandResults_Restart,
				SharedIcons.RESTART) {
			@Override
			public void run() {
	...
	public void executeCommand(final String command,
		...
		ConsoleJob job = new ConsoleJob(String.format(
				...
				session.getObjectName(nodeId)), null, Activator.PLUGIN_ID, null) {
			@Override
			...
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				try {
					session.executeServerCommand(nodeId, command, inputValues,
							true, ServerCommandResults.this, null);
	...
	private void stopCommand() {
		if (streamId > 0) {
			ConsoleJob job = new ConsoleJob("Stop server command for node: "
					+ session.getObjectName(nodeId), null, Activator.PLUGIN_ID,
					null) {
				@Override
				protected void runInternal(IProgressMonitor monitor)
						throws Exception {
				...
				@Override
				protected String getErrorMessage() {
					return "Failed to stop server command for node: "
							+ session.getObjectName(nodeId);
				}
			};
			job.start();
	...
	public int promptToSaveOnClose() {
		return MessageDialog.openConfirm(Display.getCurrent().getActiveShell(),
				"Stop command", "Do you wish to stop the command \""
						+ lastCommand + "\"? ") ? 0 : 2;
	}
}
