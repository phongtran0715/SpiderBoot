...
public class Columns extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		viewer = new TableViewer(dialogArea, SWT.BORDER | SWT.MULTI
				| SWT.FULL_SELECTION);
		...
		viewer.setLabelProvider(new ToolColumnLabelProvider(objectTool
				.getToolType() == ObjectTool.TYPE_TABLE_SNMP));
		setupTableColumns();
	...
	private void setupTableColumns() {
		...
		column = new TableColumn(viewer.getTable(), SWT.LEFT);
		column.setText(objectTool.getToolType() == ObjectTool.TYPE_TABLE_SNMP ? Messages
				.get().Columns_OID : Messages.get().Columns_Index);
		column.setWidth(200);

		viewer.getTable().setHeaderVisible(true);

		WidgetHelper.restoreColumnSettings(viewer.getTable(), Activator
				.getDefault().getDialogSettings(), "ColumnsPropertyPage"); //$NON-NLS-1$
	}
	...
	private void addColumn() {
		ObjectToolTableColumn tc = new ObjectToolTableColumn(
				Messages.get().Columns_DefName
						+ Integer.toString(columns.size() + 1));
		EditColumnDialog dlg = new EditColumnDialog(getShell(), true,
				objectTool.getToolType() == ObjectTool.TYPE_TABLE_SNMP, tc);
	...
	private void editColumn() {
		...
		EditColumnDialog dlg = new EditColumnDialog(getShell(), false,
				objectTool.getToolType() == ObjectTool.TYPE_TABLE_SNMP,
				(ObjectToolTableColumn) selection.getFirstElement());
		dlg.open();
