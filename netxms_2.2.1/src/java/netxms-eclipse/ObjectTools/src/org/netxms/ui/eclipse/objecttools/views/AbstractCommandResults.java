...
public abstract class AbstractCommandResults extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		try {
			...
			setPartName(object.getObjectName()
					+ " - " + ObjectToolsCache.getInstance().findTool(toolId).getDisplayName()); //$NON-NLS-1$
		} catch (Exception e) {
	...
	protected void createActions() {
		...
		actionClear = new Action(
				Messages.get().LocalCommandResults_ClearConsole,
		...
		actionClear
				.setActionDefinitionId("org.netxms.ui.eclipse.objecttools.commands.clear_output"); //$NON-NLS-1$
		handlerService.activateHandler(actionClear.getActionDefinitionId(),
				new ActionHandler(actionClear));

		actionScrollLock = new Action(
				Messages.get().LocalCommandResults_ScrollLock,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionCopy
				.setActionDefinitionId("org.netxms.ui.eclipse.objecttools.commands.copy"); //$NON-NLS-1$
		handlerService.activateHandler(actionCopy.getActionDefinitionId(),
				new ActionHandler(actionCopy));

		actionSelectAll = new Action(
				Messages.get().LocalCommandResults_SelectAll) {
			@Override
			public void run() {
				console.selectAll();
			}
		...
		actionSelectAll
				.setActionDefinitionId("org.netxms.ui.eclipse.objecttools.commands.select_all"); //$NON-NLS-1$
