...
public class Filter extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		ObjectAction action = (ObjectAction) getElement().getAdapter(
				ObjectAction.class);
		if (action instanceof ObjectTool)
			objectTool = (ObjectToolDetails) getElement().getAdapter(
		...
		checkMatchNodeOS
				.setSelection((filter.flags & ObjectMenuFilter.REQUIRES_NODE_OS_MATCH) != 0);
		...
		if (action instanceof ObjectTool
				&& action.getToolType() == ObjectTool.TYPE_LOCAL_COMMAND) {
			checkMatchWorkstationOS = new Button(dialogArea, SWT.CHECK);
			checkMatchWorkstationOS
					.setText("Workstation OS name should match this template(coma separated regular expression list)");
			checkMatchWorkstationOS
					.setSelection((filter.flags & ObjectMenuFilter.REQUIRES_WORKSTATION_OS_MATCH) != 0);
			checkMatchWorkstationOS
					.addSelectionListener(new SelectionListener() {
						@Override
						public void widgetSelected(SelectionEvent e) {
							textWorkstationOS
									.setEnabled(checkMatchWorkstationOS
											.getSelection());
		...
		checkMatchCustomAttributes
				.setText("The following custom attribute(s) should exist");
		checkMatchCustomAttributes
				.setSelection((filter.flags & ObjectMenuFilter.REQUIRES_CUSTOM_ATTRIBUTE_MATCH) != 0);
		checkMatchCustomAttributes
				.addSelectionListener(new SelectionListener() {
					@Override
					public void widgetSelected(SelectionEvent e) {
						textCustomAttributes
								.setEnabled(checkMatchCustomAttributes
										.getSelection());
		...
		textCustomAttributes.setEnabled(checkMatchCustomAttributes
				.getSelection());
	...
	protected void applyChanges(final boolean isApply) {
		...
		setFilter(textNodeOS.getText().trim(),
				ObjectMenuFilter.REQUIRES_NODE_OS_MATCH);

		if ((checkMatchWorkstationOS != null)
				&& checkMatchWorkstationOS.getSelection())
		...
		if (textWorkstationOS != null)
			setFilter(textWorkstationOS.getText().trim(),
					ObjectMenuFilter.REQUIRES_WORKSTATION_OS_MATCH);
		...
		setFilter(textTemplate.getText().trim(),
				ObjectMenuFilter.REQUIRES_TEMPLATE_MATCH);
		...
		setFilter(textCustomAttributes.getText().trim(),
				ObjectMenuFilter.REQUIRES_CUSTOM_ATTRIBUTE_MATCH);
	}
