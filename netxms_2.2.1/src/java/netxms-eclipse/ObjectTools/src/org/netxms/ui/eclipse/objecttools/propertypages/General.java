...
public class General extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		case ObjectTool.TYPE_FILE_DOWNLOAD:
			...
			maxFileSize
					.setLabel(Messages.get().General_LimitDownloadFileSizeLable);
			maxFileSize.setRange(0, 0x7FFFFFFF);
			try {
				maxFileSize.setSelection((parameters.length > 1) ? Integer
						.parseInt(parameters[1]) : 0);
			...
			{
				checkFollow
						.setSelection(parameters[2].equals("true") ? true : false); //$NON-NLS-1$
			}
			break;
		case ObjectTool.TYPE_TABLE_SNMP:
			...
			new Label(snmpOptGroup, SWT.NONE)
					.setText(Messages.get().General_UseAsIndex);

			radioIndexOID = new Button(snmpOptGroup, SWT.RADIO);
			radioIndexOID.setText(Messages.get().General_OIDSuffix);
			radioIndexOID
					.setSelection((objectTool.getFlags() & ObjectTool.SNMP_INDEXED_BY_VALUE) == 0);
		...
		checkConfirmation
				.setSelection((objectTool.getFlags() & ObjectTool.ASK_CONFIRMATION) != 0);
		...
		checkCommand
				.setSelection((objectTool.getFlags() & ObjectTool.SHOW_IN_COMMANDS) != 0);
		...
		checkDisable
				.setSelection((objectTool.getFlags() & ObjectTool.DISABLED) > 0);
	...
	private void createOutputGroup(Composite parent) {
		...
		checkOutput
				.setSelection((objectTool.getFlags() & ObjectTool.GENERATES_OUTPUT) != 0);
	}
	...
	private void selectIcon() {
		...
		dlg.setFilterNames(new String[] { Messages.get().General_ImageFiles,
				Messages.get().General_AllFiles });
		...
		try {
			...
			if ((image.getImageData().width <= 16)
				...
				MessageDialogHelper.openError(getShell(),
						Messages.get().General_Error,
						Messages.get().General_ImageTooLarge);
			}
		} catch (Exception e) {
			MessageDialogHelper.openError(
					getShell(),
					Messages.get().General_Error,
					String.format(Messages.get().General_CannotLoadImage,
	...
	protected void applyChanges(final boolean isApply) {
		...
		if (objectTool.getToolType() == ObjectTool.TYPE_TABLE_AGENT) {
			objectTool
					.setData(textData.getText()
							+ "\u007F" + textParameter.getText() + "\u007F" + textRegexp.getText()); //$NON-NLS-1$ //$NON-NLS-2$
		} else {
			if (objectTool.getToolType() == ObjectTool.TYPE_FILE_DOWNLOAD) {
				objectTool
						.setData(textData.getText()
								+ "\u007F" + maxFileSize.getSelection() + "\u007F" + checkFollow.getSelection()); //$NON-NLS-1$ //$NON-NLS-2$
			} else {
		...
		if (checkConfirmation.getSelection()) {
			objectTool.setFlags(objectTool.getFlags()
					| ObjectTool.ASK_CONFIRMATION);
		} else {
			objectTool.setFlags(objectTool.getFlags()
					& ~ObjectTool.ASK_CONFIRMATION);
		}
		...
		if (objectTool.getToolType() == ObjectTool.TYPE_TABLE_SNMP) {
			if (radioIndexValue.getSelection()) {
				objectTool.setFlags(objectTool.getFlags()
						| ObjectTool.SNMP_INDEXED_BY_VALUE);
			} else {
				objectTool.setFlags(objectTool.getFlags()
						& ~ObjectTool.SNMP_INDEXED_BY_VALUE);
			}
