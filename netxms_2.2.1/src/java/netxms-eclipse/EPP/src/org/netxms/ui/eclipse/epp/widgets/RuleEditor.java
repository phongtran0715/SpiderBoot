...
public class RuleEditor extends Composite {
	 ...
	 * @param rule
	 */
	public RuleEditor(Composite parent, EventProcessingPolicyRule rule,
			EventProcessingPolicyEditor editor) {
		...
		setBackground(SharedColors.getColor(
				SharedColors.RULE_EDITOR_RULE_BORDER, getDisplay()));
		...
		createPopupMenu(new Control[] { leftPanel, ruleNumberLabel, header,
				headerLabel });

		condition = new CGroup(mainArea, Messages.get().RuleEditor_Filter) {
			@Override
			protected Control createClientArea(Composite parent) {
				setBorderColor(SharedColors
						.getColor(SharedColors.RULE_EDITOR_CONDITION_BORDER,
								getDisplay()));
				setTitleColor(SharedColors.getColor(
						SharedColors.RULE_EDITOR_TITLE_TEXT, getDisplay()));
		...
		condition.addButton(new DashboardElementButton(
				Messages.get().RuleEditor_EditCondition, editor.getImageEdit(),
				editRuleCondition));
		...
		action = new CGroup(mainArea, Messages.get().RuleEditor_Action) {
			@Override
			protected Control createClientArea(Composite parent) {
				setBorderColor(SharedColors.getColor(
						SharedColors.RULE_EDITOR_ACTION_BORDER, getDisplay()));
				setTitleColor(SharedColors.getColor(
						SharedColors.RULE_EDITOR_TITLE_TEXT, getDisplay()));
		...
		action.addButton(new DashboardElementButton(
				Messages.get().RuleEditor_EditActions, editor.getImageEdit(),
				editRuleAction));
	...
	private void dragEnable() {
		...
		source.setTransfer(new Transfer[] { LocalSelectionTransfer
				.getTransfer() });
		// add a drop listener
	...
	private void createMainArea() {
		...
		mainArea.setBackground(SharedColors.getColor(
				SharedColors.RULE_EDITOR_BACKGROUND, getDisplay()));
		...
		leftPanel
				.setBackground(SharedColors.getColor(
						rule.isDisabled() ? SharedColors.RULE_EDITOR_DISABLED_TITLE_BACKGROUND
								: SharedColors.RULE_EDITOR_NORMAL_TITLE_BACKGROUND,
						getDisplay()));
		...
		ruleNumberLabel
				.setBackground(SharedColors.getColor(
						rule.isDisabled() ? SharedColors.RULE_EDITOR_DISABLED_TITLE_BACKGROUND
								: SharedColors.RULE_EDITOR_NORMAL_TITLE_BACKGROUND,
						getDisplay()));
		ruleNumberLabel.setForeground(SharedColors.getColor(
				SharedColors.RULE_EDITOR_TITLE_TEXT, getDisplay()));
	...
	private void createHeader() {
		...
		if (rule.isDisabled())
			headerLabel.setText(rule.getComments()
					+ Messages.get().RuleEditor_DisabledSuffix);
		else
			headerLabel.setText(rule.getComments());
		headerLabel
				.setBackground(SharedColors.getColor(
						rule.isDisabled() ? SharedColors.RULE_EDITOR_DISABLED_TITLE_BACKGROUND
								: SharedColors.RULE_EDITOR_NORMAL_TITLE_BACKGROUND,
						getDisplay()));
		headerLabel.setForeground(SharedColors.getColor(
				SharedColors.RULE_EDITOR_TITLE_TEXT, getDisplay()));
		...
		expandButton.setImage(collapsed ? editor.getImageExpand() : editor
				.getImageCollapse());
		expandButton
				.setToolTipText(collapsed ? Messages.get().RuleEditor_Tooltip_ExpandRule
	 ...
	 * 
	 * @param pageId
	 *            property page ID to be opened on double click
	 * @return
	 */
	 ...
	 * @return
	 */
	private Control createConditionControl(Composite parent,
			final EventProcessingPolicyRule rule) {
		Composite clientArea = new Composite(parent, SWT.NONE);

		clientArea.setBackground(SharedColors.getColor(
				SharedColors.RULE_EDITOR_BACKGROUND, getDisplay()));
		GridLayout layout = new GridLayout();
		...
		if (((rule.getSources().size() > 0) && rule.isSourceInverted())
				|| ((rule.getSources().size() == 0)
						&& (rule.getEvents().size() > 0) && rule
							.isEventsInverted()))
			createLabel(clientArea, 0, true, Messages.get().RuleEditor_IF_NOT,
					null);
		else
		...
		if (rule.getSources().size() > 0) {
			...
			addConditionGroupLabel(clientArea,
					Messages.get().RuleEditor_SourceIs, needAnd,
					rule.isSourceInverted(), listener);
		...
		if (rule.getEvents().size() > 0) {
			...
			addConditionGroupLabel(clientArea,
					Messages.get().RuleEditor_EventIs, needAnd,
					rule.isEventsInverted(), listener);

			List<EventTemplate> sortedEvents = new ArrayList<EventTemplate>(
					rule.getEvents().size());
			...
			for (EventTemplate e : sortedEvents) {
				...
				clabel.setImage(StatusDisplayInfo.getStatusImage(e
						.getSeverity()));
			}
			needAnd = true;
		}
		...
		if ((rule.getFlags() & EventProcessingPolicyRule.SEVERITY_ANY) != EventProcessingPolicyRule.SEVERITY_ANY) {
			...
			addConditionGroupLabel(clientArea,
					Messages.get().RuleEditor_SeverityIs, needAnd, false,
					listener);
		...
		if ((rule.getScript() != null) && !rule.getScript().isEmpty()) {
			...
			addConditionGroupLabel(clientArea,
					Messages.get().RuleEditor_ScriptIs, needAnd, false,
					listener);

			ScriptEditor scriptEditor = new ScriptEditor(clientArea,
					SWT.BORDER, SWT.NONE);
			GridData gd = new GridData();
	 ...
	 * @return
	 */
	private Label createLabel(Composite parent, int indent, boolean bold,
			String text, MouseListener mouseListener) {
		Label label = new Label(parent, SWT.NONE);
		label.setBackground(SharedColors.getColor(
				SharedColors.RULE_EDITOR_BACKGROUND, getDisplay()));
	 ...
	 *            group's title
	 * @param needAnd
	 *            true if AND clause have to be added
	 */
	private void addConditionGroupLabel(Composite parent, String title,
			boolean needAnd, boolean inverted, MouseListener mouseListener) {
		if (needAnd)
			createLabel(
					parent,
					0,
					true,
					inverted ? Messages.get().RuleEditor_AND_NOT : Messages
							.get().RuleEditor_AND, null);
	 ...
	 *            severity code
	 */
	private void addSeverityLabel(Composite parent, Severity severity,
	 ...
	 * @return
	 */
	private Control createActionControl(Composite parent,
			final EventProcessingPolicyRule rule) {
		Composite clientArea = new Composite(parent, SWT.NONE);
		...
		if ((rule.getFlags() & EventProcessingPolicyRule.GENERATE_ALARM) != 0) {
			...
			if (rule.getAlarmSeverity().compareTo(Severity.TERMINATE) < 0) {
				addActionGroupLabel(clientArea,
						Messages.get().RuleEditor_GenerateAlarm,
						editor.getImageAlarm(), listener);

				CLabel clabel = createCLabel(clientArea, 1, false);
				clabel.setImage(StatusDisplayInfo.getStatusImage(rule
						.getAlarmSeverity()));
				clabel.setText(rule.getAlarmMessage());
				clabel.addMouseListener(listener);

				if ((rule.getAlarmKey() != null)
						&& !rule.getAlarmKey().isEmpty()) {
					createLabel(clientArea, 1, false, String.format(
							Messages.get().RuleEditor_WithKey,
							rule.getAlarmKey()), null);
				}
			} else if (rule.getAlarmSeverity() == Severity.TERMINATE) {
				addActionGroupLabel(clientArea,
						Messages.get().RuleEditor_TerminateAlarms,
						editor.getImageTerminate(), listener);
				createLabel(
						clientArea,
						1,
						false,
						String.format(Messages.get().RuleEditor_WithKey,
								rule.getAlarmKey()), listener);
				if ((rule.getFlags() & EventProcessingPolicyRule.TERMINATE_BY_REGEXP) != 0)
					createLabel(clientArea, 1, false,
							Messages.get().RuleEditor_UserRegexpForTerminate,
							listener);
			} else if (rule.getAlarmSeverity() == Severity.RESOLVE) {
				addActionGroupLabel(clientArea,
						Messages.get().RuleEditor_ResolveAlarms,
						editor.getImageTerminate(), listener);
				createLabel(
						clientArea,
						1,
				...
				if ((rule.getFlags() & EventProcessingPolicyRule.TERMINATE_BY_REGEXP) != 0)
					createLabel(clientArea, 1, false,
							Messages.get().RuleEditor_UseRegexpForResolve,
							listener);
			}

			if (rule.getAlarmCategories().size() > 0) {
				createLabel(clientArea, 1, false, rule.getAlarmCategories()
						.size() == 1 ? "with category: " : "with categories: ",
						null);
				for (Long id : rule.getAlarmCategories()) {
					AlarmCategory category = editor.findAlarmCategoryById(id);
					if (category != null) {
						createLabel(clientArea, 2, false, category.getName(),
								null);
					}
				}
			}
			...
			if ((rule.getFlags() & EventProcessingPolicyRule.CREATE_TICKET) != 0) {
				createLabel(clientArea, 1, false, "creates helpdesk ticket",
						null);
			}
		}
		...
		if (rule.getPStorageSet().size() != 0
				|| rule.getPStorageDelete().size() != 0) {
			final MouseListener listener = createMouseListener("org.netxms.ui.eclipse.epp.propertypages.RulePStorage#20"); //$NON-NLS-1$
			addActionGroupLabel(clientArea,
					"Update persistent storage entries",
					editor.getImageSituation(), listener);

			if (rule.getPStorageSet().size() != 0) {
				createLabel(clientArea, 1, false,
						"Set Persistent storage values", listener); //$NON-NLS-1$ //$NON-NLS-2$
				for (Entry<String, String> e : rule.getPStorageSet().entrySet()) {
					createLabel(
							clientArea,
							2,
							false,
							e.getKey() + " = \"" + e.getValue() + "\"", listener); //$NON-NLS-1$ //$NON-NLS-2$
				}
			}

			if (rule.getPStorageDelete().size() != 0) {
				createLabel(clientArea, 1, false,
						"Delete persistent storage values", listener);
				List<String> pStorageList = rule.getPStorageDelete();
				for (int i = 0; i < pStorageList.size(); i++) {
					createLabel(clientArea, 2, false, pStorageList.get(i),
							listener);
				}
			}
		...
		if (rule.getActions().size() > 0) {
			...
			addActionGroupLabel(clientArea,
					Messages.get().RuleEditor_ExecuteActions,
					editor.getImageExecute(), listener);
		...
		if ((rule.getFlags() & EventProcessingPolicyRule.STOP_PROCESSING) != 0) {
			...
			addActionGroupLabel(clientArea,
					Messages.get().RuleEditor_StopProcessing,
					editor.getImageStop(), listener);
		}
	 ...
	 *            true if AND clause have to be added
	 */
	private void addActionGroupLabel(Composite parent, String title,
	 ...
	 * 
	 * @param collapsed
	 *            true to collapse widget, false to expand
	 * @param doLayout
	 *            if set to true, view layout will be updated
	 */
	...
	private static PropertyDialog createDialogOn(Shell shell,
			final String propertyPageId, Object element, String name) {
		...
		Iterator pages = pageManager.getElements(PreferenceManager.PRE_ORDER)
				.iterator();
		if (!pages.hasNext()) {
			MessageDialogHelper.openInformation(shell,
		...
		PlatformUI
				.getWorkbench()
				.getHelpSystem()
				.setHelp(propertyDialog.getShell(),
	...
	private void editRule(String pageId) {
		PropertyDialog dlg = createDialogOn(editor.getSite().getShell(),
				pageId, this, Messages.get().RuleEditor_Rule + ruleNumber);
		if (dlg != null) {
			modified = false;
			dlg.open();
			if (modified) {
	...
	public void setSelected(boolean selected) {
		...
		final Color color = SharedColors
				.getColor(
						selected ? SharedColors.RULE_EDITOR_SELECTED_TITLE_BACKGROUND
								: (rule.isDisabled() ? SharedColors.RULE_EDITOR_DISABLED_TITLE_BACKGROUND
										: SharedColors.RULE_EDITOR_NORMAL_TITLE_BACKGROUND),
						getDisplay());
	...
	private class RuleDropTargetListener implements DropTargetListener {
		...
		public void drop(DropTargetEvent event) {
			...
			Object object = ((IStructuredSelection) event.data)
					.getFirstElement();
