...
public class RuleAlarm extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		Composite alarmCreationSubgroup = new Composite(alarmCreationGroup,
				SWT.NONE);
		layout = new GridLayout();
		layout.numColumns = 2;
		...
		new Label(severityGroup, SWT.NONE)
				.setText(Messages.get().RuleAlarm_Severity);
		alarmSeverity = new ImageCombo(severityGroup, SWT.DROP_DOWN
				| SWT.READ_ONLY | SWT.BORDER);
		for (int i = 0; i < Severity.UNKNOWN.getValue(); i++)
			alarmSeverity.add(StatusDisplayInfo.getStatusImage(i),
					StatusDisplayInfo.getStatusText(i));
		alarmSeverity.add(StatusDisplayInfo.getStatusImage(Severity.UNKNOWN),
				Messages.get().RuleAlarm_FromEvent);
		...
		checkCreateHelpdeskTicket
				.setText("Create helpdesk ticket on alarm creation");
		checkCreateHelpdeskTicket
				.setSelection((rule.getFlags() & EventProcessingPolicyRule.CREATE_TICKET) != 0);
		gd = new GridData();
		...
		alarmKeyTerminate.setLabel((alarmAction == ALARM_TERMINATE) ? Messages
				.get().RuleAlarm_TerminateAll
				: Messages.get().RuleAlarm_ResolveAll);
		...
		checkTerminateWithRegexp
				.setText((alarmAction == ALARM_TERMINATE) ? Messages.get().RuleAlarm_UseRegexpForTerminate
						: Messages.get().RuleAlarm_UseRegexpForResolve);
		checkTerminateWithRegexp
				.setSelection((rule.getFlags() & EventProcessingPolicyRule.TERMINATE_BY_REGEXP) != 0);
	...
	private void changeAlarmAction(int newAction) {
		if (alarmAction != ALARM_NO_ACTION) {
			Composite group = (alarmAction == ALARM_CREATE) ? alarmCreationGroup
					: alarmTerminationGroup;
		...
		if (alarmAction != ALARM_NO_ACTION) {
			...
			alarmKeyTerminate
					.setLabel((alarmAction == ALARM_TERMINATE) ? Messages.get().RuleAlarm_TerminateAll
							: Messages.get().RuleAlarm_ResolveAll);
			checkTerminateWithRegexp
					.setText((alarmAction == ALARM_TERMINATE) ? Messages.get().RuleAlarm_UseRegexpForTerminate
							: Messages.get().RuleAlarm_UseRegexpForResolve);
	...
	private boolean doApply() {
		...
		case ALARM_CREATE:
			try {
				...
				if (t < 0) {
					MessageDialogHelper.openWarning(getShell(),
							Messages.get().RuleAlarm_Warning,
							Messages.get().RuleAlarm_WarningInvalidTimeout);
					return false;
				}
			...
			rule.setAlarmSeverity(Severity.getByValue(alarmSeverity
					.getSelectionIndex()));
			rule.setAlarmTimeoutEvent(timeoutEvent.getEventCode());
			rule.setAlarmCategories(alarmCategory.getCategoryId());
			rule.setFlags(rule.getFlags()
					| EventProcessingPolicyRule.GENERATE_ALARM);
			if (checkCreateHelpdeskTicket.getSelection())
				rule.setFlags(rule.getFlags()
						| EventProcessingPolicyRule.CREATE_TICKET);
			break;
		case ALARM_RESOLVE:
		case ALARM_TERMINATE:
			rule.setAlarmKey(alarmKeyTerminate.getText());
			rule.setAlarmSeverity((alarmAction == ALARM_TERMINATE) ? Severity.TERMINATE
					: Severity.RESOLVE);
			rule.setFlags(rule.getFlags()
					| EventProcessingPolicyRule.GENERATE_ALARM);
			if (checkTerminateWithRegexp.getSelection())
				rule.setFlags(rule.getFlags()
						| EventProcessingPolicyRule.TERMINATE_BY_REGEXP);
			else
				rule.setFlags(rule.getFlags()
						& ~EventProcessingPolicyRule.TERMINATE_BY_REGEXP);
			break;
