 ...
 * 
 */
public class EventProcessingPolicyEditor extends ViewPart implements
		ISaveablePart {
	...
	public void createPartControl(Composite parent) {
		...
		filterEnabled = settings
				.getBoolean("EventProcessingPolicyEditor.filterEnabled"); //$NON-NLS-1$
		...
		try {
			...
			Platform.getAdapterManager().loadAdapter(new ServerAction(0),
					"org.eclipse.ui.model.IWorkbenchAdapter"); //$NON-NLS-1$
		...
		imageStop = Activator
				.getImageDescriptor("icons/stop.png").createImage(); //$NON-NLS-1$
		imageAlarm = Activator
				.getImageDescriptor("icons/alarm.png").createImage(); //$NON-NLS-1$
		imageSituation = Activator
				.getImageDescriptor("icons/situation.gif").createImage(); //$NON-NLS-1$
		imageExecute = Activator
				.getImageDescriptor("icons/execute.png").createImage(); //$NON-NLS-1$
		imageTerminate = Activator
				.getImageDescriptor("icons/terminate.png").createImage(); //$NON-NLS-1$
		...
		if (Platform.getOS().equals(Platform.OS_WIN32)) {
			normalFont = new Font(parent.getDisplay(), "Verdana", 8, SWT.NORMAL); //$NON-NLS-1$
			boldFont = new Font(parent.getDisplay(), "Verdana", 8, SWT.BOLD); //$NON-NLS-1$
		} else {
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.epp.context.PolicyEditor"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionHorizontal = new Action(
				Messages.get().EventProcessingPolicyEditor_LayoutH,
		...
		actionHorizontal.setImageDescriptor(Activator
				.getImageDescriptor("icons/h_layout.gif")); //$NON-NLS-1$

		actionVertical = new Action(
				Messages.get().EventProcessingPolicyEditor_LayoutV,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
				verticalLayout = true;
		...
		actionVertical.setImageDescriptor(Activator
				.getImageDescriptor("icons/v_layout.gif")); //$NON-NLS-1$
		...
		actionCollapseAll = new Action(
				Messages.get().EventProcessingPolicyEditor_CollapseAll) {
			@Override
			public void run() {
		...
		actionExpandAll = new Action(
				Messages.get().EventProcessingPolicyEditor_ExpandAll) {
			@Override
			public void run() {
		...
		actionDelete = new Action(
				Messages.get().EventProcessingPolicyEditor_Delete) {
			@Override
			public void run() {
				deleteSelectedRules();
			}
		...
		actionInsertBefore = new Action(
				Messages.get().EventProcessingPolicyEditor_InsertBefore) {
			@Override
			public void run() {
		...
		actionInsertAfter = new Action(
				Messages.get().EventProcessingPolicyEditor_InsertAfter) {
			@Override
			public void run() {
		...
		actionPaste = new Action(
				Messages.get().EventProcessingPolicyEditor_Paste) {
			@Override
			public void run() {
				pasteRules();
			}
		...
		actionEnableRule = new Action(
				Messages.get().EventProcessingPolicyEditor_Enable) {
			@Override
			public void run() {
				enableRules(true);
			}
		...
		actionDisableRule = new Action(
				Messages.get().EventProcessingPolicyEditor_Disable) {
			@Override
			public void run() {
				enableRules(false);
			}
		...
		actionShowFilter = new Action(
				Messages.get().EventProcessingPolicyEditor_ShowFilter,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.epp.commands.show_rule_filter"); //$NON-NLS-1$
	...
	private void openEventProcessingPolicy() {
		ConsoleJob job = new ConsoleJob(
				Messages.get().EventProcessingPolicyEditor_OpenJob_Title, this,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				synchronized (EventProcessingPolicyEditor.this.actions) {
					for (ServerAction a : actions) {
						EventProcessingPolicyEditor.this.actions.put(a.getId(),
								a);
					}
				}
			...
			protected void jobFailureHandler() {
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						EventProcessingPolicyEditor.this.getViewSite()
								.getPage()
								.hideView(EventProcessingPolicyEditor.this);
					}
				});
			}
		};
	...
	private void savePolicy() {
		actionSave.setEnabled(false);
		new ConsoleJob(
				Messages.get().EventProcessingPolicyEditor_SaveJob_Title, this,
				Activator.PLUGIN_ID, JOB_FAMILY) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	public void dispose() {
		...
		if (policyLocked) {
			new ConsoleJob(
					Messages.get().EventProcessingPolicyEditor_CloseJob_Title,
					null, Activator.PLUGIN_ID, JOB_FAMILY) {
				@Override
				protected void runInternal(IProgressMonitor monitor)
						throws Exception {
	...
	public void doSave(IProgressMonitor monitor) {
		try {
			...
			MessageDialogHelper.openError(
					...
					Messages.get().EventProcessingPolicyEditor_Error,
					Messages.get().EventProcessingPolicyEditor_SaveError
							+ e.getMessage());
		}
	}
	...
	public void addToSelection(RuleEditor e, boolean allFromPrevSelection) {
		if (allFromPrevSelection && (lastSelectedRule != -1)) {
			int direction = Integer
					.signum(e.getRuleNumber() - lastSelectedRule);
	 ...
	 * 
	 * @param anchor
	 *            - where the selection is being moved
	 */
	...
	private void enableFilter(boolean enable) {
		...
		fd.top = enable ? new FormAttachment(filterControl)
	...
	private boolean isRuleVisible(EventProcessingPolicyRule rule) {
		...
		for (Long code : rule.getEvents()) {
			...
			if ((evt != null)
					&& evt.getName().toLowerCase().contains(filterText))
				return true;
		}
