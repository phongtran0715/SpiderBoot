...
public class RulePStorage extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		final int vInd = WidgetHelper.OUTER_SPACING
		...
		viewerDeleteValue = new SortableTableViewer(dialogArea,
				deleteColumnNames, deleteColumnWidths, 0, SWT.UP, SWT.BORDER
						| SWT.MULTI | SWT.FULL_SELECTION);
		viewerDeleteValue.setContentProvider(new ArrayContentProvider());
		viewerDeleteValue.setComparator(new ObjectLabelComparator(
				(ILabelProvider) viewerDeleteValue.getLabelProvider()));
		viewerDeleteValue.setInput(pStorageDelete.toArray());
		viewerDeleteValue
				.addSelectionChangedListener(new ISelectionChangedListener() {
					@Override
					public void selectionChanged(SelectionChangedEvent event) {
						int size = ((IStructuredSelection) viewerDeleteValue
								.getSelection()).size();
	...
	private void addPStorageDeleteAction() {
		SetEntryEditDialog dlg = new SetEntryEditDialog(getShell(), null, null,
				false, false);
	...
	private void editPStorageDeleteAction() {
		IStructuredSelection selection = (IStructuredSelection) viewerDeleteValue
				.getSelection();
		if (selection.size() != 1)
			return;

		String attr = (String) selection.getFirstElement();
		SetEntryEditDialog dlg = new SetEntryEditDialog(getShell(), attr, null,
				false, false);
		if (dlg.open() == Window.OK) {
			pStorageDelete.set(pStorageDelete.indexOf(attr),
					dlg.getAtributeName());
			viewerDeleteValue.setInput(pStorageDelete.toArray());
		}
	}
