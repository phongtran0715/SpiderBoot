...
public class RepositoryManager extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		viewer = new SortableTreeViewer(content, columnNames, columnWidths, 0,
				SWT.UP, SortableTableViewer.DEFAULT_STYLE);
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.market.context.RepositoryManager"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionShowFilter.setChecked(filterEnabled);
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.market.commands.showFilter"); //$NON-NLS-1$
		handlerService.activateHandler(
				actionShowFilter.getActionDefinitionId(), new ActionHandler(
						actionShowFilter));

		actionAddRepository = new Action("&Add repository...",
		...
		actionMark
				.setActionDefinitionId("org.netxms.ui.eclipse.market.commands.mark"); //$NON-NLS-1$
		handlerService.activateHandler(actionMark.getActionDefinitionId(),
				new ActionHandler(actionMark));
		...
		actionUnmark
				.setActionDefinitionId("org.netxms.ui.eclipse.market.commands.unmark"); //$NON-NLS-1$
		handlerService.activateHandler(actionUnmark.getActionDefinitionId(),
				new ActionHandler(actionUnmark));

		actionInstall = new Action("&Install",
				Activator.getImageDescriptor("icons/install.gif")) {
			@Override
			public void run() {
				install();
			}
		...
		actionInstall
				.setActionDefinitionId("org.netxms.ui.eclipse.market.commands.install"); //$NON-NLS-1$
		handlerService.activateHandler(actionInstall.getActionDefinitionId(),
				new ActionHandler(actionInstall));
	}
	...
	protected void fillContextMenu(final IMenuManager mgr) {
		...
		if ((selection.size() == 1)
				&& (selection.getFirstElement() instanceof RepositoryRuntimeInfo)) {
	 ...
	 * @return
	 */
	private static boolean selectionContainsRepositoryElements(
	...
	private void refreshRepositoryList() {
		new ConsoleJob("Get list of configured repositories", this,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				final List<RepositoryRuntimeInfo> repositoryInfo = new ArrayList<RepositoryRuntimeInfo>(
						repositories.size());
	...
	private void addRepository() {
		RepositoryPropertiesDlg dlg = new RepositoryPropertiesDlg(getSite()
				.getShell(), null);
		if (dlg.open() != Window.OK)
			return;

		final Repository repository = new Repository(dlg.getUrl(),
				dlg.getToken(), dlg.getDescription());
		new ConsoleJob("Add repository", this, Activator.PLUGIN_ID, null) {
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				runInUIThread(new Runnable() {
					...
					@Override
					public void run() {
						ArrayList<RepositoryRuntimeInfo> repositories = (ArrayList<RepositoryRuntimeInfo>) viewer
								.getInput();
	...
	private void deleteRepository() {
		...
		final RepositoryRuntimeInfo repository = (RepositoryRuntimeInfo) selection
				.getFirstElement();
		if (!MessageDialogHelper.openQuestion(getSite().getShell(),
				"Confirm Delete", String.format(
						"Repository %s will be deleted. Are you sure?",
						repository.getName())))
			return;
	...
	private void install() {
		...
		new ConsoleJob("Install items from repository", this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	 ...
	 * @throws Exception
	 */
	private void importFromRepository(RepositoryRuntimeInfo repository,
			String request) throws Exception {
		String importFile = repository.loadImportFile(request);
		Activator.logInfo("Import XML received from repository "
				+ repository.getName() + ": " + importFile);
