...
public class RepositoryRuntimeInfo implements MarketObject {
	...
	public void load(IViewPart viewPart, final Runnable completionHandler) {
		...
		ConsoleJob job = new ConsoleJob("Load repository objects", viewPart,
	...
	private Set<RepositoryElement> loadRepositoryObjects() throws Exception {
		...
		URL url = new URL(repository.getUrl()
				+ "/rest-api/get-available-items?accessToken="
				+ repository.getAuthToken());
		InputStream in = url.openStream();
		try {
			JSONTokener t = new JSONTokener(in);
			JSONObject root = new JSONObject(t);
			Activator.logInfo("JSON received for repository "
					+ repository.getDescription() + " (" + repository.getUrl()
					+ "): " + root.toString());
	 ...
	 * @throws Exception
	 */
	private void loadTemplates(JSONObject root, Set<RepositoryElement> objects)
			throws Exception {
		if (root.isNull("templates"))
			return;
		JSONObject templates = root.getJSONObject("templates");
		for (String k : templates.keySet()) {
			objects.add(new TemplateReference(UUID.fromString(k), templates
					.getJSONObject(k)));
		}
	}
	 ...
	 * @throws Exception
	 */
	private void loadEvents(JSONObject root, Set<RepositoryElement> objects)
			throws Exception {
		if (root.isNull("events"))
			return;
		JSONObject events = root.getJSONObject("events");
		for (String k : events.keySet()) {
			objects.add(new EventReference(UUID.fromString(k), events
					.getJSONObject(k)));
		}
	}
	...
	public MarketObject[] getChildren() {
		return loaded ? new MarketObject[] { events, rules, scripts, snmpTraps,
				summaryTables, templates }
				: new MarketObject[] { loadingPlaceholder };
	}
	...
	public String loadImportFile(String request) throws Exception {
		...
		URL url = new URL(repository.getUrl()
				+ "/rest-api/get-items?accessToken="
				+ repository.getAuthToken());
		...
		((HttpURLConnection) conn).setRequestProperty("User-Agent",
				"NetXMS Console/" + NXCommon.VERSION);
