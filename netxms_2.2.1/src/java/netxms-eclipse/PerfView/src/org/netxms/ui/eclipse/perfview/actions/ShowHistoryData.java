	...
	public void run(IAction action) {
		if (currentSelection != null && currentSelection.size() != 0) {
			for (Object o : currentSelection) {
				final String id = (o instanceof DataCollectionItem) ? Long
						.toString(((DataCollectionItem) o).getNodeId())
						+ "&" + Long.toString(((DataCollectionItem) o).getId()) : //$NON-NLS-1$
						Long.toString(((SimpleDciValue) o).getNodeId())
								+ "&" + Long.toString(((SimpleDciValue) o).getId()); //$NON-NLS-1$
				try {
					window.getActivePage().showView(HistoricalDataView.ID, id,
							IWorkbenchPage.VIEW_ACTIVATE);
				} catch (Exception e) {
					MessageDialogHelper.openError(window.getShell(), Messages
							.get().ShowHistoryData_Error, String.format(
							Messages.get().ShowHistoryData_ErrorOpeningView,
							e.getMessage()));
				}
			}
		}
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if ((selection instanceof IStructuredSelection) && !selection.isEmpty()) {
			...
			for (Object o : element)
				if ((o instanceof DataCollectionItem)
						|| (o instanceof SimpleDciValue))
