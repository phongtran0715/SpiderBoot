...
public class TemplateGraphView extends ViewPart implements SessionListener {
	...
	private void createActions() {
		...
		actionAdd = new Action("Create new template graph",
	...
	private void saveGraph(String graphName, String errorMessage,
			final boolean canBeOverwritten) {
	...
	private void editTemplateGraph() {
		...
		if (dlg != null) {
			if (dlg.open() == Window.OK) {
				...
				try {
					new ConsoleJob("Update template graph", null,
	...
	private void refreshData() {
		...
		new ConsoleJob(Messages.get().HistoricalDataView_RefreshJobName, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<GraphSettings> settings = session
						.getPredefinedGraphs(true);
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						graphList = settings;
