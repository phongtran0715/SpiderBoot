...
public class PredefinedGraphTree extends ViewPart implements SessionListener {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		initShowFilter = safeCast(
				settings.get("PredefinedGraphTree.showFilter"),
				settings.getBoolean("PredefinedGraphTree.showFilter"),
				initShowFilter);
	}
	...
	public void createPartControl(Composite parent) {
		...
		viewer.setComparer(new IElementComparer() {
			...
			public boolean equals(Object a, Object b) {
				if ((a instanceof GraphSettings)
						&& (b instanceof GraphSettings))
					return ((GraphSettings) a).getId() == ((GraphSettings) b)
							.getId();
				if ((a instanceof GraphFolder) && (b instanceof GraphFolder))
					return ((GraphFolder) a).getName().equals(
							((GraphFolder) b).getName());
				return a.equals(b);
			}
		});
	...
	private void createActions() {
		...
		actionDelete = new Action(Messages.get().PredefinedGraphTree_Delete,
		...
		actionProperties = new Action(
				Messages.get().PredefinedGraphTree_Properties) {
			@Override
			public void run() {
				editPredefinedGraph();
			}
		...
		actionShowFilter = new Action(
				Messages.get().PredefinedGraphTree_ShowFilter,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowFilter
				.setId("org.netxms.ui.eclipse.perfview.actions.showFilter"); //$NON-NLS-1$
		actionShowFilter.setChecked(initShowFilter);
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.perfview.commands.show_graph_filter"); //$NON-NLS-1$
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.perfview.context.PredefinedGraphTree"); //$NON-NLS-1$
		}
	}
	...
	private void reloadGraphList() {
		new ConsoleJob(Messages.get().PredefinedGraphTree_LoadJobName, this,
			...
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<GraphSettings> list = session
						.getPredefinedGraphs(false);
	...
	private void showPredefinedGraph(GraphSettings gs) {
		...
		String id = HistoricalGraphView.PREDEFINED_GRAPH_SUBID
				+ "&" + encodedName; //$NON-NLS-1$
		try {
			HistoricalGraphView g = (HistoricalGraphView) PlatformUI
					.getWorkbench()
					.getActiveWorkbenchWindow()
					.getActivePage()
					.showView(HistoricalGraphView.ID, id,
			...
			MessageDialogHelper
					.openError(
							...
							Messages.get().PredefinedGraphTree_Error,
							String.format(
									Messages.get().PredefinedGraphTree_ErrorOpeningView,
	...
	private void editPredefinedGraph() {
		...
		PropertyDialog dlg = PropertyDialog.createDialogOn(
				getSite().getShell(), null, settings);
		if (dlg != null) {
			if (dlg.open() == Window.OK) {
				final GraphSettings newSettings = settings;
				try {
					new ConsoleJob(
							Messages.get().PredefinedGraphTree_UpdateJobName,
					...
					MessageDialogHelper
							.openError(
									getSite().getShell(),
									"Internal Error", String.format("Unexpected exception: %s", e.getLocalizedMessage())); //$NON-NLS-1$ //$NON-NLS-2$
				}
			}
		}
	}
	...
	private void deletePredefinedGraph() {
		...
		if (!MessageDialogHelper.openQuestion(getSite().getShell(),
				Messages.get().PredefinedGraphTree_DeletePromptTitle,
				Messages.get().PredefinedGraphTree_DeletePromptText))
			return;
		...
		for (final Object o : selection.toList()) {
			...
			new ConsoleJob(String.format(
					Messages.get().PredefinedGraphTree_DeleteJobName,
					((GraphSettings) o).getShortName()), null,
	...
	public void notificationHandler(final SessionNotification n) {
		...
		case SessionNotification.PREDEFINED_GRAPHS_DELETED:
			viewer.getControl().getDisplay().asyncExec(new Runnable() {
				...
				@Override
				public void run() {
					List<GraphSettings> list = (List<GraphSettings>) viewer
							.getInput();
		...
		case SessionNotification.PREDEFINED_GRAPHS_CHANGED:
			...
			viewer.getControl().getDisplay().asyncExec(new Runnable() {
				...
				public void run() {
					...
					final List<GraphSettings> list = (List<GraphSettings>) viewer
							.getInput();
					...
					if ((selection.size() == 1)
							&& (selection.getFirstElement() instanceof GraphSettings)) {
						GraphSettings element = (GraphSettings) selection
								.getFirstElement();
						if (element.getId() == n.getSubCode())
							viewer.setSelection(new StructuredSelection(
									(GraphSettings) n.getObject()), true);
					}
				}
			});
			break;
