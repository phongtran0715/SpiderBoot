...
public class DciListLabelProvider extends LabelProvider implements
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		case DataSources.COLUMN_NODE:
			...
			return (object != null) ? object.getObjectName()
					: ("[" + Long.toString(dci.nodeId) + "]"); //$NON-NLS-1$ //$NON-NLS-2$
		case DataSources.COLUMN_METRIC:
			String name = dciNameCache.get(new NodeItemPair(dci.nodeId,
					dci.dciId));
		...
		case DataSources.COLUMN_COLOR:
			return dci.color.equalsIgnoreCase(ChartDciConfig.UNSET_COLOR) ? Messages
					.get().DciListLabelProvider_Auto : dci.color;
		}
		return null;
	}
	...
	public void resolveDciNames(final Collection<ChartDciConfig> dciList) {
		new ConsoleJob(Messages.get().DciListLabelProvider_ResolveJobName,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						int i = 0;
						for (ChartDciConfig dci : dciList) {
							dciNameCache.put(new NodeItemPair(dci.nodeId,
									dci.dciId), names[i++]);
						}
					}
				});
