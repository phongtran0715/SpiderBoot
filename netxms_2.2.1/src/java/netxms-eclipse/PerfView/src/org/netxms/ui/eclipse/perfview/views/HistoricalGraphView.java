...
public class HistoricalGraphView extends ViewPart implements
	...
	private static final int[] presetUnits = { GraphSettings.TIME_UNIT_MINUTE,
			GraphSettings.TIME_UNIT_MINUTE, GraphSettings.TIME_UNIT_HOUR,
			GraphSettings.TIME_UNIT_HOUR, GraphSettings.TIME_UNIT_HOUR,
			GraphSettings.TIME_UNIT_HOUR, GraphSettings.TIME_UNIT_DAY,
			GraphSettings.TIME_UNIT_DAY, GraphSettings.TIME_UNIT_DAY,
			GraphSettings.TIME_UNIT_DAY, GraphSettings.TIME_UNIT_DAY,
			GraphSettings.TIME_UNIT_DAY };
	private static final int[] presetRanges = { 10, 30, 1, 2, 4, 12, 1, 2, 5,
			7, 31, 365 };
	private static final String[] presetNames = {
			Messages.get().HistoricalGraphView_Preset10min,
			Messages.get().HistoricalGraphView_Preset30min,
			Messages.get().HistoricalGraphView_Preset1hour,
			Messages.get().HistoricalGraphView_Preset2hours,
			Messages.get().HistoricalGraphView_Preset4hours,
			Messages.get().HistoricalGraphView_Preset12hours,
			Messages.get().HistoricalGraphView_Preset1day,
			Messages.get().HistoricalGraphView_Preset2days,
			Messages.get().HistoricalGraphView_Preset5days,
			Messages.get().HistoricalGraphView_PresetWeek,
			Messages.get().HistoricalGraphView_PresetMonth,
			Messages.get().HistoricalGraphView_PresetYear };
	...
	public void init(IViewSite site) throws PartInitException {
		...
		settings.setTimeFrom(new Date(System.currentTimeMillis()
				- settings.getTimeRangeMillis()));
		...
		if (!fields[0].equals(PREDEFINED_GRAPH_SUBID)) {
			...
			for (int i = 1; i < fields.length; i++) {
				...
				if (subfields.length == 6) // item
				{
					try {
						...
						dci.dciDescription = URLDecoder.decode(subfields[5],
								"UTF-8"); //$NON-NLS-1$
						items.add(dci);
			...
			// Set view title to "host name: dci description" if we have only
			// one DCI
			if (items.size() == 1) {
				...
				if (nodeId != -1) {
					...
					if (object != null) {
						setPartName(String.format(
								Messages.get().HistoricalGraphView_PartName,
	...
	private void configureGraphFromSettings() {
		...
		refreshController.setInterval(settings.isAutoRefresh() ? settings
				.getRefreshRate() : -1);
	}
	...
	private int getDisplayType(ChartDciConfig dci) {
		...
		default:
			return settings.isArea() ? GraphItemStyle.AREA
					: GraphItemStyle.LINE;
		}
	}
	...
	private void getDataFromServer() {
		...
		ConsoleJob job = new ConsoleJob(
				Messages.get().HistoricalGraphView_JobName, this,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				for (int i = 0; i < dciList.length; i++) {
					...
					if (currentItem.type == ChartDciConfig.ITEM) {
						data[i] = session.getCollectedData(currentItem.nodeId,
								currentItem.dciId, settings.getTimeFrom(),
								settings.getTimeTo(), 0);
						thresholds[i] = session.getThresholds(
								currentItem.nodeId, currentItem.dciId);
					} else {
						data[i] = session
								.getCollectedTableData(currentItem.nodeId,
										currentItem.dciId,
										currentItem.instance,
										currentItem.column,
										settings.getTimeFrom(),
										settings.getTimeTo(), 0);
						thresholds[i] = null;
					}
					monitor.worked(1);
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						if (!((Widget) chart).isDisposed()) {
							chart.setTimeRange(settings.getTimeFrom(),
									settings.getTimeTo());
							setChartData(data);
							chart.clearErrors();
						}
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().HistoricalGraphView_JobError,
						session.getObjectName(currentItem.nodeId),
						currentItem.name);
			}
	...
	private void createActions() {
		...
		actionProperties = new Action(
				Messages.get().HistoricalGraphView_Properties) {
			@Override
			public void run() {
				PropertyDialog dlg = PropertyDialog.createDialogOn(getSite()
						.getShell(), null, settings);
				if (dlg != null) {
					dlg.open();
					configureGraphFromSettings();
					refreshMenuSelection();
		...
		actionAutoRefresh = new Action(
				Messages.get().HistoricalGraphView_Autorefresh) {
			@Override
			public void run() {
				settings.setAutoRefresh(!settings.isAutoRefresh());
				setChecked(settings.isAutoRefresh());
				refreshController
						.setInterval(settings.isAutoRefresh() ? settings
								.getRefreshRate() : -1);
			}
		};
		actionAutoRefresh.setChecked(settings.isAutoRefresh());
			...
			public void run() {
				try {
					...
					MessageDialogHelper
							.openError(
									getSite().getShell(),
									Messages.get().HistoricalGraphView_Error,
									String.format(
											Messages.get().HistoricalGraphView_LogScaleSwitchError,
		...
		actionShowLegend = new Action(
				Messages.get().HistoricalGraphView_ShowLegend) {
			@Override
			public void run() {
		...
		actionExtendedLegend = new Action(
				Messages.get().HistoricalGraphView_ExtendedLegend) {
			@Override
			public void run() {
		...
		actionLegendLeft = new Action(
				Messages.get().HistoricalGraphView_PlaceOnLeft,
		...
		actionLegendLeft
				.setChecked(settings.getLegendPosition() == GraphSettings.POSITION_LEFT);

		actionLegendRight = new Action(
				Messages.get().HistoricalGraphView_PlaceOnRight,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
		...
		actionLegendRight
				.setChecked(settings.getLegendPosition() == GraphSettings.POSITION_RIGHT);

		actionLegendTop = new Action(
				Messages.get().HistoricalGraphView_PlaceOnTop,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
		...
		actionLegendTop
				.setChecked(settings.getLegendPosition() == GraphSettings.POSITION_TOP);

		actionLegendBottom = new Action(
				Messages.get().HistoricalGraphView_PlaceOnBottom,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
		...
		actionLegendBottom
				.setChecked(settings.getLegendPosition() == GraphSettings.POSITION_BOTTOM);

		actionSave = new Action(Messages.get().HistoricalGraphView_Save,
				SharedIcons.SAVE) {
			@Override
			public void run() {
				String initalName = settings.getName().compareTo("noname") == 0 ? settings
						.getTitle() : settings.getName();
		...
		actionStacked = new Action(Messages.get().HistoricalGraphView_Stacked,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionTranslucent = new Action(
				Messages.get().HistoricalGraphView_Translucent,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		if (!System.getProperty("os.name").toLowerCase().contains("linux")) {
			actionCopyImage = new Action(
					Messages.get().HistoricalGraphView_CopyToClipboard,
					SharedIcons.COPY) {
				@Override
				public void run() {
					Image image = chart.takeSnapshot();
					ImageTransfer imageTransfer = ImageTransfer.getInstance();
					final Clipboard clipboard = new Clipboard(getSite()
							.getShell().getDisplay());
		...
		actionSaveAsImage = new Action("Save as image...",
				SharedIcons.SAVE_AS_IMAGE) {
			@Override
			public void run() {
				saveAsImage();
			}
    ...
    */
	...
	private void fillLocalPullDown(IMenuManager manager) {
		MenuManager presets = new MenuManager(
				Messages.get().HistoricalGraphView_Presets);
		for (int i = 0; i < presetActions.length; i++)
			presets.add(presetActions[i]);

		MenuManager legend = new MenuManager(
				Messages.get().HistoricalGraphView_Legend);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.netxms.client.datacollection.GraphSettingsChangeListener#
	 * onGraphSettingsChange(org.netxms.client.datacollection.GraphSettings )
	 */
	@Override
	 ...
	 */
	private void saveGraph(String graphName, String errorMessage,
			final boolean canBeOverwritten, final boolean asTemplate) {
		...
		SaveGraphDlg dlg = new SaveGraphDlg(getSite().getShell(), graphName,
				errorMessage, canBeOverwritten);
		int result = dlg.open();
		if (result == Window.CANCEL)
			return;

		final GraphSettings gs = new GraphSettings(0, session.getUserId(), 0,
				new ArrayList<AccessListElement>(0));
		...
		if (result == SaveGraphDlg.OVERRIDE) {
			new ConsoleJob(Messages.get().HistoricalGraphView_SaveSettings,
						...
						if (e.getErrorCode() == RCC.OBJECT_ALREADY_EXISTS) {
							runInUIThread(new Runnable() {
								...
								public void run() {
									saveGraph(
											gs.getName(),
											Messages.get().HistoricalGraphView_NameAlreadyExist,
											true, asTemplate);
								}
							...
							if (e.getErrorCode() == RCC.ACCESS_DENIED) {
								runInUIThread(new Runnable() {
									...
									public void run() {
										saveGraph(
												gs.getName(),
												Messages.get().HistoricalGraphView_NameAlreadyExistNoOverwrite,
												false, asTemplate);
									}
	...
	public static Action[] createPresetActions(final PresetHandler handler) {
		...
		for (int i = 0; i < presetRanges.length; i++) {
			...
			actions[i] = new Action(String.format(
					Messages.get().HistoricalGraphView_Last, presetNames[i])) {
				@Override
				public void run() {
					handler.onPresetSelected(presetUnits[presetIndex],
							presetRanges[presetIndex]);
				}
			};
		}
		return actions;
	...
	public static Action createAction(ActionType type,
			final HistoricalDataChart chart) {
		Action action = null;
		switch (type) {
		case ADJUST_BOTH:
			action = new Action() {
			...
			action.setImageDescriptor(Activator
					.getImageDescriptor("icons/adjust.png")); //$NON-NLS-1$
			break;
		case ADJUST_X:
			action = new Action() {
				@Override
			...
			action.setImageDescriptor(Activator
					.getImageDescriptor("icons/adjust_x.png")); //$NON-NLS-1$
			break;
		case ADJUST_Y:
			action = new Action() {
				@Override
			...
			action.setImageDescriptor(Activator
					.getImageDescriptor("icons/adjust_y.png")); //$NON-NLS-1$
			break;
		}
		return action;
	}
