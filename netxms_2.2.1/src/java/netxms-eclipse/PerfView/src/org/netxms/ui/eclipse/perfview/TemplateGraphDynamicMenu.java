...
public class TemplateGraphDynamicMenu extends ContributionItem implements
	...
	public void fill(Menu menu, int index) {
		...
		GraphSettings[] settings = GraphTemplateCache.getInstance()
				.getGraphTemplates(); // array should be already sorted
				...
				item.addSelectionListener(new SelectionAdapter() {
					...
					public void widgetSelected(SelectionEvent e) {
						final NXCSession session = (NXCSession) ConsoleSharedData
								.getSession();
						ConsoleJob job = new ConsoleJob("Get last values of "
								+ node.getObjectName(), null,
								Activator.PLUGIN_ID, null) {
							@Override
							protected String getErrorMessage() {
								return "Not possible to get last values for node"
										+ node.getObjectName();
							}
							...
							protected void runInternal(IProgressMonitor monitor)
								...
								final DciValue[] data = session
										.getLastValues(node.getObjectId());
								runInUIThread(new Runnable() {
									@Override
									public void run() {
										GraphTemplateCache.execute(node,
												(GraphSettings) item.getData(),
												data);
									}
								});
							}
