		...
		if (currentSelection != null) {
			...
			for (int i = 0; i < currentSelection.length; i++) {
				...
				id += "&" + Long.toString(nodeId) + "@" + Long.toString(dciId) + "@" + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
						...
						+ "@" + Integer.toString(dataType) + "@" + name + "@" + description; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
			}

			try {
				window.getActivePage().showView(HistoricalGraphView.ID, id,
						IWorkbenchPage.VIEW_ACTIVATE);
			} catch (PartInitException e) {
				MessageDialogHelper
						.openError(
								window.getShell(),
								Messages.get().ShowHistoryGraph_Error,
								String.format(
										Messages.get().ShowHistoryGraph_ErrorOpeningView,
