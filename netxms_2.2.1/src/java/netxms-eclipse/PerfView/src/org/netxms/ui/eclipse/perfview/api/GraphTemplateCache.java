...
public class GraphTemplateCache {
			...
			public void notificationHandler(SessionNotification n) {
				...
				case SessionNotification.PREDEFINED_GRAPHS_CHANGED: {
					if (!(n.getObject() instanceof GraphSettings)
							|| !((GraphSettings) n.getObject()).isTemplate()) {
						return;
					}
	...
	public GraphSettings[] getGraphTemplates() {
		GraphSettings[] graphs = templateList
				.toArray(new GraphSettings[templateList.size()]);
	...
	public static void execute(final AbstractNode node,
			final GraphSettings data, final DciValue[] values) {
		...
		for (int i = 0; i < conf.length; i++) {
			...
			Pattern descriptionPattern = Pattern
					.compile(conf[i].dciDescription);
			int j;
			for (j = 0; j < values.length; j++) {
				if (!conf[i].dciName.isEmpty()
						&& namePattern.matcher(values[j].getName()).find()) {
					newList.add(new ChartDciConfig(values[j]));
					foundDCICount++;
					if (!conf[i].multiMatch)
						break;
				}
				if (!conf[i].dciDescription.isEmpty()
						&& descriptionPattern.matcher(
								values[j].getDescription()).find()) {
			...
			if (!conf[i].multiMatch && j == values.length
					&& foundByDescription >= 0) {
				foundDCICount++;
		...
		if (foundDCICount > 0) {
			...
			MessageDialogHelper.openError(PlatformUI.getWorkbench()
					...
					"Graph creation from template error",
					"None of template graphs DCI were found on a node.");
		}
	}
	...
	private static void showPredefinedGraph(GraphSettings gs, long objectId) {
		...
		String id = HistoricalGraphView.PREDEFINED_GRAPH_SUBID
				+ "&" + encodedName + "&" + objectId; //$NON-NLS-1$
		try {
