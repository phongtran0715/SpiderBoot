...
public class HistoricalDataComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		case HistoricalDataView.COLUMN_TIME:
			rc = Long.signum(((DciDataRow) e1).getTimestamp().getTime()
					- ((DciDataRow) e2).getTimestamp().getTime());
			break;
		case HistoricalDataView.COLUMN_DATA:
			rc = NaturalOrderComparator.compare(
					((DciDataRow) e1).getValueAsString(),
					((DciDataRow) e2).getValueAsString());
			break;
		default:
			rc = 0;
			break;
