...
public class General extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		gd.verticalSpan = 2;
		gd.verticalAlignment = SWT.TOP;
		legendLocation = WidgetHelper.createLabeledCombo(optionsGroup,
				SWT.READ_ONLY, Messages.get().General_LegendPosition, gd);
		...
		legendLocation.select(31 - Integer.numberOfLeadingZeros(config
				.getLegendPosition()));
		...
		checkAutoRefresh.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				refreshIntervalSpinner.setEnabled(checkAutoRefresh
						.getSelection());
		...
		refreshIntervalScale.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				refreshIntervalSpinner.setSelection(refreshIntervalScale
						.getSelection());
			}
		...
		refreshIntervalSpinner.addSelectionListener(new SelectionListener() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				refreshIntervalScale.setSelection(refreshIntervalSpinner
						.getSelection());
			}
		...
		timeSelector = new TimePeriodSelector(dialogArea, SWT.NONE,
				config.timePeriod());
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
	...
	protected void applyChanges(final boolean isApply) {
		...
		config.setLegendPosition((int) Math.pow(2,
				legendLocation.getSelectionIndex()));
		...
		if ((config instanceof GraphSettings) && isApply) {
			...
			final NXCSession session = (NXCSession) ConsoleSharedData
					.getSession();
			new ConsoleJob(Messages.get().General_JobName, null,
