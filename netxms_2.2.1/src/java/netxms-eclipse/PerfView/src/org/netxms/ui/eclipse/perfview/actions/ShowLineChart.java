...
public class ShowLineChart implements DciOpenHandler {
	...
	public boolean open(DciValue dci) {
		...
		final String id = Long.toString(uniqueId++)
				+ "&" + Long.toString(dci.getNodeId()) + "@" + //$NON-NLS-1$ //$NON-NLS-2$
				Long.toString(dci.getId())
				+ "@" + Integer.toString(dci.getSource()) + "@" + //$NON-NLS-1$ //$NON-NLS-2$
				Integer.toString(dci.getDataType())
		...
		try {
			...
			MessageDialogHelper.openError(window.getShell(),
					Messages.get().ShowLineChart_Error, String.format(
							Messages.get().ShowLineChart_ErrorOpeningView,
