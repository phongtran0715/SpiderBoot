...
public class TableValue extends Composite {
	...
	public TableValue(Composite parent, int style, IViewPart viewPart,
		...
		labelProvider.setUseMultipliers(getBoolean(ds, configId
		...
		viewer.getTable().addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				...
				ds.put(configId + ".initShowfilter",
						actionShowFilter.isChecked());
			}
		});
	...
	private void createActions() {
		actionShowHistory = new Action(
				"History", Activator.getImageDescriptor("icons/data_history.gif")) { //$NON-NLS-1$
			@Override
			public void run() {
				showHistory();
			}
		...
		actionShowLineChart = new Action(Messages.get().TableValue_LineChart,
				Activator.getImageDescriptor("icons/chart_line.png")) { //$NON-NLS-1$
			@Override
			public void run() {
				showLineChart();
			}
		...
		actionShowBarChart = new Action(Messages.get().TableValue_BarChart,
				Activator.getImageDescriptor("icons/chart_bar.png")) { //$NON-NLS-1$
			@Override
			public void run() {
		...
		actionShowPieChart = new Action(Messages.get().TableValue_PieChart,
				Activator.getImageDescriptor("icons/chart_pie.png")) { //$NON-NLS-1$
			@Override
			public void run() {
		...
		actionUseMultipliers = new Action("Use &multipliers",
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
				labelProvider.setUseMultipliers(actionUseMultipliers
						.isChecked());
				viewer.refresh(true);
			}
		};
		...
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.perfview.commands.show_table_values_filter"); //$NON-NLS-1$
	}
	...
	public void refresh(final Runnable postRefreshHook) {
		...
		ConsoleJob job = new ConsoleJob(String.format(
				Messages.get().TableValue_JobName, dciId), viewPart,
			...
			protected IStatus createFailureStatus(final Exception e) {
				runInUIThread(new Runnable() {
					...
					public void run() {
						...
						if (errorLabel == null) {
							errorLabel = new CLabel(viewer.getControl()
									.getParent(), SWT.NONE);
							errorLabel.setFont(JFaceResources.getBannerFont());
							errorLabel.setImage(StatusDisplayInfo
	...
	private void updateViewer(final Table table) {
		...
		if (!viewer.isInitialized()) {
			...
			WidgetHelper.restoreTableViewerSettings(viewer, Activator
					.getDefault().getDialogSettings(), configId); //$NON-NLS-1$
			viewer.getTable().addDisposeListener(new DisposeListener() {
				@Override
				public void widgetDisposed(DisposeEvent e) {
			...
			viewer.setComparator(new TableItemComparator(table
					.getColumnDataTypes()));
		}
	...
	private void showHistory() {
		...
		for (int i = 0; i < cells.length; i++) {
			TableColumnDefinition column = currentData
					.getColumnDefinition(cells[i].getColumnIndex());
			...
			String id = Long.toString(objectId)
					...
					+ safeEncode(column.getDisplayName()
							+ ": " + instance.replace("~~~", " / ")) + "@" //$NON-NLS-1$
					+ safeEncode(instance) + "@" + safeEncode(column.getName());//$NON-NLS-1$ //$NON-NLS-2$

			final IWorkbenchPage page = (viewPart != null) ? viewPart.getSite()
					.getPage() : PlatformUI.getWorkbench()
					.getActiveWorkbenchWindow().getActivePage();
			try {
				page.showView(HistoricalDataView.ID, id,
						IWorkbenchPage.VIEW_ACTIVATE);
			} catch (Exception e) {
				MessageDialogHelper.openError(page.getWorkbenchWindow()
						.getShell(), Messages.get().TableValue_Error, String
						.format(Messages.get().TableValue_ErrorOpeningView,
	...
	private void showLineChart() {
		...
		for (int i = 0; i < cells.length; i++) {
			...
			id += "&" + Long.toString(objectId) + "@" + Long.toString(dciId) + "@" + Integer.toString(source) + "@" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
					+ Integer.toString(column.getDataType())
					+ "@" + safeEncode(currentData.getTitle()) + "@" //$NON-NLS-1$ //$NON-NLS-2$
					+ safeEncode(column.getDisplayName()
							+ ": " + instance.replace("~~~", " / ")) + "@" + safeEncode(instance) + "@" //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
		...
		final IWorkbenchPage page = (viewPart != null) ? viewPart.getSite()
				...
				.getActiveWorkbenchWindow().getActivePage();
		try {
			page.showView(HistoricalGraphView.ID, id,
					IWorkbenchPage.VIEW_ACTIVATE);
		} catch (Exception e) {
			MessageDialogHelper.openError(
					page.getWorkbenchWindow().getShell(),
					Messages.get().TableValue_Error,
					String.format(Messages.get().TableValue_ErrorOpeningView,
							e.getLocalizedMessage()));
		}
	}
	...
	private void showDataComparisonChart(int chartType) {
		...
		String id = Long.toString(uniqueId++)
				+ "&" + Integer.toString(chartType); //$NON-NLS-1$
		for (int i = 0; i < cells.length; i++) {
			TableColumnDefinition column = currentData
					.getColumnDefinition(cells[i].getColumnIndex());
		...
		final IWorkbenchPage page = (viewPart != null) ? viewPart.getSite()
				...
				.getActiveWorkbenchWindow().getActivePage();
		try {
			page.showView(DataComparisonView.ID, id,
					IWorkbenchPage.VIEW_ACTIVATE);
		} catch (Exception e) {
			MessageDialogHelper.openError(
					page.getWorkbenchWindow().getShell(),
