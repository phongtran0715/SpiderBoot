...
public class PerfTabGraph extends DashboardComposite {
	 ...
	 * @param style
	 */
	public PerfTabGraph(Composite parent, long nodeId, PerfTabDci dci,
			PerfTabGraphSettings settings, IViewPart viewPart,
		...
		GraphItemStyle style = new GraphItemStyle(settings.getType(),
				settings.getColorAsInt(), 2, 0);
		...
		chart.addParameter(new GraphItem(nodeId, dci.getId(), 0, 0,
				"", settings.getRuntimeName(), "%s")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public void addItem(PerfTabDci dci, PerfTabGraphSettings settings) {
		...
		synchronized (items) {
			...
			GraphItemStyle style = new GraphItemStyle(settings.getType(),
					settings.getColorAsInt(), 2, 0);
			List<GraphItemStyle> styles = new ArrayList<GraphItemStyle>(
					chart.getItemStyles());
	 ...
	 */
	public void start() {
		refreshController = new ViewRefreshController(viewPart, 30,
				new Runnable() {
					@Override
					public void run() {
	...
	public void refreshData() {
		...
		ConsoleJob job = new ConsoleJob(Messages.get().PerfTabGraph_JobTitle,
				null, Activator.PLUGIN_ID, Activator.PLUGIN_ID) {
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				synchronized (items) {
					...
					for (int i = 0; i < data.length; i++) {
						...
						data[i] = session.getCollectedData(nodeId,
								currentDci.getId(), from, to, 0);
					}
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().PerfTabGraph_JobError,
						currentDci.getId(), currentDci.getDescription());
			}
	...
	private void openHistoryGraph() {
		...
		try {
			page.showView(HistoricalGraphView.ID, sb.toString(),
					IWorkbenchPage.VIEW_ACTIVATE);
		} catch (PartInitException e) {
			MessageDialogHelper.openError(
					getShell(),
					"Error",
					"Could not open history graph view "
