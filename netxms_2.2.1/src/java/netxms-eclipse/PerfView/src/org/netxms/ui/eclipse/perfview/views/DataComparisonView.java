...
public class DataComparisonView extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		titleImages[0] = Activator
				.getImageDescriptor("icons/chart_bar.png").createImage(); //$NON-NLS-1$
		titleImages[1] = Activator
				.getImageDescriptor("icons/chart_pie.png").createImage(); //$NON-NLS-1$
		titleImages[2] = Activator
				.getImageDescriptor("icons/graph.png").createImage(); // TODO: add radar icon //$NON-NLS-1$
		titleImages[3] = Activator
				.getImageDescriptor("icons/chart_tube.png").createImage(); //$NON-NLS-1$
		titleImages[4] = Activator
				.getImageDescriptor("icons/chart_dial.png").createImage(); //$NON-NLS-1$
		...
		if (!fields[0].equals(HistoricalGraphView.PREDEFINED_GRAPH_SUBID)) {
			...
			for (int i = 2; i < fields.length; i++) {
				...
				if (subfields.length == 6) {
					try {
						items.add(new GraphItem(Long
								.parseLong(subfields[0], 10), // Node ID
	...
	public void init(IViewSite site, IMemento memento) throws PartInitException {
		...
		if (memento != null) {
			...
			autoRefreshEnabled = safeCast(memento.getBoolean(KEY_AUTO_REFRESH),
					autoRefreshEnabled);
			autoRefreshInterval = safeCast(
					memento.getInteger(KEY_REFRESH_INTERVAL),
					autoRefreshInterval);
			translucent = safeCast(memento.getBoolean(KEY_TRANSLUCENT),
					translucent);
			transposed = safeCast(memento.getBoolean(KEY_TRANSPOSED),
					transposed);
			showLegend = safeCast(memento.getBoolean(KEY_SHOW_LEGEND),
					showLegend);
			legendPosition = safeCast(memento.getInteger(KEY_LEGEND_POSITION),
					legendPosition);
			useLogScale = safeCast(memento.getBoolean(KEY_LOG_SCALE),
					useLogScale);
	...
	public void createPartControl(Composite parent) {
		...
		refreshController = new ViewRefreshController(this,
				autoRefreshEnabled ? autoRefreshInterval : -1, new Runnable() {
					@Override
					public void run() {
	...
	private void createActions() {
		...
		actionAutoRefresh = new Action(
				Messages.get().DataComparisonView_AutoRefresh) {
			@Override
			public void run() {
				autoRefreshEnabled = !autoRefreshEnabled;
				setChecked(autoRefreshEnabled);
				refreshController
						.setInterval(autoRefreshEnabled ? autoRefreshInterval
								: -1);
			}
		};
		actionAutoRefresh.setChecked(autoRefreshEnabled);

		actionUseLogScale = new Action(
				Messages.get().DataComparisonView_LogScale) {
			@Override
			public void run() {
		...
		actionShowTranslucent = new Action(
				Messages.get().DataComparisonView_Translucent) {
			@Override
			public void run() {
		...
		actionShowLegend = new Action(
				Messages.get().DataComparisonView_ShowLegend) {
			@Override
			public void run() {
		...
		actionLegendLeft = new Action(
				Messages.get().DataComparisonView_PlaceOnLeft,
		...
		actionLegendLeft
				.setChecked(legendPosition == GraphSettings.POSITION_LEFT);

		actionLegendRight = new Action(
				Messages.get().DataComparisonView_PlaceOnRight,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
		...
		actionLegendRight
				.setChecked(legendPosition == GraphSettings.POSITION_RIGHT);

		actionLegendTop = new Action(
				Messages.get().DataComparisonView_PlaceOnTop,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
		...
		actionLegendBottom = new Action(
				Messages.get().DataComparisonView_PlaceOnBottom,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
		...
		actionShowBarChart = new Action(
				Messages.get().DataComparisonView_BarChart,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
		...
		actionShowBarChart
				.setChecked(chart.getChartType() == DataComparisonChart.BAR_CHART);
		actionShowBarChart.setImageDescriptor(Activator
				.getImageDescriptor("icons/chart_bar.png")); //$NON-NLS-1$

		actionShowTubeChart = new Action(
				Messages.get().DataComparisonView_TubeChart,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
		...
		actionShowTubeChart
				.setChecked(chart.getChartType() == DataComparisonChart.TUBE_CHART);
		actionShowTubeChart.setImageDescriptor(Activator
				.getImageDescriptor("icons/chart_tube.png")); //$NON-NLS-1$

		actionShowPieChart = new Action(
				Messages.get().DataComparisonView_PieChart,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
		...
		actionShowPieChart
				.setChecked(chart.getChartType() == DataComparisonChart.PIE_CHART);
		actionShowPieChart.setImageDescriptor(Activator
				.getImageDescriptor("icons/chart_pie.png")); //$NON-NLS-1$

		actionHorizontal = new Action(
				Messages.get().DataComparisonView_ShowHorizontally,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
				transposed = true;
		...
		actionHorizontal.setImageDescriptor(Activator
				.getImageDescriptor("icons/bar_horizontal.png")); //$NON-NLS-1$

		actionVertical = new Action(
				Messages.get().DataComparisonView_ShowVertically,
				Action.AS_RADIO_BUTTON) {
			@Override
			public void run() {
				transposed = false;
		...
		actionVertical.setImageDescriptor(Activator
				.getImageDescriptor("icons/bar_vertical.png")); //$NON-NLS-1$

		if (!System.getProperty("os.name").toLowerCase().contains("linux")) {
			actionCopyImage = new Action(
					Messages.get().DataComparisonView_CopyToClipboard,
					SharedIcons.COPY) {
				@Override
				public void run() {
	...
	private void fillLocalPullDown(IMenuManager manager) {
		MenuManager legend = new MenuManager(
				Messages.get().DataComparisonView_Legend);
	...
	private void updateChart() {
		...
		ConsoleJob job = new ConsoleJob(
				Messages.get().DataComparisonView_JobName, this,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				for (int i = 0; i < items.size(); i++) {
					...
					DciData data = (item.getType() == DataCollectionObject.DCO_TYPE_ITEM) ? session
							.getCollectedData(item.getNodeId(),
									item.getDciId(), null, null, 1) : session
							.getCollectedTableData(item.getNodeId(),
									item.getDciId(), item.getInstance(),
									item.getDataColumn(), null, null, 1);
					DciDataRow value = data.getLastValue();
					values[i] = (value != null) ? value.getValueAsDouble()
							: 0.0;
				}
				...
				if (chartType == DataComparisonChart.GAUGE_CHART) {
					for (int i = 0; i < items.size(); i++) {
						GraphItem item = items.get(i);
						thresholds[i] = session.getThresholds(item.getNodeId(),
								item.getDciId());
					}
				}
					...
					public void run() {
						if (chartType == DataComparisonChart.GAUGE_CHART)
							for (int i = 0; i < thresholds.length; i++)
								chart.updateParameterThresholds(i,
										thresholds[i]);
						setChartData(values);
						chart.clearErrors();
