...
public class HistoricalDataView extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		if ((object == null)
				|| (!(object instanceof AbstractNode)
						&& !(object instanceof MobileDevice) && !(object instanceof Cluster)))
			throw new PartInitException(
					Messages.get().HistoricalDataView_InvalidObjectID);
		...
		setPartName(nodeName
				+ ": [" + (tableName == null ? Long.toString(dciId) : tableName) + "]"); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public void createPartControl(Composite parent) {
		final String[] names = {
				Messages.get().HistoricalDataView_ColTimestamp,
				Messages.get().HistoricalDataView_ColValue };
	...
	private void createActions() {
		...
		actionSelectRange = new Action(
				Messages.get().HistoricalDataView_SelectDataRange) {
			@Override
			public void run() {
				selectRange();
			}
	...
	private void refreshData() {
		...
		new ConsoleJob(Messages.get().HistoricalDataView_RefreshJobName, this,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				final DciData data;
				if (subparts != null)
					data = session.getCollectedTableData(nodeId, dciId,
							instance, column, timeFrom, timeTo, recordLimit);
				else
					data = session.getCollectedData(nodeId, dciId, timeFrom,
							timeTo, recordLimit);
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().HistoricalDataView_RefreshJobError,
						nodeName, dciId);
			}
		}.start();
	}
	...
	private void selectRange() {
		HistoricalDataSelectionDialog dlg = new HistoricalDataSelectionDialog(
				getSite().getShell(), recordLimit, timeFrom, timeTo);
