	...
	public void run(IAction action) {
		if (currentSelection != null) {
			String id = Long.toString(uniqueId++)
					+ "&" + Integer.toString(getChartType()); //$NON-NLS-1$
			for (int i = 0; i < currentSelection.length; i++) {
				...
				try {
					if (currentSelection[i] instanceof DciValue) {
						...
						dataType = ((DciValue) currentSelection[i])
								.getDataType();
						name = URLEncoder.encode(
								((DciValue) currentSelection[i]).getName(),
								"UTF-8"); //$NON-NLS-1$
						description = URLEncoder.encode(
								((DciValue) currentSelection[i])
										.getDescription(), "UTF-8"); //$NON-NLS-1$
					} else if (currentSelection[i] instanceof DataCollectionItem) {
						dciId = ((DataCollectionItem) currentSelection[i])
								.getId();
						nodeId = ((DataCollectionItem) currentSelection[i])
								.getNodeId();
						source = ((DataCollectionItem) currentSelection[i])
								.getOrigin();
						dataType = ((DataCollectionItem) currentSelection[i])
								.getDataType();
						name = URLEncoder.encode(
								((DataCollectionItem) currentSelection[i])
										.getName(), "UTF-8"); //$NON-NLS-1$
						description = URLEncoder.encode(
								((DataCollectionItem) currentSelection[i])
										.getDescription(), "UTF-8"); //$NON-NLS-1$
					}
			...
			try {
				window.getActivePage().showView(DataComparisonView.ID, id,
						IWorkbenchPage.VIEW_ACTIVATE);
			} catch (PartInitException e) {
				MessageDialogHelper
						.openError(
								window.getShell(),
								Messages.get().ShowDataComparisonChart_Error,
								String.format(
										Messages.get().ShowDataComparisonChart_ErrorOpeningView,
										e.getMessage()));
			} catch (IllegalArgumentException e) {
				MessageDialogHelper.openError(window.getShell(),
						Messages.get().ShowDataComparisonChart_14,
						String.format(
								Messages.get().ShowDataComparisonChart_15,
								e.getMessage()));
			}
		}
	}
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if ((selection instanceof IStructuredSelection) && !selection.isEmpty()) {
			...
			if ((element instanceof DciValue)
					|| (element instanceof DataCollectionItem)) {
