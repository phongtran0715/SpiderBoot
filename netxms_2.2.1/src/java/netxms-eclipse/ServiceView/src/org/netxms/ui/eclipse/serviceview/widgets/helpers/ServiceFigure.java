...
public class ServiceFigure extends Figure {
	...
	private static final Color BORDER_COLOR = new Color(Display.getCurrent(),
			0, 0, 0);
	private static final Color NORMAL_COLOR_LEFT = new Color(
			Display.getCurrent(), 164, 196, 212);
	private static final Color NORMAL_COLOR_RIGHT = new Color(
			Display.getCurrent(), 180, 208, 228);
	private static final Color SELECTION_COLOR_LEFT = new Color(
			Display.getCurrent(), 255, 242, 0);
	private static final Color SELECTION_COLOR_RIGHT = new Color(
			Display.getCurrent(), 225, 212, 0);
	...
	public ServiceFigure(ServiceTreeElement service,
			ServiceTreeLabelProvider labelProvider,
			IServiceFigureListener listener) {
		...
		if (listener != null) {
			label.addMouseListener(new MouseListener() {
				...
				public void mouseDoubleClicked(MouseEvent me) {
					if (me.button == 1)
						ServiceFigure.this.listener
								.expandButtonPressed(ServiceFigure.this.service);
				}
		...
		expandBox.setLocation(new Point(width - HORIZONTAL_MARGIN - es.width,
				height / 2 - es.height / 2));
	...
	protected void paintFigure(Graphics gc) {
		...
		if (labelProvider.isElementSelected(service)) {
			...
			Pattern pattern = new Pattern(Display.getDefault(), rect.x, rect.y,
					rect.x + rect.width, rect.y + rect.height,
					SELECTION_COLOR_LEFT, SELECTION_COLOR_RIGHT);
			...
			Pattern pattern = new Pattern(Display.getDefault(), rect.x, rect.y,
					rect.x + rect.width, rect.y + rect.height,
					NORMAL_COLOR_LEFT, NORMAL_COLOR_RIGHT);
			gc.setBackgroundPattern(pattern);
			gc.fillRoundRectangle(rect, 8, 8);
			// gc.setAlpha(255);
		}
