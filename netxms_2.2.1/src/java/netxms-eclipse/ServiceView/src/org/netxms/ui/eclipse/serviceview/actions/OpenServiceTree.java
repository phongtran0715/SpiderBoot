 ...
 * 
 */
public class OpenServiceTree implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		if (window != null) {
			try {
				window.getActivePage().showView(ServiceTreeView.ID);
			} catch (PartInitException e) {
				MessageDialogHelper.openError(
						window.getShell(),
						Messages.get().OpenServiceTree_Error,
						Messages.get().OpenServiceTree_OpenViewError
								+ e.getMessage());
			}
		}
	}
