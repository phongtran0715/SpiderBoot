...
public class ServiceTreeModel {
	 ...
	 *            NetXMS object
	 */
	private void addToModel(ServiceTreeElement parent, AbstractObject object,
			int level) {
		final ServiceTreeElement element = new ServiceTreeElement(parent,
				object);
		elements.add(element);
		for (AbstractObject o : object.getChildsAsArray()) {
			if ((o instanceof Container) || (o instanceof Node)
					|| (o instanceof Cluster) || (o instanceof Condition)) {
	...
	public ServiceTreeElement[] getVisibleElements() {
		List<ServiceTreeElement> list = new ArrayList<ServiceTreeElement>(
				elements.size());
