...
public class BarGaugeWidget extends GaugeWidget {
	...
	protected void renderElement(GC gc, DataComparisonElement dci, int x,
		...
		if (legendVisible) {
			...
			case GraphSettings.POSITION_TOP:
				gc.drawText(dci.getName(), rect.x
						+ ((rect.width - legendExt.x) / 2), rect.y + 4, true);
			...
			case GraphSettings.POSITION_BOTTOM:
				rect.height -= legendExt.y + 8;
				gc.drawText(dci.getName(), rect.x
						+ ((rect.width - legendExt.x) / 2), rect.y
						+ rect.height + 4, true);
				break;
			case GraphSettings.POSITION_LEFT:
				gc.drawText(dci.getName(), rect.x + 4, rect.y
						+ ((rect.height - legendExt.y) / 2), true);
				rect.x += legendExt.x + 8;
				rect.width -= legendExt.x + 8;
				break;
			case GraphSettings.POSITION_RIGHT:
				rect.width -= legendExt.x + 8;
				gc.drawText(dci.getName(), rect.x + rect.width + 4, rect.y
						+ ((rect.height - legendExt.y) / 2), true);
				break;
			}
		}
		...
		final double pointValue = isVertical() ? (maxValue - minValue)
				/ rect.width : (maxValue - minValue) / rect.height;
		if (dci.getValue() > minValue) {
			if (colorMode == GaugeColorMode.ZONE) {
				...
				if (leftRedZone > minValue) {
					...
					drawZone(gc, rect, minValue, right, pointValue,
							RED_ZONE_COLOR, YELLOW_ZONE_COLOR);
				}

				left = right;
				right = leftYellowZone + (rightYellowZone - leftYellowZone) / 2;
				drawZone(gc, rect, left, right, pointValue,
						(leftYellowZone > minValue) ? YELLOW_ZONE_COLOR
								: GREEN_ZONE_COLOR, GREEN_ZONE_COLOR);

				left = right;
				right = rightYellowZone + (rightRedZone - rightYellowZone) / 2;
				if (rightYellowZone < rightRedZone) {
					drawZone(gc, rect, left, right, pointValue,
							GREEN_ZONE_COLOR, YELLOW_ZONE_COLOR);
					left = right;
					drawZone(gc, rect, left, maxValue, pointValue,
							YELLOW_ZONE_COLOR,
							(rightRedZone < maxValue) ? RED_ZONE_COLOR
									: YELLOW_ZONE_COLOR);
				} else if (rightRedZone < maxValue) {
					drawZone(gc, rect, left, right, pointValue,
							GREEN_ZONE_COLOR, RED_ZONE_COLOR);
				}
				...
				if (v < maxValue) {
					...
					if (isVertical()) {
						...
						gc.fillRectangle(rect.x + points, rect.y, rect.width
								- points, rect.height);
					} else {
				...
				if (colorMode == GaugeColorMode.THRESHOLD)
					gc.setBackground(StatusDisplayInfo.getStatusColor(dci
				...
				if (isVertical()) {
					...
					if (points > rect.height)
						points = rect.height;
					gc.fillRectangle(rect.x, rect.y + rect.height - points,
							rect.width, points);
				}
			}
		}
	 ...
	 * @param prevColor
	 */
	private void drawZone(GC gc, Rectangle rect, double startValue,
			double endValue, double pointValue, RGB startColor, RGB endColor) {
		...
		if (isVertical()) {
			...
			gc.fillGradientRectangle(rect.x + start, rect.y, points,
					rect.height, false);
		} else {
	...
	private void drawScale(GC gc, Rectangle rect, double pointValue) {
		...
		final Font markFont = WidgetHelper.getBestFittingFont(gc, scaleFonts,
				"900MM", SCALE_TEXT_WIDTH, SCALE_TEXT_HEIGHT); //$NON-NLS-1$
		gc.setFont(markFont);

		double step = getStepMagnitude(Math.max(Math.abs(minValue),
				Math.abs(maxValue)));
		...
		if (isVertical()) {
			for (float x = 0; x < rect.width; x += pointsStep, value += step) {
				if (gridVisible && (x > 0)) {
					...
					gc.drawLine(rect.x + (int) x, rect.y - 2, rect.x + (int) x,
							rect.y + rect.height + 1);
				}
				String text = DataFormatter.roundDecimalValue(value, step, 5);
				gc.setForeground(scaleTextColor);
				gc.drawText(text, rect.x + (int) x, rect.y + rect.height + 4,
			...
			for (float y = rect.height; y > 0; y -= pointsStep, value += step) {
				if (gridVisible && (y < rect.height)) {
					gc.setForeground(scaleColor);
					gc.drawLine(rect.x - 2, rect.y + (int) y, rect.x
							+ rect.width + 1, rect.y + (int) y);
				}
				String text = DataFormatter.roundDecimalValue(value, step, 5);
				gc.setForeground(scaleTextColor);
				gc.drawText(text, rect.x + rect.width + 4, rect.y + (int) y
						- textHeight * 3 / 4, SWT.DRAW_TRANSPARENT);
			}
		}
	}
