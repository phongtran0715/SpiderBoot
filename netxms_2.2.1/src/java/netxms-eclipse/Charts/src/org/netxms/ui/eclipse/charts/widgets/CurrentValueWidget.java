...
public class CurrentValueWidget extends GaugeWidget {
	...
	protected void createFonts() {
		...
		for (int i = 0; i < valueFonts.length; i++)
			valueFonts[i] = new Font(getDisplay(), fontName, i * 6 + 12,
					SWT.BOLD); //$NON-NLS-1$
	}
	...
	protected void renderElement(GC gc, DataComparisonElement dci, int x,
		...
		final Font font = WidgetHelper.getBestFittingFont(gc, valueFonts,
				value, rect.width, rect.height); //$NON-NLS-1$
		gc.setFont(font);
		switch (colorMode) {
		case ZONE:
			if ((dci.getValue() <= leftRedZone)
					|| (dci.getValue() >= rightRedZone)) {
				gc.setForeground(colors.create(RED_ZONE_COLOR));
			} else if ((dci.getValue() <= leftYellowZone)
					|| (dci.getValue() >= rightYellowZone)) {
		...
		case CUSTOM:
			...
			break;
		case THRESHOLD:
			gc.setForeground(StatusDisplayInfo.getStatusColor(dci
		...
		gc.drawText(value, rect.x + rect.width / 2 - ext.x / 2, rect.y
				+ rect.height / 2 - ext.y / 2, SWT.DRAW_TRANSPARENT);
