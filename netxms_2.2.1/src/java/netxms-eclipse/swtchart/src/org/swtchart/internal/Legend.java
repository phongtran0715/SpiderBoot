...
public class Legend extends Canvas implements ILegend, PaintListener {
	...
	public void updateLayoutData() {
		...
		final int cellExtraWidth = extended ? (Util.getExtentInGC(getFont(),
				VALUE_PLACEHOLDER).x + MARGIN) * 4 : 0;
		...
		if (position == SWT.RIGHT || position == SWT.LEFT) {
			...
			for (ISeries series : seriesArray) {
				int textWidth = Util.getExtentInGC(getFont(), series.getName()).x;
				int cellWidth = textWidth + SYMBOL_WIDTH + MARGIN * 3;
				if ((!extended && (xPosition + cellWidth < r.width))
						|| (xPosition == 0)) {
					xPosition += cellWidth;
				} else {
					rows++;
				...
				cellBounds.put(series.getId(), new Rectangle(xPosition
						...
						+ topOffset, cellWidth + cellExtraWidth, cellHeight));
		...
		if (extended) {
			...
			cellBounds.put(HEADER_ID, new Rectangle(0, MARGIN, cellExtraWidth,
					cellHeight));
		}
	...
	private void drawExtendedInfo(GC gc, Series series, Rectangle r) {
		...
		gc.drawText(
				DataFormatter.roundDecimalValue(series.getCurY(), 0.005, 3), x,
				r.y, true);
		x += shift;

		gc.drawText(
				DataFormatter.roundDecimalValue(series.getMinY(), 0.005, 3), x,
				r.y, true);
		x += shift;

		gc.drawText(
				DataFormatter.roundDecimalValue(series.getAvgY(), 0.005, 3), x,
				r.y, true);
		x += shift;

		gc.drawText(
				DataFormatter.roundDecimalValue(series.getMaxY(), 0.005, 3), x,
				r.y, true);
	}
	...
	public void paintControl(PaintEvent e) {
		...
		for (int i = 0; i < seriesArray.length; i++) {
			...
			gc.setBackground(getBackground());
			gc.setForeground(getForeground());
			gc.drawText(seriesArray[i].getName(), r.x + SYMBOL_WIDTH + MARGIN
					* 2, r.y, true);
