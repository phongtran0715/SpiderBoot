...
public class Axis implements IAxis {
	...
	public int getPixelCoordinate(double dataCoordinate, double lower,
		...
		if (isHorizontalAxis) {
			if (logScaleEnabled) {
				double logLower = Math.log10((lower == 0) ? Math.min(0.1,
						upper / 10.0) : lower);
				pixelCoordinate = (dataCoordinate == 0) ? 0 : (int) ((Math
						.log10(dataCoordinate) - logLower)
						/ (Math.log10(upper) - logLower) * width);
			...
			if (logScaleEnabled) {
				double logLower = Math.log10((lower == 0) ? Math.min(0.1,
						upper / 10.0) : lower);
				pixelCoordinate = (dataCoordinate == 0) ? height : (int) ((Math
						.log10(upper) - Math.log10(dataCoordinate))
						/ (Math.log10(upper) - logLower) * height);
			} else if (categoryAxisEnabled) {
		...
		if (isHorizontalAxis) {
			if (logScaleEnabled) {
				...
				dataCoordinate = (pixelCoordinate == 0) ? min : Math.pow(10,
						pixelCoordinate / (double) width
								* (Math.log10(upper) - logLower) + logLower);
			} else if (categoryAxisEnabled) {
			...
			if (logScaleEnabled) {
				double logLower = Math.log10((lower == 0) ? Math.min(0.1,
						upper / 10.0) : lower);
				dataCoordinate = (pixelCoordinate == height) ? min : Math.pow(
						10, Math.log10(upper) - pixelCoordinate
								/ (double) height
								* (Math.log10(upper) - logLower));
			} else if (categoryAxisEnabled) {
