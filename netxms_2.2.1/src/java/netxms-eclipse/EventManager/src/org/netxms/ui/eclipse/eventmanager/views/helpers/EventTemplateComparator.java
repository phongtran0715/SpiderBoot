...
public class EventTemplateComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		case EventConfigurator.COLUMN_CODE:
			result = (int) (((EventTemplate) e1).getCode() - ((EventTemplate) e2)
					.getCode());
			break;
		case EventConfigurator.COLUMN_NAME:
			result = ((EventTemplate) e1).getName().compareToIgnoreCase(
					((EventTemplate) e2).getName());
			break;
		case EventConfigurator.COLUMN_SEVERITY:
			result = ((EventTemplate) e1).getSeverity().compareTo(
					((EventTemplate) e2).getSeverity());
			break;
		case EventConfigurator.COLUMN_FLAGS:
			result = ((EventTemplate) e1).getFlags()
					- ((EventTemplate) e2).getFlags();
			break;
		case EventConfigurator.COLUMN_MESSAGE:
			result = ((EventTemplate) e1).getMessage().compareToIgnoreCase(
					((EventTemplate) e2).getMessage());
			break;
		case EventConfigurator.COLUMN_DESCRIPTION:
			result = ((EventTemplate) e1).getDescription().compareToIgnoreCase(
					((EventTemplate) e2).getDescription());
			break;
		default:
			result = 0;
			break;
