...
public class EventSelectionDialog extends Dialog {
	...
	protected void configureShell(Shell newShell) {
		...
		try {
			newShell.setSize(
					settings.getInt("SelectEvent.cx"), settings.getInt("SelectEvent.cy")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected Control createDialogArea(Composite parent) {
		...
		final String[] names = { Messages.get().EventSelectionDialog_Code,
				Messages.get().EventSelectionDialog_Name };
		final int[] widths = { 80, 350 };
		eventList = new SortableTableViewer(dialogArea, names, widths, 1,
				SWT.UP, SWT.BORDER | SWT.FULL_SELECTION
		...
		eventList.setInput(ConsoleSharedData.getSession()
	...
	protected void okPressed() {
		final IStructuredSelection selection = (IStructuredSelection) eventList
				.getSelection();
