...
public class SyslogMonitorFilter extends AbstractTraceViewFilter {
	...
	public boolean select(Viewer viewer, Object parentElement, Object element) {
		...
		if (((SyslogRecord) element).getMessage().toLowerCase()
				.contains(filterString))
			return true;

		if (((SyslogRecord) element).getHostname().toLowerCase()
				.contains(filterString))
			return true;

		if (((SyslogRecord) element).getTag().toLowerCase()
				.contains(filterString))
			return true;

		AbstractObject object = session.findObjectById(((SyslogRecord) element)
				.getSourceObjectId());
		if (object != null) {
