...
/**
 * Event selector widget. Provides uniform way to display selected event and
 * change selection.
 */
public class EventSelector extends AbstractSelector {
	...
	protected void selectionButtonHandler() {
		...
		if (dlg.open() == Window.OK) {
			...
			if (events.length > 0) {
				...
				setImage(StatusDisplayInfo.getStatusImage(events[0]
						.getSeverity()));
	...
	public void setEventCode(long eventCode) {
		...
		if (eventCode != 0) {
			EventTemplate event = ((NXCSession) ConsoleSharedData.getSession())
