...
public class EventConfigurator extends ViewPart implements SessionListener {
	...
	public void createPartControl(Composite parent) {
		...
		final String[] names = { Messages.get().EventConfigurator_ColCode,
				Messages.get().EventConfigurator_ColName,
				Messages.get().EventConfigurator_ColSeverity,
				Messages.get().EventConfigurator_ColFlags,
				Messages.get().EventConfigurator_ColMessage,
				Messages.get().EventConfigurator_ColDescription };
		final int[] widths = { 70, 200, 90, 50, 400, 400 };
		viewer = new SortableTableViewer(parent, names, widths, 0, SWT.UP,
	...
	private void refreshView() {
		new ConsoleJob(Messages.get().EventConfigurator_OpenJob_Title, this,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						eventTemplates = new HashMap<Long, EventTemplate>(
								list.size());
			...
			viewer.getControl().getDisplay().asyncExec(new Runnable() {
				...
				public void run() {
					...
					if (oldTmpl != null) {
						...
						eventTemplates.put(n.getSubCode(),
								(EventTemplate) n.getObject());
						viewer.setInput(eventTemplates.values().toArray());
					}
				}
			});
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.eventmanager.contexts.EventConfigurator"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionNew = new Action(Messages.get().EventConfigurator_NewEvent,
		...
		actionNew
				.setActionDefinitionId("org.netxms.ui.eclipse.eventmanager.commands.new_event_template"); //$NON-NLS-1$
		handlerService.activateHandler(actionNew.getActionDefinitionId(),
				new ActionHandler(actionNew));

		actionEdit = new Action(Messages.get().EventConfigurator_Properties,
				SharedIcons.EDIT) { //$NON-NLS-1$
			@Override
			public void run() {
				editEventTemplate();
			}
		...
		actionDelete = new Action(Messages.get().EventConfigurator_Delete,
		...
		actionShowFilter = new Action(
				Messages.get().EventConfigurator_ShowFilter,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.eventmanager.commands.show_filter"); //$NON-NLS-1$
	...
	protected void createNewEventTemplate() {
		...
		EditEventTemplateDialog dlg = new EditEventTemplateDialog(getSite()
				.getShell(), etmpl, false);
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().EventConfigurator_CreateJob_Title,
	...
	protected void editEventTemplate() {
		...
		final EventTemplate etmpl = new EventTemplate(
				(EventTemplate) selection.getFirstElement());
		EditEventTemplateDialog dlg = new EditEventTemplateDialog(getSite()
				.getShell(), etmpl, false);
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().EventConfigurator_UpdateJob_Title,
					this, Activator.PLUGIN_ID, JOB_FAMILY) {
				@Override
				protected String getErrorMessage() {
	...
	protected void deleteEventTemplate() {
		...
		final String message = ((selection.size() > 1) ? Messages.get().EventConfigurator_DeleteConfirmation_Plural
				: Messages.get().EventConfigurator_DeleteConfirmation_Singular);
		final Shell shell = getViewSite().getShell();
		if (!MessageDialogHelper.openQuestion(shell,
				Messages.get().EventConfigurator_DeleteConfirmationTitle,
				message)) {
			return;
		}

		new ConsoleJob(Messages.get().EventConfigurator_DeleteJob_Title, this,
				Activator.PLUGIN_ID, JOB_FAMILY) {
			@Override
			protected String getErrorMessage() {
