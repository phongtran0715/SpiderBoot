...
public class EditEventTemplateDialog extends Dialog {
	 ...
	 * @param parentShell
	 */
	public EditEventTemplateDialog(Shell parentShell, EventTemplate object,
			boolean isNew) {
		super(parentShell);
		this.object = object;
		this.isNew = isNew;
	...
	protected Control createDialogArea(Composite parent) {
		...
		severity = WidgetHelper.createLabeledCombo(dialogArea, SWT.DROP_DOWN
				| SWT.READ_ONLY,
				Messages.get().EditEventTemplateDialog_Severity, gd);
		...
		optionLog
				.setSelection((object.getFlags() & EventTemplate.FLAG_WRITE_TO_LOG) != 0);
		gd = new GridData();
		...
		description = new LabeledText(dialogArea, SWT.NONE, SWT.MULTI
				| SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL | SWT.WRAP);
		description
				.setLabel(Messages.get().EditEventTemplateDialog_Description);
	...
	protected void okPressed() {
		...
		object.setFlags(optionLog.getSelection() ? EventTemplate.FLAG_WRITE_TO_LOG
				: 0);
		super.okPressed();
	}
	...
	protected void configureShell(Shell newShell) {
		newShell.setText(isNew ? Messages.get().EditEventTemplateDialog_TitleCreate
				: Messages.get().EditEventTemplateDialog_TitleEdit);
