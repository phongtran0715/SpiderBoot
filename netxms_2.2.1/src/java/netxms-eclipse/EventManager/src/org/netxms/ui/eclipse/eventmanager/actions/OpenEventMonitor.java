 ...
 */
public class OpenEventMonitor implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		if (window != null) {
			try {
				window.getActivePage().showView(EventMonitor.ID);
			} catch (PartInitException e) {
				MessageDialogHelper.openError(
						window.getShell(),
						Messages.get().OpenEventMonitor_Error,
						Messages.get().OpenEventMonitor_ErrorText
								+ e.getMessage());
			}
		}
	}
