 ...
 * Syslog trace widget
 */
public class SyslogTraceWidget extends AbstractTraceWidget implements
		SessionListener {
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.widgets.AbstractTraceWidget#setupViewer(org.eclipse
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.widgets.AbstractTraceWidget#getDialogSettings()
	 */
	@Override
	...
	protected void createActions() {
		...
		actionShowColor = new Action(
				Messages.get().SyslogMonitor_ShowStatusColors,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowIcons = new Action(
				Messages.get().SyslogMonitor_ShowStatusIcons,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
	...
	public void setRootObject(long objectId) {
		...
		if (objectId == 0)
			return;
		setFilter(session.findObjectById(objectId).getObjectName()
				.toLowerCase());
	}
}
