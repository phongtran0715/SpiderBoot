...
public class AlarmCategoryList extends Composite implements SessionListener {
	 ...
	 * @param showFilter
	 */
	public AlarmCategoryList(ViewPart viewPart, Composite parent, int style,
			final String configPrefix, boolean showFilter) {
	 ...
	 */
	public AlarmCategoryList(Composite parent, int style,
			final String configPrefix, boolean showCloseButton) {
		super(parent, style);
		...
		viewer = new SortableTableViewer(this, names, widths, 1, SWT.UP,
				SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI | SWT.H_SCROLL
						| SWT.V_SCROLL);
		...
		addListener(SWT.Resize, new Listener() {
			...
			public void handleEvent(Event e) {
				viewer.getControl().setBounds(
						AlarmCategoryList.this.getClientArea());
			}
		});
		...
		// Get filter settings
		initShowfilter = safeCast(ds.get(configPrefix + "initShowfilter"),
				ds.getBoolean(configPrefix + "initShowfilter"), initShowfilter);
	...
	public void refreshView() {
		new ConsoleJob("Open alarm category list", null, Activator.PLUGIN_ID,
				null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
				...
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						alarmCategories = new HashMap<Long, AlarmCategory>(
								list.size());
	...
	private void createActions() {
		...
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.alarmviewer.commands.show_filter"); //$NON-NLS-1$
	}
	...
	public void createCategory() {
		AlarmCategoryEditor editor = new AlarmCategoryEditor(
				new AlarmCategory());
		PropertyDialog dlg = PropertyDialog.createDialogOn(getShell(), null,
				editor);
		dlg.open();
	}
	...
	public void editCategory() {
		...
		AlarmCategoryEditor editor = new AlarmCategoryEditor(
				(AlarmCategory) selection.getFirstElement());
		PropertyDialog dlg = PropertyDialog.createDialogOn(getShell(), null,
				editor);
		dlg.open();
	}
	...
	private void deleteCategory() {
		...
		if (!MessageDialogHelper.openQuestion(getShell(),
				"Confirm category deletion", message))
			return;

		new ConsoleJob("Delete alarm category database entries", viewPart,
	 ...
	 * 
	 * @see
	 * org.netxms.client.SessionListener#notificationHandler(org.netxms.client
	 * .SessionNotification)
	 */
	@Override
			...
			viewer.getControl().getDisplay().asyncExec(new Runnable() {
				...
				public void run() {
					...
					if (oldCat != null) {
						...
						alarmCategories.put(n.getSubCode(),
								(AlarmCategory) n.getObject());
						viewer.setInput(alarmCategories.values().toArray());
					}
				}
			});
