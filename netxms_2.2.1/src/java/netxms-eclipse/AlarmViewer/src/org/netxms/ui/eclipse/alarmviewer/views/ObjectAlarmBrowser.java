...
/**
 * Alarm browser view - alarms for specific objects. List of object IDs is
 * passed as a '&' separated secondaryId
 */
public class ObjectAlarmBrowser extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		for (String id : site.getSecondaryId().split("&")) //$NON-NLS-1$
		{
			try {
				...
				Activator.logError(
						"Invalid number in AlarmViewer secondary ID", e); //$NON-NLS-1$
			}
		}
	}
	...
	public void createPartControl(Composite parent) {
		...
		alarmView = new AlarmList(this, content.getContent(), SWT.NONE,
				"ObjectAlarmBrowser", null); //$NON-NLS-1$
		alarmView.setRootObjects(objects);

		if (objects.size() == 1) {
			NXCSession session = (NXCSession) ConsoleSharedData.getSession();
			setPartName(String.format(Messages.get().ObjectAlarmBrowser_Title,
					session.getObjectName(objects.get(0))));
			// content.hideMessage();
		} else {
	...
	private void showObjectList() {
		...
		content.showMessage(CompositeWithMessageBar.INFORMATION, String.format(
				Messages.get().ObjectAlarmBrowser_SelectedObjects,
				sb.toString()));
	}
	...
	private void createActions() {
		...
		actionExportToCsv = new ExportToCsvAction(this, alarmView.getViewer(),
				false);
	}
