...
public class AlarmCategoryListComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		case AlarmCategoryList.COLUMN_ID:
			rc = Long.signum(((AlarmCategory) e1).getId()
					- ((AlarmCategory) e2).getId());
			break;
		case AlarmCategoryList.COLUMN_NAME:
			rc = (((AlarmCategory) e1).getName()
					.compareToIgnoreCase(((AlarmCategory) e2).getName()));
			break;
		case AlarmCategoryList.COLUMN_DESCRIPTION:
			rc = (((AlarmCategory) e1).getDescription()
					.compareToIgnoreCase(((AlarmCategory) e2).getDescription()));
			break;
		default:
			rc = 0;
			break;
