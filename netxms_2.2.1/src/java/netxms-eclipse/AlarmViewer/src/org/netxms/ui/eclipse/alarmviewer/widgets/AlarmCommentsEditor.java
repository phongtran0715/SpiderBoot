...
public class AlarmCommentsEditor extends Composite {
	 ...
	 * @param comment
	 */
	public AlarmCommentsEditor(Composite parent, FormToolkit toolkit,
			ImageCache imageCache, AlarmComment comment,
			HyperlinkAdapter editAction, HyperlinkAdapter deleteAction) {
		...
		final AbstractUserObject userObject = session
				.findUserDBObjectById(comment.getUserId());

		final CLabel user = new CLabel(this, SWT.NONE);
		toolkit.adapt(user);
		user.setImage(imageCache.add(Activator
				.getImageDescriptor("icons/user.png"))); //$NON-NLS-1$
		user.setText((userObject != null) ? userObject.getName() : Messages
				.get().AlarmCommentsEditor_Unknown);
		GridData gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = false;
		user.setLayoutData(gd);

		final Label time = toolkit.createLabel(this, RegionalSettings
				.getDateTimeFormat().format(comment.getLastChangeTime()));
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		time.setLayoutData(gd);

		final ImageHyperlink linkEdit = toolkit.createImageHyperlink(this,
				SWT.NONE);
		...
		final ImageHyperlink linkDelete = toolkit.createImageHyperlink(this,
				SWT.NONE);
