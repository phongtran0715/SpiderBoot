 ...
 */
public class EventTreeLabelProvider extends LabelProvider implements
		ITableLabelProvider {
	...
	public Image getColumnImage(Object element, int columnIndex) {
		if (columnIndex == AlarmDetails.EV_COLUMN_SEVERITY)
			return StatusDisplayInfo.getStatusImage(((EventInfo) element)
					.getSeverity());
		return null;
	}
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		case AlarmDetails.EV_COLUMN_SEVERITY:
			return StatusDisplayInfo.getStatusText(((EventInfo) element)
					.getSeverity());
		case AlarmDetails.EV_COLUMN_SOURCE:
			AbstractObject o = session.findObjectById(((EventInfo) element)
					.getSourceObjectId());
			return (o != null) ? o.getObjectName()
					: ("[" + ((EventInfo) element).getSourceObjectId() + "]"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		case AlarmDetails.EV_COLUMN_TIMESTAMP:
			return RegionalSettings.getDateTimeFormat().format(
					((EventInfo) element).getTimeStamp());
		}
		return null;
	}
}
