...
public class AlarmLogViewer extends LogViewer {
	...
	protected void createActions() {
		super.createActions();
		actionCreateIssue = new Action(
				"Create &ticket in helpdesk system", Activator.getImageDescriptor("icons/helpdesk_ticket.png")) { //$NON-NLS-1$
			@Override
			public void run() {
				createIssue();
			}
		...
		actionShowIssue = new Action("Show helpdesk ticket in &web browser",
				SharedIcons.BROWSER) {
			@Override
			public void run() {
				showIssue();
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.logviewer.views.LogViewer#fillContextMenu(org.eclipse
	...
	protected void fillContextMenu(final IMenuManager mgr) {
		...
		if (selection.size() == 0 && session.isHelpdeskLinkActive()) {
			try {
				Alarm alarm = session.getAlarm(Long
						.parseLong(((TableRow) selection.getFirstElement())
								.get(0).getValue()));
	...
	private void createIssue() {
		...
		final long id = Long.parseLong(((TableRow) selection.getFirstElement())
				.get(0).getValue());
						...
						try {
							...
							MessageDialogHelper
									.openError(getSite().getShell(), "Error",
											"Internal error: unable to open web browser");
						}
					}
				});
			}
	...
	private void unlinkIssue() {
		...
		final long id = Long.parseLong(((TableRow) selection.getFirstElement())
				.get(0).getValue());
		new ConsoleJob("Unlink alarm from helpdesk ticket", this,
