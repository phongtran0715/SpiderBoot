...
public class AccessControl extends PropertyPage {
	...
	private HashMap<Long, AbstractUserObject> accessMap = new HashMap<Long, AbstractUserObject>(
			0);
	...
	protected Control createContents(Composite parent) {
		...
		editor = (AlarmCategoryEditor) getElement().getAdapter(
				AlarmCategoryEditor.class);
		...
		userList = new SortableTableViewer(dialogArea, columnNames,
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (dlg.open() == Window.OK) {
					...
					userList.setInput(accessMap.values().toArray(
							new AbstractUserObject[accessMap.size()]));
				}
			}
		...
		deleteButton.addSelectionListener(new SelectionListener() {
			...
			@Override
			public void widgetSelected(SelectionEvent e) {
				IStructuredSelection selecion = (IStructuredSelection) userList
						.getSelection();
		...
		for (long userId : category.getAccessControl()) {
			final AbstractUserObject user = session
					.findUserDBObjectById(userId);
			if (user != null) {
				accessMap.put(user.getId(), user);
			}
		}
