...
public class AlarmStateChangeFailureDialog extends Dialog {
	 ...
	 * @param parentShell
	 */
	public AlarmStateChangeFailureDialog(Shell parentShell,
			Map<Long, Integer> failures) {
		super(parentShell);
	...
	protected Control createDialogArea(Composite parent) {
		...
		alarmList = new SortableTableViewer(dialogArea, names, widths, 0,
				SWT.UP, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI
						| SWT.V_SCROLL);
	 ...
	 */
	private class FailuresLabelProvider extends LabelProvider implements
			ITableLabelProvider {
		@Override
		...
		public String getColumnText(Object element, int columnIndex) {
			return (columnIndex == 0) ? ((Entry<?, ?>) element).getKey()
					.toString() : RCC.getText((Integer) ((Entry<?, ?>) element)
					.getValue(), Locale.getDefault().getLanguage(), null);
		}
	}
