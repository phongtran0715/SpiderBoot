...
public class AlarmCategorySelector extends AbstractSelector {
	...
	protected void selectionButtonHandler() {
		AlarmCategorySelectionDialog dlg = new AlarmCategorySelectionDialog(
				getShell());
	...
	public void setCategoryId(List<Long> categoryId) {
		...
		if (categoryId != null && !categoryId.isEmpty()) {
			if (categoryId.get(0) == 0) {
				...
				List<AlarmCategory> categories = ((NXCSession) ConsoleSharedData
						.getSession()).findMultipleAlarmCategories(categoryId);
				if (categories != null) {
					StringBuilder sb = new StringBuilder();
