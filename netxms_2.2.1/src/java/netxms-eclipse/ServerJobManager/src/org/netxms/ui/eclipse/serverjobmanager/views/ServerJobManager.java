...
public class ServerJobManager extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		final String[] names = { Messages.get().ServerJobManager_ColStatus,
				Messages.get().ServerJobManager_ColInitiator,
				Messages.get().ServerJobManager_ColNode,
				Messages.get().ServerJobManager_ColDescription,
				Messages.get().ServerJobManager_ColProgress,
				Messages.get().ServerJobManager_ColMessage };
		...
		// Create listener for notifications received from server via client
		// library
	...
	private void createActions() {
		...
		actionCancelJob.setImageDescriptor(Activator
				.getImageDescriptor("icons/cancel.png")); //$NON-NLS-1$
		...
		actionHoldJob.setImageDescriptor(Activator
				.getImageDescriptor("icons/hold.gif")); //$NON-NLS-1$
		...
		actionUnholdJob.setImageDescriptor(Activator
				.getImageDescriptor("icons/unhold.gif")); //$NON-NLS-1$
	...
	private void refreshJobList(boolean userInitiated) {
		ConsoleJob job = new ConsoleJob(
				Messages.get().ServerJobManager_RefreshJobName, this,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						...
						// Build new list of selected jobs - add object to
						// selection if
						// object with same id was selected before
						List<ServerJob> selectedJobs = new ArrayList<ServerJob>(
								selectedObjects.length);
						for (int i = 0; i < selectedObjects.length; i++) {
							for (int j = 0; j < jobList.length; j++) {
								if (((ServerJob) selectedObjects[i]).getId() == jobList[j]
										.getId()) {
	...
	private void cancelServerJob() {
		doJobAction(Messages.get().ServerJobManager_ActionName_Cancel,
				Messages.get().ServerJobManager_ActionErrorName_Cancel,
				CANCEL_JOB);
	}
	...
	private void holdServerJob() {
		doJobAction(Messages.get().ServerJobManager_ActionName_Hold,
				Messages.get().ServerJobManager_ActionErrorName_Hold, HOLD_JOB);
	}
	...
	private void unholdServerJob() {
		doJobAction(Messages.get().ServerJobManager_ActionName_Unhold,
				Messages.get().ServerJobManager_ActionErrorName_Unhold,
				UNHOLD_JOB);
	}
	 ...
	 * @param actionId
	 */
	private void doJobAction(final String actionName,
			final String actionErrorName, final int actionId) {
		final IStructuredSelection selection = (IStructuredSelection) viewer
				.getSelection();
		new ConsoleJob(String.format(
				Messages.get().ServerJobManager_ActionJobName, actionName),
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().ServerJobManager_ActionJobError,
						actionErrorName);
			}
