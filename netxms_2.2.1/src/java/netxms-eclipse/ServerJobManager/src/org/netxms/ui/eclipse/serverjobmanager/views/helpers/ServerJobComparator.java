...
public class ServerJobComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		case ServerJobManager.COLUMN_STATUS:
			result = ((ServerJob) e1).getStatus()
					- ((ServerJob) e2).getStatus();
			break;
		case ServerJobManager.COLUMN_NODE:
			AbstractObject object1 = session.findObjectById(((ServerJob) e1)
					.getNodeId());
			AbstractObject object2 = session.findObjectById(((ServerJob) e2)
					.getNodeId());
			String name1 = (object1 != null) ? object1.getObjectName()
					: "<unknown>"; //$NON-NLS-1$
			String name2 = (object2 != null) ? object2.getObjectName()
					: "<unknown>"; //$NON-NLS-1$
			result = name1.compareToIgnoreCase(name2);
			break;
		case ServerJobManager.COLUMN_DESCRIPTION:
			result = ((ServerJob) e1).getDescription().compareToIgnoreCase(
					((ServerJob) e2).getDescription());
			break;
		case ServerJobManager.COLUMN_MESSAGE:
			result = ((ServerJob) e1).getFailureMessage().compareToIgnoreCase(
					((ServerJob) e2).getFailureMessage());
			break;
		default:
			result = 0;
			break;
