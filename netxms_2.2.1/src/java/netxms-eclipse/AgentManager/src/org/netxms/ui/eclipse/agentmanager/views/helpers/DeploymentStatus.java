...
public class DeploymentStatus {
	...
	public DeploymentStatus(long nodeId, int status, String message) {
		this.nodeId = nodeId;
		this.nodeObject = (AbstractNode) ((NXCSession) ConsoleSharedData
				.getSession()).findObjectById(nodeId, AbstractNode.class);
		this.status = status;
	...
	public String getNodeName() {
		if (nodeObject == null) {
			nodeObject = (AbstractNode) ((NXCSession) ConsoleSharedData
					.getSession()).findObjectById(nodeId, AbstractNode.class);
		}
		return (nodeObject != null) ? nodeObject.getObjectName()
				: ("[" + Long.toString(nodeId) + "]"); //$NON-NLS-1$ //$NON-NLS-2$
	}
