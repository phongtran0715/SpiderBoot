...
public class TunnelManagerFilter extends ViewerFilter {
	...
	public boolean select(Viewer viewer, Object parentElement, Object element) {
		...
		if ((filterString == null) || (filterString.isEmpty()))
			...
			return true;
		else if (t.isBound() ? Integer.toString(t.getActiveChannelCount())
				.toLowerCase().contains(filterString) : false)
			return true;
		else if (Integer.toString(t.getId()).toLowerCase()
				.contains(filterString))
			return true;
		else if (t.getAddress().getHostAddress().toLowerCase()
				.contains(filterString))
			return true;
		else if (t.isBound() ? ConsoleSharedData.getSession()
				.getObjectName(t.getNodeId()).toLowerCase()
				.contains(filterString) : false)
			return true;
		else if (t.getPlatformName().toLowerCase().contains(filterString))
			return true;
		else if (t.isBound() ? "bound".contains(filterString) : "unbound"
				.contains(filterString))
			return true;
