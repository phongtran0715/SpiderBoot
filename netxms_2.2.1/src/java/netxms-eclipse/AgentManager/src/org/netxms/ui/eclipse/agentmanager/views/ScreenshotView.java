...
public class ScreenshotView extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		setPartName(String.format(Messages.get().ScreenshotView_PartTitle,
				session.getObjectName(nodeId)));
	}
		...
		canvas.addPaintListener(new PaintListener() {
			public void paintControl(PaintEvent e) {
				...
				if (image != null) {
					...
					gc.setForeground(canvas.getDisplay().getSystemColor(
							SWT.COLOR_DARK_RED));
	...
	private void updateScrollerSize() {
		if (image != null) {
			...
			scroller.setMinSize(new Point(Math.max(d.width, pt.x + 20),
					d.height + pt.y + 15));
		} else if (errorMessage != null) {
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.agentmanager.context.ScreenshotView"); //$NON-NLS-1$
		}
	}
	...
	public void refresh() {
		...
		ConsoleJob job = new ConsoleJob(Messages.get().ScreenshotView_JobTitle,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				try {
					if (userSession == null) {
						Table sessions = session.queryAgentTable(nodeId,
								"Agent.SessionAgents"); //$NON-NLS-1$
						if ((sessions != null) && (sessions.getRowCount() > 0)) {
							int colIndexName = sessions
									.getColumnIndex("SESSION_NAME"); //$NON-NLS-1$
							int colIndexUser = sessions
									.getColumnIndex("USER_NAME"); //$NON-NLS-1$
							for (int i = 0; i < sessions.getRowCount(); i++) {
								String n = sessions.getCellValue(i,
										colIndexName);
								if ("Console".equalsIgnoreCase(n)) //$NON-NLS-1$
								{
									userSession = n;
									userName = sessions.getCellValue(i,
											colIndexUser);
									break;
								}
							}
							...
							if (userSession == null) {
								// Console session not found, use first
								// available
								userSession = sessions.getCellValue(0,
										colIndexName);
								userName = sessions.getCellValue(0,
										colIndexUser);
							}
						}
					}
					...
					final ImageData data = new ImageData(
							new ByteArrayInputStream(byteImage));
					runInUIThread(new Runnable() {
						@Override
						public void run() {
							if (image != null)
					...
					runInUIThread(new Runnable() {
						...
						public void run() {
							...
							image = null;
							errorMessage = (emsg != null) ? String.format(
									Messages.get().ScreenshotView_ErrorWithMsg,
									emsg)
									: Messages.get().ScreenshotView_ErrorWithoutMsg;
							canvas.redraw();
	...
	private void createActions() {
		...
		actionCopyToClipboard = new Action(
				Messages.get().ScreenshotView_CopyToClipboard, SharedIcons.COPY) {
			@Override
			public void run() {
				if (image == null)
					return;
		...
		actionCopyToClipboard
				.setActionDefinitionId("org.netxms.ui.eclipse.agentmanager.commands.copy_screenshot"); //$NON-NLS-1$
		handlerService.activateHandler(
				actionCopyToClipboard.getActionDefinitionId(),
				new ActionHandler(actionCopyToClipboard));
		actionCopyToClipboard.setEnabled(false);

		actionSave = new Action(Messages.get().ScreenshotView_Save,
				SharedIcons.SAVE) {
			@Override
			public void run() {
				saveImage();
		...
		actionSave
				.setActionDefinitionId("org.netxms.ui.eclipse.agentmanager.commands.save_screenshot"); //$NON-NLS-1$
	...
	private void saveImage() {
		...
		fd.setFilterNames(new String[] {
				Messages.get().ScreenshotView_PngFiles,
				Messages.get().ScreenshotView_AllFiles });
		...
		try {
			...
			MessageDialogHelper.openError(getViewSite().getShell(), Messages
					.get().ScreenshotView_Error, String.format(
					Messages.get().ScreenshotView_CannotCreateFile, name,
		...
		try {
			...
			MessageDialogHelper.openError(getViewSite().getShell(), Messages
					.get().ScreenshotView_Error, String.format(
					Messages.get().ScreenshotView_CannotSaveImage, name,
					e.getLocalizedMessage()));
		}
	}
