 ...
 */
public class DeploymentStatusLabelProvider extends LabelProvider implements
		ITableLabelProvider {
	private static final String[] statusText = {
			Messages.get().DeploymentStatusLabelProvider_Pending,
			Messages.get().DeploymentStatusLabelProvider_Uploading,
			Messages.get().DeploymentStatusLabelProvider_Installing,
			Messages.get().DeploymentStatusLabelProvider_Completed,
			Messages.get().DeploymentStatusLabelProvider_Failed,
			Messages.get().DeploymentStatusLabelProvider_Init };
	...
	public DeploymentStatusLabelProvider() {
		...
		imageActive = Activator
				.getImageDescriptor("icons/active.gif").createImage(); //$NON-NLS-1$
		imagePending = Activator
				.getImageDescriptor("icons/pending.gif").createImage(); //$NON-NLS-1$
		imageCompleted = Activator
				.getImageDescriptor("icons/complete.png").createImage(); //$NON-NLS-1$
		imageFailed = Activator
				.getImageDescriptor("icons/failed.png").createImage(); //$NON-NLS-1$
	}
	...
	public Image getColumnImage(Object element, int columnIndex) {
		...
		case PackageDeploymentMonitor.COLUMN_NODE:
			...
			return (node != null) ? workbenchLabelProvider.getImage(node)
					: SharedIcons.IMG_UNKNOWN_OBJECT;
