...
public class PackageManager extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		final String[] names = { Messages.get().PackageManager_ColumnID,
				Messages.get().PackageManager_ColumnName,
				Messages.get().PackageManager_ColumnVersion,
				Messages.get().PackageManager_ColumnPlatform,
				Messages.get().PackageManager_ColumnFile,
				Messages.get().PackageManager_ColumnDescription };
		final int[] widths = { 70, 120, 90, 120, 150, 400 };
		viewer = new SortableTableViewer(parent, names, widths, COLUMN_ID,
				SWT.UP, SWT.FULL_SELECTION | SWT.MULTI);
		...
		new ConsoleJob(Messages.get().PackageManager_OpenDatabase, this,
	...
	private void createActions() {
		...
		actionInstall = new Action(Messages.get().PackageManager_InstallAction,
		...
		actionRemove = new Action(Messages.get().PackageManager_RemoveAction,
		...
		actionDeploy = new Action(Messages.get().PackageManager_DeployAction,
				Activator.getImageDescriptor("icons/package_deploy.gif")) { //$NON-NLS-1$
			@Override
			public void run() {
				deployPackage();
			}
	...
	private void refresh() {
		...
		new ConsoleJob(Messages.get().PackageManager_LoadPkgList, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	private void installPackage() {
		...
		fd.setFilterNames(new String[] {
				Messages.get().PackageManager_FileTypePackage,
				Messages.get().PackageManager_FileTypeAll });
		...
		if (npiName != null) {
			try {
				...
				final NXCSession session = (NXCSession) ConsoleSharedData
						.getSession();
				new ConsoleJob(Messages.get().PackageManager_InstallPackage,
						this, Activator.PLUGIN_ID, null) {
					@Override
					protected void runInternal(final IProgressMonitor monitor)
							throws Exception {
						final long id = session.installPackage(p, new File(
								npiFile.getParent(), p.getFileName()),
								new ProgressListener() {
									...
									public void setTotalWorkAmount(long amount) {
										monitor.beginTask(
												Messages.get().PackageManager_UploadPackage,
												(int) amount);
									}
				...
				MessageDialogHelper.openError(
						getSite().getShell(),
						Messages.get().PackageManager_Error,
						Messages.get().PackageManager_PkgFileOpenError
	...
	private void removePackage() {
		if (!MessageDialogHelper.openConfirm(getSite().getShell(),
				Messages.get().PackageManager_ConfirmDeleteTitle,
				Messages.get().PackageManager_ConfirmDeleteText))
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		final Object[] packages = ((IStructuredSelection) viewer.getSelection())
				.toArray();
		final List<Object> removedPackages = new ArrayList<Object>();
		new ConsoleJob(Messages.get().PackageManager_DeletePackages, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	private void deployPackage() {
		...
		ConsoleJob job = new ConsoleJob(
				Messages.get().PackageManager_DeployAgentPackage, null,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				session.deployPackage(pkg.getId(),
						objects.toArray(new Long[objects.size()]),
						new PackageDeploymentListener() {
							private PackageDeploymentMonitor monitor = null;

							@Override
							public void statusUpdate(long nodeId, int status,
									String message) {
								if (monitor != null)
									monitor.viewStatusUpdate(nodeId, status,
											message);
							}
							...
							public void deploymentStarted() {
								...
								synchronized (sync) {
									runInUIThread(new Runnable() {
										...
										public void run() {
											try {
												monitor = (PackageDeploymentMonitor) PlatformUI
														.getWorkbench()
														.getActiveWorkbenchWindow()
														.getActivePage()
														.showView(
																PackageDeploymentMonitor.ID,
																toString(),
																IWorkbenchPage.VIEW_ACTIVATE);
												monitor.setPackageId(pkg
														.getId());
												...
												MessageDialogHelper
														.openError(
																getSite()
																		.getShell(),
																Messages.get().PackageManager_Error,
																Messages.get().PackageManager_ErrorOpenView
																		+ e.getLocalizedMessage());
											}
											synchronized (sync) {
												sync.notify();
								...
								runInUIThread(new Runnable() {
									...
									public void run() {
										MessageDialogHelper.openInformation(
												getSite().getShell(),
												Messages.get().PackageManager_Information,
												Messages.get().PackageManager_PkgDepCompleted);
									}
								});
							}
						});
	...
	public void dispose() {
		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().PackageManager_UnlockDatabase, null,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
