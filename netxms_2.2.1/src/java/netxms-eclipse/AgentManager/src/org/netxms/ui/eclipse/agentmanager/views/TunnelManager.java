...
public class TunnelManager extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		final String[] names = { "ID", "State", "Node", "IP address",
				"Channels", "System name", "Platform", "System information",
				"Agent version" };
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.agentmanager.context.TunnelManager"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.agentmanager.commands.show_filter"); //$NON-NLS-1$
	...
	protected void fillContextMenu(IMenuManager manager) {
		IStructuredSelection selection = (IStructuredSelection) viewer
				.getSelection();
		if ((selection.size() == 1)
				&& !((AgentTunnel) selection.getFirstElement()).isBound()) {
	...
	private void refresh() {
		...
		new ConsoleJob("Get list of active agent tunnels", this,
	...
	private void createNode() {
		...
		final NXCObjectCreationData cd = new NXCObjectCreationData(
				AbstractObject.OBJECT_NODE, dlg.getObjectName(), 2);
		...
		final NXCSession session = ConsoleSharedData.getSession();
		new ConsoleJob("Create new node and bind tunnel", this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	private void unbindTunnel() {
		...
		if (!MessageDialogHelper.openQuestion(getSite().getShell(),
				"Unbind Tunnel",
				"Selected tunnels will be unbound. Are you sure?"))
			return;
