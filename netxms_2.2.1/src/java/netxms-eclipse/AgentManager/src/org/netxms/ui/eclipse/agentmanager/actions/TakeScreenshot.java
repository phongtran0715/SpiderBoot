...
/**
 * Take screenshot from remote node via agent
 */
public class TakeScreenshot implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		if (window != null) {
			try {
				window.getActivePage().showView(ScreenshotView.ID,
						Long.toString(node.getObjectId()),
						IWorkbenchPage.VIEW_ACTIVATE);
			} catch (PartInitException e) {
				MessageDialogHelper.openError(window.getShell(),
						Messages.get().TakeScreenshot_Error, String.format(
								Messages.get().TakeScreenshot_ErrorOpeningView,
