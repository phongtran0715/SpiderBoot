 ...
 */
public class ServerStoredAgentConfigEditorView extends ViewPart implements
		ISaveablePart2 {
	...
	public void createPartControl(Composite parent) {
		...
		configList = new TableViewer(listContainer, SWT.FULL_SELECTION
				| SWT.SINGLE);
		...
		configList.getControl().setLayoutData(
				new GridData(SWT.FILL, SWT.FILL, true, true));

		new Label(listContainer, SWT.SEPARATOR | SWT.VERTICAL)
		...
		new Label(formContainer, SWT.SEPARATOR | SWT.VERTICAL)
				.setLayoutData(new GridData(SWT.RIGHT, SWT.FILL, false, true));
		...
		configEditor = new AgentConfigEditor(section, SWT.BORDER, SWT.H_SCROLL
				| SWT.V_SCROLL, 0);
	...
	private void onSelectionChange(SelectionChangedEvent event) {
		...
		if (modified) {
			SaveStoredConfigDialog dlg = new SaveStoredConfigDialog(getSite()
					.getShell());
			int rc = dlg.open();
		...
		final ConfigListElement element = (ConfigListElement) selection
				.getFirstElement();

		new ConsoleJob(
				Messages.get().ServerStoredAgentConfigEditorView_JobTitle_GetContent,
    ...
    */
	...
	private void createActions() {
		...
		actionSave = new Action(Messages.get().AgentConfigEditorView_Save,
				SharedIcons.SAVE) {
			@Override
			public void run() {
				intermediateSave();
		...
		actionCreate = new Action(
				Messages.get().ServerStoredAgentConfigEditorView_JobTitle_CreateNew,
		...
		actionDelete = new Action(
				Messages.get().ServerStoredAgentConfigEditorView_Delete,
		...
		actionMoveUp = new Action(
				Messages.get().ServerStoredAgentConfigEditorView_MoveUp,
				SharedIcons.UP) {
			@Override
			public void run() {
				moveUp();
		...
		actionMoveDown = new Action(
				Messages.get().ServerStoredAgentConfigEditorView_MoveDown,
				SharedIcons.DOWN) {
			@Override
			public void run() {
				moveDown();
	 ...
	 */
	private void moveUp() {
		IStructuredSelection selection = (IStructuredSelection) configList
				.getSelection();
		if (selection == null)
			return;

		final ConfigListElement element1 = (ConfigListElement) selection
				.getFirstElement();
		...
		new ConsoleJob(
				Messages.get().ServerStoredAgentConfigEditorView_JobMoveUp,
				this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	private void moveDown() {
		...
		new ConsoleJob(
				Messages.get().ServerStoredAgentConfigEditorView_JobMoveDown,
				this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	private void deleteConfig() {
		...
		new ConsoleJob(
				Messages.get().ServerStoredAgentConfigEditorView_JobDelete,
				this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						...
						configList
								.setInput(elements
										.toArray(new ConfigListElement[elements
												.size()]));
					}
				});
			}
	...
	private void createNewConfig() {
		...
		configList.setInput(elements.toArray(new ConfigListElement[elements
				.size()]));
	...
	public void doSave(IProgressMonitor monitor) {
		try {
			...
			MessageDialogHelper.openError(
					...
					Messages.get().AgentConfigEditorView_Error,
					Messages.get().AgentConfigEditorView_SaveError
							+ e.getMessage());
		}
	}
    ...
    */
	public void intermediateSave() {
		...
		new ConsoleJob(
				Messages.get().ServerStoredAgentConfigEditorView_JobSave, this,
	...
	public int promptToSaveOnClose() {
		...
		return (rc == IDialogConstants.CANCEL_ID) ? CANCEL
				: ((rc == SaveConfigDialog.DISCARD_ID) ? NO : YES);
	}
	...
	public void getConfigList() {
		IStructuredSelection selection = (IStructuredSelection) configList
				.getSelection();
		final ConfigListElement element = selection == null ? null
				: (ConfigListElement) selection.getFirstElement();
