...
public class TunnelListComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		case TunnelManager.COL_AGENT_VERSION:
			result = t1.getAgentVersion().compareToIgnoreCase(
					t2.getAgentVersion());
			break;
		...
		case TunnelManager.COL_IP_ADDRESS:
			result = ComparatorHelper.compareInetAddresses(t1.getAddress(),
					t2.getAddress());
			break;
		case TunnelManager.COL_NODE:
			NXCSession session = ConsoleSharedData.getSession();
			result = session.getObjectName(t1.getNodeId()).compareToIgnoreCase(
					session.getObjectName(t2.getNodeId()));
			break;
		case TunnelManager.COL_PLATFORM:
			result = t1.getPlatformName().compareToIgnoreCase(
					t2.getPlatformName());
			break;
		...
		case TunnelManager.COL_SYSINFO:
			result = t1.getSystemInformation().compareToIgnoreCase(
					t2.getSystemInformation());
			break;
