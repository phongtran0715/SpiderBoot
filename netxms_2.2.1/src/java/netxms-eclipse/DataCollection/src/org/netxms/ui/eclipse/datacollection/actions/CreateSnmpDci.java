...
public class CreateSnmpDci implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		MibObject mibObject = MibCache.findObject(objects.get(0).getName(),
				false);
		String description = (mibObject != null) ? mibObject.getName()
				: objects.get(0).getName();
		if (objects.size() > 1)
			description += " @@instance@@"; //$NON-NLS-1$

		final CreateSnmpDciDialog dlg = new CreateSnmpDciDialog(shell,
				description);
		...
		for (SnmpValue v : values) {
			AbstractNode node = (AbstractNode) session.findObjectById(
					v.getNodeId(), AbstractNode.class);
			if (node != null) {
				nodes.add(node);
			}
		}
		...
		for (AbstractNode n : nodes) {
			IViewReference ref = page.findViewReference(
					DataCollectionEditor.ID, Long.toString(n.getObjectId()));
			lockRequired.put(n.getObjectId(),
					!((ref != null) && (ref.getView(false) != null)));
		}

		new ConsoleJob(Messages.get().CreateSnmpDci_JobTitle, viewPart,
			...
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				monitor.beginTask(Messages.get().CreateSnmpDci_TaskTitle,
						values.size());
				for (SnmpValue v : values) {
					final String description = dlg
							.getDescription()
							.replaceAll(
									"@@instance@@", Long.toString(v.getObjectId().getIdFromPos(v.getObjectId().getLength() - 1))); //$NON-NLS-1$
					createDci(session, v, description,
							dlg.getPollingInterval(), dlg.getRetentionTime(),
							dlg.getDeltaCalculation(), lockRequired);
					monitor.worked(1);
				}
				monitor.done();
			}
	 ...
	 * @throws Exception
	 */
	private static void createDci(NXCSession session, SnmpValue value,
			String description, int pollingInterval, int retentionTime,
			int deltaCalculation, Map<Long, Boolean> lockRequired)
			throws Exception {
		AbstractNode node = (AbstractNode) session.findObjectById(
				value.getNodeId(), AbstractNode.class);
		if (node == null)
