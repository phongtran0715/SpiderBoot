...
public class LastValues extends ObjectTab {
	...
	protected void createTabContent(Composite parent) {
		final IDialogSettings settings = Activator.getDefault()
				.getDialogSettings();
		initShowFilter = safeCast(settings.get("LastValuesTab.showFilter"),
				settings.getBoolean("LastValuesTab.showFilter"), initShowFilter);

		dataView = new LastValuesWidget(getViewPart(), parent, SWT.NONE,
				getObject(), "LastValuesTab", new VisibilityValidator() { //$NON-NLS-1$
					@Override
		...
		dataView.addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				settings.put("LastValuesTab.showFilter",
						dataView.isFilterEnabled());
			}
		...
		dataView.setFilterCloseAction(new Action() {
			...
			public void run() {
				...
				Command command = service
						.getCommand("org.netxms.ui.eclipse.datacollection.commands.show_dci_filter"); //$NON-NLS-1$
				State state = command
						.getState("org.netxms.ui.eclipse.datacollection.commands.show_dci_filter.state"); //$NON-NLS-1$
	...
	public void objectChanged(AbstractObject object) {
		...
		if (getViewPart().getSite().getPage().isPartVisible(getViewPart())
				&& isActive())
			dataView.refresh();
	}
