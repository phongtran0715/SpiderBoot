...
public class ClusterOptions extends DCIPropertyPageDialog {
	...
	protected Control createContents(Composite parent) {
		...
		clusterResource = WidgetHelper.createLabeledCombo(dialogArea,
				SWT.BORDER | SWT.READ_ONLY, Messages.get().General_ClRes,
		...
		aggregationGroup.setLayoutData(new GridData(SWT.FILL, SWT.TOP, true,
				false));
		...
		checkAggregate
				.setText(Messages.get().ClusterOptions_AggregateFromNodes);
		...
		checkAggregateWithErrors
				.setText("Use last known value for aggregation in case of data collection error");
		checkAggregateWithErrors.setSelection(editor.getObject()
				.isAggregateWithErrors());

		checkRunScript = new Button(aggregationGroup, SWT.CHECK);
		checkRunScript
				.setText(Messages.get().ClusterOptions_RunScriptOnAggregatedData);
		...
		if (editor.getObject() instanceof DataCollectionItem) {
			checkAggregate.addSelectionListener(new SelectionListener() {
				...
				public void widgetSelected(SelectionEvent e) {
					aggregationFunction.setEnabled(checkAggregate
							.getSelection());
				}
			...
			aggregationFunction = WidgetHelper.createLabeledCombo(
					aggregationGroup, SWT.BORDER | SWT.READ_ONLY,
					Messages.get().ClusterOptions_AggrFunction,
					WidgetHelper.DEFAULT_LAYOUT_DATA);
			...
			aggregationFunction.select(editor.getObjectAsItem()
					.getAggregationFunction());
			aggregationFunction.setEnabled(editor.getObjectAsItem()
	...
	protected boolean applyChanges(final boolean isApply) {
		if (cluster != null) {
			editor.getObject()
					.setResourceId(
							clusterResourceMap.get(clusterResource
									.getSelectionIndex()));
		}
		editor.getObject().setAggregateOnCluster(checkAggregate.getSelection());
		editor.getObject().setAggregateWithErrors(
				checkAggregateWithErrors.getSelection());
		editor.getObject()
				.setTransformAggregated(checkRunScript.getSelection());
		if (editor.getObject() instanceof DataCollectionItem) {
			editor.getObjectAsItem().setAggregationFunction(
					aggregationFunction.getSelectionIndex());
		}
		editor.modify();
		return true;
	}
