 ...
 */
public abstract class AbstractSelectParamDlg extends Dialog implements
		IParameterSelectionDialog {
	...
	public AbstractSelectParamDlg(Shell parentShell, long nodeId,
			boolean selectTables) {
		super(parentShell);
		this.selectTables = selectTables;
		setShellStyle(getShellStyle() | SWT.RESIZE);
		object = ((NXCSession) ConsoleSharedData.getSession())
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText(selectTables ? Messages.get().AbstractSelectParamDlg_TableSelection
				: Messages.get().AbstractSelectParamDlg_Title);
		IDialogSettings settings = Activator.getDefault().getDialogSettings();
		final String prefix = getConfigurationPrefix();
		try {
			newShell.setSize(
					settings.getInt(prefix + ".cx"), settings.getInt(prefix + ".cy")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected Control createDialogArea(Composite parent) {
		...
		label.setText(selectTables ? Messages.get().AbstractSelectParamDlg_AvailTables
				: Messages.get().AbstractSelectParamDlg_AvailableParameters);
		...
		final String[] names = {
				Messages.get().AbstractSelectParamDlg_Name,
				selectTables ? Messages.get().AbstractSelectParamDlg_InstanceColumn
						: Messages.get().AbstractSelectParamDlg_Type,
				Messages.get().AbstractSelectParamDlg_Description };
		final int[] widths = { 150, selectTables ? 150 : 100, 350 };
		viewer = new SortableTableViewer(dialogArea, names, widths, 0, SWT.UP,
				SWT.FULL_SELECTION | SWT.BORDER);
		WidgetHelper.restoreTableViewerSettings(viewer, Activator.getDefault()
				.getDialogSettings(), getConfigurationPrefix() + ".viewer"); //$NON-NLS-1$
		viewer.setContentProvider(new ArrayContentProvider());
		viewer.setLabelProvider(selectTables ? new AgentTableLabelProvider()
				: new AgentParameterLabelProvider());
		viewer.setComparator(selectTables ? new AgentTableComparator()
				: new AgentParameterComparator());
		...
		viewer.getTable().addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveTableViewerSettings(viewer, Activator
						...
						getConfigurationPrefix() + ".viewer"); //$NON-NLS-1$
			}
		});
	...
	public String getParameterDescription() {
		return selectTables ? ((AgentTable) selection).getDescription()
				: ((AgentParameter) selection).getDescription();
	}
	...
	public String getParameterName() {
		return selectTables ? ((AgentTable) selection).getName()
				: ((AgentParameter) selection).getName();
	}
	...
	public String getInstanceColumn() {
		return selectTables ? ((AgentTable) selection)
				.getInstanceColumnsAsList() : ""; //$NON-NLS-1$
	}
	...
	protected void okPressed() {
		...
		if (selection.isEmpty()) {
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().AbstractSelectParamDlg_Warning,
					Messages.get().AbstractSelectParamDlg_WarningText);
			return;
		}
