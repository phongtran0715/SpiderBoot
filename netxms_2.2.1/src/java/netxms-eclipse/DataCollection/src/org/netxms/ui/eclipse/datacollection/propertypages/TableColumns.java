...
public class TableColumns extends DCIPropertyPageDialog {
	...
	protected Control createContents(Composite parent) {
		...
		new Label(columnListArea, SWT.NONE)
				.setText(Messages.get().TableColumns_Columns);

		columnList = new TableViewer(columnListArea, SWT.BORDER | SWT.MULTI
				| SWT.FULL_SELECTION);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		...
		columnList.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (selection.size() == 1) {
					...
					upButton.setEnabled(columns.indexOf(selection
							.getFirstElement()) > 0);
					downButton.setEnabled(columns.indexOf(selection
							.getFirstElement()) < columns.size() - 1);
				} else {
					modifyButton.setEnabled(false);
		...
		final DataCollectionObjectListener listener = new DataCollectionObjectListener() {
			@Override
			public void onSelectItem(int origin, String name,
					String description, int dataType) {
			}

			@Override
			public void onSelectTable(int origin, String name,
					String description) {
	...
	private void setupColumnList() {
		...
		WidgetHelper.restoreColumnSettings(table, Activator.getDefault()
				.getDialogSettings(), COLUMN_SETTINGS_PREFIX);
	...
	private void deleteColumns() {
		final IStructuredSelection selection = (IStructuredSelection) columnList
				.getSelection();
	...
	private void editColumn() {
		final IStructuredSelection selection = (IStructuredSelection) columnList
				.getSelection();
		if (selection.size() == 1) {
			final ColumnDefinition column = (ColumnDefinition) selection
					.getFirstElement();
	...
	private void addColumn() {
		final InputDialog idlg = new InputDialog(getShell(),
				Messages.get().TableColumns_NewColumn,
				Messages.get().TableColumns_ColumnName,
		...
		if (idlg.open() == Window.OK) {
			final ColumnDefinition column = new ColumnDefinition(
					idlg.getValue(), idlg.getValue());
			final EditColumnDialog dlg = new EditColumnDialog(getShell(),
					column);
			if (dlg.open() == Window.OK) {
				columns.add(column);
				columnList.setInput(columns.toArray());
	...
	private void saveSettings() {
		WidgetHelper.saveColumnSettings(columnList.getTable(), Activator
				.getDefault().getDialogSettings(), COLUMN_SETTINGS_PREFIX);
	}
	...
	private void queryColumns() {
		if (!MessageDialogHelper
				.openQuestion(
						getShell(),
						"Warning",
						"Current column definition will be replaced by definition provided by agent. Continue?"))
			return;

		AbstractObject object = ConsoleSharedData.getSession().findObjectById(
				dci.getNodeId());
		if ((editor.getSourceNode() == 0)
				&& ((object instanceof Template) || (object instanceof Cluster))) {
	...
	private void updateColumnsFromAgent(final String name,
			final boolean interactive, final AbstractObject queryObject) {
		final NXCSession session = ConsoleSharedData.getSession();
		ConsoleJob job = new ConsoleJob(Messages.get().TableColumns_JobName,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				try {
					...
					if (editor.getSourceNode() != 0) {
						table = session.queryAgentTable(editor.getSourceNode(),
								name);
					} else {
						table = session.queryAgentTable(
								(queryObject != null) ? queryObject
										.getObjectId() : dci.getNodeId(), name);
					}
						...
						public void run() {
							...
							for (int i = 0; i < table.getColumnCount(); i++) {
								ColumnDefinition c = new ColumnDefinition(
										...
										table.getColumnDisplayName(i));
								c.setDataType(table.getColumnDefinition(i)
										.getDataType());
								c.setInstanceColumn(table
					...
					Activator
							.logError(
									"Cannot read table column definition from agent",
									e);
					if (interactive) {
						final String msg = (e instanceof NXCException) ? e
								.getLocalizedMessage() : "Internal error";
						runInUIThread(new Runnable() {
							@Override
							public void run() {
								MessageDialogHelper
										.openError(
												...
												"Error",
												String.format(
														"Cannot read table column definition from agent (%s)",
														msg));
							}
						});
					}
