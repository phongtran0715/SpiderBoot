...
public class SummaryTableColumns extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		table = (DciSummaryTable) getElement()
				.getAdapter(DciSummaryTable.class);

		columns = new ArrayList<DciSummaryTableColumn>(table.getColumns()
				.size());
		...
		new Label(dialogArea, SWT.NONE)
				.setText(Messages.get().SummaryTableColumns_Columns);
	...
	protected void applyChanges(final boolean isApply) {
		...
		new ConsoleJob(Messages.get().SummaryTableColumns_JobName, null,
	...
	private void moveUp() {
		final IStructuredSelection selection = (IStructuredSelection) viewer
				.getSelection();
		if (selection.size() == 1) {
			final DciSummaryTableColumn column = (DciSummaryTableColumn) selection
					.getFirstElement();
	...
	private void editColumn() {
		...
		if (selection.size() == 1) {
			final DciSummaryTableColumn column = (DciSummaryTableColumn) selection
					.getFirstElement();
			EditDciSummaryTableColumnDlg dlg = new EditDciSummaryTableColumnDlg(
					getShell(), column);
		...
		if (dialog.open() == Dialog.OK) {
			...
			for (DciValue item : selection) {
				DciSummaryTableColumn column = new DciSummaryTableColumn(
						item.getDescription(), item.getName(), 0, ";");
				select.add(column);
				columns.add(column);
			}
			viewer.setInput(columns.toArray());
