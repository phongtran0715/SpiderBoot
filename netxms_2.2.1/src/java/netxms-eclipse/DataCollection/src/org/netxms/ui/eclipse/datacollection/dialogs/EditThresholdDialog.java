...
public class EditThresholdDialog extends Dialog {
	...
	protected Control createDialogArea(final Composite parent) {
		...
		conditionGroup.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false));
		...
		function = WidgetHelper.createLabeledCombo(conditionGroup, SWT.BORDER
				| SWT.DROP_DOWN | SWT.READ_ONLY,
				Messages.get().EditThresholdDialog_Function,
		...
		function.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if ((f != Threshold.F_SCRIPT)
						&& (selectedFunction == Threshold.F_SCRIPT)) {
					disposeScriptGroup();
					createOperGroup();
					parent.getParent().layout(true, true);
				} else if ((f == Threshold.F_SCRIPT)
						&& (selectedFunction != Threshold.F_SCRIPT)) {
					disposeOperGroup();
					createScriptGroup();
					parent.getParent().layout(true, true);
				}
				if (f == Threshold.F_ERROR
						&& selectedFunction != Threshold.F_ERROR) {
					if (!operation.isDisposed())
						operation.setEnabled(false);
					value.setEnabled(false);
				} else if (f != Threshold.F_ERROR
						&& selectedFunction == Threshold.F_ERROR) {
					if (!operation.isDisposed())
		...
		samples = WidgetHelper.createLabeledText(conditionGroup, SWT.BORDER,
				60, Messages.get().EditThresholdDialog_Samples,
				Integer.toString(threshold.getSampleCount()),
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		deactivationEvent
				.setLabel(Messages.get().EditThresholdDialog_DeactEvent);
		...
		repeatGroup.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false));
		...
		repeatInterval.setText((threshold.getRepeatInterval() > 0) ? Integer
				.toString(threshold.getRepeatInterval()) : "3600"); //$NON-NLS-1$
		repeatInterval.setEnabled(threshold.getRepeatInterval() > 0);

		new Label(repeatGroup, SWT.NONE)
				.setText(Messages.get().EditThresholdDialog_Seconds);
	...
	private void createOperGroup() {
		operation = WidgetHelper.createLabeledCombo(conditionGroup, SWT.BORDER
				| SWT.DROP_DOWN | SWT.READ_ONLY,
				Messages.get().EditThresholdDialog_Operation,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		operation.select((savedOperation != -1) ? savedOperation : threshold
				.getOperation());

		value = WidgetHelper.createLabeledText(conditionGroup, SWT.BORDER, 120,
				Messages.get().EditThresholdDialog_Value,
				(savedValue != null) ? savedValue : threshold.getValue(),
				WidgetHelper.DEFAULT_LAYOUT_DATA);
	}
	...
	private void createScriptGroup() {
		...
		scriptGroup.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false, 1, 1));
		...
		script.setText(savedScript != null ? savedScript : threshold
				.getScript());
		...
		gd.horizontalAlignment = SWT.FILL;
		gd.heightHint = script.getTextControl().computeSize(SWT.DEFAULT,
				SWT.DEFAULT).y;
		editButton.setLayoutData(gd);
		editButton
				.setToolTipText(Messages.get().EditThresholdDialog_OpenScriptEditor);
		...
		final Listener keyListener = new Listener() {
			...
			public void handleEvent(Event e) {
				if (((e.stateMask & SWT.MOD1) == SWT.CTRL)
						&& (e.keyCode == 'e')) {
					if (disabled)
						return;
					disabled = true;
	...
	private void openScriptEditor() {
		ScriptEditDialog dlg = new ScriptEditDialog(
				getShell(),
				script.getText(),
	...
	protected void okPressed() {
		if (!WidgetHelper.validateTextInput(samples,
				Messages.get().EditThresholdDialog_Samples,
				new NumericTextFieldValidator(1, 1000), null))
			return;
		...
		if (repeatDefault.getSelection()) {
			...
			if (!WidgetHelper.validateTextInput(repeatInterval,
					Messages.get().EditThresholdDialog_RepeatInterval,
					new NumericTextFieldValidator(1, 1000000), null))
				return;
