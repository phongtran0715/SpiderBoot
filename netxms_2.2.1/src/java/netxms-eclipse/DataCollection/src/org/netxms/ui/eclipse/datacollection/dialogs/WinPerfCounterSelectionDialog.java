 ...
 * 
 */
public class WinPerfCounterSelectionDialog extends Dialog implements
	...
	protected Control createDialogArea(Composite parent) {
		...
		new Label(dialogArea, SWT.NONE)
				.setText(Messages.get().WinPerfCounterSelectionDialog_ObjectsAndCounters);
		new Label(dialogArea, SWT.NONE)
				.setText(Messages.get().WinPerfCounterSelectionDialog_Instances);
		...
		objectTree.setComparator(new ViewerComparator() {
			...
			public int compare(Viewer viewer, Object e1, Object e2) {
				String s1 = (e1 instanceof WinPerfCounter) ? ((WinPerfCounter) e1)
						.getName() : ((WinPerfObject) e1).getName();
				String s2 = (e2 instanceof WinPerfCounter) ? ((WinPerfCounter) e2)
						.getName() : ((WinPerfObject) e2).getName();
		...
		instanceList = new TableViewer(dialogArea, SWT.BORDER
				| SWT.FULL_SELECTION);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.verticalAlignment = SWT.FILL;
	...
	private void fillData() {
		...
		new ConsoleJob(Messages.get().WinPerfCounterSelectionDialog_JobName,
				null, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<WinPerfObject> objects = session
						.getNodeWinPerfObjects(nodeId);
	...
	private void onObjectSelection() {
		...
		WinPerfObject object = (selection.getFirstElement() instanceof WinPerfObject) ? (WinPerfObject) selection
				.getFirstElement() : ((WinPerfCounter) selection
				.getFirstElement()).getObject();
		instanceList.setInput(object.getInstances());
		instanceList.getTable().setEnabled(object.hasInstances());
		selectedCounter = (selection.getFirstElement() instanceof WinPerfCounter) ? (WinPerfCounter) selection
	...
	protected void okPressed() {
		if (selectedCounter == null) {
			MessageDialogHelper
					.openWarning(
							getShell(),
							Messages.get().WinPerfCounterSelectionDialog_Warning,
							Messages.get().WinPerfCounterSelectionDialog_PleaseSelectCounter);
			return;
		}

		if (selectedCounter.getObject().hasInstances()) {
			IStructuredSelection selection = (IStructuredSelection) instanceList
					.getSelection();
			if (selection.size() != 1) {
				MessageDialogHelper
						.openWarning(
	...
	public String getParameterDescription() {
		if (selectedInstance == null)
			return selectedCounter.getObject().getName()
					+ ": " + selectedCounter.getName(); //$NON-NLS-1$
		return selectedCounter.getObject().getName()
				+ ": " + selectedCounter.getName() + Messages.get().WinPerfCounterSelectionDialog_for + selectedInstance; //$NON-NLS-1$
	}
