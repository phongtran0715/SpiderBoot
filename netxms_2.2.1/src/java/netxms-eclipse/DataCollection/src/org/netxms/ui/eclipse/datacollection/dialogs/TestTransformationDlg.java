...
public class TestTransformationDlg extends Dialog {
	...
	protected void createButtonsForButtonBar(Composite parent) {
		Button b = createButton(parent, RUN,
				Messages.get().TestTransformationDlg_Run, true);
		...
		createButton(parent, Window.CANCEL,
				Messages.get().TestTransformationDlg_Close, false);
	}
	...
	protected Control createDialogArea(Composite parent) {
		imageWaiting = Activator
				.getImageDescriptor("icons/waiting.png").createImage(); //$NON-NLS-1$
	...
	private void runScript() {
		...
		new ConsoleJob(Messages.get().TestTransformationDlg_JobTitle, null,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final TransformationTestResult r = session
						.testTransformationScript(nodeId, script, input);
				runInUIThread(new Runnable() {
					...
					public void run() {
						...
						if (r.success) {
							...
							status.setImage(StatusDisplayInfo
