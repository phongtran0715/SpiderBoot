 ...
 */
public class SummaryTablesDynamicMenu extends ContributionItem implements
	...
	public void initialize(IServiceLocator serviceLocator) {
		evalService = (IEvaluationService) serviceLocator
	...
	public void fill(Menu menu, int index) {
		final Object selection = evalService.getCurrentState().getVariable(
				ISources.ACTIVE_MENU_SELECTION_NAME);
		if ((selection == null) || !(selection instanceof IStructuredSelection))
			return;

		final AbstractObject baseObject = (AbstractObject) ((IStructuredSelection) selection)
				.getFirstElement();
		...
		Arrays.sort(tables, new Comparator<DciSummaryTableDescriptor>() {
			@Override
			public int compare(DciSummaryTableDescriptor arg0,
					DciSummaryTableDescriptor arg1) {
				return arg0
						.getMenuPath()
						.replace("&", "").compareToIgnoreCase(arg1.getMenuPath().replace("&", "")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
			}
		});
		...
		for (int i = 0; i < tables.length; i++) {
			...
			item.addSelectionListener(new SelectionAdapter() {
				...
				public void widgetSelected(SelectionEvent e) {
					queryTable(baseObject.getObjectId(),
							((DciSummaryTableDescriptor) item.getData())
									.getId());
				}
			});
		...
		if (added > 0) {
			...
			tablesMenuItem
					.setText(Messages.get().SummaryTablesDynamicMenu_MenuName);
	...
	private void queryTable(final long baseObjectId, final int tableId) {
		...
		new ConsoleJob(Messages.get().SummaryTablesDynamicMenu_QueryTableJob,
				null, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final Table results = session.queryDciSummaryTable(tableId,
						baseObjectId);
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						String secondaryId = Integer.toString(tableId)
								+ "&" + Long.toString(baseObjectId); //$NON-NLS-1$
						IWorkbenchWindow window = PlatformUI.getWorkbench()
								.getActiveWorkbenchWindow();
						try {
							SummaryTable view = (SummaryTable) window
									.getActivePage().showView(SummaryTable.ID,
											secondaryId,
							...
							MessageDialogHelper
									.openError(
											window.getShell(),
											Messages.get().SummaryTablesDynamicMenu_Error,
											String.format(
													Messages.get().SummaryTablesDynamicMenu_CannotOpenView,
