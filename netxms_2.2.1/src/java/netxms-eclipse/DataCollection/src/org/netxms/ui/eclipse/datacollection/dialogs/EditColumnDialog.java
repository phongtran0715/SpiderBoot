...
public class EditColumnDialog extends Dialog {
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText(Messages.get().EditColumnDialog_ColumnDefinition
				+ column.getName());
	}
	...
	protected Control createDialogArea(Composite parent) {
		...
		dataType = WidgetHelper.createLabeledCombo(dialogArea, SWT.READ_ONLY,
				Messages.get().EditColumnDialog_DataType, new GridData(
		...
		aggregationFunction = WidgetHelper.createLabeledCombo(dialogArea,
				SWT.READ_ONLY, Messages.get().EditColumnDialog_AggFunc,
		...
		checkInstanceColumn
				.setText(Messages.get().EditColumnDialog_InstanceCol);
		checkInstanceColumn.setLayoutData(new GridData(SWT.FILL, SWT.CENTER,
				true, false, 2, 1));
		checkInstanceColumn.setSelection(column.isInstanceColumn());

		checkInstanceLabelColumn = new Button(dialogArea, SWT.CHECK);
		checkInstanceLabelColumn
				.setText(Messages.get().EditColumnDialog_InstanceLabelCol);
		checkInstanceLabelColumn.setLayoutData(new GridData(SWT.FILL,
		...
		snmpOid.setText((column.getSnmpObjectId() != null) ? column
				.getSnmpObjectId().toString() : ""); //$NON-NLS-1$
		snmpOid.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false,
				2, 1));
		...
		if (!oidText.isEmpty()) {
			try {
				...
				MessageDialogHelper.openWarning(getShell(),
						...
						Messages.get().EditColumnDialog_InvalidOID);
				return;
			}
		} else {
