...
public class ClearCollectedData implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		if (!MessageDialogHelper.openQuestion(part.getSite().getShell(),
				Messages.get().ClearCollectedData_ConfirmDialogTitle,
				Messages.get().ClearCollectedData_ConfirmationText))
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		final Set<DCI> dciToClear = objects;
		new ConsoleJob(Messages.get().ClearCollectedData_TaskName, part,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				monitor.beginTask(Messages.get().ClearCollectedData_TaskName,
						dciToClear.size());
		...
		if (selection instanceof IStructuredSelection) {
			...
			for (Object o : ((IStructuredSelection) selection).toList()) {
				if (o instanceof DciValue) {
					dciList.add(new DCI(((DciValue) o).getNodeId(),
							((DciValue) o).getId()));
				} else if (o instanceof DataCollectionObject) {
					dciList.add(new DCI(((DataCollectionObject) o).getNodeId(),
							((DataCollectionObject) o).getId()));
				} else {
					dciList = null;
					break;
				}
