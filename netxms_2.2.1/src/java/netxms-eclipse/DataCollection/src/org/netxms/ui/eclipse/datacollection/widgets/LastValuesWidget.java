...
public class LastValuesWidget extends CompositeWithMessageBar {
	 ...
	 * @param validator
	 *            additional visibility validator (used to suppress unneded
	 *            refresh calls, may be null)
	 */
	public LastValuesWidget(ViewPart viewPart, Composite parent, int style,
			AbstractObject dcTarget, final String configPrefix,
		...
		refreshController = new ViewRefreshController(viewPart, -1,
				new Runnable() {
					@Override
					public void run() {
						if (isDisposed())
		...
		// Setup table columns
		final String[] names = { Messages.get().LastValuesWidget_ColID,
				Messages.get().LastValuesWidget_ColDescr,
				Messages.get().LastValuesWidget_ColValue,
				Messages.get().LastValuesWidget_ColTime,
				Messages.get().LastValuesWidget_ColThreshold };
		final int[] widths = { 70, 250, 150, 120, 150 };
		dataViewer = new SortableTableViewer(getContent(), names, widths, 0,
		...
		dataViewer.addDoubleClickListener(new IDoubleClickListener() {
			...
			public void doubleClick(DoubleClickEvent event) {
				IStructuredSelection selection = (IStructuredSelection) dataViewer
						.getSelection();
				if (selection.size() == 1) {
					callOpenObjectHandler((DciValue) selection
							.getFirstElement());
				}
			}
		});
		...
		dataViewer.getTable().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveTableViewerSettings(dataViewer, ds,
						configPrefix);
		...
		try {
			autoRefreshInterval = ds.getInt(configPrefix
					+ ".autoRefreshInterval"); //$NON-NLS-1$
		...
		if (ds.get(configPrefix + ".useMultipliers") != null) //$NON-NLS-1$
			labelProvider.setUseMultipliers(ds.getBoolean(configPrefix
					+ ".useMultipliers")); //$NON-NLS-1$
		else
			labelProvider.setUseMultipliers(true);
		if (ds.get(configPrefix + ".showErrors") != null) //$NON-NLS-1$
		{
			labelProvider.setShowErrors(ds.getBoolean(configPrefix
					+ ".showErrors")); //$NON-NLS-1$
			comparator.setShowErrors(ds
					.getBoolean(configPrefix + ".showErrors")); //$NON-NLS-1$
		} else {
		...
		filter.setShowUnsupported(ds.getBoolean(configPrefix
				+ ".showUnsupported")); //$NON-NLS-1$
		...
		clientListener = new SessionListener() {
			...
			public void notificationHandler(SessionNotification n) {
				if ((n.getCode() == SessionNotification.FORCE_DCI_POLL)
						...
						&& (n.getSubCode() == LastValuesWidget.this.dcTarget
								.getObjectId()))
					refresh();
			}
		};
	...
	private void createActions() {
		actionUseMultipliers = new Action(
				Messages.get().LastValuesWidget_UseMultipliers,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowErrors = new Action(
				Messages.get().LastValuesWidget_ShowErrors, Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowUnsupported = new Action(
				Messages.get().LastValuesWidget_ShowUnsupported,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowDisabled = new Action(
				Messages.get().LastValuesWidget_ShowDisabled,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionCopyToClipboard = new Action(
				Messages.get().LastValuesWidget_CopyToClipboard,
				SharedIcons.COPY) {
			@Override
			public void run() {
				copySelection();
	...
	private void getDataFromServer() {
		...
		ConsoleJob job = new ConsoleJob(
				Messages.get().LastValuesWidget_JobTitle
						+ dcTarget.getObjectName(), viewPart,
				Activator.PLUGIN_ID, LastValuesWidget.JOB_FAMILY) {
			@Override
			protected String getErrorMessage() {
				return String.format(Messages.get().LastValuesWidget_JobError,
						(dcTarget != null) ? dcTarget.getObjectName() : null);
			}
			...
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				try {
					final DciValue[] data = session.getLastValues(dcTarget
							.getObjectId());
						...
						public void run() {
							if (!isDisposed()) {
								showMessage(ERROR, String.format(
										"Cannot read data from server: %s",
	 ...
	 * 
	 * @param dcTarget
	 *            new data collection target object
	 */
	public void setDataCollectionTarget(AbstractObject dcTarget) {
		this.dcTarget = (dcTarget instanceof DataCollectionTarget) ? (DataCollectionTarget) dcTarget
				: null;
	}
	 ...
	 *            the autoRefreshEnabled to set
	 */
	public void setAutoRefreshEnabled(boolean autoRefreshEnabled) {
		this.autoRefreshEnabled = autoRefreshEnabled;
		refreshController.setInterval(autoRefreshEnabled ? autoRefreshInterval
				: -1);
	}
	...
	public boolean areMultipliersUsed() {
		return (labelProvider != null) ? labelProvider.areMultipliersUsed()
				: false;
	}

	/**
	 * @param useMultipliers
	 *            the useMultipliers to set
	 */
	...
	private void registerOpenHandlers() {
		...
		IConfigurationElement[] elements = reg
				.getConfigurationElementsFor("org.netxms.ui.eclipse.datacollection.dciOpenHandlers"); //$NON-NLS-1$
		for (int i = 0; i < elements.length; i++) {
			try {
				final OpenHandlerData h = new OpenHandlerData();
				h.handler = (DciOpenHandler) elements[i]
