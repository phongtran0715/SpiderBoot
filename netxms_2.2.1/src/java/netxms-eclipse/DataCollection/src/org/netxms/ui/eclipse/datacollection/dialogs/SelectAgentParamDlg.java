...
public class SelectAgentParamDlg extends AbstractSelectParamDlg {
	...
	protected void fillParameterList() {
		...
		new ConsoleJob(Messages.get().SelectAgentParamDlg_JobTitle
			...
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				if (selectTables) {
					final List<AgentTable> tables = session
							.getSupportedTables(object.getObjectId());
					...
					final List<AgentParameter> parameters = session
							.getSupportedParameters(object.getObjectId());
					runInUIThread(new Runnable() {
						@Override
						public void run() {
	...
	protected void querySelectedParameter() {
		...
		if (queryObject instanceof Template) {
			final ObjectSelectionDialog sDlg = new ObjectSelectionDialog(
					getShell(), null,
		...
		if (p.getName().contains("(*)")) //$NON-NLS-1$
		{
			InputDialog dlg = new InputDialog(getShell(),
					Messages.get().SelectAgentParamDlg_InstanceTitle,
					Messages.get().SelectAgentParamDlg_InstanceMessage,
					"", null); //$NON-NLS-1$
		...
		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		final String name = n;
		new ConsoleJob(Messages.get().SelectAgentParamDlg_QueryJobTitle, null,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final String value = session.queryParameter(
						queryObject.getObjectId(), DataCollectionItem.AGENT,
						name);
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						MessageDialogHelper
								.openInformation(
										getShell(),
										Messages.get().SelectAgentParamDlg_CurrentValueTitle,
										String.format(
												Messages.get().SelectAgentParamDlg_CurrentValue,
												value));
					}
				});
			}
