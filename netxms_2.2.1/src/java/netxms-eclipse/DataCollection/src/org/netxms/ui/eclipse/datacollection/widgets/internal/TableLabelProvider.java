...
public class TableLabelProvider extends LabelProvider implements
	...
	private static final Color[] FOREGROUND_COLORS = { null,
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		if (columnIndex >= row.size())
			return null;
		return useMultipliers ? new DataFormatter("%*s",
				columnDataTypes[columnIndex]).format(row.get(columnIndex)
				.getValue()) : row.get(columnIndex).getValue();
	}
	...
	public Color getForeground(Object element, int columnIndex) {
		...
		return (cell.getStatus() >= 0)
				&& (cell.getStatus() < FOREGROUND_COLORS.length) ? FOREGROUND_COLORS[cell
				.getStatus()] : null;
	}
	...
	public Color getBackground(Object element, int columnIndex) {
		...
		TableCell cell = row.get(columnIndex);
		return (cell.getStatus() > 0) ? StatusDisplayInfo.getStatusColor(cell
				.getStatus()) : null;
	}
