...
public class DataCollectionEditor extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		object = ((obj != null) && ((obj instanceof DataCollectionTarget) || (obj instanceof Template))) ? obj
				: null;
		setPartName(Messages.get().DataCollectionEditor_PartNamePrefix
				+ ((object != null) ? object.getObjectName()
						: Messages.get().DataCollectionEditor_Error));
	}
	...
	public void createPartControl(Composite parent) {
		...
		final String[] names = { Messages.get().DataCollectionEditor_ColID,
				Messages.get().DataCollectionEditor_ColOrigin,
				Messages.get().DataCollectionEditor_ColDescription,
				Messages.get().DataCollectionEditor_ColParameter,
				Messages.get().DataCollectionEditor_ColDataType,
				Messages.get().DataCollectionEditor_ColPollingInterval,
				Messages.get().DataCollectionEditor_ColRetentionTime,
				Messages.get().DataCollectionEditor_ColStatus,
				Messages.get().DataCollectionEditor_ColThresholds,
				Messages.get().DataCollectionEditor_ColTemplate };
		final int[] widths = { 60, 100, 250, 200, 90, 90, 90, 100, 200, 150 };
		viewer = new SortableTableViewer(content, names, widths, 0, SWT.UP,
				SortableTableViewer.DEFAULT_STYLE);
		viewer.setContentProvider(new ArrayContentProvider());
		viewer.setLabelProvider(new DciLabelProvider());
		viewer.setComparator(new DciComparator((DciLabelProvider) viewer
				.getLabelProvider()));
		filter = new DciFilter();
		viewer.addFilter(filter);
		WidgetHelper.restoreTableViewerSettings(viewer, settings,
				"DataCollectionEditor"); //$NON-NLS-1$
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (selection != null) {
					...
					while (it.hasNext() && (!canActivate || !canDisable)) {
						DataCollectionObject dci = (DataCollectionObject) it
								.next();
		...
		viewer.getTable().addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveTableViewerSettings(viewer, settings,
						"DataCollectionEditor"); //$NON-NLS-1$
				settings.put("DataCollectionEditor.hideModificationWarnings",
						hideModificationWarnings);
			}
		});
		...
		new ConsoleJob(Messages.get().DataCollectionEditor_OpenJob_Title
				+ object.getObjectName(), this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				dciConfig = session.openDataCollectionConfiguration(object
						.getObjectId());
			...
			protected void jobFailureHandler() {
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						DataCollectionEditor.this.getViewSite().getPage()
								.hideView(DataCollectionEditor.this);
					}
				});
			}
			...
			protected String getErrorMessage() {
				return Messages.get().DataCollectionEditor_OpenJob_Error
		...
		hideModificationWarnings = getBooleanFromSettings(
				"DataCollectionEditor.hideModificationWarnings", false);
	...
	private void createActions() {
		...
		actionCreateItem = new Action(
				Messages.get().DataCollectionEditor_NewParam,
				Activator.getImageDescriptor("icons/new.png")) { //$NON-NLS-1$
			@Override
			public void run() {
				createItem();
			}
		...
		actionCreateTable = new Action(
				Messages.get().DataCollectionEditor_NewTable) {
			@Override
			public void run() {
				createTable();
			}
		...
		actionEdit = new Action(Messages.get().DataCollectionEditor_ActionEdit,
				SharedIcons.EDIT) {
			@Override
			public void run() {
				editSelectedObject();
		...
		actionEdit.setImageDescriptor(Activator
				.getImageDescriptor("icons/edit.png")); //$NON-NLS-1$
		actionEdit.setEnabled(false);

		actionDelete = new Action(Messages.get().DataCollectionEditor_Delete,
				Activator.getImageDescriptor("icons/delete.png")) { //$NON-NLS-1$
			@Override
			public void run() {
				deleteItems();
			}
		...
		actionDuplicate = new Action(
				Messages.get().DataCollectionEditor_Duplicate) {
			@Override
			public void run() {
				duplicateItems();
			}
		...
		actionActivate = new Action(
				Messages.get().DataCollectionEditor_Activate,
				Activator.getImageDescriptor("icons/active.gif")) { //$NON-NLS-1$
			@Override
			public void run() {
		...
		actionDisable = new Action(Messages.get().DataCollectionEditor_Disable,
				Activator.getImageDescriptor("icons/disabled.gif")) { //$NON-NLS-1$
			@Override
			public void run() {
		...
		actionShowFilter = new Action(
				Messages.get().DataCollectionEditor_ShowFilter,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowFilter.setChecked(getBooleanFromSettings(
				"DataCollectionEditor.showFilter", true));
		actionShowFilter
	...
	public void dispose() {
		if (dciConfig != null) {
			new ConsoleJob(Messages.get().DataCollectionEditor_UnlockJob_Title
				...
				@Override
				protected String getErrorMessage() {
					return Messages.get().DataCollectionEditor_UnlockJob_Error
							+ object.getObjectName();
				}
			}.start();
		}
	...
	private void setItemStatus(final int newStatus) {
		...
		new ConsoleJob(Messages.get().DataCollectionEditor_ChStatusJob_Title
				+ object.getObjectName(), this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						for (Object dci : selection.toList()) {
							...
							new DataCollectionObjectEditor(
									(DataCollectionObject) dci).modify();
						}
					}
				});
			}
			...
			@Override
			protected String getErrorMessage() {
				return Messages.get().DataCollectionEditor_ChStatusJob_Error
						+ object.getObjectName();
			}
		}.start();
	}
	...
	private void deleteItems() {
		...
		if (!MessageDialogHelper.openConfirm(getSite().getShell(),
				Messages.get().DataCollectionEditor_DeleteConfirmTitle,
				Messages.get().DataCollectionEditor_DeleteConfirmText))
			return;

		new ConsoleJob(Messages.get().DataCollectionEditor_DeleteJob_Title
				+ object.getObjectName(), this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				for (Object dci : selection.toList()) {
					dciConfig
							.deleteObject(((DataCollectionObject) dci).getId());
				}
				runInUIThread(new Runnable() {
					@Override
					public void run() {
			...
			@Override
			protected String getErrorMessage() {
				return Messages.get().DataCollectionEditor_DeleteJob_Error
						+ object.getObjectName();
			}
		}.start();
	}
	...
	private void editSelectedObject() {
		...
		DataCollectionObject dco = (DataCollectionObject) selection
				.getFirstElement();

		DialogData data = null;
		if (!hideModificationWarnings && dco.getTemplateId() != 0) {
			String message = DataCollectionObjectEditor
					.createModificationWarningMessage(dco);
			if (message != null) {
				data = MessageDialogHelper.openWarningWithCheckbox(getSite()
						.getShell(), "Warning",
						"Don't show this message again", message);
				hideModificationWarnings = data.getSaveSelection();
			}
		}

		if ((data == null) || data.isOkPressed()) {
			ExtendedPropertyDialog dlg = ExtendedPropertyDialog.createDialogOn(
					getSite().getShell(), null, dco, ""); //$NON-NLS-1$
			dlg.createAllPages();
			int result = dlg.open();
			if (((DataCollectionObject) selection.getFirstElement())
					.isNewItem() && result != SWT.OK) {
	...
	private void duplicateItems() {
		...
		new ConsoleJob(Messages.get().DataCollectionEditor_DupJob_Title
				+ object.getObjectName(), this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
			...
			@Override
			protected String getErrorMessage() {
				return Messages.get().DataCollectionEditor_DupJob_Error
						+ object.getObjectName();
			}
		}.start();
	}
	...
	private void copyItems(final boolean doMove) {
		final ObjectSelectionDialog dlg = new ObjectSelectionDialog(getSite()
				.getShell(), null,
				ObjectSelectionDialog
						.createNodeAndTemplateSelectionFilter(true));
		...
		new ConsoleJob(Messages.get().DataCollectionEditor_CopyJob_Title
				+ object.getObjectName(), this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				for (AbstractObject o : dlg
						.getSelectedObjects(AbstractNode.class))
				...
				for (AbstractObject o : dlg.getSelectedObjects(Cluster.class))
					dciConfig.copyObjects(o.getObjectId(), dciList);
				for (AbstractObject o : dlg
						.getSelectedObjects(MobileDevice.class))
					dciConfig.copyObjects(o.getObjectId(), dciList);
				if (doMove) {
					for (long id : dciList)
			...
			@Override
			protected String getErrorMessage() {
				return Messages.get().DataCollectionEditor_CopyJob_Error
						+ object.getObjectName();
			}
		}.start();
	}
	...
	private void convertToTemplate() {
		final ObjectSelectionDialog dlg = new ObjectSelectionDialog(getSite()
				.getShell(), null,
				ObjectSelectionDialog.createTemplateSelectionFilter());
		...
		new ConsoleJob(
				Messages.get().DataCollectionEditor_ConvertJob_TitlePrefix
						+ object.getObjectName()
						+ Messages.get().DataCollectionEditor_ConvertJob_TitleSuffix,
				this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				monitor.beginTask(
						Messages.get().DataCollectionEditor_ConvertJob_TaskName,
						4);
				...
				if (needApply) {
					...
					do {
						try {
							session.applyTemplate(template.getObjectId(),
									dciConfig.getNodeId());
							success = true;
			...
			@Override
			protected String getErrorMessage() {
				return Messages.get().DataCollectionEditor_ConvertJob_ErrorPrefix
						+ object.getObjectName()
						+ Messages.get().DataCollectionEditor_ConvertJob_ErrorSuffix;
			}
		}.start();
	}
