 ...
 */
public class ThresholdTreeLabelProvider extends LabelProvider implements
		ITableLabelProvider {
	...
	public Image getColumnImage(Object element, int columnIndex) {
		if ((element instanceof ThresholdViolationSummary)
				&& (columnIndex == ThresholdSummaryWidget.COLUMN_NODE)) {
			AbstractObject node = session
					.findObjectById(((ThresholdViolationSummary) element)
							.getNodeId());
			return (node != null) ? wbLabelProvider.getImage(node) : null;
		} else if ((element instanceof ThresholdViolationSummary)
				&& (columnIndex == ThresholdSummaryWidget.COLUMN_STATUS)) {
			return StatusDisplayInfo
					.getStatusImage(((ThresholdViolationSummary) element)
							.getCurrentSeverity());
		} else if ((element instanceof DciValue)
				&& (columnIndex == ThresholdSummaryWidget.COLUMN_STATUS)) {
			return StatusDisplayInfo.getStatusImage(((DciValue) element)
					.getActiveThreshold().getCurrentSeverity());
		}
		return null;
	}
	...
	public String getColumnText(Object element, int columnIndex) {
		if (element instanceof ThresholdViolationSummary) {
			...
			case ThresholdSummaryWidget.COLUMN_STATUS:
				return StatusDisplayInfo
						.getStatusText(((ThresholdViolationSummary) element)
								.getCurrentSeverity());
			default:
				return null;
			}
			...
			switch (columnIndex) {
			case ThresholdSummaryWidget.COLUMN_STATUS:
				return StatusDisplayInfo.getStatusText(((DciValue) element)
						.getActiveThreshold().getCurrentSeverity());
			...
			case ThresholdSummaryWidget.COLUMN_CONDITION:
				return thresholdLabelProvider.getColumnText(
						((DciValue) element).getActiveThreshold(),
						Thresholds.COLUMN_OPERATION);
