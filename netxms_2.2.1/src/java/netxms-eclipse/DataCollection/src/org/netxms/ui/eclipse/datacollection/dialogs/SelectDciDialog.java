...
public class SelectDciDialog extends Dialog {
	...
	protected void configureShell(Shell newShell) {
		...
		try {
			newShell.setSize(
					settings.getInt("SelectDciDialog.width"), settings.getInt("SelectDciDialog.hight")); //$NON-NLS-1$ //$NON-NLS-2$
			newShell.setLocation(
					settings.getInt("SelectDciDialog.cx"), settings.getInt("SelectDciDialog.cy")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected void createButtonsForButtonBar(Composite parent) {
		if (enableEmptySelection) {
			Button button = createButton(parent, 1000,
					Messages.get().SelectDciDialog_None, false);
	...
	protected Control createDialogArea(Composite parent) {
		...
		dciList = new DciList(
				null,
				(fixedNode == 0) ? splitter : dialogArea,
				SWT.BORDER,
				null,
		...
		if (fixedNode == 0) {
			...
			objectTree.getTreeViewer().addSelectionChangedListener(
					new ISelectionChangedListener() {
						...
						public void selectionChanged(SelectionChangedEvent event) {
							AbstractObject object = objectTree
									.getFirstSelectedObject2();
							if ((object != null)
									&& ((object instanceof AbstractNode)
											|| (object instanceof MobileDevice)
											|| (object instanceof Cluster) || (allowTemplateItems && (object instanceof Template)))) {
			...
			dciList.setNode(((NXCSession) ConsoleSharedData.getSession())
					.findObjectById(fixedNode));
		}
	...
	protected void okPressed() {
		...
		if (selection == null || selection.size() == 0) {
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().SelectDciDialog_Warning,
					Messages.get().SelectDciDialog_WarningMessage);
			return;
		}
		saveSettings();
		super.okPressed();
	...
	/**
	 * @param enableEmptySelection
	 *            the enableEmptySelection to set
	 */
	...
	/**
	 * @param allowTemplateItems
	 *            the allowTemplateItems to set
	 */
	...
	/**
	 * @param allowSingleSelection
	 *            false to have multiple selection, true to have single
	 *            selection in DCI list. By default multiple selection is set.
	 */
	...
	/**
	 * @param allowNoValueObjects
	 *            the allowNoValueObjects to set
	 */
