...
public class ThresholdTreeComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		if ((e1 instanceof ThresholdViolationSummary)
				&& (e2 instanceof ThresholdViolationSummary)) {
			switch ((Integer) ((SortableTreeViewer) viewer).getTree()
					.getSortColumn().getData("ID")) //$NON-NLS-1$
			{
			case ThresholdSummaryWidget.COLUMN_NODE:
				String name1 = session
						.getObjectName(((ThresholdViolationSummary) e1)
								.getNodeId());
				String name2 = session
						.getObjectName(((ThresholdViolationSummary) e2)
								.getNodeId());
				result = name1.compareToIgnoreCase(name2);
				break;
			case ThresholdSummaryWidget.COLUMN_STATUS:
				result = ((ThresholdViolationSummary) e1).getCurrentSeverity()
						.compareTo(
								((ThresholdViolationSummary) e2)
										.getCurrentSeverity());
				break;
			default:
				break;
			...
			case ThresholdSummaryWidget.COLUMN_STATUS:
				result = ((DciValue) e1)
						...
						.getCurrentSeverity()
						.compareTo(
								((DciValue) e2).getActiveThreshold()
										.getCurrentSeverity());
				break;
			case ThresholdSummaryWidget.COLUMN_PARAMETER:
				result = ((DciValue) e1).getDescription().compareToIgnoreCase(
						((DciValue) e2).getDescription());
				break;
			case ThresholdSummaryWidget.COLUMN_VALUE:
				result = ((DciValue) e1).getValue().compareToIgnoreCase(
						((DciValue) e2).getValue());
				break;
			case ThresholdSummaryWidget.COLUMN_TIMESTAMP:
				Date t1 = ((DciValue) e1).getActiveThreshold()
						.getLastEventTimestamp();
				Date t2 = ((DciValue) e2).getActiveThreshold()
						.getLastEventTimestamp();
		...
		return (((SortableTreeViewer) viewer).getTree().getSortDirection() == SWT.UP) ? result
				: -result;
	}
}
