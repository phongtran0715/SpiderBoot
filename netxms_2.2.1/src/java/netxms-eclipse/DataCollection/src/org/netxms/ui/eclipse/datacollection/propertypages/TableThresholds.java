...
public class TableThresholds extends DCIPropertyPageDialog {
	...
	protected Control createContents(Composite parent) {
		...
		new Label(thresholdListArea, SWT.NONE)
				.setText(Messages.get().TableThresholds_Thresholds);

		thresholdList = new TableViewer(thresholdListArea, SWT.BORDER
				...
				.addSelectionChangedListener(new ISelectionChangedListener() {
					...
					public void selectionChanged(SelectionChangedEvent event) {
						...
						if (selection.size() == 1) {
							...
							upButton.setEnabled(thresholds.indexOf(selection
									.getFirstElement()) > 0);
							downButton.setEnabled(thresholds.indexOf(selection
									.getFirstElement()) < thresholds.size() - 1);
						} else {
							modifyButton.setEnabled(false);
	...
	private void deleteThresholds() {
		final IStructuredSelection selection = (IStructuredSelection) thresholdList
				.getSelection();
	...
	private void editThreshold() {
		final IStructuredSelection selection = (IStructuredSelection) thresholdList
				.getSelection();
		if (selection.size() == 1) {
			final TableThreshold t = (TableThreshold) selection
					.getFirstElement();
			EditTableThresholdDialog dlg = new EditTableThresholdDialog(
					getShell(), t, editor.getCallback());
	...
	private void addThreshold() {
		...
		final EditTableThresholdDialog dlg = new EditTableThresholdDialog(
				getShell(), t, editor.getCallback());
		if (dlg.open() == Window.OK) {
			thresholds.add(t);
			thresholdList.setInput(thresholds.toArray());
	...
	private void duplicateThreshold() {
		...
		if (selection.size() > 0) {
			...
			for (TableThreshold t : list) {
				thresholds
						.add(thresholds.indexOf(t) + 1, new TableThreshold(t));
				thresholdList.setInput(thresholds.toArray());
			}
		}
	}
	...
	private void saveSettings() {
		WidgetHelper.saveColumnSettings(thresholdList.getTable(), Activator
