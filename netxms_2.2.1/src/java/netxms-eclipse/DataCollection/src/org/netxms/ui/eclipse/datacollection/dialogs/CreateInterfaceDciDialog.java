...
public class CreateInterfaceDciDialog extends Dialog {
	...
	private static final boolean[] DEFAULT_ENABLED = { true, true, false,
	...
	protected Control createDialogArea(Composite parent) {
		...
		for (int i = 0; i < names.length; i++) {
			...
			forms[i] = new InterfaceDciForm(
					dataGroup,
					names[i],
					(object != null) ? descriptions[i].replaceAll(
							"@@ifName@@", object.getObjectName()) : descriptions[i], enabled[i]); //$NON-NLS-1$
			gd = new GridData();
			gd.horizontalAlignment = SWT.FILL;
			gd.grabExcessHorizontalSpace = true;
		...
		textInterval
				.setLabel(Messages.get().CreateInterfaceDciDialog_PollingInterval);
		...
		textRetention
				.setLabel(Messages.get().CreateInterfaceDciDialog_RetentionTime);
	...
	protected void okPressed() {
		...
		try {
			...
			MessageDialogHelper.openError(getShell(),
					Messages.get().CreateInterfaceDciDialog_Error,
					Messages.get().CreateInterfaceDciDialog_BadPollingInterval);
		}
		...
		try {
			...
			MessageDialogHelper.openError(getShell(),
					Messages.get().CreateInterfaceDciDialog_Error,
					Messages.get().CreateInterfaceDciDialog_BadRetentionTime);
		}

		settings.put(
				"CreateInterfaceDciDialog.pollingInterval", pollingInterval); //$NON-NLS-1$
		...
		for (int i = 0; i < forms.length; i++) {
			dciInfo[i] = new InterfaceDciInfo(forms[i].isDciEnabled(),
					forms[i].isDelta(), forms[i].getDescription());
			settings.put(
					"CreateInterfaceDciDialog.enabled_" + i, forms[i].isDciEnabled()); //$NON-NLS-1$
			settings.put(
					"CreateInterfaceDciDialog.delta_" + i, forms[i].isDelta()); //$NON-NLS-1$
			settings.put(
					"CreateInterfaceDciDialog.description_" + i, forms[i].getDescription()); //$NON-NLS-1$
		}
	...
	private class InterfaceDciForm extends Composite {
		...
		public InterfaceDciForm(Composite parent, String name,
				String defaultDescription, boolean enabled) {
			...
			new Label(textRow, SWT.NONE)
					.setText(Messages.get().CreateInterfaceDciDialog_Description);
