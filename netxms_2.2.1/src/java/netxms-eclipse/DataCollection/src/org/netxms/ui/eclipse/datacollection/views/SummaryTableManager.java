...
public class SummaryTableManager extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		initShowFilter = safeCast(
				settings.get("SummaryTableManager.showFilter"),
				settings.getBoolean("SummaryTableManager.showFilter"),
				initShowFilter);
	}
	...
	public void createPartControl(Composite parent) {
		...
		final String[] names = { Messages.get().SummaryTableManager_ID,
				Messages.get().SummaryTableManager_MenuPath,
				Messages.get().SummaryTableManager_Title };
		final int[] widths = { 90, 250, 200 };
		viewer = new SortableTableViewer(tableArea, names, widths,
				COLUMN_MENU_PATH, SWT.UP, SortableTableViewer.DEFAULT_STYLE);
	...
	private void refresh() {
		new ConsoleJob(
				"Get configured DCI summary tables", this, Activator.PLUGIN_ID, null) { //$NON-NLS-1$
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<DciSummaryTableDescriptor> list = session
	...
	private void createActions() {
		...
		actionCreateSingleValue = new Action("Create new summary table...",
		...
		actionCreateTableValue = new Action(
				"Create new summary table for table DCIs...",
		...
		// create edit action
		actionEdit = new Action(Messages.get().SummaryTableManager_ActionEdit,
				SharedIcons.EDIT) {
			@Override
			public void run() {
				editSummaryTable();
		...
		actionDelete = new Action(
				Messages.get().SummaryTableManager_ActionDelete,
	...
	private void createSummaryTable(boolean isTableSource) {
		...
		PropertyDialog dlg = PropertyDialog.createDialogOn(
				getSite().getShell(), null, t);
		if (dlg != null) {
			dlg.getShell().setText(
					Messages.get().SummaryTableManager_TitleCreate);
			dlg.open();
			if (t.getId() != 0) {
				// was saved to server
	...
	private void editSummaryTable() {
		...
		final DciSummaryTableDescriptor d = (DciSummaryTableDescriptor) selection
				.getFirstElement();
		new ConsoleJob(Messages.get().SummaryTableManager_ReadJobName, this,
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						PropertyDialog dlg = PropertyDialog.createDialogOn(
								getSite().getShell(), null, t);
						dlg.getShell().setText(
								Messages.get().SummaryTableManager_TitleEdit);
						dlg.open();
						d.updateFromTable(t);
	...
	private void deleteSelection() {
		...
		if (!MessageDialogHelper.openQuestion(getSite().getShell(),
				Messages.get().SummaryTableManager_ConfirmDelete,
				Messages.get().SummaryTableManager_Confirmation))
			return;
		...
		new ConsoleJob(Messages.get().SummaryTableManager_DeleteJobName, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
