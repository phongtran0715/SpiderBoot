...
public class CreateTemplate implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateObjectDialog dlg = new CreateObjectDialog(
				window.getShell(), Messages.get().CreateTemplate_Template);
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateTemplate_JobTitle, part,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_TEMPLATE, dlg.getObjectName(),
						parentId);
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().CreateTemplate_JobError,
						dlg.getObjectName());
			}
		}.start();
	}
