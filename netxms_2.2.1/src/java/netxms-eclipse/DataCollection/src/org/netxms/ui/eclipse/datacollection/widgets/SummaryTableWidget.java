...
public class SummaryTableWidget extends Composite {
	 ...
	 *            widget style
	 * @param viewPart
	 *            view part this widget is associated with
	 * @param tableId
	 *            summary table ID
	 * @param baseObjectId
	 *            base object ID
	 */
	public SummaryTableWidget(Composite parent, int style, IViewPart viewPart,
			int tableId, long baseObjectId) {
		super(parent, style);
		...
		viewer.getTree().addMouseListener(new MouseListener() {
			...
			public void mouseDown(MouseEvent e) {
				if (e.button == 3)
					currentColumn = viewer
							.getColumnAtPoint(new Point(e.x, e.y));
			}
	...
	private void createActions() {
		...
		actionUseMultipliers = new Action(
				Messages.get().LastValues_UseMultipliers, Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionForcePollAll = new Action(
				Messages.get().SummaryTableWidget_ForcePollForAllColumns) {
			@Override
			public void run() {
				forcePoll(true);
			}
		...
		actionShowObjectDetails = new Action(
				Messages.get().SummaryTableWidget_ShowObjectDetails) {
			@Override
			public void run() {
				showObjectDetails();
			}
		...
		actionShowObjectDetails
				.setId("org.netxms.ui.eclipse.datacollection.popupActions.ShowObjectDetails"); //$NON-NLS-1$
	}
	...
	private void createPopupMenu() {
		...
		nodeMenuManager.addMenuListener(new IMenuListener() {
			public void menuAboutToShow(IMenuManager mgr) {
				ObjectContextMenu.fill(mgr, viewPart.getSite(),
		...
		if (viewPart != null) {
			viewPart.getSite()
					.registerContextMenu(
							viewPart.getSite().getId() + ".data", rowMenuManager, viewer); //$NON-NLS-1$
			viewPart.getSite()
					.registerContextMenu(
							viewPart.getSite().getId() + ".node", nodeMenuManager, objectSelectionProvider); //$NON-NLS-1$
		}
	}
	...
	protected void fillContextMenu(IMenuManager manager,
		...
		if ((currentColumn != null)
				&& ((Integer) currentColumn.getData("ID") > 0)) //$NON-NLS-1$
		{
			manager.add(new Action(String.format(
					Messages.get().SummaryTableWidget_ForcePollForNode,
	...
	public void refresh() {
		...
		new ConsoleJob(Messages.get().SummaryTable_JobName, viewPart,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final Table table = session.queryDciSummaryTable(tableId,
						baseObjectId);
	...
	public void update(final Table table) {
		if (!viewer.isInitialized()) {
			...
			final IDialogSettings settings = Activator.getDefault()
					.getDialogSettings();
			final String key = viewPart.getViewSite().getId()
					+ ".SummaryTable." + Integer.toString(tableId); //$NON-NLS-1$
		...
		if (sortingColumnList != null && sortingColumnList.size() > 0) {
			...
			for (int i = 0; i < sortingColumnList.size(); i++) {
				boolean isDesc = sortingColumnList.get(i).charAt(0) == '>' ? true
						: false;
				int index = table.getColumnIndex(sortingColumnList.get(i)
						.substring(1));
				if (index >= 0) {
			...
			table.sort(new Comparator<TableRow>() {
				public int compare(TableRow row1, TableRow row2) {
					...
					while (result == 0 && i < sortItemFin.size()) {
						result = compareItem(row1, row2,
								sortItemFin.get(i).colIndex,
								sortItemFin.get(i).isDesc);
						i++;
					}
					return result;
				}

				private int compareItem(TableRow row1, TableRow row2,
						int index, boolean sortDesc) {
	...
	/**
	 * Set automatic refresh interval. If less or equal 0, automatic refresh
	 * will be disabled.
	 * 
	 * @param interval
	...
	private void showObjectDetails() {
		...
		AbstractObject object = ((ObjectWrapper) selection.getFirstElement())
				.getObject();
		if (object != null) {
			try {
				...
				MessageDialogHelper
						.openError(
								getShell(),
								Messages.get().SummaryTableWidget_Error,
								String.format(
										Messages.get().SummaryTableWidget_CannotOpenObjectDetails,
	...
	private void forcePoll(boolean pollAll) {
		...
		for (Object o : selection.toList()) {
			...
			if (pollAll) {
				...
				int index = ((Table) viewer.getInput())
						.getColumnIndex(currentColumn.getText());
		...
		new ConsoleJob(Messages.get().SummaryTableWidget_ForceDciPoll,
				viewPart, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				monitor.beginTask(Messages.get().SummaryTableWidget_DciPoll,
						requests.size());
	 ...
	 * 
	 * @param sortingColumnList
	 *            columns to be used while sorting
	 */
