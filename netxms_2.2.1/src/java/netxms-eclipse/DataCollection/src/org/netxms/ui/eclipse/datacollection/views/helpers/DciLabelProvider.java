...
public class DciLabelProvider implements ITableLabelProvider, IColorProvider {
	private static final Color FONT_COLOR = new Color(Display.getDefault(),
			new RGB(126, 137, 185));
	...
	public DciLabelProvider() {
		...
		statusImages[DataCollectionItem.ACTIVE] = Activator.getImageDescriptor(
				"icons/active.gif").createImage(); //$NON-NLS-1$
		statusImages[DataCollectionItem.DISABLED] = Activator
				.getImageDescriptor("icons/disabled.gif").createImage(); //$NON-NLS-1$
		statusImages[DataCollectionItem.NOT_SUPPORTED] = Activator
				.getImageDescriptor("icons/unsupported.gif").createImage(); //$NON-NLS-1$

		originTexts.put(DataCollectionItem.AGENT,
				Messages.get().DciLabelProvider_SourceAgent);
		originTexts.put(DataCollectionItem.SNMP,
				Messages.get().DciLabelProvider_SourceSNMP);
		originTexts.put(DataCollectionItem.CHECKPOINT_SNMP,
				Messages.get().DciLabelProvider_SourceCPSNMP);
		originTexts.put(DataCollectionItem.INTERNAL,
				Messages.get().DciLabelProvider_SourceInternal);
		originTexts.put(DataCollectionItem.PUSH,
				Messages.get().DciLabelProvider_SourcePush);
		originTexts.put(DataCollectionItem.WINPERF,
				Messages.get().DciLabelProvider_SourceWinPerf);
		originTexts.put(DataCollectionItem.SMCLP,
				Messages.get().DciLabelProvider_SourceILO);
		originTexts.put(DataCollectionItem.SCRIPT,
				Messages.get().DciLabelProvider_SourceScript);
		originTexts.put(DataCollectionItem.SSH,
				Messages.get().DciLabelProvider_SourceSSH);
		originTexts.put(DataCollectionItem.MQTT,
				Messages.get().DciLabelProvider_SourceMQTT);

		statusTexts.put(DataCollectionItem.ACTIVE,
				Messages.get().DciLabelProvider_Active);
		statusTexts.put(DataCollectionItem.DISABLED,
				Messages.get().DciLabelProvider_Disabled);
		statusTexts.put(DataCollectionItem.NOT_SUPPORTED,
				Messages.get().DciLabelProvider_NotSupported);

		dtTexts.put(DataCollectionItem.DT_INT,
				Messages.get().DciLabelProvider_DT_int32);
		dtTexts.put(DataCollectionItem.DT_UINT,
				Messages.get().DciLabelProvider_DT_uint32);
		dtTexts.put(DataCollectionItem.DT_INT64,
				Messages.get().DciLabelProvider_DT_int64);
		dtTexts.put(DataCollectionItem.DT_UINT64,
				Messages.get().DciLabelProvider_DT_uint64);
		dtTexts.put(DataCollectionItem.DT_FLOAT,
				Messages.get().DciLabelProvider_DT_float);
		dtTexts.put(DataCollectionItem.DT_STRING,
				Messages.get().DciLabelProvider_DT_string);
	}
	...
	public Image getColumnImage(Object element, int columnIndex) {
		...
		return ((status >= 0) && (status < statusImages.length)) ? statusImages[status]
				: null;
	}
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		case DataCollectionEditor.COLUMN_RETENTION:
			...
			return Integer.toString(days)
					+ ((days == 1) ? Messages.get().DciLabelProvider_Day
							: Messages.get().DciLabelProvider_Days);
		...
		case DataCollectionEditor.COLUMN_THRESHOLD:
			...
			if ((dci instanceof DataCollectionItem)) {
				ArrayList<Threshold> list = ((DataCollectionItem) dci)
						.getThresholds();
				for (int i = 0; i < list.size(); i++) {
					Threshold tr = list.get(i);
					int f = tr.getFunction();
					StringBuilder text = new StringBuilder(
							ThresholdLabelProvider.FUNCTIONS[f]);
					text.append(tr.getSampleCount());
					text.append(") "); //$NON-NLS-1$
					if (f != Threshold.F_SCRIPT) {
						text.append(ThresholdLabelProvider.OPERATIONS[tr
								.getOperation()]);
						text.append(' ');
			...
			if ((dci instanceof DataCollectionTable)) {
				List<TableThreshold> list = ((DataCollectionTable) dci)
						.getThresholds();
				for (int i = 0; i < list.size(); i++) {
