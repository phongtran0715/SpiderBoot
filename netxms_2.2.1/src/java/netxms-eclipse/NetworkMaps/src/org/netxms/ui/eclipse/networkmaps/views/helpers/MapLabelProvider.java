...
public class MapLabelProvider extends LabelProvider implements IFigureProvider,
	...
	public MapLabelProvider(ExtendedGraphViewer viewer) {
		...
		for (int i = 0; i < statusImages.length; i++)
			statusImages[i] = StatusDisplayInfo.getStatusImageDescriptor(i)
					.createImage();

		imgNetMap = Activator
				.getImageDescriptor("icons/objects/netmap.png").createImage(); //$NON-NLS-1$
		imgNodeGeneric = Activator
				.getImageDescriptor("icons/objects/node.png").createImage(); //$NON-NLS-1$
		imgNodeOSX = Activator
				.getImageDescriptor("icons/objects/macserver.png").createImage(); //$NON-NLS-1$
		imgNodeWindows = Activator.getImageDescriptor(
				"icons/objects/windowsserver.png").createImage(); //$NON-NLS-1$
		imgNodeLinux = Activator.getImageDescriptor(
				"icons/objects/linuxserver.png").createImage(); //$NON-NLS-1$
		imgNodeFreeBSD = Activator.getImageDescriptor(
				"icons/objects/freebsdserver.png").createImage(); //$NON-NLS-1$
		imgNodeSwitch = Activator
				.getImageDescriptor("icons/objects/switch.png").createImage(); //$NON-NLS-1$
		imgNodeRouter = Activator
				.getImageDescriptor("icons/objects/router.png").createImage(); //$NON-NLS-1$
		imgNodePrinter = Activator.getImageDescriptor(
				"icons/objects/printer.png").createImage(); //$NON-NLS-1$
		imgSubnet = Activator
				.getImageDescriptor("icons/objects/subnet.png").createImage(); //$NON-NLS-1$
		imgService = Activator
				.getImageDescriptor("icons/objects/service.png").createImage(); //$NON-NLS-1$
		imgCluster = Activator
				.getImageDescriptor("icons/objects/cluster.png").createImage(); //$NON-NLS-1$
		imgAccessPoint = Activator.getImageDescriptor(
				"icons/objects/accesspoint.png").createImage(); //$NON-NLS-1$
		imgInterface = Activator.getImageDescriptor(
				"icons/objects/interface.png").createImage(); //$NON-NLS-1$
		imgOther = Activator
				.getImageDescriptor("icons/objects/other.png").createImage(); //$NON-NLS-1$
		imgUnknown = Activator
				.getImageDescriptor("icons/objects/unknown.png").createImage(); //$NON-NLS-1$
		imgResCluster = Activator.getImageDescriptor(
				"icons/resources/cluster_res.png").createImage(); //$NON-NLS-1$

		final Display display = viewer.getControl().getDisplay();
		fontLabel = new Font(display,
				"Verdana", WidgetHelper.scaleTextPoints(display, 6), SWT.NORMAL); //$NON-NLS-1$
		fontTitle = new Font(
				display,
				"Verdana", WidgetHelper.scaleTextPoints(display, 10), SWT.NORMAL); //$NON-NLS-1$
	...
	public String getText(Object element) {
		if (element instanceof NetworkMapObject) {
			AbstractObject object = session
					.findObjectById(((NetworkMapObject) element).getObjectId());
	...
	public Image getImage(Object element) {
		if (element instanceof NetworkMapObject) {
			...
			if (object != null) {
				...
				if (objectImageGuid != null
						&& !objectImageGuid.equals(NXCommon.EMPTY_GUID)) {
					return ImageProvider.getInstance()
							.getImage(objectImageGuid);
				}

				// Try registered network map image providers
				Image img = MapImageProvidersManager.getInstance().getMapImage(
						object);
				if (img != null)
					return img;
	 ...
	 * 
	 * @see
	 * org.eclipse.gef4.zest.core.viewers.IFigureProvider#getFigure(java.lang
	 * .Object)
	 */
	@Override
	public IFigure getFigure(Object element) {
		if (element instanceof NetworkMapObject) {
			...
			case LARGE_LABEL:
				return new ObjectFigureLargeLabel((NetworkMapObject) element,
						this);
			case SMALL_LABEL:
				return new ObjectFigureSmallLabel((NetworkMapObject) element,
						this);
			case ICON:
		...
		if (element instanceof NetworkMapDCIContainer) {
			return new DCIContainerFigure((NetworkMapDCIContainer) element,
					this, viewer);
		}
		if (element instanceof NetworkMapDCIImage) {
			return new DCIImageFigure((NetworkMapDCIImage) element, this,
					viewer);
		}
	...
	public Image getStatusImage(AbstractObject object) {
		Image image = MapImageProvidersManager.getInstance().getStatusIcon(
				object.getStatus());
		if (image == null) {
			try {
	...
	public boolean isElementSelected(NetworkMapElement element) {
		IStructuredSelection selection = (IStructuredSelection) viewer
				.getSelection();
		return (selection != null) ? selection.toList().contains(element)
				: false;
	}
	 ...
	 * 
	 * @see
	 * org.eclipse.gef4.zest.core.viewers.ISelfStyleProvider#selfStyleConnection
	 * (java .lang.Object, org.eclipse.gef4.zest.core.widgets.GraphConnection)
	 */
	@Override
	public void selfStyleConnection(Object element, GraphConnection connection) {
		...
		if (link.hasConnectorName1() && connectionLabelsVisible) {
			ConnectionEndpointLocator sourceEndpointLocator = new ConnectionEndpointLocator(
					connection.getConnectionFigure(), false);
		...
		if (link.hasConnectorName2() && connectionLabelsVisible) {
			ConnectionEndpointLocator targetEndpointLocator = new ConnectionEndpointLocator(
					connection.getConnectionFigure(), true);
		...
		if ((hasName || hasDciData) && connectionLabelsVisible) {
			ConnectionLocator nameLocator = new ConnectionLocator(
		...
		if (link.getStatusObject() != null
				&& link.getStatusObject().size() != 0) {
			ObjectStatus status = ObjectStatus.UNKNOWN;
			for (Long id : link.getStatusObject()) {
				AbstractObject object = session.findObjectById(id);
				if (object != null) {
					ObjectStatus s = object.getStatus();
					if ((s.compareTo(ObjectStatus.UNKNOWN) < 0)
							&& ((status.compareTo(s) < 0) || (status == ObjectStatus.UNKNOWN))) {
						status = s;
			...
			connection.setLineColor(colors.create(ColorConverter
					.rgbFromInt(link.getColor())));
		...
		if (points != null) {
			...
			for (int i = 0; i < points.length; i += 2) {
				if (points[i] == 0x7FFFFFFF)
					break;
				list.add(new AbsoluteBendpoint((int) points[i],
						(int) points[i + 1]));
			}
			return list;
		} else {
	...
	/**
	 * @param objectFigureType
	 *            the objectFigureType to set
	 */
	 ...
	 * 
	 * @see
	 * org.eclipse.gef4.zest.core.viewers.ISelfStyleProvider#selfStyleNode(java
	 * .lang.Object, org.eclipse.gef4.zest.core.widgets.GraphNode)
	 */
	@Override
	...
	/**
	 * @param defaultLinkColor
	 *            the defaultLinkColor to set
	 */
	...
	public DciValue[] getNodeLastValues(long nodeId) {
		return ((MapContentProvider) viewer.getContentProvider())
				.getNodeLastValues(nodeId);
	}
	...
	/**
	 * @param connectionLabelsVisible
	 *            the connectionLabelsVisible to set
	 */
	...
	/**
	 * @param connectionsVisible
	 *            the connectionsVisible to set
	 */
