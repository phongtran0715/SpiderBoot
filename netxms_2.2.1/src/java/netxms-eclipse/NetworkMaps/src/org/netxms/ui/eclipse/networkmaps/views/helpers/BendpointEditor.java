...
public class BendpointEditor {
	...
	private List<AbsoluteBendpoint> bendpoints = new ArrayList<AbsoluteBendpoint>(
			16);
	private Map<AbsoluteBendpoint, BendpointHandle> handles = new HashMap<AbsoluteBendpoint, BendpointHandle>(
			16);
	 ...
	 */
	public BendpointEditor(NetworkMapLink linkObject,
			GraphConnection connection, ExtendedGraphViewer viewer) {
		...
		if (points != null) {
			for (int i = 0; i < points.length; i += 2) {
				if (points[i] == 0x7FFFFFFF)
					break;
				AbsoluteBendpoint bp = new AbsoluteBendpoint((int) points[i],
						(int) points[i + 1]);
				bendpoints.add(bp);
	...
	private void setRoutingConstraints() {
		...
		ConnectionRouter router = (ConnectionRouter) connection
				.getData("ROUTER"); //$NON-NLS-1$
	...
	private void addBendPoint(int x, int y) {
		...
		if (bendpoints.size() > 1) {
			...
			if (index == 0) {
				Point pnode = connectionFigure.getTargetAnchor()
						.getReferencePoint();
				AbsoluteBendpoint plast = bendpoints.get(bendpoints.size() - 1);
				if (isLineContainsPoint(pnode.x, pnode.y, plast.x, plast.y, x,
						y))
	 ...
	 * @return
	 */
	private boolean isLineContainsPoint(int x1, int y1, int x2, int y2, int px,
			int py) {
		int tolerance = 7;
	 ...
	 */
	private class BendpointHandle extends RoundedRectangle implements
		...
		BendpointHandle(AbsoluteBendpoint bp) {
			...
			setLocation(new Point(bp.x - HANDLE_SIZE / 2, bp.y - HANDLE_SIZE
					/ 2));
		...
		public void mouseDragged(MouseEvent me) {
			if (drag) {
				viewer.showCrosshair(me.x, me.y);
				setLocation(new Point(me.x - HANDLE_SIZE / 2, me.y
						- HANDLE_SIZE / 2));
				me.consume();
			}
		}
