...
public class MapObjectFilter extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		checkboxEnableFilter
				.setText(Messages.get().MapObjectFilter_FilterObjects);
	...
	protected void applyChanges(final boolean isApply) {
		...
		if (initialFilter.equals(filter)
				&& (checkboxEnableFilter.getSelection() == initialEnable))
		...
		md.setFilter(filter);
		md.setObjectFlags(
				checkboxEnableFilter.getSelection() ? NetworkMap.MF_FILTER_OBJECTS
						: 0, NetworkMap.MF_FILTER_OBJECTS);

		new ConsoleJob(Messages.get().MapObjectFilter_JobTitle, null,
