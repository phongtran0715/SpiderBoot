...
public class VlanMap extends AbstractNetworkMapView {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		if (parts.length < 2)
			throw new PartInitException(
					Messages.get().VlanMap_IncorrectSecondaryId);
		try {
			vlanId = Integer.parseInt(parts[1]);
		} catch (NumberFormatException e) {
			throw new PartInitException(
					Messages.get().VlanMap_IncorrectSecondaryId, e);
		}
		setPartName(String.format(Messages.get().VlanMap_PartName, vlanId,
	...
	protected void buildMapPage() {
		...
		new ConsoleJob(String.format(Messages.get().VlanMap_JobTitle,
				rootObject.getObjectName()), this, Activator.PLUGIN_ID, null) {
			@Override
			...
			protected void jobFailureHandler() {
				...
				NetworkMapPage page = new NetworkMapPage(ID + vlanId);
				page.addElement(new NetworkMapObject(mapPage.createElementId(),
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().VlanMap_JobError,
						rootObject.getObjectName());
			}
		}.start();
	}
	 ...
	 * @throws NXCException
	 */
	private long collectVlanInfo(NetworkMapPage page, Node root)
			throws Exception {
	...
	/**
	 * Process single member port of VLAN. Will add connected switch on other
	 * site to the map.
	 * 
	 * @param page
	 * @param root
	 ...
	 * @throws Exception
	 */
	private void processVlanPort(NetworkMapPage page, Node root, Port port,
			long rootElementId) throws Exception {
		Interface iface = (Interface) session.findObjectById(
				port.getObjectId(), Interface.class);
		if (iface != null) {
			Node peerNode = (Node) session.findObjectById(
					iface.getPeerNodeId(), Node.class);
			if ((peerNode != null)
					&& ((peerNode.getFlags() & Node.NF_IS_BRIDGE) != 0)) {
				try {
					long nodeElementId = collectVlanInfo(page, peerNode);
					if (nodeElementId != -1) {
						Interface peerIf = (Interface) session.findObjectById(
								iface.getPeerInterfaceId(), Interface.class);
						page.addLink(new NetworkMapLink(null,
								NetworkMapLink.NORMAL, rootElementId,
								nodeElementId, iface.getObjectName(),
								(peerIf != null) ? peerIf.getObjectName()
