 ...
 */
public class MapContentProvider implements
		...
		while (syncRunning) {
			synchronized (cachedDciValues) {
				for (Entry<Long, DciValue[]> e : cachedDciValues.entrySet()) {
					try {
						DciValue[] values = session.getLastValues(e.getKey(),
								true, false, false);
				...
				display.asyncExec(new Runnable() {
					...
					public void run() {
						...
						synchronized (cachedDciValues) {
							for (Entry<Long, DciValue[]> e : cachedDciValues
									.entrySet()) {
								if ((e.getValue() == null)
										|| (e.getValue().length == 0))
									continue;
								NetworkMapObject o = page.findObjectElement(e
										.getKey());
								if (o != null)
									viewer.refresh(o);
							}
							...
							if (page != null && page.getElements() != null) {
								for (NetworkMapElement e : page.getElements()) {
									if ((!(e instanceof NetworkMapDCIContainer) || !((NetworkMapDCIContainer) e)
											.hasDciData())
											&& !(e instanceof NetworkMapDCIImage))
										continue;
	...
	public Object[] getElements(Object inputElement) {
		...
		List<NetworkMapElement> elements = new ArrayList<NetworkMapElement>(
				((NetworkMapPage) inputElement).getElements().size());
		for (NetworkMapElement e : ((NetworkMapPage) inputElement)
				.getElements())
			if (!(e instanceof NetworkMapDecoration)
					&& !(e instanceof NetworkMapDCIContainer)
					&& !(e instanceof NetworkMapDCIImage)
					&& !(e instanceof NetworkMapTextBox))
				elements.add(e);
	 ...
	 * 
	 * @see
	 * org.eclipse.gef4.zest.core.viewers.IGraphEntityRelationshipContentProvider
	 * #getRelationships(java.lang.Object, java.lang.Object)
	 */
	@Override
	public Object[] getRelationships(Object source, Object dest) {
		return page.findLinks((NetworkMapElement) source,
				(NetworkMapElement) dest).toArray();
	}
