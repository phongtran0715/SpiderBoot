...
public class CreateMapGroup implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateObjectDialog dlg = new CreateObjectDialog(
				window.getShell(), Messages.get().CreateMapGroup_DialogTitle);
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateMapGroup_JobName, part,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_NETWORKMAPGROUP,
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().CreateMapGroup_JobError,
						dlg.getObjectName());
			}
		}.start();
	}
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if ((selection instanceof IStructuredSelection)
			...
			final Object object = ((IStructuredSelection) selection)
					.getFirstElement();
			if ((object instanceof NetworkMapGroup)
					|| (object instanceof NetworkMapRoot)) {
