	...
	protected Control createContents(Composite parent) {
		...
		final SelectionListener listener = new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				enableControls(
						radioTypeImage.getSelection(),
						disableGeolocationBackground ? false : radioTypeGeoMap
								.getSelection());
			}
		...
		radioTypeNone.setSelection(object.getBackground().equals(
				NXCommon.EMPTY_GUID));
		...
		radioTypeImage
				.setSelection(!object.getBackground().equals(
						NXCommon.EMPTY_GUID)
						&& !object.getBackground().equals(
								NetworkMap.GEOMAP_BACKGROUND));
		...
		if (!disableGeolocationBackground) {
			...
			radioTypeGeoMap.setSelection(object.getBackground().equals(
					NetworkMap.GEOMAP_BACKGROUND));
		...
		backgroundColor.setColorValue(ColorConverter.rgbFromInt(object
				.getBackgroundColor()));
	...
	protected boolean applyChanges(final boolean isApply) {
		...
		if (radioTypeNone.getSelection()) {
			md.setMapBackground(NXCommon.EMPTY_GUID, new GeoLocation(false), 0,
					ColorConverter.rgbToInt(backgroundColor.getColorValue()));
		} else if (radioTypeImage.getSelection()) {
			md.setMapBackground(image.getImageGuid(), new GeoLocation(false),
					0, ColorConverter.rgbToInt(backgroundColor.getColorValue()));
		} else if (!disableGeolocationBackground
				&& radioTypeGeoMap.getSelection()) {
			...
			try {
				...
				MessageDialogHelper.openError(getShell(),
						Messages.get().MapBackground_Error,
						Messages.get().MapBackground_GeoLocFormatError);
				return false;
			}
			md.setMapBackground(NetworkMap.GEOMAP_BACKGROUND, location,
					zoomSpinner.getSelection(),
					ColorConverter.rgbToInt(backgroundColor.getColorValue()));
		}
		...
		new ConsoleJob(Messages.get().MapBackground_JobTitle
			...
			protected String getErrorMessage() {
				return Messages.get().MapBackground_JobError
