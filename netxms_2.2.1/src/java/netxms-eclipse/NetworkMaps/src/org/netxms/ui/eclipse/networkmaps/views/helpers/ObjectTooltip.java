...
public class ObjectTooltip extends Figure {
	...
	public ObjectTooltip(AbstractObject object, MapLabelProvider labelProvider) {
		...
		if ((object instanceof Node)
				...
				&& !((Node) object).getPrimaryIP().getAddress()
						.isAnyLocalAddress()) {
			StringBuilder sb = new StringBuilder(((Node) object).getPrimaryIP()
					.getHostAddress());
	 ...
	 * Status chart
	 */
	private void addStatusChart(AbstractObject object,
			MapLabelProvider labelProvider) {
		BirtChartFigure chart = new BirtChartFigure(BirtChartFigure.BAR_CHART,
				labelProvider.getColors());
		add(chart);
		GridData gd = new GridData();
		gd.horizontalSpan = 2;
