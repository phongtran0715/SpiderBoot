...
public class MapOptions extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		objectDisplayGroup
				.setText(Messages.get().MapOptions_DefaultDispOptions);
		...
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		objectDisplayMode = WidgetHelper.createLabeledCombo(objectDisplayGroup,
				SWT.READ_ONLY, Messages.get().MapOptions_DisplayObjectsAs, gd);
		...
		checkShowStatusIcon
				.setSelection((object.getFlags() & NetworkMap.MF_SHOW_STATUS_ICON) != 0);
		...
		checkShowStatusFrame
				.setSelection((object.getFlags() & NetworkMap.MF_SHOW_STATUS_FRAME) != 0);
		...
		checkShowStatusBkgnd
				.setSelection((object.getFlags() & NetworkMap.MF_SHOW_STATUS_BKGND) != 0);
		...
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		routingAlgorithm = WidgetHelper.createLabeledCombo(linkGroup,
				SWT.READ_ONLY, Messages.get().MapOptions_RoutingAlg, gd);
		...
		linkColor.setColorValue(ColorConverter.rgbFromInt(object
				.getDefaultLinkColor()));
		...
		if (object.getMapType() != NetworkMap.TYPE_CUSTOM) {
			...
			checkIncludeEndNodes
					.setText(Messages.get().MapOptions_IncludeEndNodes);
			checkIncludeEndNodes
					.setSelection((object.getFlags() & NetworkMap.MF_SHOW_END_NODES) != 0);

			checkCustomRadius = new Button(topoGroup, SWT.CHECK);
			checkCustomRadius
					.setText(Messages.get().MapOptions_CustomDiscoRadius);
			...
			topologyRadius = WidgetHelper.createLabeledSpinner(topoGroup,
					SWT.BORDER, Messages.get().MapOptions_TopoDiscoRadius, 1,
					255, WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		checkCalculateStatus
				.setText(Messages.get().MapOptions_CalcStatusFromObjects);
		checkCalculateStatus
				.setSelection((object.getFlags() & NetworkMap.MF_CALCULATE_STATUS) != 0);
	...
	protected boolean applyChanges(final boolean isApply) {
		...
		int flags = 0;
		if ((checkIncludeEndNodes != null)
				&& checkIncludeEndNodes.getSelection())
		...
		md.setMapObjectDisplayMode(MapObjectDisplayMode
				.getByValue(objectDisplayMode.getSelectionIndex()));
		...
		new ConsoleJob(Messages.get().MapOptions_JobTitle
			...
			protected String getErrorMessage() {
				return Messages.get().MapOptions_JobError
