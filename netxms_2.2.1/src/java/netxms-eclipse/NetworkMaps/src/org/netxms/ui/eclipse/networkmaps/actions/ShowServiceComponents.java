 ...
 * 
 */
public class ShowServiceComponents implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		if (object != null) {
			try {
				window.getActivePage().showView(ServiceComponents.ID,
						Long.toString(object.getObjectId()),
						IWorkbenchPage.VIEW_ACTIVATE);
			} catch (PartInitException e) {
				MessageDialogHelper.openError(window.getShell(),
						Messages.get().ShowServiceComponents_Error,
						String.format(
								Messages.get().ShowServiceComponents_ErrorText,
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if ((selection instanceof IStructuredSelection)
			...
			if ((obj instanceof Container) || (obj instanceof ServiceRoot)) {
