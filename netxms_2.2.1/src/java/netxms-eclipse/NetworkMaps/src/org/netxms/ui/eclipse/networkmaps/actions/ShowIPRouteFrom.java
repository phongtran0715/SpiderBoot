...
/**
 * Show IP route between two nodes (selection as destination)
 */
	...
	public void run(IAction action) {
		...
		if (selection.length == 0) {
			MessageDialogHelper.openError(window.getShell(),
					Messages.get().ShowIPRoute_Error,
					Messages.get().ShowIPRoute_InvalidTarget);
			return;
		}
		...
		try {
			window.getActivePage()
					.showView(
							IPRouteMap.ID,
							Long.toString(selection[0].getObjectId())
									+ "&" + Long.toString(node.getObjectId()), IWorkbenchPage.VIEW_ACTIVATE); //$NON-NLS-1$
		} catch (PartInitException e) {
			MessageDialogHelper.openError(
					window.getShell(),
					Messages.get().ShowIPRoute_Error,
					String.format(Messages.get().ShowIPRoute_ErrorOpenView,
