...
public class MapLinkGeneral extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		radioColorDefault.setSelection((object.getColor() < 0)
				&& (object.getStatusObject().size() == 0));
		radioColorDefault.addSelectionListener(listener);

		radioColorObject = new Button(colorGroup, SWT.RADIO);
		radioColorObject
				.setText(Messages.get().MapLinkGeneral_BasedOnObjStatus);
		...
		list = new List(nodeSelectionGroup, SWT.BORDER | SWT.SINGLE
		...
		if (object.getStatusObject() != null) {
			for (int i = 0; i < object.getStatusObject().size(); i++) {
				final AbstractObject obj = ((NXCSession) ConsoleSharedData
						.getSession()).findObjectById(object.getStatusObject()
						.get(i));
				list.add((obj != null) ? obj.getObjectName()
						: ("<" + Long.toString(object.getStatusObject().get(i)) + ">")); //$NON-NLS-1$ //$NON-NLS-2$
			}
		}
		list.setEnabled(radioColorObject.getSelection());
		...
		radioColorCustom.setSelection((object.getColor() >= 0)
				&& (object.getStatusObject().size() == 0));
		...
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		routingAlgorithm = WidgetHelper.createLabeledCombo(dialogArea,
				SWT.READ_ONLY, Messages.get().MapLinkGeneral_RoutingAlg, gd);
	...
	private void addObject() {
		...
		if (dlg.open() == Window.OK) {
			AbstractObject[] objects = dlg
					.getSelectedObjects(AbstractObject.class);
			if (objects.length > 0) {
				for (AbstractObject obj : objects) {
					object.addStatusObject(obj.getObjectId());
					list.add((obj != null) ? obj.getObjectName()
							: ("<" + Long.toString(obj.getObjectId()) + ">")); //$NON-NLS-1$ //$NON-NLS-2$
				}
			}
		}
	}
