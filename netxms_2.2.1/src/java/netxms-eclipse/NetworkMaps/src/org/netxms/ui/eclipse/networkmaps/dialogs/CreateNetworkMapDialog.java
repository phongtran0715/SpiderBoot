...
public class CreateNetworkMapDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		mapType = WidgetHelper.createLabeledCombo(dialogArea, SWT.READ_ONLY,
				Messages.get().CreateNetworkMapDialog_MapType,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		seedObjectSelector
				.setLabel(Messages.get().CreateNetworkMapDialog_SeedNode);
		seedObjectSelector.setObjectClass(Node.class);
		seedObjectSelector.setClassFilter(ObjectSelectionDialog
	...
	protected void okPressed() {
		...
		if (name.isEmpty()) {
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().CreateNetworkMapDialog_Warning,
					Messages.get().CreateNetworkMapDialog_PleaseEnterName);
			return;
		}
		...
		if (type > 0) {
			...
			if (seedObject == 0) {
				MessageDialogHelper.openWarning(getShell(),
						Messages.get().CreateNetworkMapDialog_Warning,
						Messages.get().CreateNetworkMapDialog_PleaseSelectSeed);
				return;
			}
		}
