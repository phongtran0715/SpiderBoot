...
public class SparseTree implements LayoutAlgorithm {
	...
	public void applyLayout(boolean clean) {
		...
		Collections.sort(rows, new Comparator<ArrayList<EntityLayout>>() {
			@Override
			public int compare(ArrayList<EntityLayout> o1,
					ArrayList<EntityLayout> o2) {
				return (int) (o1.get(0).getLocation().y - o2.get(0)
						.getLocation().y);
			}
		});
		...
		for (int currRowIdx = 0; currRowIdx < rows.size(); currRowIdx++) {
			...
			for (int currNodeIdx = 1; currNodeIdx < row.size(); currNodeIdx++) {
				...
				double minimalPos = prevNodePos + prevNode.getSize().width
						+ MINIMAL_DISTANCE;
		...
		for (int currRowIdx = rows.size() - 1; currRowIdx > 0; currRowIdx--) {
				...
				if (parent != nextParent) {
					if (parent != null)
						centerNode(
								...
								parent,
								leftmostX,
								row.get(currNodeIdx - 1).getLocation().x
										+ row.get(currNodeIdx - 1).getSize().width);
					parent = nextParent;
			...
			if (parent != null) {
				...
				centerNode(rows, currRowIdx, firstChildIdx, parent, leftmostX,
						currNode.getLocation().x + currNode.getSize().width);
			}
		}
	}
	 ...
	 *            row list
	 * @param currRowIdx
	 *            current row (contains child nodes)
	 * @param firstChildIdx
	 *            index of first child node
	 * @param node
	 *            current node
	 * @param leftmostX
	 *            position of leftmost child node
	 * @param rightmostX
	 *            position of rightmost child node + it's length
	 */
	private void centerNode(ArrayList<ArrayList<EntityLayout>> rows,
			int currRowIdx, int firstChildIdx, EntityLayout node,
			double leftmostX, double rightmostX) {
		double newX = leftmostX + (rightmostX - leftmostX) / 2
	 ...
	 *            shift value
	 */
	private void shiftChildNodes(ArrayList<ArrayList<EntityLayout>> rows,
			int currRowIdx, EntityLayout parent, double shift) {
	 ...
	 *            node
	 */
	private void addToRow(ArrayList<ArrayList<EntityLayout>> rows,
			EntityLayout node) {
