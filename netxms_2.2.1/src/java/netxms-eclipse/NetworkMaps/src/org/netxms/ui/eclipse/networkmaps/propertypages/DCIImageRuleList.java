...
public class DCIImageRuleList extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		new Label(RuleArea, SWT.NONE)
				.setText(Messages.get().DCIImageRuleList_Rules);

		RuleList = new TableViewer(RuleArea, SWT.BORDER | SWT.MULTI
				| SWT.FULL_SELECTION);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
	...
	private void deleteRules() {
		final IStructuredSelection selection = (IStructuredSelection) RuleList
				.getSelection();
	...
	private void editRule() {
		final IStructuredSelection selection = (IStructuredSelection) RuleList
				.getSelection();
		if (selection.size() == 1) {
			final DCIImageRule rule = (DCIImageRule) selection
					.getFirstElement();
			EditDCIImageRuleDialog dlg = new EditDCIImageRuleDialog(getShell(),
					rule);
	...
	private void saveSettings() {
		WidgetHelper.saveColumnSettings(RuleList.getTable(), Activator
