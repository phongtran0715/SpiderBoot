...
public class ObjectFigureIcon extends ObjectFigure {
	 ...
	 * Constructor
	 */
	public ObjectFigureIcon(NetworkMapObject element,
	...
	private void updateSize(MapLabelProvider labelProvider) {
		...
		setSize(Math.max(Math.max(ls.width, imageWidth), imageWidth
				+ BACKGROUND_MARGIN_X * 2 + FRAME_LINE_WIDTH), imageHeight
	...
	protected void paintFigure(Graphics gc) {
		...
		if (labelProvider.isShowStatusBackground() || isElementSelected()) {
			...
			gc.setBackgroundColor(isElementSelected() ? SELECTION_COLOR
					: StatusDisplayInfo.getStatusColor(object.getStatus()));
			gc.setAlpha(64);
		...
		if (labelProvider.isShowStatusFrame()) {
			...
			gc.setForegroundColor(StatusDisplayInfo.getStatusColor(object
					.getStatus()));
