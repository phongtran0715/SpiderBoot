...
public class DCIContainerDataSources extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		final String[] columnNames = {
				Messages.get().DCIContainerDataSources_ColPos,
				Messages.get().DCIContainerDataSources_ColNode,
				Messages.get().DCIContainerDataSources_ColParameter,
				Messages.get().DCIContainerDataSources_ColName };
	...
	private void addItem() {
		...
		if (dlg.open() == Window.OK) {
			...
			for (DciValue item : selection) {
				...
				select.add(dci);
				labelProvider.addCacheEntry(dci.getNodeId(), dci.dciId,
						dci.name);
	...
	private boolean applyChanges(final boolean isApply) {
		container.setObjectDCIArray(dciList.toArray(new SingleDciConfig[dciList
				.size()]));
		return true;
	}
