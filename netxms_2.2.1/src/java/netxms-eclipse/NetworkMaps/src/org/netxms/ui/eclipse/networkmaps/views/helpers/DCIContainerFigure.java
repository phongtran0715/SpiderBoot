...
public class DCIContainerFigure extends DecorationLayerAbstractFigure {
	 ...
	 * @param labelProvider
	 */
	public DCIContainerFigure(NetworkMapDCIContainer container,
			MapLabelProvider labelProvider, ExtendedGraphViewer viewer) {
		super(container, viewer);

		this.container = container;
		dciValueProvider = LinkDciValueProvider.getInstance();
		String text = dciValueProvider.getDciDataAsString(container
				.getDciAsList());

		backgroundColor = labelProvider.getColors().create(
				ColorConverter.rgbFromInt(container.getBackgroundColor()));
		final Color textColor = labelProvider.getColors().create(
				ColorConverter.rgbFromInt(container.getTextColor()));
		borderColor = labelProvider.getColors().create(
				ColorConverter.rgbFromInt(container.getBorderColor()));

		label = new Label(
				text.isEmpty() ? Messages.get().DCIContainerFigure_NoValue
						: text);
		...
		setSize(d.width + (border * 2) + MARGIN, d.height + (border * 2)
				+ MARGIN);
	...
	public void refresh() {
		...
		String text = dciValueProvider.getDciDataAsString(container
				.getDciAsList());
		label.setText(text.isEmpty() ? Messages.get().DCIContainerFigure_NoValue
				: text);
