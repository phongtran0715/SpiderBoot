...
public class NetworkMapOpenHandler implements ObjectOpenHandler {
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.objectbrowser.api.ObjectOpenHandler#openObject(
	...
	public boolean openObject(AbstractObject object) {
		...
		try {
			window.getActivePage().showView(PredefinedMap.ID,
					Long.toString(object.getObjectId()),
					IWorkbenchPage.VIEW_ACTIVATE);
		} catch (PartInitException e) {
			MessageDialogHelper.openError(window.getShell(),
					Messages.get().NetworkMapOpenHandler_Error, String.format(
							Messages.get().NetworkMapOpenHandler_ErrorText,
