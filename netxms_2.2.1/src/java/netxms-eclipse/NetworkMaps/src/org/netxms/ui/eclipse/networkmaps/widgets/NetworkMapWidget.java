...
public class NetworkMapWidget extends Composite {
		...
		viewer.addDoubleClickListener(new IDoubleClickListener() {
			...
			public void doubleClick(DoubleClickEvent event) {
				...
				if (selection.isEmpty()
						|| !(selection.getFirstElement() instanceof NetworkMapObject))
					return;

				AbstractObject object = ConsoleSharedData
						.getSession()
						.findObjectById(
								((NetworkMapObject) selection.getFirstElement())
										.getObjectId());
				if (object == null)
					return;
	...
	private void openDrillDownObject(AbstractObject object) {
		long objectId = (object instanceof NetworkMap) ? object.getObjectId()
				: object.getDrillDownObjectId();
		if (objectId == 0)
			return;
		...
		if (viewPart != null && test instanceof Dashboard) {
			try {
				...
				MessageDialogHelper.openError(viewPart.getSite().getShell(),
						Messages.get().AbstractNetworkMapView_Error,
						String.format("Cannot open drill-down object view: %s",
								e.getMessage()));
			}
		}
	}
	...
	private void goBack() {
		...
		NetworkMap map = ConsoleSharedData.getSession().findObjectById(
				submapId, NetworkMap.class);
		if (map != null) {
	 ...
	 * @param mapObject
	 * @param resetHistory
	 *            true to reset drill-down history
	 */
	...
	public void setLayoutAlgorithm(MapLayoutAlgorithm alg) {
		...
		case VTREE:
			...
			break;
		case SPARSE_VTREE:
			TreeLayoutAlgorithm mainLayoutAlgorithm = new TreeLayoutAlgorithm(
			...
			algorithm = new CompositeLayoutAlgorithm(new LayoutAlgorithm[] {
					mainLayoutAlgorithm, new SparseTree() });
			break;
		default:
