...
public class DecorationFigure extends DecorationLayerAbstractFigure {
	 ...
	 * @param labelProvider
	 */
	public DecorationFigure(NetworkMapDecoration decoration,
		...
		if (decoration.getDecorationType() == NetworkMapDecoration.GROUP_BOX) {
			...
			label.setBackgroundColor(labelProvider.getColors().create(
					ColorConverter.rgbFromInt(decoration.getColor())));
			label.setForegroundColor(SharedColors.getColor(
					SharedColors.MAP_GROUP_BOX_TITLE, Display.getCurrent()));
		...
		} else // Image
		{
			try {
				...
				org.eclipse.swt.graphics.Rectangle bounds = ImageProvider
						.getInstance().getImage(guid).getBounds();
	...
	private void createResizeHandle(int pos) {
		...
		case TOP_LEFT:
			...
			handle.setCursor(Display.getCurrent().getSystemCursor(
					SWT.CURSOR_SIZENWSE));
			break;
		case TOP_RIGHT:
	...
	private void drawGroupBox(Graphics gc) {
		...
		final Color color = labelProvider.getColors().create(
				ColorConverter.rgbFromInt(decoration.getColor()));
