...
public class IPRouteMap extends AbstractNetworkMapView {
	...
	public void init(IViewSite site) throws PartInitException {
		super.init(site);
		setPartName(Messages.get().IPRouteMap_PartTitle
				+ " - [" + rootObject.getObjectName() + " - " + targetObject.getObjectName() + "]"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	}
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.networkmaps.views.NetworkMap#parseSecondaryId(java
	 * .lang.String[])
	 */
	@Override
	protected void parseSecondaryId(String[] parts) throws PartInitException {
		...
		if (targetObject == null)
			throw new PartInitException(
					Messages.get().IPRouteMap_TargetObjectNotExist);
	}
	...
	protected void buildMapPage() {
		...
		new ConsoleJob(String.format(Messages.get().IPRouteMap_JobTitle,
				rootObject.getObjectName(), targetObject.getObjectName()),
			...
			protected void jobFailureHandler() {
				...
				NetworkMapPage page = new NetworkMapPage(ID
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().IPRouteMap_JobError,
						rootObject.getObjectName(),
						targetObject.getObjectName());
			}
		}.start();
	...
	private void getRoute(Display display) throws Exception {
		final NetworkPath path = session.getNetworkPath(
				rootObject.getObjectId(), targetObject.getObjectId());
		final NetworkMapPage page = new NetworkMapPage(
				ID
						+ "@" + rootObject.getObjectName() + "@" + targetObject.getObjectName()); //$NON-NLS-1$ //$NON-NLS-2$
		...
		for (final HopInfo h : path.getPath()) {
			...
			if (prevElementId != 0) {
				NetworkMapLink link = new NetworkMapLink(
						prevHop.isVpn() ? NetworkMapLink.VPN
								: NetworkMapLink.NORMAL, prevElementId,
						elementId);
