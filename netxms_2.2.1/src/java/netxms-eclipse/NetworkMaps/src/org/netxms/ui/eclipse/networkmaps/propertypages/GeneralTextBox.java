...
public class GeneralTextBox extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		textBox = (NetworkMapTextBox) getElement().getAdapter(
				NetworkMapTextBox.class);
		...
		labeledText = WidgetHelper.createLabeledText(dialogArea, SWT.MULTI
				| SWT.BORDER, SWT.DEFAULT, "Text", textBox.getText(), gd);
		labeledText.setTextLimit(255);

		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		backgroundSelector = WidgetHelper.createLabeledColorSelector(
				dialogArea, "Background color", gd);
		backgroundSelector.setColorValue(ColorConverter.rgbFromInt(textBox
				.getBackgroundColor()));

		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		textSelector = WidgetHelper.createLabeledColorSelector(dialogArea,
				"Text color", gd);
		textSelector.setColorValue(ColorConverter.rgbFromInt(textBox
				.getTextColor()));

		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		borderSelector = WidgetHelper.createLabeledColorSelector(dialogArea,
				"Border color", gd);
		borderSelector.setColorValue(ColorConverter.rgbFromInt(textBox
				.getBorderColor()));
		...
		fontSize = WidgetHelper.createLabeledSpinner(dialogArea, SWT.BORDER,
				"Text size", 1, 100, WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		drillDownObject.setClassFilter(ObjectSelectionDialog
	...
	private boolean applyChanges(final boolean isApply) {
		...
		textBox.setBackgroundColor(ColorConverter.rgbToInt(backgroundSelector
				.getColorValue()));
		textBox.setTextColor(ColorConverter.rgbToInt(textSelector
				.getColorValue()));
		textBox.setBorderRequired(borderCheck.getSelection());
		textBox.setBorderColor(ColorConverter.rgbToInt(borderSelector
				.getColorValue()));
