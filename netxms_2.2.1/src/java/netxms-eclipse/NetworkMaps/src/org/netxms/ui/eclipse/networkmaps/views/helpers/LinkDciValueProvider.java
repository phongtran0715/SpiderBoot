...
public class LinkDciValueProvider {
	...
	private Set<MapDCIInstance> dciIDList = Collections
			.synchronizedSet(new HashSet<MapDCIInstance>());
		...
		while (syncRunning) {
			synchronized (cachedDciValues) {
				synchronized (dciIDList) {
					...
					try {
						if (dciIDList.size() > 0) {
							DciValue[] values = session
									.getLastValues(dciIDList);
	...
	public void addDci(long nodeID, long dciID, NetworkMapPage mapPage) {
		synchronized (dciIDList) {
			...
			if (!exists) {
				dciIDList.add(new MapDCIInstance(dciID, nodeID,
						DataCollectionItem.DCO_TYPE_ITEM, mapPage.getId()));
				syncThread.interrupt();
			}
		}
	}
	 ...
	 * @param mapPage
	 */
	public void addDci(long nodeID, long dciID, String column, String instance,
			NetworkMapPage mapPage) {
		synchronized (dciIDList) {
			boolean exists = false;
			for (MapDCIInstance item : dciIDList) {
				if (item.getDciID() == dciID) {
			...
			if (!exists) {
				dciIDList.add(new MapDCIInstance(dciID, nodeID, column,
						instance, DataCollectionItem.DCO_TYPE_TABLE, mapPage
								.getId()));
				syncThread.interrupt();
			}
		}
	...
	public String getDciDataAsString(NetworkMapLink link) {
		...
		for (int i = 0; i < dciList.length;) {
			...
			if (v != null) {
				...
				result += v
						.format(formatString.isEmpty() ? "%s" : formatString); //$NON-NLS-1$
			}
			if (++i != dciList.length)
				result += "\n"; //$NON-NLS-1$
		}
