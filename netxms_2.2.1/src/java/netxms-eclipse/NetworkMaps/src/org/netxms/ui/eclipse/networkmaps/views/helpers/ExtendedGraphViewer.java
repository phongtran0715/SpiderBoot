...
public class ExtendedGraphViewer extends GraphViewer {
	private static final double[] zoomLevels = { 0.10, 0.25, 0.50, 0.75, 1.00,
	...
	public ExtendedGraphViewer(Composite composite, int style) {
		...
		MouseListener backgroundMouseListener = new MouseListener() {
			...
			public void mousePressed(MouseEvent me) {
				...
				IFigure figureUnderMouse = graph.getFigureAt(mousePoint.x,
						mousePoint.y);
		...
		graph.getLightweightSystem().getRootFigure()
		...
		snapToGridListener = new MouseListener() {
			...
			public void mouseReleased(MouseEvent me) {
				if (snapToGrid
						&& (graph.getRootLayer().findFigureAt(me.x, me.y) != null))
					alignToGrid(true);
			}
	 ...
	 * 
	 * @see
	 * org.eclipse.gef4.zest.core.viewers.GraphViewer#inputChanged(java.lang
	...
	protected void inputChanged(Object input, Object oldInput) {
		...
		if ((getContentProvider() instanceof MapContentProvider)
				&& (getLabelProvider() instanceof MapLabelProvider)) {
			mapDecorations = ((MapContentProvider) getContentProvider())
					.getDecorations(input);
			if (mapDecorations != null) {
				MapLabelProvider lp = (MapLabelProvider) getLabelProvider();
				for (NetworkMapElement d : mapDecorations) {
					DecorationLayerAbstractFigure figure = (DecorationLayerAbstractFigure) lp
							.getFigure(d);
					figure.setLocation(new org.eclipse.draw2d.geometry.Point(d
							.getX(), d.getY()));
	...
	/**
	 * Set selection on decoration layer. Intended to be called from decoration
	 * figure.
	 * 
	 * @param d
	 *            decoartion object
	 * @param addToExisting
	 *            if true, add to existing selection
	 */
	protected void setDecorationSelection(NetworkMapElement d,
	 ...
	 * 
	 * @see
	 * org.eclipse.gef4.zest.core.viewers.GraphViewer#setSelectionToWidget(java
	 * .util.List, boolean)
	 */
		...
		if (l != null) {
			...
			for (Object o : l) {
				if ((o instanceof NetworkMapDecoration)
						|| (o instanceof NetworkMapDCIContainer)
						|| (o instanceof NetworkMapDCIImage)
						|| (o instanceof NetworkMapDCIContainer)) {
					selectedDecorations.add((NetworkMapElement) o);
					DecorationLayerAbstractFigure f = decorationFigures
							.get(((NetworkMapElement) o).getId());
					if (f != null)
						f.setSelected(true);
				}
			}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.eclipse.gef4.zest.core.viewers.AbstractStructuredGraphViewer#
	 ...
	 * 
	 * @param image
	 *            new image or null to clear background
	 */
	...
	private void reloadMapBackground() {
		...
		final org.eclipse.draw2d.geometry.Rectangle rootLayerSize = graph
				.getZestRootLayer().getClientArea();
		final Point mapSize = new Point(Math.min(controlSize.width,
				rootLayerSize.width), Math.max(controlSize.height,
				rootLayerSize.height));
		ConsoleJob job = new ConsoleJob(
				Messages.get().ExtendedGraphViewer_DownloadTilesJob, null,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final TileSet tiles = mapLoader.getAllTiles(mapSize,
						backgroundLocation, MapLoader.TOP_LEFT, backgroundZoom,
						false);
	...
	public void zoomFit() {
		...
		org.eclipse.draw2d.geometry.Rectangle mapArea = getGraphControl()
	...
	public Action[] createZoomActions(IHandlerService handlerService) {
		...
		for (int i = 0; i < zoomLevels.length; i++) {
			...
			if (zoomLevels[i] == 1.00) {
				...
				actions[i]
						.setId("org.netxms.ui.eclipse.networkmaps.localActions.AbstractMap.Zoom100Pct"); //$NON-NLS-1$
				actions[i]
						.setActionDefinitionId("org.netxms.ui.eclipse.networkmaps.localCommands.AbstractMap.Zoom100Pct"); //$NON-NLS-1$
				handlerService
						.activateHandler(actions[i].getActionDefinitionId(),
								new ActionHandler(actions[i]));
			}
		}
		return actions;
	}
	...
	public void showBackButton(final Runnable action) {
		if (backButton != null) {
			...
			backButton
					.setLocation(new org.eclipse.draw2d.geometry.Point(10, 10));
		}
	}
	...
	private class OverlayButton extends Figure {
		...
		public Dimension getPreferredSize(int wHint, int hHint) {
			return new Dimension(icon.getImageData().width,
					icon.getImageData().height).expand(6, 6);
		}
