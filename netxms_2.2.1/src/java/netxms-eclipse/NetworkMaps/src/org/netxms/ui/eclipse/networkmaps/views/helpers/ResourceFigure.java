...
public class ResourceFigure extends Figure {
	 ...
	 * 
	 * @param object
	 *            Object represented by this figure
	 */
	public ResourceFigure(NetworkMapResource element,
			MapLabelProvider labelProvider) {
		...
		Dimension ls = label.getPreferredSize(
				IMAGE_SIZE_X + IMAGE_MARGIN_X * 2, -1);
		setSize(IMAGE_SIZE_X + IMAGE_MARGIN_X * 2, IMAGE_SIZE_Y
				+ IMAGE_MARGIN_Y * 2 + ls.height);
	...
	protected void paintFigure(Graphics gc) {
		...
		if (labelProvider.isShowStatusBackground()) {
			...
			gc.setBackgroundColor(StatusDisplayInfo
					.getStatusColor(getElementStatus()));
			gc.setAlpha(64);
		...
		if (labelProvider.isShowStatusFrame()) {
			...
			gc.setForegroundColor(StatusDisplayInfo
					.getStatusColor(getElementStatus()));
			gc.setLineWidth(3);
			gc.setLineStyle(labelProvider.isElementSelected(element) ? SWT.LINE_DOT
					: SWT.LINE_SOLID);
		...
		if (image != null) {
			gc.drawImage(image, rect.x + IMAGE_MARGIN_X, rect.y
					+ IMAGE_MARGIN_Y);
		}
		...
		if (labelProvider.isShowStatusIcons()) {
			...
			if (image != null) {
				...
				gc.drawImage(image, rect.x + rect.width - imgSize.width, rect.y); // rect.y
																					// +
																					// rect.height
																					// -
																					// imgSize.height
	...
	private ObjectStatus getElementStatus() {
		switch (element.getType()) {
		case NetworkMapResource.CLUSTER_RESOURCE:
			return (((ClusterResource) element.getData()).getCurrentOwner() == 0) ? ObjectStatus.MAJOR
					: ObjectStatus.NORMAL;
		default:
