...
public class ClusterTab extends ObjectTab {
	...
	protected void createTabContent(Composite parent) {
		...
		final String[] names = { Messages.get().ClusterTab_Resource,
				Messages.get().ClusterTab_VirtualIP,
				Messages.get().ClusterTab_Owner };
		final int[] widths = { 200, 120, 150 };
		resourceList = new SortableTableViewer(content, names, widths,
				COLUMN_NAME, SWT.UP, SortableTableViewer.DEFAULT_STYLE);
	 ...
	 * @param resources
	 */
	private void addOwnedResources(NetworkMapPage page, long rootElementId,
			long objectId, List<ClusterResource> resources) {
		long elementId = rootElementId * 100000;
		for (ClusterResource r : resources) {
			if (r.getCurrentOwner() == objectId) {
				page.addElement(new NetworkMapResource(elementId,
						NetworkMapResource.CLUSTER_RESOURCE, r));
