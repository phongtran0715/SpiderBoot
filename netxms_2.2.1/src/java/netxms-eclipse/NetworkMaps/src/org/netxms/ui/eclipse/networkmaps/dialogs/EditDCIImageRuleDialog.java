...
public class EditDCIImageRuleDialog extends Dialog {
	...
	protected Control createDialogArea(final Composite parent) {
		...
		operation = WidgetHelper.createLabeledCombo(conditionGroup, SWT.BORDER
				...
				Messages.get().EditDCIImageRuleDialog_Operation,
		...
		operation.select((rule.getComparisonType() != -1) ? rule
				.getComparisonType() : 0);

		value = WidgetHelper.createLabeledText(conditionGroup, SWT.BORDER, 120,
				Messages.get().EditDCIImageRuleDialog_Value,
				rule.getCompareValue(), WidgetHelper.DEFAULT_LAYOUT_DATA);

		comment = WidgetHelper.createLabeledText(dialogArea, SWT.BORDER, 120,
				Messages.get().EditDCIImageRuleDialog_Comment,
	...
	protected void okPressed() {
		...
		if (selectedImage == null || selectedImage == NXCommon.EMPTY_GUID) {
			MessageDialogHelper
					.openError(
							getShell(),
							Messages.get().EditDCIImageRuleDialog_Error,
							Messages.get().EditDCIImageRuleDialog_ImageShouldBeSelected);
			return;
		}
		...
		if (compareValue == null || compareValue.isEmpty()) {
			MessageDialogHelper
					.openError(
							getShell(),
							Messages.get().EditDCIImageRuleDialog_Error,
							Messages.get().EditDCIImageRuleDialog_ValueShouldNotBeEmpty);
			return;
		}
