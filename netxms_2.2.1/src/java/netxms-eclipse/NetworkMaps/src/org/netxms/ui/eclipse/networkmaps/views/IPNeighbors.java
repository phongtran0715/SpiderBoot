...
public class IPNeighbors extends AbstractNetworkMapView {
	...
	public void init(IViewSite site) throws PartInitException {
		super.init(site);
		setPartName(Messages.get().IPNeighbors_PartName
	...
	protected void buildMapPage() {
		...
		mapPage.addElement(new NetworkMapObject(rootElementId, rootObject
				.getObjectId()));
		...
		for (long objectId : rootObject.getChildIdList()) {
			...
			if ((object != null) && (object instanceof VPNConnector)) {
				AbstractObject peer = session
						.findObjectById(((VPNConnector) object)
								.getPeerGatewayId());
				if (peer != null) {
					long elementId = mapPage.createElementId();
					mapPage.addElement(new NetworkMapObject(elementId, peer
							.getObjectId()));
					NetworkMapLink link = new NetworkMapLink(
							NetworkMapLink.VPN, rootElementId, elementId);
	...
	private void addSubnets(AbstractObject root, long rootElementId) {
		for (long objectId : root.getParentIdList()) {
			...
			if ((object != null) && (object instanceof Subnet)) {
				...
				mapPage.addLink(new NetworkMapLink(NetworkMapLink.NORMAL,
						rootElementId, elementId));
				addNodesFromSubnet((Subnet) object, elementId,
						root.getObjectId());
			}
		}
	}
	 ...
	 *            Subnet object
	 * @param rootNodeId
	 *            ID of map's root node (used to prevent recursion)
	 */
	private void addNodesFromSubnet(Subnet subnet, long subnetElementId,
			long rootNodeId) {
