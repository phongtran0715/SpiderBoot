...
public class ScheduleLabelProvider extends LabelProvider implements
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		case ReportExecutionForm.SCHEDULE_START_TIME:
			...
			String HHmm = timeFormat
					.format(job.getStartTime().getTime() / 1000);

			switch (job.getType()) {
			case ReportingJob.TYPE_ONCE:
				return RegionalSettings.getDateTimeFormat().format(
						job.getStartTime().getTime() / 1000);
			...
			case ReportingJob.TYPE_MONTHLY:
				...
				for (int i = 0; i < 31; i++) {
					if (((job.getDaysOfMonth() >> i) & 0x01) != 0)
						result = String.valueOf(31 - i)
								+ (result.length() > 0 ? "," : "") + result; //$NON-NLS-1$ //$NON-NLS-2$
				}
			...
			case ReportingJob.TYPE_WEEKLY:
				...
				for (int i = 0; i < 7; i++) {
					if (((job.getDaysOfWeek() >> i) & 0x01) != 0)
						result1 = dayOfWeek[7 - (i + 1)]
								+ (result1.length() > 0 ? "," : "") + result1; //$NON-NLS-1$ //$NON-NLS-2$
				}
		...
		case ReportExecutionForm.SCHEDULE_OWNER:
			AbstractUserObject user = ((NXCSession) ConsoleSharedData
					.getSession()).findUserDBObjectById(job.getUserId());
			return (user != null) ? user.getName()
					: ("[" + job.getUserId() + "]"); //$NON-NLS-1$ //$NON-NLS-2$
