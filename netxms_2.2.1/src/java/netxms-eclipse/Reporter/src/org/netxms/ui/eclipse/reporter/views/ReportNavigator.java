		...
		new ConsoleJob("Load Reports", null, Activator.PLUGIN_ID, null) {
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				for (UUID reportId : reportIds) {
					try {
						final ReportDefinition definition = session
				...
				Collections.sort(definitions,
						new Comparator<ReportDefinition>() {

							@Override
							public int compare(ReportDefinition o1,
									ReportDefinition o2) {
					...
					public void run() {
						...
						if (notGeneratedReport.size() > 0) {
							...
							for (int i = 0; i < notGeneratedReport.size(); i++)
								errorMessage += notGeneratedReport.get(i)
										.toString()
										+ (i == notGeneratedReport.size() - 1 ? ""
												: ", ");

							MessageDialog
									.openError(null, "Error", errorMessage);
						}
					}
				});
			}
