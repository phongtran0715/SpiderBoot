...
public class ReportExecutionForm extends Composite {
	 ...
	 * @param style
	 */
	public ReportExecutionForm(Composite parent, int style,
			ReportDefinition report, IWorkbenchPart workbenchPart) {
		super(parent, style);
		this.report = report;
		...
		Section section = toolkit.createSection(form.getBody(),
				Section.DESCRIPTION | Section.TITLE_BAR | Section.COMPACT
		...
		section = toolkit.createSection(form.getBody(), Section.DESCRIPTION
				| Section.TITLE_BAR | Section.COMPACT | Section.EXPANDED
				| Section.TWISTIE);
	...
	private void createActions() {
		...
		actionRenderPDF = new Action("Render to &PDF",
				Activator.getImageDescriptor("icons/pdf.png")) {
			@Override
			public void run() {
		...
		actionRenderXLS = new Action("Render to &XLS",
				Activator.getImageDescriptor("icons/xls.png")) {
			@Override
			public void run() {
	...
	private void createSchedulesSection(Composite parent) {
		...
		scheduleList = new SortableTableViewer(parent, names, widths, 0,
				SWT.DOWN, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI);
		GridData gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.verticalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		...
		WidgetHelper.restoreTableViewerSettings(scheduleList, Activator
				.getDefault().getDialogSettings(),
				"ReportExecutionForm.ScheduleList");
		scheduleList.getControl().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveTableViewerSettings(scheduleList, Activator
						.getDefault().getDialogSettings(),
						"ReportExecutionForm.ScheduleList");
			}
		});
		...
		link.setImage(imageCache.add(Activator
				.getImageDescriptor("icons/schedule.png"))); //$NON-NLS-1$
		...
		scheduleList
				.addSelectionChangedListener(new ISelectionChangedListener() {
					...
					public void selectionChanged(SelectionChangedEvent event) {
						IStructuredSelection selection = (IStructuredSelection) scheduleList
								.getSelection();
	...
	private void createResultsSection(Composite parent) {
		...
		resultList = new SortableTableViewer(parent, names, widths, 0,
				SWT.DOWN, SWT.BORDER | SWT.FULL_SELECTION | SWT.MULTI);
		GridData gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.verticalAlignment = SWT.FILL;
		...
		WidgetHelper.restoreTableViewerSettings(resultList, Activator
				.getDefault().getDialogSettings(),
				"ReportExecutionForm.ResultList");
		resultList.getControl().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveTableViewerSettings(resultList, Activator
						.getDefault().getDialogSettings(),
						"ReportExecutionForm.ResultList");
			}
		});
		...
		resultList.addSelectionChangedListener(new ISelectionChangedListener() {
			@Override
			public void selectionChanged(SelectionChangedEvent event) {
				IStructuredSelection selection = (IStructuredSelection) resultList
						.getSelection();
	...
	private void createParamEntryFields(Composite parent) {
		...
		IConfigurationElement[] elements = reg
				.getConfigurationElementsFor("org.netxms.ui.eclipse.reporter.customfields"); //$NON-NLS-1$
		...
		for (ReportParameter parameter : parameters) {
			...
			for (int i = 0; i < elements.length; i++) {
				try {
					final CustomControlFactory provider = (CustomControlFactory) elements[i]
					...
					Activator.logError(
							"Cannot create CustomControlFactory instance", e);
				}
			}
			if (editor == null) {
		...
		for (ReportParameter parameter : parameters) {
			...
			final FieldEditor parentEditor = editors.get(parameter
					.getDependsOn());
	 ...
	 * @param format
	 */
	private void renderReport(final UUID jobId, Date executionTime,
		...
		nameTemplate
				.append(new SimpleDateFormat("ddMMyyyy HHmm").format(executionTime)); //$NON-NLS-1$
		nameTemplate.append("."); //$NON-NLS-1$
		...
		case PDF:
			fileDialog
					.setFilterNames(new String[] { "PDF Files", "All Files" });
			fileDialog.setFilterExtensions(new String[] { "*.pdf", "*.*" }); //$NON-NLS-1$ //$NON-NLS-2$
			break;
		case XLS:
			fileDialog
					.setFilterNames(new String[] { "Excel Files", "All Files" });
		...
		new ConsoleJob("Rendering report", workbenchPart, Activator.PLUGIN_ID,
				null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final File reportFile = session.renderReport(report.getId(),
						jobId, format);
			...
			protected String getErrorMessage() {
				return String.format("Cannot render report %1$s (job ID %2$s)",
						report.getName(), jobId);
			}
		}.start();
	}
	...
	private void executeReport() {
		...
		if (parameters != null) {
			for (int i = 0; i < parameters.size(); i++)
				execParameters.put(parameters.get(i).getName(), fields.get(i)
						.getValue());
		}

		new ConsoleJob("Execute report", workbenchPart, Activator.PLUGIN_ID,
				null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				...
				getDisplay().asyncExec(new Runnable() {
					...
					public void run() {
						MessageDialogHelper.openInformation(getShell(),
								"Report Execution", report.getName()
			...
			@Override
			protected String getErrorMessage() {
				return String.format("Cannot execute report %s",
						report.getName());
			}
		}.start();
	}
	...
	private void refreshResultList() {
		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob("Refresh result list for report " + report.getName(),
				workbenchPart, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<ReportResult> results = session
						.listReportResults(report.getId());
			...
			@Override
			protected String getErrorMessage() {
				return String.format("Cannot get result list for report %s",
						report.getName());
			}
		}.start();
	...
	private void refreshScheduleList() {
		new ConsoleJob(String.format("Refresh schedule list for report %s",
				report.getName()), workbenchPart, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<ReportingJob> results = session
						.listScheduledJobs(report.getId());
				runInUIThread(new Runnable() {
					@Override
					public void run() {
			...
			@Override
			protected String getErrorMessage() {
				return String.format("Cannot get schedule list for report %s",
						report.getName());
			}
		}.start();
	...
	private void deleteResults() {
		...
		if (!MessageDialogHelper.openConfirm(getShell(),
		...
		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob("Delete report execution results", workbenchPart,
	...
	protected void addSchedule() {
		...
		final PropertyDialog dialog = PropertyDialog.createDialogOn(
				workbenchPart.getSite().getShell(), General.ID, job);
		dialog.getShell().setText("Report Execution Schedule");
		if (dialog.open() != Window.OK)
			return;

		new ConsoleJob("Adding report schedule", workbenchPart,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
			...
			@Override
			protected String getErrorMessage() {
				return String.format("Cannot schedule report %s",
						report.getName());
			}
		}.start();
	}
	...
	private void deleteSchedules() {
		...
		if (!MessageDialogHelper.openConfirm(getShell(), "Delete schedules",
				"Do you really want to delete selected schedules?")) {
			return;
		}
		...
		new ConsoleJob("Delete report schedules", workbenchPart,
				Activator.PLUGIN_ID, null) {
