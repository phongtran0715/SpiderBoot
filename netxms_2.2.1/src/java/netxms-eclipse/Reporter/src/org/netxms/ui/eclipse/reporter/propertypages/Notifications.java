...
public class Notifications extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		sendNotify.setLayoutData(new GridData(SWT.LEFT, SWT.TOP, true, false,
				2, 1));
		...
		sendNotify.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				xlsFormat.setEnabled(sendReport.isEnabled()
						&& sendReport.getSelection());
				pdfFormat.setEnabled(sendReport.isEnabled()
						&& sendReport.getSelection());
		...
		listViewer = new ListViewer(emailGroup, SWT.BORDER | SWT.MULTI
				| SWT.V_SCROLL);
		...
		attachmentGroup.setLayoutData(new GridData(SWT.FILL, SWT.TOP, true,
				false));

		sendReport = new Button(attachmentGroup, SWT.CHECK);
		sendReport.setLayoutData(new GridData(SWT.LEFT, SWT.FILL, true, false,
				2, 1));
	...
	private void addMail() {
		InputDialog dlg = new InputDialog(emailGroup.getShell(),
				"Add mail", "Enter mail", "", new IInputValidator() { //$NON-NLS-1$
	...
	public boolean performOk() {
		if (sendReport.getSelection())
			job.setRenderFormat(pdfFormat.getSelection() ? ReportRenderFormat.PDF
					: ReportRenderFormat.XLS);
		else
