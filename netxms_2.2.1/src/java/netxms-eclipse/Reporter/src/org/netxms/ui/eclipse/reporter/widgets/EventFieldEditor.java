...
public class EventFieldEditor extends FieldEditor {
	private static final String EMPTY_SELECTION_TEXT = Messages.get().EventFieldEditor_Any;
	 ...
	 * @param parent
	 */
	public EventFieldEditor(ReportParameter parameter, FormToolkit toolkit,
			Composite parent) {
	...
	protected Control createContent(Composite parent) {
		...
		selectionLink
				.setToolTipText(Messages.get().EventFieldEditor_SelectEvent);
		...
		final ImageHyperlink clearLink = toolkit.createImageHyperlink(content,
				SWT.NONE);
		clearLink.setImage(SharedIcons.IMG_CLEAR);
		clearLink
				.setToolTipText(Messages.get().EventFieldEditor_ClearSelection);
	...
	private void selectEvent() {
		...
		if (dlg.open() == Window.OK) {
			...
			if (events.length > 0) {
				...
				text.setImage(StatusDisplayInfo.getStatusImage(events[0]
						.getSeverity()));
			} else {
				eventCode = 0;
				text.setText(EMPTY_SELECTION_TEXT);
