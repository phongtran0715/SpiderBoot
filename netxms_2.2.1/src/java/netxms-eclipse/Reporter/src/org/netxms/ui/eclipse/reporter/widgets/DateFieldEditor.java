...
public class DateFieldEditor extends FieldEditor {
	 ...
	 * @param parent
	 */
	public DateFieldEditor(ReportParameter parameter, FormToolkit toolkit,
			Composite parent) {
	...
	protected Control createContent(Composite parent) {
		...
		try {
			dateTime.setTime(new Date(Long.parseLong(parameter
					.getDefaultValue()) * 1000));
		} catch (NumberFormatException e) {
			dateTime.setTime(new Date());
		}

		final String[] dateElementNames = {
				Messages.get().DateFieldEditor_Year,
				Messages.get().DateFieldEditor_Month,
				Messages.get().DateFieldEditor_Day };
		dateElements = new Combo[dateElementNames.length];
		for (int idx = 0; idx < dateElementNames.length; idx++) {
			Combo cb = WidgetHelper.createLabeledCombo(content, SWT.BORDER,
					dateElementNames[idx], WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		final ImageHyperlink link = toolkit.createImageHyperlink(content,
				SWT.NONE);
		link.setImage(imageCache.add(Activator
				.getImageDescriptor("icons/calendar.png"))); //$NON-NLS-1$
	...
	private void createPopupCalendar(Control anchor) {
		...
		calendar.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				date.set(calendar.getYear(), calendar.getMonth(),
						calendar.getDay(), calendar.getHours(),
						calendar.getMinutes(), calendar.getSeconds());
		...
		Rectangle rect = getDisplay().map(anchor.getParent(), null,
				anchor.getBounds());
