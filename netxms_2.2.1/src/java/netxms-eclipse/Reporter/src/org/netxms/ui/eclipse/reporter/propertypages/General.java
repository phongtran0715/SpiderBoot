...
public class General extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		onceButton.addSelectionListener(new GranularitySelectorListener(
				ReportingJob.TYPE_ONCE, true, false, false, false));

		dailyButton = new Button(dialogArea, SWT.RADIO);
		dailyButton.setText("Daily");
		dailyButton.addSelectionListener(new GranularitySelectorListener(
				ReportingJob.TYPE_DAILY, false, true, false, false));

		weeklyButton = new Button(dialogArea, SWT.RADIO);
		weeklyButton.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false,
				false, 1, 1));
		weeklyButton.setText("Weekly");
		weeklyButton.addSelectionListener(new GranularitySelectorListener(
				ReportingJob.TYPE_WEEKLY, false, false, true, false));

		monthlyButton = new Button(dialogArea, SWT.RADIO);
		monthlyButton.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, false,
				false, 1, 1));
		monthlyButton.setText("Monthly");
		monthlyButton.addSelectionListener(new GranularitySelectorListener(
				ReportingJob.TYPE_MONTHLY, false, false, false, true));
	...
	private Composite createOnce(Composite group) {
		...
		onceComposite.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 4, 1));
		onceComposite.setLayout(new GridLayout(4, false));

		final DateTime startDate = new DateTime(onceComposite, SWT.DATE
				| SWT.DROP_DOWN);
	...
	private Composite createDaily(Composite group) {
		...
		dailyComposite.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 4, 1));
	...
	protected Composite createWeekly(Composite group) {
		...
		weeklyComposite.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 4, 1));
		...
		startTime.setLayoutData(new GridData(SWT.LEFT, SWT.TOP, false, false,
				7, 1));
	...
	private Composite createMonthly(Composite group) {
		...
		monthlyComposite.setLayoutData(new GridData(SWT.LEFT, SWT.CENTER, true,
				false, 4, 1));
	 ...
	 */
	private void changeVisibility(boolean onceVisible, boolean dailyVisible,
			boolean weeklyVisible, boolean monthlyVisible) {
    ...
    *
    */
	private final class GranularitySelectorListener implements
			SelectionListener {
		...
		private GranularitySelectorListener(int type, Boolean onceVisible,
				boolean dailyVisible, boolean weeklyVisible,
				boolean monthlyVisible) {
			this.type = type;
		...
		public void widgetSelected(SelectionEvent e) {
			...
			changeVisibility(onceVisible, dailyVisible, weeklyVisible,
					monthlyVisible);
		}
