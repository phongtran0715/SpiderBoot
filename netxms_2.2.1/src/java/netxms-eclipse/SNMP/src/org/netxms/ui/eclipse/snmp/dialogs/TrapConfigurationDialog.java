...
public class TrapConfigurationDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		description = WidgetHelper.createLabeledText(dialogArea, SWT.BORDER,
				300, Messages.get().TrapConfigurationDialog_Description,
				trap.getDescription(), WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		oid = WidgetHelper.createLabeledText(oidSelection, SWT.BORDER, 300,
				Messages.get().TrapConfigurationDialog_TrapOID, (trap
						.getObjectId() != null) ? trap.getObjectId().toString()
						: "", WidgetHelper.DEFAULT_LAYOUT_DATA); //$NON-NLS-1$
		...
		eventTag = WidgetHelper.createLabeledText(dialogArea, SWT.BORDER,
				SWT.DEFAULT, Messages.get().TrapConfigurationDialog_UserTag,
				trap.getUserTag(), WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		buttonAdd.setLayoutData(new RowData(WidgetHelper.BUTTON_WIDTH_HINT,
				SWT.DEFAULT));
	...
	private void addParameter() {
		SnmpTrapParameterMapping pm = new SnmpTrapParameterMapping(
				new SnmpObjectId());
	...
	private void editParameter() {
		IStructuredSelection selection = (IStructuredSelection) paramList
				.getSelection();
		if (selection.size() != 1)
			return;

		SnmpTrapParameterMapping pm = (SnmpTrapParameterMapping) selection
				.getFirstElement();
		ParamMappingEditDialog dlg = new ParamMappingEditDialog(getShell(), pm);
		if (dlg.open() == Window.OK) {
	...
	private void setupParameterList() {
		...
		pmap = new ArrayList<SnmpTrapParameterMapping>(
				trap.getParameterMapping());
		...
		WidgetHelper.restoreColumnSettings(table, Activator.getDefault()
				.getDialogSettings(), PARAMLIST_TABLE_SETTINGS);
	...
	private void saveSettings() {
		WidgetHelper.saveColumnSettings(paramList.getTable(), Activator
				.getDefault().getDialogSettings(), PARAMLIST_TABLE_SETTINGS);
	}
	...
	protected void okPressed() {
		try {
			...
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().TrapConfigurationDialog_Warning,
					Messages.get().TrapConfigurationDialog_WarningInvalidOID);
			return;
		}
