...
public class ParamMappingEditDialog extends Dialog {
	 ...
	 *            parent shell
	 * @param pm
	 *            trap parameter mapping to be edited
	 */
	...
	protected Control createDialogArea(Composite parent) {
		...
		description = WidgetHelper.createLabeledText(dialogArea, SWT.BORDER,
				300, Messages.get().ParamMappingEditDialog_Description,
				pm.getDescription(), WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		radioByOid
				.setSelection(pm.getType() == SnmpTrapParameterMapping.BY_OBJECT_ID);
		...
		radioByPosition
				.setSelection(pm.getType() == SnmpTrapParameterMapping.BY_POSITION);
		...
		new Label(positionSelection, SWT.NONE)
				.setText(Messages.get().ParamMappingEditDialog_EnterVarbindPos);
		...
		checkForceText
				.setText(Messages.get().ParamMappingEditDialog_NeverConvertToHex);
		checkForceText
				.setSelection((pm.getFlags() & SnmpTrapParameterMapping.FORCE_TEXT) != 0);
	 ...
	 * 
	 * @param mappingByOid
	 *            true if current mapping type is "by OID"
	 */
	...
	protected void okPressed() {
		int type = radioByOid.getSelection() ? SnmpTrapParameterMapping.BY_OBJECT_ID
				: SnmpTrapParameterMapping.BY_POSITION;
		if (type == SnmpTrapParameterMapping.BY_OBJECT_ID) {
			try {
				pm.setObjectId(SnmpObjectId.parseSnmpObjectId(objectId
						.getText()));
			} catch (SnmpObjectIdFormatException e) {
				MessageDialogHelper
						.openWarning(
								getShell(),
								Messages.get().ParamMappingEditDialog_Warning,
								Messages.get().ParamMappingEditDialog_WarningInvalidOID);
				return;
			}
		} else {
		...
		pm.setFlags(checkForceText.getSelection() ? SnmpTrapParameterMapping.FORCE_TEXT
				: 0);
		super.okPressed();
	}
}
