...
public class SnmpCredentials extends ViewPart implements ISaveablePart {
	...
	public void createPartControl(Composite parent) {
		...
		// Load config
		new ConsoleJob(Messages.get().SnmpCredentials_LoadingConfig, this,
	...
	private void createActions() {
		actionSave = new Action(Messages.get().SnmpConfigurator_Save,
				SharedIcons.SAVE) {
			@Override
			public void run() {
				save();
	...
	private void createSnmpCommunitySection() {
		...
		snmpCommunityList = new TableViewer(clientArea, SWT.BORDER | SWT.MULTI
				| SWT.FULL_SELECTION);
	...
	private void createSnmpUsmCredSection() {
		...
		snmpUsmCredList = new TableViewer(clientArea, SWT.BORDER | SWT.MULTI
				| SWT.FULL_SELECTION);
	...
	private void createSnmpPortList() {
		...
		snmpPortList = new TableViewer(clientArea, SWT.BORDER | SWT.MULTI
				| SWT.FULL_SELECTION);
	...
	public void doSave(IProgressMonitor monitor) {
		try {
			...
			MessageDialogHelper.openError(getSite().getShell(),
					Messages.get().SnmpConfigurator_Error, String.format(
							Messages.get().SnmpCredentials_CannotSaveConfig,
	 ...
	 */
	private void save() {
		new ConsoleJob(Messages.get().SnmpCredentials_SaveConfig, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	private void addCommunity() {
		InputDialog dlg = new InputDialog(getSite().getShell(),
				Messages.get().SnmpConfigurator_AddCommunity,
	...
	private void removeCommunity() {
		final List<String> list = config.getCommunities();
		IStructuredSelection selection = (IStructuredSelection) snmpCommunityList
				.getSelection();
		if (selection.size() > 0) {
	...
	private void removeUsmCredentials() {
		final List<SnmpUsmCredential> list = config.getUsmCredentials();
		IStructuredSelection selection = (IStructuredSelection) snmpUsmCredList
				.getSelection();
		if (selection.size() > 0) {
			for (Object o : selection.toList()) {
				list.remove(o);
	...
	private void addSnmpPort() {
		InputDialog dlg = new InputDialog(getSite().getShell(),
				Messages.get().SnmpCredentials_AddPort,
	...
	private void removeSnmpPort() {
		final List<String> list = config.getPorts();
		IStructuredSelection selection = (IStructuredSelection) snmpPortList
				.getSelection();
		if (selection.size() > 0) {
			for (Object o : selection.toList()) {
				list.remove(o);
