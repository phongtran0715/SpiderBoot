...
public class SnmpTrapEditor extends ViewPart implements SessionListener {
	...
	public void createPartControl(Composite parent) {
		...
		final String[] columnNames = { Messages.get().SnmpTrapEditor_ColID,
				Messages.get().SnmpTrapEditor_ColOID,
				Messages.get().SnmpTrapEditor_ColEvent,
				Messages.get().SnmpTrapEditor_ColDescription };
	...
	private void createActions() {
		...
		actionNew.setImageDescriptor(Activator
				.getImageDescriptor("icons/new.png")); //$NON-NLS-1$
		...
		actionDelete.setImageDescriptor(Activator
				.getImageDescriptor("icons/delete.png")); //$NON-NLS-1$
	}
	...
	private void refreshTrapList() {
		new ConsoleJob(Messages.get().SnmpTrapEditor_LoadJob_Title, this,
	...
	protected void deleteTraps() {
		...
		new ConsoleJob(Messages.get().SnmpTrapEditor_DeleteJob_Title, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected String getErrorMessage() {
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				for (int i = 0; i < objects.length; i++) {
					session.deleteSnmpTrapConfiguration(((SnmpTrap) objects[i])
							.getId());
				}
			}
		}.start();
	...
	protected void editTrap() {
		...
		TrapConfigurationDialog dlg = new TrapConfigurationDialog(getViewSite()
				.getShell(), trap);
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().SnmpTrapEditor_ModifyJob_Title, this,
					Activator.PLUGIN_ID, null) {
				@Override
				protected String getErrorMessage() {
	...
	protected void createTrap() {
		...
		TrapConfigurationDialog dlg = new TrapConfigurationDialog(getViewSite()
				.getShell(), trap);
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().SnmpTrapEditor_CreateJob_Title, this,
					Activator.PLUGIN_ID, null) {
				@Override
				protected String getErrorMessage() {
