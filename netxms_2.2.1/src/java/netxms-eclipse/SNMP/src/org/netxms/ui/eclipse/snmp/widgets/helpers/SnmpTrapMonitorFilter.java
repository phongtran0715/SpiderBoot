...
public class SnmpTrapMonitorFilter extends AbstractTraceViewFilter {
	...
	public boolean select(Viewer viewer, Object parentElement, Object element) {
		...
		if (((SnmpTrapLogRecord) element).getTrapObjectId().contains(
				filterString))
			return true;

		if (((SnmpTrapLogRecord) element).getVarbinds().toLowerCase()
				.contains(filterString))
			return true;

		if (((SnmpTrapLogRecord) element).getSourceAddress().getHostAddress()
				.contains(filterString))
			return true;

		AbstractObject object = session
				.findObjectById(((SnmpTrapLogRecord) element).getSourceNode());
		if (object != null) {
