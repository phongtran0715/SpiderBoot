...
public class MibExplorer extends ViewPart implements SnmpWalkListener {
	...
	public void init(IViewSite site, IMemento memento) throws PartInitException {
		if (memento != null) {
			...
			if (nodeId != 0) {
				AbstractObject object = ((NXCSession) ConsoleSharedData
						.getSession()).findObjectById(nodeId);
		...
		initShowFilter = safeCast(settings.get("MibExplorer.showFilter"),
				settings.getBoolean("MibExplorer.showFilter"), initShowFilter);

		super.init(site, memento);
	}

	/**
	 * Safely cast Integer to int - return default value if Integer object is
	 * null
	 * 
	 * @param i
	 * @param defval
	...
	public void createPartControl(Composite parent) {
		...
		header.setBackground(SharedColors.getColor(
				SharedColors.MIB_EXPLORER_HEADER_BACKGROUND,
				parent.getDisplay()));
		header.setForeground(SharedColors.getColor(
				SharedColors.MIB_EXPLORER_HEADER_TEXT, parent.getDisplay()));
		...
		details = new MibObjectDetails(mibViewSplitter, SWT.BORDER, true,
				mibBrowser);
		...
		viewer.getTable().addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveColumnSettings(viewer.getTable(), Activator
						.getDefault().getDialogSettings(), "MibExplorer"); //$NON-NLS-1$
			}
		});
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.snmp.context.SNMP"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionSetNode = new Action(Messages.get().MibExplorer_SetNodeObject) {
			...
			public void run() {
				ObjectSelectionDialog dlg = new ObjectSelectionDialog(
						MibExplorer.this.getSite().getShell(), null,
		...
		actionCopySymbolicName = new Action(
				Messages.get().MibExplorer_CopySymbolicName) {
			@Override
			public void run() {
		...
		actionShowFilter.setChecked(initShowFilter);
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.snmp.commands.showFilter"); //$NON-NLS-1$
	...
	private void selectInTree() {
		...
		if (selection.length == 1) {
			MibObject o = MibCache.findObject(
					selection[0].getText(COLUMN_NAME), false);
			if (o != null) {
	...
	private void setupViewerColumns() {
		...
		WidgetHelper.restoreColumnSettings(viewer.getTable(), Activator
				.getDefault().getDialogSettings(), "MibExplorer"); //$NON-NLS-1$
	}
	...
	public void saveState(IMemento memento) {
		...
		memento.putInteger(
				"CurrentNode", (currentNode != null) ? (int) currentNode.getObjectId() : 0); //$NON-NLS-1$
	}
	...
	private void doWalk() {
		...
		ConsoleJob job = new ConsoleJob(
				Messages.get().MibExplorer_WalkJob_Title, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				session.snmpWalk(currentNode.getObjectId(), object
						.getObjectId().toString(), MibExplorer.this);
			}
	 ...
	 * 
	 * @see
	 * org.netxms.client.snmp.SnmpWalkListener#onSnmpWalkData(java.util.List)
	 */
	@Override
	public void onSnmpWalkData(final List<SnmpValue> data) {
		viewer.getControl().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				...
				try {
					viewer.getTable().showItem(
							viewer.getTable().getItem(
