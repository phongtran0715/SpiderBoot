...
public class MibSelectionDialog extends Dialog {
	 ...
	 *            initial selection for MIB tree or null
	 */
	...
	protected void configureShell(Shell newShell) {
		...
		try {
			newShell.setSize(
					settings.getInt("MibSelectionDialog.cx"), settings.getInt("MibSelectionDialog.cy")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected Control createDialogArea(Composite parent) {
		...
		oid = WidgetHelper.createLabeledText(dialogArea, SWT.BORDER, 500,
				Messages.get().MibSelectionDialog_OID,
		...
		if (initialSelection != null) {
			MibObject object = MibCache.getMibTree().findObject(
	...
	protected void okPressed() {
		try {
			...
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().MibSelectionDialog_Warning,
					Messages.get().MibSelectionDialog_EnterValidOID);
			return;
		}

		selectedObject = mibTree.getSelection();
		if (selectedObject == null) {
			selectedObject = MibCache.getMibTree().findObject(selectedObjectId,
					false);
			if (selectedObject == null) {
				MessageDialogHelper.openWarning(getShell(),
						Messages.get().MibSelectionDialog_Warning,
						Messages.get().MibSelectionDialog_WarningText);
				return;
			}
		}
		saveSettings();
	...
	private void doWalk() {
		try {
			final SnmpObjectId root = SnmpObjectId.parseSnmpObjectId(oid
					.getText());
			...
			MessageDialogHelper.openError(getShell(),
					Messages.get().MibSelectionDialog_Error,
					Messages.get().MibSelectionDialog_OIDParseError);
		}
	}
