...
public class MibWalkDialog extends Dialog implements SnmpWalkListener {
	 ...
	 * @param parentShell
	 * @param rootObject
	 *            root object to start walk from
	 */
	...
	protected void configureShell(Shell newShell) {
		...
		try {
			newShell.setSize(
					settings.getInt("MibWalkDialog.cx"), settings.getInt("MibWalkDialog.cy")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected Control createDialogArea(Composite parent) {
		...
		viewer = new TableViewer(dialogArea, SWT.FULL_SELECTION | SWT.SINGLE
				| SWT.BORDER);
		...
		viewer.getTable().addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveColumnSettings(viewer.getTable(), Activator
						.getDefault().getDialogSettings(), "MibWalkDialog"); //$NON-NLS-1$
			}
		});
	...
	private void startWalk() {
		...
		new ConsoleJob(Messages.get().MibExplorer_WalkJob_Title, null,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				session.snmpWalk(nodeId, rootObject.toString(),
						MibWalkDialog.this);
			}
