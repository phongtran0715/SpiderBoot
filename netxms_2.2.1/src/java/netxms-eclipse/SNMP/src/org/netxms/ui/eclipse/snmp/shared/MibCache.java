...
public final class MibCache implements ConsoleLoginListener {
	...
	public void afterLogin(final NXCSession session, Display display) {
		...
		ConsoleJob job = new ConsoleJob(Messages.get().LoginListener_JobTitle,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				if (loc != null) {
					...
					if (!mibFile.exists()
							|| (serverMibTimestamp.getTime() > mibFile
									.lastModified())) {
						File file = session.downloadMibFile();
						Activator
								.logInfo("MIB file downloaded to: " + file.getPath() + " (size " + file.length() + " bytes)"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
	...
	/**
	 * Find matching object in tree. If exactMatch set to true, method will
	 * search for object with ID equal to given. If exactMatch set to false, and
	 * object with given id cannot be found, closest upper level object will be
	 * returned (i.e., if object .1.3.6.1.5 does not exist in the tree, but
	 * .1.3.6.1 does, .1.3.6.1 will be returned in search for .1.3.6.1.5).
	 * 
	 * @param oid
	 *            object id to find
	 * @param exactMatch
