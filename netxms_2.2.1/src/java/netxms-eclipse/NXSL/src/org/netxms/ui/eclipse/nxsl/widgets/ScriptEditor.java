...
public class ScriptEditor extends Composite {
	 ...
	 */
	public ScriptEditor(Composite parent, int style, int editorStyle,
			boolean showLineNumbers, String hints) {
		super(parent, style);
		...
		proposalIcons[0] = Activator
				.getImageDescriptor("icons/function.gif").createImage(); //$NON-NLS-1$
		proposalIcons[1] = Activator
				.getImageDescriptor("icons/var_global.gif").createImage(); //$NON-NLS-1$
		proposalIcons[2] = Activator
				.getImageDescriptor("icons/var_local.gif").createImage(); //$NON-NLS-1$
		proposalIcons[3] = Activator
				.getImageDescriptor("icons/constant.gif").createImage(); //$NON-NLS-1$
	...
	private void createHintsArea() {
		...
		hintArea.setBackground(SharedColors.getColor(
				SharedColors.MESSAGE_BAR_BACKGROUND, getDisplay()));

		CLabel hintsTitle = new CLabel(hintArea, SWT.NONE);
		hintsTitle.setBackground(SharedColors.getColor(
				SharedColors.MESSAGE_BAR_BACKGROUND, getDisplay()));
		hintsTitle.setForeground(SharedColors.getColor(
				SharedColors.MESSAGE_BAR_TEXT, getDisplay()));
		...
		hintsExpandButton.setCursor(getDisplay().getSystemCursor(
				SWT.CURSOR_HAND));
		hintsExpandButton.setImage(SharedIcons.IMG_EXPAND);
		hintsExpandButton
				.setToolTipText(Messages.get().ScriptEditor_HideMessage);
	...
	/**
	 * Get icon for given autocompletion proposal type. Proposal types defined
	 * in NXSLProposalProcessor.
	 * 
	 * @param type
	 *            proposal type
	...
	private void toggleHints() {
		if (hintTextControl != null) {
			...
			hintTextControl.setLayoutData(new GridData(SWT.FILL, SWT.FILL,
					true, false, 2, 1));
			hintTextControl.setEditable(false);
			hintTextControl.setText(hintText);
			hintTextControl.setBackground(SharedColors.getColor(
					SharedColors.MESSAGE_BAR_BACKGROUND, getDisplay()));
			hintTextControl.setForeground(SharedColors.getColor(
					SharedColors.MESSAGE_BAR_TEXT, getDisplay()));
