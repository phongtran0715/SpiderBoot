...
public class ScriptExecutor extends ViewPart implements ISaveablePart2,
	...
	public void init(IViewSite site) throws PartInitException {
		...
		setPartName(String.format(Messages.get().ScriptExecutor_PartName,
	...
	public void createPartControl(Composite parent) {
		...
		scriptCombo = WidgetHelper.createLabeledCombo(form.getBody(),
				SWT.READ_ONLY, Messages.get().ScriptExecutor_LibScript,
				WidgetHelper.DEFAULT_LAYOUT_DATA, toolkit);
		...
		scriptEditor = new ScriptEditor(section, SWT.BORDER, SWT.H_SCROLL
				| SWT.V_SCROLL, true);
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.nxsl.context.ScriptExecutor"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionSave = new Action(Messages.get().ScriptExecutor_Save,
				SharedIcons.SAVE) {
			@Override
			public void run() {
		...
		actionSaveAs = new Action(Messages.get().ScriptExecutor_SaveAs,
				SharedIcons.SAVE_AS) {
			@Override
			public void run() {
		...
		actionSaveAs
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.save_as"); //$NON-NLS-1$
		handlerService.activateHandler(actionSaveAs.getActionDefinitionId(),
				new ActionHandler(actionSaveAs));

		actionClear = new Action(Messages.get().ScriptExecutor_Clear,
				SharedIcons.CLEAR) {
			@Override
			public void run() {
				if (modified) {
		...
		actionClearOutput = new Action(
				Messages.get().ScriptExecutor_ClearOutput,
		...
		actionClearOutput
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.clear_output"); //$NON-NLS-1$
		handlerService.activateHandler(actionClearOutput
				.getActionDefinitionId(), new ActionHandler(actionClearOutput));
		...
		actionExecute = new Action(Messages.get().ScriptExecutor_Execute,
				SharedIcons.EXECUTE) {
			@Override
			public void run() {
				executeScript();
		...
		actionExecute
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.execute_script"); //$NON-NLS-1$
		handlerService.activateHandler(actionExecute.getActionDefinitionId(),
				new ActionHandler(actionExecute));
	}
	...
	private boolean saveIfRequired(boolean onSelectionChange) {
		SaveScriptDialog dlg = new SaveScriptDialog(getSite().getShell(),
				actionSave.isEnabled());
		int rc = dlg.open();
	...
	private void createNewScript(final boolean saveOnSelectionChange) {
		final CreateScriptDialog dlg = new CreateScriptDialog(getSite()
				.getShell(), null);
		if (dlg.open() == Window.OK) {
			final String scriptSource = scriptEditor.getText();
			new ConsoleJob(Messages.get().ScriptExecutor_JobName_Create, this,
					...
					runInUIThread(new Runnable() {
						...
						public void run() {
							Runnable run = new Runnable() {
								@Override
								public void run() {
									scriptCombo.select(scriptCombo.indexOf(dlg
											.getName()));
								}
							};
	...
	protected void getScriptContent() {
		...
		new ConsoleJob(Messages.get().ScriptExecutor_JobName_Update, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final Script script = session.getScript(library.get(index)
						.getId());
	...
	protected void executeScript() {
		...
		new ConsoleJob(Messages.get().ScriptExecutor_JobName_Execute, null,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	private void updateScriptList(final Runnable postProcessor) {
		final String selection = (scriptCombo.getSelectionIndex() != -1) ? scriptCombo
				.getItem(scriptCombo.getSelectionIndex()) : null;

		new ConsoleJob(Messages.get().ScriptExecutor_JobName_ReadList, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						...
						if (postProcessor != null) {
							...
							if (selection != null) {
								scriptCombo.select(scriptCombo
										.indexOf(selection));
							}
						}
					}
	...
	public void intermediateSave(boolean saveOnSelectionChange) {
		final Script s = library.get(saveOnSelectionChange ? previousSelection
				: scriptCombo.getSelectionIndex());
		final String scriptSource = scriptEditor.getText();
		new ConsoleJob(Messages.get().ScriptExecutor_JobName_Update, this,
				Activator.PLUGIN_ID, null) {
			@Override
	...
	public void doSave(IProgressMonitor monitor) {
		try {
			...
			MessageDialogHelper.openError(
					...
					Messages.get().ScriptExecutor_Error,
					String.format(Messages.get().ScriptExecutor_SaveError,
	...
	public int promptToSaveOnClose() {
		...
		return (rc == IDialogConstants.CANCEL_ID) ? CANCEL
				: ((rc == SaveScriptDialog.DISCARD_ID) ? NO : YES);
	}
	 ...
	 * 
	 * @see
	 * org.netxms.client.ActionExecutionListener#messageReceived(java.lang.String
	 * )
	 */
	@Override
