...
public class ScriptLibrary extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		final String[] names = { Messages.get().ScriptLibrary_ColumnId,
				Messages.get().ScriptLibrary_ColumnName };
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.nxsl.context.ScriptLibrary"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionNew = new Action(Messages.get().ScriptLibrary_New,
		...
		actionEdit = new Action(Messages.get().ScriptLibrary_Edit,
				Activator.getImageDescriptor("icons/edit.png")) { //$NON-NLS-1$
			@Override
			public void run() {
				editScript();
			}
		...
		actionDelete = new Action(Messages.get().ScriptLibrary_Delete,
		...
		actionCopyName
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.CopyNameToClipboard");
		handlerService.activateHandler(actionCopyName.getActionDefinitionId(),
				new ActionHandler(actionCopyName));
	}
	...
	private void refreshScriptList() {
		new ConsoleJob(Messages.get().ScriptLibrary_LoadJobTitle, this,
	...
	private void createNewScript() {
		...
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().ScriptLibrary_CreateJobTitle, this,
					Activator.PLUGIN_ID, null) {
				@Override
				protected void runInternal(IProgressMonitor monitor)
						throws Exception {
	...
	private void editScript() {
		...
		try {
			getSite().getPage()
					.showView(ScriptEditorView.ID,
							Long.toString(script.getId()),
							IWorkbenchPage.VIEW_ACTIVATE);
		} catch (PartInitException e) {
			MessageDialogHelper.openError(getSite().getWorkbenchWindow()
					.getShell(), Messages.get().ScriptLibrary_Error, String
					.format(Messages.get().ScriptLibrary_EditScriptError,
							e.getMessage()));
		}
	}
	...
	private void renameScript() {
		...
		final CreateScriptDialog dlg = new CreateScriptDialog(getSite()
				.getShell(), script.getName());
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().ScriptLibrary_RenameJobTitle, this,
					Activator.PLUGIN_ID, null) {
				@Override
				protected void runInternal(IProgressMonitor monitor)
						throws Exception {
					...
					runInUIThread(new Runnable() {
						...
						public void run() {
							...
							final Script newScript = new Script(script.getId(),
									dlg.getName(), script.getSource());
							list.add(newScript);
							viewer.setInput(list.toArray());
	...
	private void deleteScript() {
		if (!MessageDialogHelper.openQuestion(getSite().getShell(),
				Messages.get().ScriptLibrary_Confirmation,
				Messages.get().ScriptLibrary_ConfirmationText))
			return;

		final IStructuredSelection selection = (IStructuredSelection) viewer
				.getSelection();

		new ConsoleJob(Messages.get().ScriptLibrary_DeleteJobTitle, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	private void copyNameToClipboard() {
		IStructuredSelection selection = (IStructuredSelection) viewer
				.getSelection();
		WidgetHelper.copyToClipboard(((Script) selection.getFirstElement())
				.getName());
	}
}
