...
/**
 * Line style listener for NXSL syntax highlighting in StyledText. Object of
 * this class should be set both as line style listener and extended modify
 * listener
 * 
 */
public class NXSLLineStyleListener implements LineStyleListener,
	...
	private static final Color COMMENT_COLOR = new Color(Display.getCurrent(),
			128, 128, 128);
	private static final Color CONSTANT_COLOR = new Color(Display.getCurrent(),
			0, 0, 192);
	private static final Color ERROR_COLOR = new Color(Display.getCurrent(),
			255, 255, 255);
	private static final Color ERROR_BK_COLOR = new Color(Display.getCurrent(),
			255, 0, 0);
	...
	private static final String[] keywords = { "abort", "array", "break",
			"case", "catch", "classof", "const", "continue", "default", "do",
			"else", "exit", "for", "foreach", "global", "if", "ilike",
			"imatch", "int32", "int64", "like", "match", "print", "println",
			"real", "return", "string", "sub", "switch", "try", "typeof",
			"uint32", "uint64", "use", "while" };
	private static final String[] systemConstants = { "null", "NULL", "true",
			"TRUE", "false", "FALSE" };
	...
	public void lineGetStyle(LineStyleEvent event) {
		...
		int state = isOffsetWithinComment(event.lineOffset) ? STATE_COMMENT
				: STATE_DEFAULT;
		int start = 0;
		...
		for (int i = 0; i < length; i++) {
			...
			case STATE_DEFAULT:
				switch (ch) {
				case '/':
					final char next = (i < length - 1) ? event.lineText
							.charAt(i + 1) : 0;
					if (next == '*') {
						...
						processKeyword(word, event.lineOffset + start, styles);
						styles.add(newCommentStyle(event.lineOffset + i, length
								- i));
						i = length;
			...
			case STATE_STRING:
				...
				case '\\':
					...
					break;
				case '"':
					styles.add(newStringStyle(event.lineOffset + start, i
							- start + 1));
					start = i + 1;
					state = STATE_DEFAULT;
					break;
			...
			case STATE_COMMENT:
				if (ch == '*') {
					...
					if (next == '/') {
						i++;
						styles.add(newCommentStyle(event.lineOffset + start, i
								- start + 1));
						start = i + 1;
						state = STATE_DEFAULT;
					}
		...
		case STATE_COMMENT:
			...
			break;
		case STATE_STRING:
			styles.add(newOpenStringStyle(event.lineOffset + start, length
					- start));
			break;
		}
	...
	/**
	 * Check if collected word is a language keyword and set style if
	 * appropriate. Will clear supplied string builder after processing.
	 * 
	 * @param word
	 *            String builder containing potential keyword
	 * @param start
	 *            Start offset for the keyword
	 */
	private void processKeyword(StringBuilder word, int start,
	 ...
	 * 
	 * @see
	 * org.eclipse.swt.custom.ExtendedModifyListener#modifyText(org.eclipse.
