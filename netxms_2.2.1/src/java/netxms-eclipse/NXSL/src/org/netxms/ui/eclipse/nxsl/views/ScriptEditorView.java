...
public class ScriptEditorView extends ViewPart implements ISaveablePart {
	...
	private static final Color ERROR_COLOR = new Color(Display.getDefault(),
			255, 0, 0);
	...
	public void init(IViewSite site) throws PartInitException {
		...
		if (settings.get("ScriptEditor.showLineNumbers") != null) //$NON-NLS-1$
			showLineNumbers = settings
					.getBoolean("ScriptEditor.showLineNumbers"); //$NON-NLS-1$
	}
	...
	public void createPartControl(Composite parent) {
		...
		editor = new ScriptEditor(editorMessageBar, SWT.NONE, SWT.H_SCROLL
				| SWT.V_SCROLL, showLineNumbers);
		editor.getTextWidget().addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				if (!modified) {
					...
					boolean selected = editor.getTextWidget()
							.getSelectionCount() > 0;
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.nxsl.context.ScriptEditor"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionSave = new Action(Messages.get().ScriptEditorView_Save,
				SharedIcons.SAVE) {
			@Override
			public void run() {
				saveScript();
		...
		actionSave
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.save"); //$NON-NLS-1$
		handlerService.activateHandler(actionSave.getActionDefinitionId(),
				new ActionHandler(actionSave));

		actionCompile = new Action(Messages.get().ScriptEditorView_Compile,
				Activator.getImageDescriptor("icons/compile.gif")) { //$NON-NLS-2$
			@Override
			public void run() {
				compileScript();
			}
		...
		actionCompile
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.compile"); //$NON-NLS-1$
		handlerService.activateHandler(actionCompile.getActionDefinitionId(),
				new ActionHandler(actionCompile));

		actionShowLineNumbers = new Action(
				Messages.get().ScriptEditorView_ShowLineNumbers,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowLineNumbers
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.showLineNumbers"); //$NON-NLS-1$
		handlerService.activateHandler(
				actionShowLineNumbers.getActionDefinitionId(),
				new ActionHandler(actionShowLineNumbers));
		...
		actionGoToLine
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.goToLine"); //$NON-NLS-1$
		handlerService.activateHandler(actionGoToLine.getActionDefinitionId(),
				new ActionHandler(actionGoToLine));
		...
		actionSelectAll
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.selectAll"); //$NON-NLS-1$
		handlerService.activateHandler(actionSelectAll.getActionDefinitionId(),
				new ActionHandler(actionSelectAll));

		actionCut = new Action(Messages.get().ScriptEditorView_Cut,
				SharedIcons.CUT) {
			@Override
			public void run() {
		...
		actionCut
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.cut"); //$NON-NLS-1$
		handlerService.activateHandler(actionCut.getActionDefinitionId(),
				new ActionHandler(actionCut));
		actionCut.setEnabled(false);

		actionCopy = new Action(Messages.get().ScriptEditorView_Copy,
				SharedIcons.COPY) {
			@Override
			public void run() {
		...
		actionCopy
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.copy"); //$NON-NLS-1$
		handlerService.activateHandler(actionCopy.getActionDefinitionId(),
				new ActionHandler(actionCopy));
		actionCopy.setEnabled(false);

		actionPaste = new Action(Messages.get().ScriptEditorView_Paste,
				SharedIcons.PASTE) {
			@Override
			public void run() {
		...
		actionPaste
				.setActionDefinitionId("org.netxms.ui.eclipse.nxsl.commands.paste"); //$NON-NLS-1$
		handlerService.activateHandler(actionPaste.getActionDefinitionId(),
				new ActionHandler(actionPaste));
	...
	private void goToLine() {
		...
		InputDialog dlg = new InputDialog(getSite().getShell(),
				Messages.get().ScriptEditorView_GoToLine_DlgTitle,
				String.format(Messages.get().ScriptEditorView_EnterLineNumber,
						maxLine), Integer.toString(textControl
						.getLineAtOffset(textControl.getCaretOffset()) + 1),
				new IInputValidator() {
					@Override
	...
	private void reloadScript() {
		new ConsoleJob(String.format(
				Messages.get().ScriptEditorView_LoadJobTitle, scriptId), this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected String getErrorMessage() {
				return String.format(
						Messages.get().ScriptEditorView_LoadJobError, scriptId);
			}
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						...
						setPartName(String.format(
								Messages.get().ScriptEditorView_PartName,
								scriptName));
	...
	private void compileScript() {
		...
		new ConsoleJob(Messages.get().ScriptEditorView_CompileScript, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected String getErrorMessage() {
			...
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final ScriptCompilationResult result = session.compileScript(
						source, false);
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						...
						if (result.success) {
							editorMessageBar
									.showMessage(
											CompositeWithMessageBar.INFORMATION,
											Messages.get().ScriptEditorView_ScriptCompiledSuccessfully);
						} else {
							editorMessageBar.showMessage(
									CompositeWithMessageBar.WARNING,
									result.errorMessage);
							s.setLineBackground(result.errorLine - 1, 1,
									ERROR_COLOR);
						}
	...
	private void saveScript() {
		final String source = editor.getText();
		editor.getTextWidget().setEditable(false);
		new ConsoleJob(Messages.get().ScriptEditorView_SaveJobTitle, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected String getErrorMessage() {
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				if (result.success) {
					...
					getDisplay().syncExec(new Runnable() {
						@Override
						public void run() {
							if (MessageDialogHelper.openQuestion(
									getSite().getShell(),
									Messages.get().ScriptEditorView_CompilationErrors,
									String.format(
											Messages.get().ScriptEditorView_ScriptCompilationFailed,
											result.errorMessage)))
								result.success = true;
							editorMessageBar.showMessage(
									CompositeWithMessageBar.WARNING,
	 ...
	 * @throws Exception
	 */
	private void doScriptSave(String source, IProgressMonitor monitor)
			throws Exception {
	...
	public void doSave(IProgressMonitor monitor) {
		...
		try {
			...
			MessageDialogHelper.openError(getViewSite().getShell(), Messages
					.get().ScriptEditorView_Error, String.format(
					Messages.get().ScriptEditorView_SaveErrorMessage,
					e.getMessage()));
		}
	}
