...
public class SelectScriptDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		new Label(dialogArea, SWT.NONE)
				.setText(Messages.get().SelectScriptDialog_AvailableScripts);

		viewer = new TableViewer(dialogArea, SWT.BORDER | SWT.FULL_SELECTION
				| (multiSelection ? SWT.MULTI : 0));
		...
		new ConsoleJob(Messages.get().SelectScriptDialog_JobTitle, null,
	...
	protected void okPressed() {
		...
		if (s.isEmpty()) {
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().SelectScriptDialog_Warning,
					Messages.get().SelectScriptDialog_WarningEmptySelection);
			return;
		}
