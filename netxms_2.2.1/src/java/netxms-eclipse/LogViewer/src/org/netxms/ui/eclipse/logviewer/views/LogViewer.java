...
public class LogViewer extends ViewPart {
	...
	public void createPartControl(Composite parent) {
			...
			public void widgetDisposed(DisposeEvent e) {
				if (logHandle != null)
					WidgetHelper.saveColumnSettings(viewer.getTable(),
							...
							"LogViewer." + logHandle.getName()); //$NON-NLS-1$
			}
		});
		...
		new ConsoleJob(String.format(Messages.get().LogViewer_OpenLogJobName,
				logName), this, Activator.PLUGIN_ID, JOB_FAMILY) {
			@Override
			protected String getErrorMessage() {
				return String.format(Messages.get().LogViewer_OpenLogError,
						logName);
			}
	...
	protected void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.logviewer.context.LogViewer"); //$NON-NLS-1$
		}
	}
	...
	private void setupLogViewer() {
		...
		WidgetHelper.restoreColumnSettings(table, Activator.getDefault()
				.getDialogSettings(), "LogViewer." + logHandle.getName()); //$NON-NLS-1$
	...
	protected void createActions() {
		...
		actionExecute = new Action(Messages.get().LogViewer_ActionExec,
				SharedIcons.EXECUTE) {
			@Override
			public void run() {
				doQuery();
		...
		actionExecute
				.setActionDefinitionId("org.netxms.ui.eclipse.logviewer.commands.execute"); //$NON-NLS-1$
		handlerService.activateHandler(actionExecute.getActionDefinitionId(),
				new ActionHandler(actionExecute));

		actionClearFilter = new Action(
				Messages.get().LogViewer_ActionClearFilter,
		...
		actionGetMoreData = new Action(
				Messages.get().LogViewer_ActionGetMoreData,
				Activator.getImageDescriptor("icons/get_more_data.png")) { //$NON-NLS-1$
			@Override
			public void run() {
				getMoreData();
			}
		...
		actionGetMoreData
				.setActionDefinitionId("org.netxms.ui.eclipse.logviewer.commands.get_more_data"); //$NON-NLS-1$
		handlerService.activateHandler(actionGetMoreData
				.getActionDefinitionId(), new ActionHandler(actionGetMoreData));

		actionShowFilter = new Action(
				Messages.get().LogViewer_ActionShowFilter, Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.logviewer.commands.show_filter"); //$NON-NLS-1$
		handlerService.activateHandler(
				actionShowFilter.getActionDefinitionId(), new ActionHandler(
						actionShowFilter));

		actionCopyToClipboard = new Action(Messages.get().LogViewer_ActionCopy,
				SharedIcons.COPY) {
			@Override
			public void run() {
		...
		handlerService.activateHandler(
				actionCopyToClipboard.getActionDefinitionId(),
				new ActionHandler(actionCopyToClipboard));
	...
	public void doQuery() {
		...
		actionGetMoreData.setEnabled(false);
		filter = filterBuilder.createFilter();
		new ConsoleJob(Messages.get().LogViewer_QueryJob, this,
	...
	private void getMoreData() {
		...
		new ConsoleJob(Messages.get().LogViewer_GetDataJob, this,
				Activator.PLUGIN_ID, JOB_FAMILY) {
			@Override
			protected String getErrorMessage() {
			...
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final Table data = logHandle.retrieveData(
						resultSet.getRowCount(), PAGE_SIZE);
				runInUIThread(new Runnable() {
					@Override
					public void run() {
	...
	protected void refreshData() {
		new ConsoleJob(Messages.get().LogViewer_RefreshJob, this,
			...
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final Table data = logHandle.retrieveData(0,
						resultSet.getRowCount(), true);
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						resultSet = data;
	...
	protected void showFilter(boolean show) {
		...
		fd.top = show ? new FormAttachment(filterBuilder) : new FormAttachment(
				0, 0);
