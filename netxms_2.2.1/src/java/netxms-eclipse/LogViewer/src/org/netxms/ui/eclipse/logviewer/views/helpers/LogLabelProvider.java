...
public class LogLabelProvider implements ITableLabelProvider {
	public static final String[] ALARM_STATE_TEXTS = {
			Messages.get().LogLabelProvider_Outstanding,
			Messages.get().LogLabelProvider_Acknowledged,
			Messages.get().LogLabelProvider_Resolved,
			Messages.get().LogLabelProvider_Terminated };
	public static final String[] ALARM_HD_STATE_TEXTS = {
			Messages.get().LogLabelProvider_Ignored,
			Messages.get().LogLabelProvider_Open,
			Messages.get().LogLabelProvider_Closed };
	...
	public LogLabelProvider(Log logHandle) {
		...
		alarmStateImages[Alarm.STATE_OUTSTANDING] = Activator
				.getImageDescriptor("icons/outstanding.png").createImage(); //$NON-NLS-1$
		alarmStateImages[Alarm.STATE_ACKNOWLEDGED] = Activator
				.getImageDescriptor("icons/acknowledged.png").createImage(); //$NON-NLS-1$
		alarmStateImages[Alarm.STATE_RESOLVED] = Activator.getImageDescriptor(
				"icons/resolved.png").createImage(); //$NON-NLS-1$
		alarmStateImages[Alarm.STATE_TERMINATED] = Activator
	...
	public Image getColumnImage(Object element, int columnIndex) {
		...
		case LogColumn.LC_OBJECT_ID:
			try {
				...
				return (object != null) ? wbLabelProvider.getImage(object)
						: null;
			} catch (NumberFormatException e) {
				return null;
			}
