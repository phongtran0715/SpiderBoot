...
public class SeverityConditionEditor extends ConditionEditor {
	private static final String[] OPERATIONS = {
			Messages.get().SeverityConditionEditor_Is,
			Messages.get().SeverityConditionEditor_IsNot,
			Messages.get().SeverityConditionEditor_Below,
			Messages.get().SeverityConditionEditor_Above };
	...
	protected void createContent(Composite parent, ColumnFilter initialFilter) {
		...
		for (int i = Severity.NORMAL.getValue(); i <= Severity.CRITICAL
				.getValue(); i++)
			severity.add(StatusDisplayInfo.getStatusImage(i),
	...
	public ColumnFilter createFilter() {
		...
		case 0: // IS
			return new ColumnFilter(ColumnFilterType.EQUALS,
					severity.getSelectionIndex());
		case 1: // IS NOT
			...
			return filter;
		case 2: // BELOW
			return new ColumnFilter(ColumnFilterType.LESS,
					severity.getSelectionIndex());
		case 3: // ABOVE
			return new ColumnFilter(ColumnFilterType.GREATER,
					severity.getSelectionIndex());
		}
		return new ColumnFilter(ColumnFilterType.EQUALS,
				severity.getSelectionIndex());
