...
public class ColumnFilterEditor extends DashboardComposite {
	 ...
	 * @param column
	 */
	public ColumnFilterEditor(Composite parent, FormToolkit toolkit,
			LogColumn column, ColumnFilterSetOperation operation,
			final Runnable deleteHandler) {
		...
		final Label title = toolkit
				.createLabel(header, column.getDescription());
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		...
		final Button radioAnd = toolkit.createButton(buttons,
				Messages.get().ColumnFilterEditor_AndCondition, SWT.RADIO);
		...
		final Button radioOr = toolkit.createButton(buttons,
				Messages.get().ColumnFilterEditor_OrCondition, SWT.RADIO);
	...
	private void setBooleanOperation(ColumnFilterSetOperation op) {
		booleanOperation = op;
		final String opName = (op == ColumnFilterSetOperation.AND) ? Messages
				.get().ColumnFilterEditor_And
				: Messages.get().ColumnFilterEditor_Or;
	...
	private void addCondition(ColumnFilter initialFilter) {
		...
		if (conditions.size() > 1)
			ce.setLogicalOperation((booleanOperation == ColumnFilterSetOperation.AND) ? Messages
					.get().ColumnFilterEditor_And
					: Messages.get().ColumnFilterEditor_Or);
