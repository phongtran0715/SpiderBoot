	...
	public Object execute(ExecutionEvent event) throws ExecutionException {
		...
		try {
			LogViewer view = (LogViewer) window.getActivePage().showView(
					LogViewer.ID, "syslog", IWorkbenchPage.VIEW_ACTIVATE); //$NON-NLS-1$
			...
			orderingColumns
					.add(new OrderingColumn(
							"msg_timestamp", Messages.get().ShowObjectSyslog_Time, true)); //$NON-NLS-1$
			filter.setOrderingColumns(orderingColumns);

			view.queryWithFilter(filter);
		} catch (PartInitException e) {
			MessageDialogHelper.openError(window.getShell(),
					Messages.get().ShowObjectSyslog_Error, String.format(
							Messages.get().ShowObjectSyslog_ErrorOpenLogViewer,
							e.getMessage()));
		}
