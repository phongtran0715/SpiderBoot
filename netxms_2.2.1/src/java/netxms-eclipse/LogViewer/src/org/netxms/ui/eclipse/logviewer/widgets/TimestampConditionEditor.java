...
public class TimestampConditionEditor extends ConditionEditor {
	private static final String[] OPERATIONS = {
			Messages.get().TimestampConditionEditor_Between,
			Messages.get().TimestampConditionEditor_Before,
			Messages.get().TimestampConditionEditor_After, "TODAY" };
	...
	protected void createContent(Composite parent, ColumnFilter initialFilter) {
		...
		datePicker1.setDate(c.get(Calendar.YEAR), c.get(Calendar.MONTH),
				c.get(Calendar.DAY_OF_MONTH));

		timePicker1 = new DateTime(group, SWT.TIME);
		timePicker1.setTime(c.get(Calendar.HOUR_OF_DAY),
				c.get(Calendar.MINUTE), c.get(Calendar.SECOND));

		andLabel = toolkit.createLabel(group,
				Messages.get().TimestampConditionEditor_And);

		datePicker2 = new DateTime(group, SWT.DATE | SWT.DROP_DOWN);
		datePicker2.setDate(c.get(Calendar.YEAR), c.get(Calendar.MONTH),
				c.get(Calendar.DAY_OF_MONTH));

		timePicker2 = new DateTime(group, SWT.TIME);
		timePicker2.setTime(c.get(Calendar.HOUR_OF_DAY),
				c.get(Calendar.MINUTE), c.get(Calendar.SECOND));
	}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.netxms.ui.eclipse.logviewer.widgets.ConditionEditor#
	 * operationSelectionChanged(int)
	 */
	@Override
	...
	public ColumnFilter createFilter() {
		final Calendar c = Calendar.getInstance();
		c.clear();
		c.set(datePicker1.getYear(), datePicker1.getMonth(),
				datePicker1.getDay(), timePicker1.getHours(),
				timePicker1.getMinutes(), timePicker1.getSeconds());
		...
		case 0: // between
			c.clear();
			c.set(datePicker2.getYear(), datePicker2.getMonth(),
					datePicker2.getDay(), timePicker2.getHours(),
					timePicker2.getMinutes(), timePicker2.getSeconds());
		...
		case 3: // today
			c.clear();
			c.set(datePicker1.getYear(), datePicker1.getMonth(),
					datePicker1.getDay(), 0, 0, 0);
			long from = c.getTimeInMillis() / 1000;
			c.clear();
			c.set(datePicker1.getYear(), datePicker1.getMonth(),
					datePicker1.getDay(), 23, 59, 59);
