...
public class OpenHistoryMap implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		if (window != null) {
			try {
				window.getActivePage().showView(HistoryView.ID,
						Long.toString(object.getObjectId()),
						IWorkbenchPage.VIEW_ACTIVATE);
			} catch (PartInitException e) {
				MessageDialogHelper.openError(window.getShell(),
						Messages.get().OpenHistoryMap_Error, String.format(
								Messages.get().OpenHistoryMap_CannotOpenView,
								e.getMessage()));
			}
		}
	}
	...
	public void selectionChanged(IAction action, ISelection selection) {
		...
		action.setEnabled((object != null)
				&& (object.getGeolocation().getType() != GeoLocation.UNSET));
	}
}
