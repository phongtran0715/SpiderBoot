 ...
 */
public class GeoLocationHistoryViewer extends AbstractGeoMapViewer implements
		MouseTrackListener {
	private static final int START = 1;
	private static final int END = 2;
	private static final String pointInformation[] = {
			Messages.get().GeoMapViewer_Start, Messages.get().GeoMapViewer_End };

	private static final int LABEL_ARROW_OFFSET = 10;

	private static final Color LABEL_BACKGROUND = new Color(
			Display.getCurrent(), 240, 254, 192);
	private static final Color LABEL_TEXT = new Color(Display.getCurrent(), 0,
			0, 0);
	private static final Color TRACK_COLOR = new Color(Display.getCurrent(),
			163, 73, 164);
	 ...
	 * @param object
	 */
	public GeoLocationHistoryViewer(Composite parent, int style,
	...
	/**
	 * Get geolocations adjacent to given screen coordinates ordered by distance
	 * from that point
	 * 
	 * @param p
	 * @return
	...
	public List<GeoLocation> getAdjacentLocations(int x, int y) {
		...
		Area area = new Area(topLeft.getLatitude(), topLeft.getLongitude(),
		...
		Collections.sort(locations, new Comparator<GeoLocation>() {
			...
			public int compare(GeoLocation l1, GeoLocation l2) {
				double d1 = Math.pow(
						Math.pow(l1.getLatitude() - center.getLatitude(), 2)
								+ Math.pow(
										l1.getLongitude()
												- center.getLongitude(), 2),
						0.5);
				double d2 = Math.pow(
						Math.pow(l2.getLatitude() - center.getLatitude(), 2)
								+ Math.pow(
										l2.getLongitude()
												- center.getLongitude(), 2),
						0.5);
	...
	private void updateHistory() {
		...
		ConsoleJob job = new ConsoleJob(
			...
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<GeoLocation> pl = session.getLocationHistory(
						historyObject.getObjectId(),
						timePeriod.getPeriodStart(), timePeriod.getPeriodEnd());
				runInUIThread(new Runnable() {
					...
					public void run() {
						...
						for (int i = 0; i < points.size(); i++)
							locationTree
									.insert(points.get(i).getLatitude(), points
											.get(i).getLongitude(), points
											.get(i));
						redraw();
					}
				});
	...
	protected void drawContent(GC gc, GeoLocation currentLocation, int imgW,
		...
		for (int i = 0; i < points.size(); i++) {
			final Point virtualXY = GeoLocationCache.coordinateToDisplay(
					points.get(i), accessor.getZoom());
			final int dx = virtualXY.x - centerXY.x;
			final int dy = virtualXY.y - centerXY.y;

			if (i != points.size() - 1) {
				final Point virtualXY2 = GeoLocationCache.coordinateToDisplay(
						points.get(i + 1), accessor.getZoom());
			...
			if (i == selectedPoint) {
				...
				pointToolTip.setText(String.format(
						"%s\r\n%s - %s", //$NON-NLS-1$
						points.get(i), df.format(points.get(i).getTimestamp()),
						df.format(points.get(i).getEndTimestamp())));
			...
			if (i == 0) {
				...
				drawPoint(gc, imgW / 2 + dx, imgH / 2 + dy, START, nextX,
						nextY, color);
				continue;
			}

			if (i == points.size() - 1) {
				drawPoint(gc, imgW / 2 + dx, imgH / 2 + dy, END, nextX, nextY,
						color);
				continue;
			}
	 ...
	 * @param object
	 */
	private void drawPoint(GC gc, int x, int y, int flag, int prevX, int prevY,
			int color) {
		if (flag == START || flag == END) {
			...
			Rectangle rect = new Rectangle(x - LABEL_ARROW_OFFSET, y
					- LABEL_ARROW_HEIGHT - textSize.y, textSize.x
					+ LABEL_X_MARGIN * 2 + LABEL_SPACING, textSize.y
					+ LABEL_Y_MARGIN * 2);
			...
			gc.fillRoundRectangle(rect.x, rect.y, rect.width, rect.height, 8, 8);
			gc.drawRoundRectangle(rect.x, rect.y, rect.width, rect.height, 8, 8);
			final int[] arrow = new int[] { rect.x + LABEL_ARROW_OFFSET - 4,
					rect.y + rect.height, x, y,
					rect.x + LABEL_ARROW_OFFSET + 4, rect.y + rect.height };
			...
			gc.drawText(text, rect.x + LABEL_X_MARGIN + LABEL_SPACING, rect.y
					+ LABEL_Y_MARGIN);
		} else {
			gc.setForeground(TRACK_COLOR);
			gc.setLineWidth(3);
