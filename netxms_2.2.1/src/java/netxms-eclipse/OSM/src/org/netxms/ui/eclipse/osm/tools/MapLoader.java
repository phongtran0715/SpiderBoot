...
/**
 * Map Loader - loads geographic map from tile server. Uses cached tiles when
 * possible.
 */
public class MapLoader {
	...
	private Image getMissingTileImage() {
		if (missingTile == null)
			missingTile = Activator
					.getImageDescriptor("icons/missing_tile.png").createImage(); //$NON-NLS-1$
		return missingTile;
	}
	...
	private Image getLoadingTileImage() {
		if (loadingTile == null)
			loadingTile = Activator
					.getImageDescriptor("icons/loading_tile.png").createImage(); //$NON-NLS-1$
		return loadingTile;
	}
	...
	private Image getBorderTileImage() {
		if (borderTile == null)
			borderTile = Activator
					.getImageDescriptor("icons/border_tile.png").createImage(); //$NON-NLS-1$
		return borderTile;
	}
	...
	public Tile getTile(int zoom, int x, int y, boolean cachedOnly) {
		...
		return (tileImage != null) ? new Tile(x, y, tileImage, true, false)
				: new Tile(x, y, getMissingTileImage(), true, true);
	}
	 ...
	 * @return
	 */
	public static Rectangle calculateBoundingBox(Point mapSize,
			GeoLocation centerPoint, int zoom) {
		Area coverage = GeoLocationCache.calculateCoverage(mapSize,
				centerPoint, CENTER, zoom);
		Point topLeft = tileFromLocation(coverage.getxLow(),
				coverage.getyLow(), zoom);
		Point bottomRight = tileFromLocation(coverage.getxHigh(),
				coverage.getyHigh(), zoom);
	 ...
	 * @param zoom
	 * @param cachedOnly
	 *            if true, only locally cached tiles will be returned (missing
	 *            tiles will be replaced by placeholder image)
	 * @return
	 */
	public TileSet getAllTiles(Point mapSize, GeoLocation basePoint,
			int pointLocation, int zoom, boolean cachedOnly) {
		if ((mapSize.x < 32) || (mapSize.y < 32) || (basePoint == null))
			return null;

		Area coverage = GeoLocationCache.calculateCoverage(mapSize, basePoint,
				pointLocation, zoom);
		Point bottomLeft = tileFromLocation(coverage.getxLow(),
				coverage.getyLow(), zoom);
		Point topRight = tileFromLocation(coverage.getxHigh(),
				coverage.getyHigh(), zoom);

		Tile[][] tiles = new Tile[bottomLeft.y - topRight.y + 1][topRight.x
				- bottomLeft.x + 1];

		int x = bottomLeft.x;
		int y = topRight.y;
		int l = (bottomLeft.y - topRight.y + 1)
		...
		Point realTopLeft = GeoLocationCache.coordinateToDisplay(
				new GeoLocation(lat, lon), zoom);
		Point reqTopLeft = GeoLocationCache.coordinateToDisplay(
				new GeoLocation(coverage.getxHigh(), coverage.getyLow()), zoom);

		return new TileSet(tiles, realTopLeft.x - reqTopLeft.x, realTopLeft.y
				- reqTopLeft.y, zoom);
	}
