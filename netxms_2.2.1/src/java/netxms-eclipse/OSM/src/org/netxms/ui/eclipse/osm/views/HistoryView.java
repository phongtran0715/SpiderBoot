...
public class HistoryView extends ViewPart {
	...
	private static final String[] presetNames = {
			Messages.get().HistoryView_Preset10min,
			Messages.get().HistoryView_Preset30min,
			Messages.get().HistoryView_Preset1hour,
			Messages.get().HistoryView_Preset2hours,
			Messages.get().HistoryView_Preset4hours,
			Messages.get().HistoryView_Preset12hours,
			Messages.get().HistoryView_Preset1day,
			Messages.get().HistoryView_Preset2days,
			Messages.get().HistoryView_Preset5days,
			Messages.get().HistoryView_Preset1week,
			Messages.get().HistoryView_Preset1month,
			Messages.get().HistoryView_Preset1year };
	...
	public void init(IViewSite site) throws PartInitException {
		...
		try {
			Platform.getAdapterManager()
					.loadAdapter(
							((NXCSession) ConsoleSharedData.getSession())
									.getTopLevelObjects()[0],
		...
		try {
			...
			object = ((NXCSession) ConsoleSharedData.getSession())
					.findObjectById(id);
			setPartName(Messages.get().LocationMap_PartNamePrefix
					+ object.getObjectName());
		} catch (Exception e) {
			throw new PartInitException(Messages.get().LocationMap_InitError1,
					e);
		}
		if (object == null)
	...
	protected void createActions(final Composite parent) {
		...
		actionZoomOut = new Action(
				Messages.get().AbstractGeolocationView_ZoomOut) {
			@Override
			public void run() {
				setZoomLevel(zoomLevel - 1);
			}
			...
			presetActions[i] = new Action(presetNames[i]) {
				...
				public void run() {
					map.changeTimePeriod(presetRanges[presetIndex],
							presetUnits[presetIndex]);
				}
			};
		}

		setConfigurableTime = new Action(
				Messages.get().HistoryView_CustomTimeFrame) {
			@Override
			public void run() {
				TimeSelectionDialog dialog = new TimeSelectionDialog(
						parent.getShell(), map.getTimePeriod());
	...
	protected void fillLocalPullDown(IMenuManager manager) {
		MenuManager presets = new MenuManager(
				Messages.get().HistoryView_Presets);
