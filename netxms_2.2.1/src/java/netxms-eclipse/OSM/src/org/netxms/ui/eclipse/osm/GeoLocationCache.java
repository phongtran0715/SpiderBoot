...
public class GeoLocationCache implements SessionListener {
			...
			for (AbstractObject object : session.getAllObjects()) {
				if ((object.getObjectClass() == AbstractObject.OBJECT_NODE)
					...
					if (gl.getType() != GeoLocation.UNSET) {
						...
						locationTree.insert(gl.getLatitude(),
								gl.getLongitude(), object.getObjectId());
					}
				}
			}
		}
	...
	private void onObjectChange(AbstractObject object) {
		...
		synchronized (locationTree) {
			GeoLocation gl = object.getGeolocation();
			if (gl.getType() == GeoLocation.UNSET) {
				AbstractObject prevObject = objects
				...
				if (!objects.containsKey(object.getObjectId())) {
					locationTree.insert(gl.getLatitude(), gl.getLongitude(),
							object.getObjectId());
					cacheChanged = true;
				} else {
					prevLocation = objects.get(object.getObjectId())
							.getGeolocation();
	...
	/**
	 * Get all objects in given area. If parent ID is set, only objects under
	 * given parent will be included.
	 * 
	 * @param area
	 *            geographical area
	 * @param parentId
	 *            parent object ID or 0
	 * @param filterString
	 *            object name filter
	 * @return
	 */
	public List<AbstractObject> getObjectsInArea(Area area, long parentId,
		...
		synchronized (locationTree) {
			...
			for (Long id : idList) {
				...
				if ((o != null)
						&& ((parentId == 0) || (o.getObjectId() == parentId) || o
								.isChildOf(parentId))
						&& (filterString != null ? o.getObjectName()
								.toLowerCase().contains(filterString) : true))
					list.add(o);
			}
		}
		return list;
	...
	/**
	 * Convert location to abstract display coordinates (coordinates on virtual
	 * map covering entire world)
	 * 
	 * @param location
	 *            geolocation
	 * @param zoom
	...
	public static Point coordinateToDisplay(GeoLocation location, int zoom) {
		...
		// LonToX
		double x = (location.getLongitude() + 180)
				* (numberOfTiles * TILE_SIZE) / 360.0;

		// LatToY
		double y = 1 - (Math.log(Math.tan(Math.PI / 4
				+ Math.toRadians(location.getLatitude()) / 2)) / Math.PI);
		y = y / 2 * (numberOfTiles * TILE_SIZE);
		return new Point((int) x, (int) y);
	}

	/**
	 * Convert abstract display coordinates (coordinates on virtual map covering
	 * entire world) to location
	 * 
	 * @param point
	 *            display coordinates
	 * @param zoom
	...
	public static GeoLocation displayToCoordinates(Point point, int zoom) {
		...
		double latitude = (1 - (point.y * (2 / (Math.pow(2, zoom) * 256))))
				* Math.PI;
		latitude = Math.toDegrees(Math.atan(Math.sinh(latitude)));
		if (latitude <= (-89.998))
			latitude = -89.997; // Needed because tan() is undefined for angles
								// of 90 degrees
	 ...
	 *            map size in pixels
	 * @param point
	 *            coordinates of map's base point
	 * @param pointLocation
	 *            location of base point: TOP-LEFT, BOTTOM-RIGHT, CENTER
	 * @param zoom
	 *            zoom level
	 * @return area covered by map
	 */
	public static Area calculateCoverage(Point mapSize, GeoLocation basePoint,
			int pointLocation, int zoom) {
		Point bp = coordinateToDisplay(basePoint, zoom);
		GeoLocation topLeft = null;
		GeoLocation bottomRight = null;
		switch (pointLocation) {
		case CENTER:
			topLeft = displayToCoordinates(new Point(bp.x - mapSize.x / 2, bp.y
					- mapSize.y / 2), zoom);
			bottomRight = displayToCoordinates(new Point(bp.x + mapSize.x / 2,
					bp.y + mapSize.y / 2), zoom);
			break;
		case TOP_LEFT:
			topLeft = displayToCoordinates(new Point(bp.x, bp.y), zoom);
			bottomRight = displayToCoordinates(new Point(bp.x + mapSize.x, bp.y
					+ mapSize.y), zoom);
			break;
		case BOTTOM_RIGHT:
			topLeft = displayToCoordinates(new Point(bp.x - mapSize.x, bp.y
					- mapSize.y), zoom);
		...
		return new Area(topLeft.getLatitude(), topLeft.getLongitude(),
