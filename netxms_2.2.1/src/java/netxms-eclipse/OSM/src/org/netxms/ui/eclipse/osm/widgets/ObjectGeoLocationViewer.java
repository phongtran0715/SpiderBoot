 ...
 */
public class ObjectGeoLocationViewer extends AbstractGeoMapViewer implements
		MouseTrackListener {
	...
	private static final Color ACTIVE_BORDER_COLOR = new Color(
	...
	public ObjectGeoLocationViewer(Composite parent, int style) {
		...
		objectToolTipHeaderFont = FontTools
				.createFont(TITLE_FONTS, 1, SWT.BOLD);
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.osm.widgets.AbstractGeoMapViewer#onCacheChange(
	 * org.netxms.client.objects.AbstractObject, org.netxms.base.GeoLocation)
	 */
	@Override
	protected void onCacheChange(AbstractObject object, GeoLocation prevLocation) {
		...
		if (((currLocation.getType() != GeoLocation.UNSET) && coverage
				.contains(currLocation.getLatitude(),
						currLocation.getLongitude()))
				|| ((prevLocation != null)
						&& (prevLocation.getType() != GeoLocation.UNSET) && coverage
							.contains(prevLocation.getLatitude(),
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.osm.widgets.AbstractGeoMapViewer#mouseMove(org.
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.osm.widgets.AbstractGeoMapViewer#mouseDown(org.
	 * eclipse.swt.events.MouseEvent)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.osm.widgets.AbstractGeoMapViewer#drawContent(org
	 * .eclipse.swt.graphics.GC, org.netxms.base.GeoLocation, int, int)
	 */
	@Override
	protected void drawContent(GC gc, GeoLocation currentLocation, int imgW,
			int imgH) {
		objectIcons.clear();

		final Point centerXY = GeoLocationCache.coordinateToDisplay(
				currentLocation, accessor.getZoom());
		for (AbstractObject object : objects) {
			final Point virtualXY = GeoLocationCache.coordinateToDisplay(
					object.getGeolocation(), accessor.getZoom());
	...
	private void drawObject(GC gc, int x, int y, AbstractObject object) {
		boolean selected = (currentObject != null)
				&& (currentObject.getObjectId() == object.getObjectId());
		...
		Rectangle rect = new Rectangle(x - w / 2 - 1, y - LABEL_ARROW_HEIGHT
				- h, w, h);

		Color bgColor = ColorConverter.adjustColor(StatusDisplayInfo
				.getStatusColor(object.getStatus()), new RGB(255, 255, 255),
				0.5f, colorCache);
		...
		final int[] arrow = new int[] { x - 4, rect.y + rect.height, x, y,
				x + 4, rect.y + rect.height };
	...
	private void drawObjectToolTip(GC gc) {
		...
		if ((currentObject.getGeolocation().getTimestamp().getTime() > 0)
			...
			locationDetails = String
					.format(Messages.get().ObjectGeoLocationViewer_ObtainedFrom,
							RegionalSettings.getDateTimeFormat().format(
									currentObject.getGeolocation()
											.getTimestamp()),
							(currentObject.getGeolocation().getType() == GeoLocation.GPS) ? Messages
									.get().ObjectGeoLocationViewer_GPS
									: Messages.get().ObjectGeoLocationViewer_Network);
		...
		final String postalAddress = currentObject.getPostalAddress()
				.getAddressLine();
		...
		if (currentObject instanceof MobileDevice) {
			lastReport = String.format(
					Messages.get().ObjectGeoLocationViewer_LastReport,
					((MobileDevice) currentObject).getLastReportTime()
							.getTime() > 0 ? RegionalSettings
							.getDateTimeFormat().format(
									((MobileDevice) currentObject)
											.getLastReportTime()) : Messages
							.get().ObjectGeoLocationViewer_Never);
			...
			if (((MobileDevice) currentObject).getBatteryLevel() >= 0) {
				batteryLevel = String.format(
						Messages.get().ObjectGeoLocationViewer_BatteryLevel,
						((MobileDevice) currentObject).getBatteryLevel());
		...
		if ((currentObject.getComments() != null)
				&& !currentObject.getComments().isEmpty()) {
		...
		gc.setBackground(StatusDisplayInfo.getStatusColor(currentObject
				.getStatus()));
		...
		gc.setFont(objectToolTipHeaderFont);
		gc.drawText(currentObject.getObjectName(), rect.x
		...
		y = rect.y + OBJECT_TOOLTIP_Y_MARGIN + titleSize.y
				+ OBJECT_TOOLTIP_SPACING + 2;
		gc.drawText(location, rect.x + OBJECT_TOOLTIP_X_MARGIN, y, true);
		if (locationDetails != null) {
			y += textLineHeight;
			gc.drawText(locationDetails, rect.x + OBJECT_TOOLTIP_X_MARGIN, y,
					true);
		}
		if (!postalAddress.isEmpty()) {
			y += textLineHeight;
			gc.drawText(postalAddress, rect.x + OBJECT_TOOLTIP_X_MARGIN, y,
					true);
		}
		...
		if (lastReport != null) {
			...
			if (batteryLevel != null) {
				y += textLineHeight;
				gc.drawText(batteryLevel, rect.x + OBJECT_TOOLTIP_X_MARGIN, y,
						true);
			}
		}
		...
		if ((currentObject.getComments() != null)
			...
			y += OBJECT_TOOLTIP_SPACING;
			gc.setForeground(colorCache.create(0, 0, 0));
			gc.drawText(currentObject.getComments(), rect.x
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.osm.widgets.AbstractGeoMapViewer#getObjectAtPoint
