 ...
 */
public abstract class AbstractGeoMapViewer extends Canvas implements
		PaintListener, GeoLocationCacheListener, MouseWheelListener,
		MouseListener, MouseMoveListener {
	protected static final String[] TITLE_FONTS = { "Segoe UI",
			"Liberation Sans", "DejaVu Sans", "Verdana", "Arial" };

	protected static final Color BORDER_COLOR = new Color(Display.getCurrent(),
			0, 0, 0);
	...
	private static final Color MAP_BACKGROUND = new Color(Display.getCurrent(),
			255, 255, 255);
	private static final Color INFO_BLOCK_BACKGROUND = new Color(
			Display.getCurrent(), 0, 0, 0);
	private static final Color INFO_BLOCK_TEXT = new Color(
			Display.getCurrent(), 255, 255, 255);
	private static final Color SELECTION_COLOR = new Color(
			Display.getCurrent(), 0, 0, 255);
	...
	public AbstractGeoMapViewer(Composite parent, int style) {
		...
		imageZoomIn = Activator
				.getImageDescriptor("icons/map_zoom_in.png").createImage(); //$NON-NLS-1$
		imageZoomOut = Activator
				.getImageDescriptor("icons/map_zoom_out.png").createImage(); //$NON-NLS-1$

		mapTitleFont = FontTools.createFont(TITLE_FONTS, 2, SWT.BOLD);

		labelProvider = WorkbenchLabelProvider
		...
		addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				...
				GeoLocationCache.getInstance().removeListener(
						AbstractGeoMapViewer.this);
				if (bufferImage != null)
					bufferImage.dispose();
	...
	public void reloadMap() {
		...
		final Point mapSize = new Point(accessor.getMapWidth(),
				accessor.getMapHeight());
		final GeoLocation centerPoint = accessor.getCenterPoint();
		ConsoleJob job = new ConsoleJob(
				Messages.get().GeoMapViewer_DownloadJob_Title, viewPart,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final TileSet tiles = mapLoader
						.getAllTiles(mapSize, centerPoint, MapLoader.CENTER,
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						...
						Point mapSize = new Point(
								currentImage.getImageData().width,
								currentImage.getImageData().height);
						coverage = GeoLocationCache.calculateCoverage(mapSize,
								accessor.getCenterPoint(),
	...
	private void loadMissingTiles(final TileSet tiles) {
		ConsoleJob job = new ConsoleJob(
				Messages.get().GeoMapViewer_LoadMissingJob_Title, viewPart,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	public void paintControl(PaintEvent e) {
		...
		if (dragStartPoint == null) {
			...
			Point cp = GeoLocationCache.coordinateToDisplay(
					accessor.getCenterPoint(), accessor.getZoom());
			cp.x += offsetX;
			cp.y += offsetY;
			currentLocation = GeoLocationCache.displayToCoordinates(cp,
					accessor.getZoom());

			Point size = getSize();
			TileSet tileSet = mapLoader.getAllTiles(size, currentLocation,
					MapLoader.CENTER, accessor.getZoom(), true);
			int x = tileSet.xOffset;
			int y = tileSet.yOffset;
			final Tile[][] tiles = tileSet.tiles;
			for (int i = 0; i < tiles.length; i++) {
		...
		if ((title != null) && !title.isEmpty()) {
			...
			gc.setForeground(SharedColors.getColor(SharedColors.GEOMAP_TITLE,
					getDisplay()));
		...
		gc.drawText(text, rect.x + rect.width / 2 - textSize.x / 2, rect.y + 5,
				true);
	 ...
	 *            current location (map center)
	 * @param imgW
	 *            map image width
	 * @param imgH
	 *            map image height
	 */
	protected abstract void drawContent(GC gc, GeoLocation currentLocation,
			int imgW, int imgH);
	 ...
	 */
	@Override
	public void geoLocationCacheChanged(final AbstractObject object,
			final GeoLocation prevLocation) {
	 ...
	 * @param prevLocation
	 */
	protected abstract void onCacheChange(final AbstractObject object,
			final GeoLocation prevLocation);
	...
	public void mouseDoubleClick(MouseEvent e) {
		...
		if (zoom < MapAccessor.MAX_MAP_ZOOM) {
			...
			zoom = (zoom + step > MapAccessor.MAX_MAP_ZOOM) ? MapAccessor.MAX_MAP_ZOOM
					: zoom + step;

			final GeoLocation geoLocation = getLocationAtPoint(new Point(e.x,
					e.y));
			accessor.setZoom(zoom);
	...
	public void mouseDown(MouseEvent e) {
		if (e.button == 1) // left button, ignore if map is currently loading
		{
			if (zoomControlRect.contains(e.x, e.y)) {
				Rectangle r = new Rectangle(zoomControlRect.x + 5,
						zoomControlRect.y + zoomControlRect.height - 37, 32, 32);
				int zoom = accessor.getZoom();
	...
	public void mouseUp(MouseEvent e) {
		if ((e.button == 1) && (dragStartPoint != null)) {
			if (Math.abs(offsetX) > DRAG_JITTER
					|| Math.abs(offsetY) > DRAG_JITTER) {
				final Point centerXY = GeoLocationCache.coordinateToDisplay(
						accessor.getCenterPoint(), accessor.getZoom());
				centerXY.x += offsetX;
				centerXY.y += offsetY;
				final GeoLocation geoLocation = GeoLocationCache
						.displayToCoordinates(centerXY, accessor.getZoom());
				accessor.setLatitude(geoLocation.getLatitude());
				accessor.setLongitude(geoLocation.getLongitude());
				reloadMap();
				notifyOnPositionChange();
		...
		if ((e.button == 1) && (selectionStartPoint != null)) {
			if (selectionEndPoint != null) {
				...
				final GeoLocation lc = getLocationAtPoint(new Point(x2
						- (x2 - x1) / 2, y2 - (y2 - y1) / 2));

				int zoom = accessor.getZoom();
				while (zoom < MapAccessor.MAX_MAP_ZOOM) {
					zoom++;
					final Area area = GeoLocationCache.calculateCoverage(
							getSize(), lc, GeoLocationCache.CENTER, zoom);
					if (!area.contains(l1.getLatitude(), l1.getLongitude())
							|| !area.contains(l2.getLatitude(),
									l2.getLongitude())) {
						zoom--;
						break;
					}
	...
	public void mouseMove(MouseEvent e) {
		if (dragStartPoint != null) {
			...
			if (Math.abs(deltaX) > DRAG_JITTER
					|| Math.abs(deltaY) > DRAG_JITTER) {
				offsetX = deltaX;
				offsetY = deltaY;
				redraw();
			}
	 ...
	 * 
	 * @param p
	 *            widget-related coordinates
	 * @return location (latitude/longitude) at given point
	 */
	public GeoLocation getLocationAtPoint(Point p) {
		Point cp = GeoLocationCache.coordinateToDisplay(new GeoLocation(
				coverage.getxHigh(), coverage.getyLow()), accessor.getZoom());
		return GeoLocationCache.displayToCoordinates(new Point(cp.x + p.x, cp.y
				+ p.y), accessor.getZoom());
	}
