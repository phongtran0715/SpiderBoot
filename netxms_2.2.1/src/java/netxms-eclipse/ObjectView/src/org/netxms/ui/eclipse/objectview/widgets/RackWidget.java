 ...
 * Rack display widget
 */
public class RackWidget extends Canvas implements PaintListener,
		DisposeListener, ImageUpdateListener, MouseListener,
	...
	private static final String[] FONT_NAMES = {
			"Segoe UI", "Liberation Sans", "DejaVu Sans", "Verdana", "Arial" }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$
	...
	private Set<RackSelectionListener> selectionListeners = new HashSet<RackSelectionListener>(
			0);
	...
	public RackWidget(Composite parent, int style, Rack rack,
			RackOrientation view) {
		super(parent, style | SWT.DOUBLE_BUFFERED);
		this.rack = rack;
		this.view = (view == RackOrientation.FILL) ? RackOrientation.FRONT
				: view;
		...
		setBackground(SharedColors.getColor(SharedColors.RACK_BACKGROUND,
				getDisplay()));
		...
		imageDefaultTop = Activator.getImageDescriptor(
				"icons/rack-default-top.png").createImage(); //$NON-NLS-1$
		imageDefaultMiddle = Activator.getImageDescriptor(
				"icons/rack-default-middle.png").createImage(); //$NON-NLS-1$
		imageDefaultBottom = Activator.getImageDescriptor(
				"icons/rack-default-bottom.png").createImage(); //$NON-NLS-1$
		imageDefaultRear = Activator.getImageDescriptor(
				"icons/rack-default-rear.png").createImage(); //$NON-NLS-1$
	...
	public void paintControl(PaintEvent e) {
		...
		unitHeight = (double) (rect.height - ((borderWidth + 1) / 2) * 2)
				/ (double) rack.getHeight();
		unitWidth = (int) (unitHeight * UNIT_WH_RATIO);
		rect.width = unitWidth + borderWidth * 2;

		// Title
		gc.setFont(WidgetHelper.getBestFittingFont(gc, titleFonts,
				VIEW_LABELS[0], rect.width, TITLE_HEIGHT)); //$NON-NLS-1$
		Point titleSize = gc.textExtent(VIEW_LABELS[view.getValue() - 1]);
		gc.drawText(VIEW_LABELS[view.getValue() - 1],
				(rect.width / 2 - titleSize.x / 2) + UNIT_NUMBER_WIDTH
						+ MARGIN_WIDTH, rect.y - TITLE_HEIGHT - MARGIN_HEIGHT
						/ 2);

		// Rack itself
		gc.setBackground(SharedColors.getColor(SharedColors.RACK_EMPTY_SPACE,
				getDisplay()));
		gc.fillRoundRectangle(rect.x, rect.y, rect.width, rect.height, 3, 3);
		gc.setLineWidth(borderWidth);
		gc.setForeground(SharedColors.getColor(SharedColors.RACK_BORDER,
				getDisplay()));
		gc.drawRoundRectangle(rect.x, rect.y, rect.width, rect.height, 3, 3);

		// Rack bottom
		gc.setBackground(SharedColors.getColor(SharedColors.RACK_BORDER,
				getDisplay()));
		gc.fillRectangle(rect.x + borderWidth * 2 - (borderWidth + 1) / 2,
				rect.y + rect.height, borderWidth * 2,
				(int) (borderWidth * 1.5));
		gc.fillRectangle(rect.x + rect.width - borderWidth * 3
				- (borderWidth + 1) / 2, rect.y + rect.height, borderWidth * 2,
				(int) (borderWidth * 1.5));

		// Draw unit numbers
		int[] unitBaselines = new int[rack.getHeight() + 1];
		gc.setFont(WidgetHelper.getBestFittingFont(gc, labelFonts,
				"00", UNIT_NUMBER_WIDTH, (int) unitHeight - 2)); //$NON-NLS-1$
		gc.setForeground(SharedColors.getColor(SharedColors.RACK_TEXT,
				getDisplay()));
		gc.setBackground(SharedColors.getColor(SharedColors.RACK_BACKGROUND,
				getDisplay()));
		gc.setLineWidth(1);
		double dy = rack.isTopBottomNumbering() ? rect.y + unitHeight
				+ (borderWidth + 1) / 2 : rect.y + rect.height
				- (borderWidth + 1) / 2;
		...
		for (int u = 1; u <= rack.getHeight(); u++) {
			...
			gc.drawText(label, UNIT_NUMBER_WIDTH - textExtent.x, y
					- (int) unitHeight / 2 - textExtent.y / 2);
			if (rack.isTopBottomNumbering()) {
				unitBaselines[u] = y;
				dy += unitHeight;
			} else {
		...
		for (RackElement n : units) {
			if ((n.getRackPosition() < 1)
					|| (n.getRackPosition() > rack.getHeight())
					|| (rack.isTopBottomNumbering() && (n.getRackPosition()
							+ n.getRackHeight() > rack.getHeight() + 1))
					|| (!rack.isTopBottomNumbering() && (n.getRackPosition()
							- n.getRackHeight() < 0))
					|| ((n.getRackOrientation() != view) && (n
							.getRackOrientation() != RackOrientation.FILL)))
				continue;

			int topLine, bottomLine;
			if (rack.isTopBottomNumbering()) {
				bottomLine = unitBaselines[n.getRackPosition()
						+ n.getRackHeight() - 1]; // lower border
				topLine = unitBaselines[n.getRackPosition() - 1]; // upper
																	// border
			} else {
				bottomLine = unitBaselines[n.getRackPosition()
						- n.getRackHeight()]; // lower border
				topLine = unitBaselines[n.getRackPosition()]; // upper border
			}
			final Rectangle unitRect = new Rectangle(rect.x + (borderWidth + 1)
					/ 2, topLine + 1, rect.width - borderWidth, bottomLine
					- topLine);
			...
			gc.fillRectangle(unitRect.x - borderWidth + borderWidth / 4 + 1,
					unitRect.y + 1, borderWidth / 2 - 1,
					Math.min(borderWidth, (int) unitHeight - 2));

			if ((n.getRackImage() != null)
					&& !n.getRackImage().equals(NXCommon.EMPTY_GUID)) {
				Image image = ImageProvider.getInstance().getImage(
						n.getRackImage());
				Rectangle r = image.getBounds();
				gc.drawImage(image, 0, 0, r.width, r.height, unitRect.x,
						unitRect.y, unitRect.width, unitRect.height);
			} else // Draw default representation
			{
				Image imageTop = (view == RackOrientation.REAR && n
						.getRackOrientation() == RackOrientation.FILL) ? imageDefaultRear
						: imageDefaultTop;

				Rectangle r = imageTop.getBounds();
				if (n.getRackHeight() == 1) {
					gc.drawImage(imageTop, 0, 0, r.width, r.height, unitRect.x,
							unitRect.y, unitRect.width, unitRect.height);
				} else {
					Image imageMiddle = (view == RackOrientation.REAR && n
							.getRackOrientation() == RackOrientation.FILL) ? imageDefaultRear
							: imageDefaultMiddle;
					Image imageBottom = (view == RackOrientation.REAR && n
							.getRackOrientation() == RackOrientation.FILL) ? imageDefaultRear
							: imageDefaultBottom;
					if (rack.isTopBottomNumbering()) {
						unitRect.height = unitBaselines[n.getRackPosition()]
								- topLine;
						gc.drawImage(imageTop, 0, 0, r.width, r.height,
								unitRect.x, unitRect.y, unitRect.width,
								unitRect.height);
						...
						for (int i = 1; i < n.getRackHeight() - 1; i++, u++) {
							...
							gc.drawImage(imageMiddle, 0, 0, r.width, r.height,
									unitRect.x, unitRect.y, unitRect.width,
									unitRect.height);
						}
						...
						unitRect.y = unitBaselines[u - 1];
						unitRect.height = unitBaselines[u] - unitRect.y;
						gc.drawImage(imageBottom, 0, 0, r.width, r.height,
								unitRect.x, unitRect.y, unitRect.width,
								unitRect.height);
					} else {
						unitRect.height = unitBaselines[n.getRackPosition() - 1]
								- topLine;
						gc.drawImage(imageTop, 0, 0, r.width, r.height,
								unitRect.x, unitRect.y, unitRect.width,
								unitRect.height);
	...
	private void drawObjectToolTip(GC gc) {
		...
		int height = OBJECT_TOOLTIP_Y_MARGIN * 2 + titleSize.y + 2
				+ OBJECT_TOOLTIP_SPACING;

		List<String> texts = new ArrayList<String>();
		if (tooltipObject instanceof AbstractNode) {
			texts.add(((AbstractNode) tooltipObject).getPrimaryIP()
					.getHostAddress());
		...
		List<DciValue> values = ((DataCollectionTarget) tooltipObject)
				.getTooltipDciData();
		if (!values.isEmpty()) {
		...
		if ((tooltipObject.getComments() != null)
				&& !tooltipObject.getComments().isEmpty()) {
		...
		Rectangle rect = new Rectangle(objectToolTipLocation.x - width / 2,
				objectToolTipLocation.y - height / 2, width, height);
		if (rect.x < 0)
			rect.x = 0;
		...
		gc.setBackground(StatusDisplayInfo.getStatusColor(tooltipObject
				.getStatus()));
		gc.fillOval(rect.x + OBJECT_TOOLTIP_X_MARGIN, rect.y
				+ OBJECT_TOOLTIP_Y_MARGIN + titleSize.y / 2 - 4, 8, 8);

		gc.setForeground(colorCache.create(0, 0, 0));
		gc.setFont(objectToolTipHeaderFont);
		gc.drawText(tooltipObject.getObjectName(), rect.x
				+ OBJECT_TOOLTIP_X_MARGIN + 12, rect.y
				+ OBJECT_TOOLTIP_Y_MARGIN, true);

		gc.setFont(JFaceResources.getDefaultFont());
		int textLineHeight = gc.textExtent("M").y; //$NON-NLS-1$
		y = rect.y + OBJECT_TOOLTIP_Y_MARGIN + titleSize.y
				+ OBJECT_TOOLTIP_SPACING + 2 - textLineHeight;
		for (String s : texts) {
			if ((s == null) || s.isEmpty())
				continue;
		...
		if (!values.isEmpty()) {
			...
			for (DciValue v : values) {
				gc.drawText(v.getName(), rect.x + OBJECT_TOOLTIP_X_MARGIN, y,
						true);
				Point pt = gc.textExtent(v.getValue());
				gc.drawText(v.getValue(), rect.x + rect.width
						- OBJECT_TOOLTIP_X_MARGIN - pt.x, y, true);
				y += textLineHeight;
			}
			y -= textLineHeight;
		}
		...
		if ((tooltipObject.getComments() != null)
			...
			y += OBJECT_TOOLTIP_SPACING;
			gc.setForeground(colorCache.create(0, 0, 0));
			gc.drawText(tooltipObject.getComments(), rect.x
					+ OBJECT_TOOLTIP_X_MARGIN, y, true);
		}
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		...
		if (hHint == SWT.DEFAULT) {
			...
			return new Point(FULL_UNIT_WIDTH + MARGIN_WIDTH * 2
					+ UNIT_NUMBER_WIDTH + borderWidth * 2, rack.getHeight()
					* FULL_UNIT_HEIGHT + MARGIN_HEIGHT * 2 + borderWidth * 2);
		}
		...
		return new Point(unitWidth + MARGIN_WIDTH * 2 + UNIT_NUMBER_WIDTH
				+ borderWidth * 2, hHint);
	}
	...
	public void mouseHover(MouseEvent e) {
		if (objectTooltipRectangle != null) // ignore hover if tooltip already
											// open
			return;

		AbstractObject object = getObjectAtPoint(new Point(e.x, e.y));
		if (object != selectedObject) {
			objectToolTipLocation = (object != null) ? new Point(e.x, e.y)
					: null;
	...
	public void mouseMove(MouseEvent e) {
		if ((objectTooltipRectangle != null)
				&& !objectTooltipRectangle.contains(e.x, e.y)) {
			objectTooltipRectangle = null;
			objectToolTipLocation = null;
			tooltipObject = null;
			redraw();
