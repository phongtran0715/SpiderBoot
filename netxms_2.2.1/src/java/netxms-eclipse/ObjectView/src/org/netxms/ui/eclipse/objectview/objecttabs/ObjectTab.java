...
public abstract class ObjectTab implements IPluginContribution {
	 ...
	 * 
	 * @param ce
	 *            Eclipse registry configuration element
	 */
	public void configure(IConfigurationElement ce, ViewPart viewPart) {
		...
		if (path != null) {
			icon = AbstractUIPlugin.imageDescriptorFromPlugin(ce
	...
	public void selected() {
		...
		// sometimes recursive activation can happen when tab selection changed
		// - it's OK here
		try {
			clientArea.setFocus();
		...
		if ((contextId != null) && !contextId.isEmpty()) {
			IContextService contextService = (IContextService) getViewPart()
	...
	/**
	 * Test if tab should be shown for given NetXMS object. Default
	...
	/**
	 * Called by parent view to inform tab that currently selected object was
	 * changed.
	 * 
	 * @param object
	...
	/**
	 * Called by parent view to inform tab that update notification for current
	 * object was received from server. Default implementation do nothing. This
	 * method always called in UI thread.
	 * 
	 * @param object
	...
	public void hide() {
		if (tabItem != null) {
			...
			// sometimes recursive activation can happen when we hide unneeded
			// tabs - it's OK here
			try {
				tabItem.dispose();
			} catch (RuntimeException e) {
	 ...
	 * Disposes the tab.
	 * 
	 * This is the last method called on the ObjectTab. There is no guarantee
	 * that createTabContent() has been called, so the tab controls may never
	 * have been created.
	 * 
	 * Within this method a tab may release any resources, fonts, images, etc.
	 * held by this tab. It is also very important to deregister all listeners
	 * from the workbench.
	 * 
	 * Clients should not call this method (the tab manager calls this method at
	 * appropriate times).
	 */
	public void dispose() {
		if (tabImage != null) {
	...
	/**
	 * Called by tab manager when user press refresh button in tab view. Default
