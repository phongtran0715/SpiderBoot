 ...
 */
public class InterfaceListLabelProvider extends LabelProvider implements
	...
	private static final String[] ifaceExpectedState = {
			...
			Messages.get().InterfaceListLabelProvider_Auto };
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		case InterfacesTab.COLUMN_TYPE:
			...
			return (typeName != null) ? String.format(
	...
	private String getPeerIpAddress(Interface iface) {
		AbstractNode peer = (AbstractNode) session.findObjectById(
				iface.getPeerNodeId(), AbstractNode.class);
		if (peer == null)
			return null;
		...
		case InterfacesTab.COLUMN_OPER_STATE:
			...
			case Interface.OPER_STATE_DOWN:
				return StatusDisplayInfo
						.getStatusColor((iface.getAdminState() == Interface.ADMIN_STATE_DOWN) ? ObjectStatus.DISABLED
	...
	public static String ifSpeedTotext(long speed) {
		if (speed < 1000)
			return Long.toString(speed)
					+ Messages.get().InterfaceListLabelProvider_bps;

		if (speed < 1000000)
			return divideSpeed(speed, 3)
					+ Messages.get().InterfaceListLabelProvider_Kbps;

		if (speed < 1000000000)
			return divideSpeed(speed, 6)
					+ Messages.get().InterfaceListLabelProvider_Mbps;

		if (speed < 1000000000000L)
			return divideSpeed(speed, 9)
					+ Messages.get().InterfaceListLabelProvider_Gbps;

		return divideSpeed(speed, 12)
				+ Messages.get().InterfaceListLabelProvider_Tbps;
	}
}
