...
public class Connection extends OverviewPageElement {
	...
	protected Control createClientArea(Composite parent) {
		...
		area.setBackground(SharedColors.getColor(
	...
	protected void onObjectChange() {
		...
		if (peerNodeId != 0) {
			...
			nodeLabel.setText((node != null) ? node.getObjectName()
					: "<" + peerNodeId + ">"); //$NON-NLS-1$ //$NON-NLS-2$
		...
		if (peerInterfaceId != 0) {
			...
			interfaceLabel.setText((peerIface != null) ? peerIface
					.getObjectName() : "<" + peerInterfaceId + ">"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	public boolean isApplicableForObject(AbstractObject object) {
		return (object instanceof Interface)
				&& (((Interface) object).getPeerNodeId() != 0);
	}
}
