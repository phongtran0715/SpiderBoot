...
public class SoftwareInventory extends Composite {
	...
	private static final String[] names = {
			Messages.get().SoftwareInventory_Name,
			Messages.get().SoftwareInventory_Version,
			Messages.get().SoftwareInventory_Vendor,
			Messages.get().SoftwareInventory_InstallDate,
			Messages.get().SoftwareInventory_Description,
			Messages.get().SoftwareInventory_URL };
	...
	private void createTableViewer() {
		viewer = new SortableTableViewer(this, names, widths, 0, SWT.UP,
				SWT.MULTI | SWT.FULL_SELECTION);
		WidgetHelper.restoreColumnViewerSettings(viewer, Activator.getDefault()
				.getDialogSettings(), configPrefix);
		viewer.getControl().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveColumnViewerSettings(viewer, Activator
	...
	private void createTreeViewer() {
		viewer = new SortableTreeViewer(this, names, widths, 0, SWT.UP,
				SWT.MULTI | SWT.FULL_SELECTION);
		WidgetHelper.restoreColumnViewerSettings(viewer, Activator.getDefault()
				.getDialogSettings(), configPrefix);
		viewer.getControl().addDisposeListener(new DisposeListener() {
	...
	public void refresh() {
		...
		new ConsoleJob(Messages.get().SoftwareInventory_JobName, viewPart,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				if (object instanceof AbstractNode) {
					final List<SoftwarePackage> packages = session
							.getNodeSoftwarePackages(rootObjectId);
					...
					for (final AbstractObject o : object
							.getAllChilds(AbstractObject.OBJECT_NODE)) {
						try {
							List<SoftwarePackage> packages = session
									.getNodeSoftwarePackages(o.getObjectId());
							nodes.add(new SoftwareInventoryNode((Node) o,
									packages));
	...
	public void setRootObjectId(long rootObjectId) {
		...
		AbstractObject object = ((NXCSession) ConsoleSharedData.getSession())
				.findObjectById(rootObjectId);
