...
public class SoftwarePackageComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		int result;
		final int column = (viewer instanceof SortableTableViewer) ? (Integer) ((SortableTableViewer) viewer)
				.getTable().getSortColumn().getData("ID") : (Integer) ((SortableTreeViewer) viewer).getTree().getSortColumn().getData("ID"); //$NON-NLS-1$ //$NON-NLS-2$

		if (e1 instanceof SoftwareInventoryNode) {
			if (column == 0) {
				result = ((SoftwareInventoryNode) e1)
						.getNode()
						.getObjectName()
						.compareToIgnoreCase(
								((SoftwareInventoryNode) e2).getNode()
										.getObjectName());
			} else {
				result = 0;
			}
			...
			case SoftwareInventory.COLUMN_DATE:
				result = Long.signum(p1.getInstallDateMs()
						- p2.getInstallDateMs());
				break;
			case SoftwareInventory.COLUMN_DESCRIPTION:
				result = p1.getDescription().compareToIgnoreCase(
						p2.getDescription());
				break;
			...
			case SoftwareInventory.COLUMN_URL:
				result = p1.getSupportUrl().compareToIgnoreCase(
						p2.getSupportUrl());
				break;
		...
		int direction = (viewer instanceof SortableTableViewer) ? ((SortableTableViewer) viewer)
				.getTable().getSortDirection() : ((SortableTreeViewer) viewer)
				.getTree().getSortDirection();
