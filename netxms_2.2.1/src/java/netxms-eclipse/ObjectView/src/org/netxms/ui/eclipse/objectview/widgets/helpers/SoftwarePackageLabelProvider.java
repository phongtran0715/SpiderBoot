 ...
 */
public class SoftwarePackageLabelProvider extends LabelProvider implements
		ITableLabelProvider {
	private WorkbenchLabelProvider wbl = new WorkbenchLabelProvider();
	private Image imgPackage = Activator
			.getImageDescriptor("icons/package.gif").createImage(); //$NON-NLS-1$
	...
	public Image getColumnImage(Object element, int columnIndex) {
		if (columnIndex == 0) {
			if (element instanceof SoftwareInventoryNode)
				return wbl
						.getImage(((SoftwareInventoryNode) element).getNode());
	...
	public String getColumnText(Object element, int columnIndex) {
		if (element instanceof SoftwareInventoryNode) {
			if (columnIndex == 0)
				return ((SoftwareInventoryNode) element).getNode()
						.getObjectName();
		} else {
			SoftwarePackage p = (SoftwarePackage) element;
			switch (columnIndex) {
			case SoftwareInventory.COLUMN_DATE:
				if ((p.getInstallDate() == null)
						|| (p.getInstallDate().getTime() == 0))
					return null;
				return RegionalSettings.getDateFormat().format(
						p.getInstallDate());
