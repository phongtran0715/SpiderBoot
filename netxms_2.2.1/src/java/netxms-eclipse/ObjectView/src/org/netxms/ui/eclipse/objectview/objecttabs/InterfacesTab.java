...
public class InterfacesTab extends ObjectTab {
	...
	protected void createTabContent(Composite parent) {
		final IDialogSettings settings = Activator.getDefault()
				.getDialogSettings();
		showFilter = safeCast(
				settings.get("InterfacesTab.showFilter"), settings.getBoolean("InterfacesTab.showFilter"), showFilter); //$NON-NLS-1$ //$NON-NLS-2$
		hideSubInterfaces = safeCast(
				settings.get("InterfacesTab.hideSubInterfaces"), settings.getBoolean("InterfacesTab.hideSubInterfaces"), hideSubInterfaces); //$NON-NLS-1$ //$NON-NLS-2$
		...
		filterText.addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				...
				settings.put(
						"InterfacesTab.hideSubInterfaces", hideSubInterfaces); //$NON-NLS-1$
			}
		});
		...
		Action action = new Action() {
			...
			public void run() {
				...
				Command command = service
						.getCommand("org.netxms.ui.eclipse.objectview.commands.show_filter"); //$NON-NLS-1$
				State state = command
						.getState("org.netxms.ui.eclipse.objectview.commands.show_filter.state"); //$NON-NLS-1$
		...
		command = service
				.getCommand("org.netxms.ui.eclipse.objectview.commands.hideSubInterfaces"); //$NON-NLS-1$
		state = command
				.getState("org.netxms.ui.eclipse.objectview.commands.hideSubInterfaces.state"); //$NON-NLS-1$
		...
		final String[] names = { Messages.get().InterfacesTab_ColId,
		...
		final int[] widths = { 60, 150, 150, 150, 70, 70, 70, 70, 90, 150, 100,
				90, 150, 100, 90, 80, 80, 80, 80, 80, 80, 80 };
		viewer = new SortableTableViewer(interfacesArea, names, widths,
				COLUMN_NAME, SWT.UP, SWT.FULL_SELECTION | SWT.MULTI);
		...
		WidgetHelper.restoreTableViewerSettings(viewer, Activator.getDefault()
				.getDialogSettings(), "InterfaceTable.V4"); //$NON-NLS-1$
		viewer.getTable().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveColumnSettings(viewer.getTable(), Activator
			...
			public void notificationHandler(SessionNotification n) {
				if (n.getCode() == SessionNotification.OBJECT_CHANGED) {
					...
					if ((object != null)
							...
							&& (getObject() != null)
							&& object
									.isDirectChildOf(getObject().getObjectId())) {
	...
	private void createActions() {
		actionCopyToClipboard = new Action(
				Messages.get().InterfacesTab_ActionCopy, SharedIcons.COPY) {
			@Override
			public void run() {
				copyToClipboard(-1);
			}
		...
		actionCopyMacAddressToClipboard = new Action(
				Messages.get().InterfacesTab_ActionCopyMAC) {
			@Override
			public void run() {
		...
		actionCopyIpAddressToClipboard = new Action(
				Messages.get().InterfacesTab_ActionCopyIP) {
			@Override
			public void run() {
		...
		actionCopyPeerNameToClipboard = new Action(
				Messages.get().InterfacesTab_ActionCopyPeerName) {
			@Override
			public void run() {
		...
		actionCopyPeerMacToClipboard = new Action(
				Messages.get().InterfacesTab_ActionCopyPeerMAC) {
			@Override
			public void run() {
		...
		actionCopyPeerIpToClipboard = new Action(
				Messages.get().InterfacesTab_ActionCopyPeerIP) {
			@Override
			public void run() {
	...
	public void refresh() {
		if (getObject() != null)
			viewer.setInput(getObject().getAllChilds(
					AbstractObject.OBJECT_INTERFACE).toArray());
		else
	 ...
	 * 
	 * @see
	 * org.netxms.ui.eclipse.objectview.objecttabs.ObjectTab#getSelectionProvider
	 * ()
	 */
	@Override
	 ...
	 * 
	 * @param column
	 *            column number or -1 to copy all columns
	 */
