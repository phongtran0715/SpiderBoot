	...
	private void createActions() {
		...
		actionWakeup = new Action(Messages.get().Commands_ActionWakeup) {
			...
			public void run() {
				...
				new ConsoleJob(Messages.get().Commands_WakeupJobName, null,
		...
		actionWakeup.setImageDescriptor(Activator
				.getImageDescriptor("icons/wol.png")); //$NON-NLS-1$
	}
	...
	protected void onObjectChange() {
		...
		if (getObject() instanceof AbstractNode) {
			...
			for (final ObjectTool tool : tools) {
				if (!tool.isVisibleInCommands()
						|| !tool.isEnabled()
						|| !tool.isApplicableForNode((AbstractNode) getObject()))
					continue;
