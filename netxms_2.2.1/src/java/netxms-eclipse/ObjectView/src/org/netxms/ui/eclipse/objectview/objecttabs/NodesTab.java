...
public class NodesTab extends ObjectTab {
	...
	protected void createTabContent(Composite parent) {
		final String[] names = { Messages.get().NodesTab_ColId,
		...
		WidgetHelper.restoreTableViewerSettings(viewer, Activator.getDefault()
				.getDialogSettings(), "NodeTable.V2"); //$NON-NLS-1$
		viewer.getTable().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveColumnSettings(viewer.getTable(), Activator
						.getDefault().getDialogSettings(), "NodeTable.V2"); //$NON-NLS-1$
			}
		});
		createActions();
		createPopupMenu();
	...
	public boolean showForObject(AbstractObject object) {
		return (object instanceof Subnet) || (object instanceof Cluster)
				|| (object instanceof Container)
				|| (object instanceof ServiceRoot) || (object instanceof Rack);
	}
