...
public class LastValues extends OverviewPageElement {
	 ...
	 * @param objectTab
	 */
	public LastValues(Composite parent, OverviewPageElement anchor,
			ObjectTab objectTab) {
		super(parent, anchor, objectTab);
		refreshController = new ViewRefreshController(getObjectTab()
				.getViewPart(), -1, new Runnable() {
			@Override
			public void run() {
				refresh();
			}
	 ...
	 * @see
	 * org.netxms.ui.eclipse.objectview.objecttabs.elements.OverviewPageElement
	 * #createClientArea(org.eclipse.swt.widgets.Composite)
	 */
	@Override
	protected Control createClientArea(Composite parent) {
		viewer = new TableViewer(parent, SWT.FULL_SELECTION
				| SWT.HIDE_SELECTION | SWT.H_SCROLL | SWT.MULTI);
		setupTable();
		...
		viewer.setComparator(new ViewerComparator() {
			...
			public int compare(Viewer viewer, Object e1, Object e2) {
				return ((DciValue) e1).getDescription().compareToIgnoreCase(
						((DciValue) e2).getDescription());
			}
		});

		viewer.setInput(((DataCollectionTarget) getObject())
				.getOverviewDciData().toArray());
	...
	private void setupTable() {
		...
		viewer.getTable().setBackground(
				SharedColors.getColor(SharedColors.OBJECT_TAB_BACKGROUND,
						getDisplay()));
	...
	private void createPopupMenu() {
		...
		getObjectTab().getViewPart().getSite()
				.registerContextMenu(menuMgr, viewer);
	}
	...
	private void refresh() {
		...
		ConsoleJob job = new ConsoleJob(Messages.get().LastValues_JobName,
				getObjectTab().getViewPart(), Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				try {
					final DciValue[] list = session.getLastValues(nodeId,
							false, true, false);
					runInUIThread(new Runnable() {
						@Override
						public void run() {
							if (viewer.getControl().isDisposed()
									|| (getObject().getObjectId() != nodeId))
								return;
					...
					Activator.log(
							"Exception in last values overview element", e); //$NON-NLS-1$
				}
			}
