...
public class ObjectStatusMapView extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		setPartName(String.format(Messages.get().ObjectStatusMapView_PartName,
				(object != null) ? object.getObjectName()
						: ("[" + rootObjectId + "]"))); //$NON-NLS-1$ //$NON-NLS-2$		
	}
	...
	public void createPartControl(Composite parent) {
		final IDialogSettings settings = Activator.getDefault()
				.getDialogSettings();
		initialGroupFlag = (settings.get("GroupObjects") != null) ? settings
				.getBoolean("GroupObjects") : true;
		initialShowFilter = (settings.get("ShowFilter") != null) ? settings
				.getBoolean("ShowFilter") : true;
	...
	private void createActions() {
		...
		actionGroupNodes = new Action(
				Messages.get().ObjectStatusMapView_ActionGroupNodes,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowFilter = new Action(
				Messages.get().ObjectStatusMapView_ActionShowFilter,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
