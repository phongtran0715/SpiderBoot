...
public class ComponentsTab extends ObjectTab {
	...
	protected void createTabContent(Composite parent) {
		...
		WidgetHelper.restoreColumnSettings(viewer.getTree(), Activator
				.getDefault().getDialogSettings(), "ComponentTree"); //$NON-NLS-1$
		viewer.getTree().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveColumnSettings(viewer.getTree(), Activator
						.getDefault().getDialogSettings(), "ComponentTree"); //$NON-NLS-1$
			}
		});
		createActions();
	...
	private void createActions() {
		actionCopy = new Action(Messages.get().ComponentsTab_ActionCopy,
				SharedIcons.COPY) {
			@Override
			public void run() {
		...
		actionCopyModel = new Action(
				Messages.get().ComponentsTab_ActionCopyModel) {
			@Override
			public void run() {
		...
		actionCopySerial = new Action(
				Messages.get().ComponentsTab_ActionCopySerial) {
			@Override
			public void run() {
		...
		actionCollapseAll = new Action(
				Messages.get().ComponentsTab_ActionCollapseAll,
				SharedIcons.COLLAPSE_ALL) {
			@Override
			public void run() {
				viewer.collapseAll();
			}
		...
		actionExpandAll = new Action(
				Messages.get().ComponentsTab_ActionExpandAll,
				SharedIcons.EXPAND_ALL) {
			@Override
			public void run() {
				viewer.expandAll();
			}
	...
	public void objectChanged(final AbstractObject object) {
		...
		ConsoleJob job = new ConsoleJob(Messages.get().ComponentsTab_JobName,
				getViewPart(), Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				try {
					final PhysicalComponent root = session
							.getNodePhysicalComponents(object.getObjectId());
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().ComponentsTab_JobError,
