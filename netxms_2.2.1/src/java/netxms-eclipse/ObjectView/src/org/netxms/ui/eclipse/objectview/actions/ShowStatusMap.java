...
public class ShowStatusMap implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		if (object != null) {
			try {
				window.getActivePage().showView(ObjectStatusMapView.ID,
						Long.toString(object.getObjectId()),
						IWorkbenchPage.VIEW_ACTIVATE);
			} catch (PartInitException e) {
				MessageDialogHelper.openError(window.getShell(),
						Messages.get().ShowStatusMap_Error, String.format(
								Messages.get().ShowStatusMap_ErrorOpeningView,
								e.getMessage()));
			}
		}
	}
