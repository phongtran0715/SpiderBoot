...
public class InterfacesTabFilter extends ViewerFilter {
	...
	public boolean select(Viewer viewer, Object parentElement, Object element) {
		...
		return matchOId(iface) || matchName(iface) || matchAlias(iface)
				|| matchIfType(iface) || matchIfIndex(iface)
				|| matchIfTypeName(iface) || matchSlot(iface)
				|| matchPort(iface) || matchMtu(iface) || matchSpeed(iface)
				|| matchDescription(iface) || matchMac(iface) || matchIp(iface)
				|| matchPeerNode(iface) || matchPeerMac(iface)
				|| matchPeerIp(iface) || matchPeerDiscoveryProtocol(iface)
				|| matchAdminState(iface) || matchOperState(iface)
				|| matchExpState(iface) || matchStatus(iface)
				|| matchDot1xPaeState(iface) || matchDot1xBackendState(iface);
	}
	...
	private boolean matchIfType(Interface interf) {
		if (Integer.toString(interf.getIfType()).toLowerCase()
	...
	private boolean matchIfIndex(Interface interf) {
		if (Integer.toString(interf.getIfIndex()).toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchSlot(Interface interf) {
		if (Integer.toString(interf.getSlot()).toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchPort(Interface interf) {
		if (Integer.toString(interf.getPort()).toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchMtu(Interface interf) {
		if (Integer.toString(interf.getMtu()).toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchSpeed(Interface interf) {
		if (Long.toString(interf.getSpeed()).toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchMac(Interface interf) {
		if (interf.getMacAddress().toString().toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchIp(Interface interf) {
		if (interf.getIpAddressListAsString().toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchPeerNode(Interface interf) {
		if (Long.toString(interf.getPeerNodeId()).toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchPeerMac(Interface interf) {
		Interface peer = (Interface) session.findObjectById(
				interf.getPeerInterfaceId(), Interface.class);
		if (peer == null)
			return false;
		if (peer.getMacAddress().toString().toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchPeerIp(Interface interf) {
		...
		if (peer == null)
			return false;
		if (peer.getIpAddressListAsString().toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchPeerDiscoveryProtocol(Interface interf) {
		if (interf.getPeerDiscoveryProtocol().toString().toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchExpState(Interface interf) {
		if (Integer.toString(interf.getExpectedState()).toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchDot1xPaeState(Interface interf) {
		if (interf.getDot1xPaeStateAsText().toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	private boolean matchDot1xBackendState(Interface interf) {
		if (interf.getDot1xBackendStateAsText().toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
	...
	/**
	 * @param hideSubInterfaces
	 *            the hideSubInterfaces to set
	 */
