...
public class AvailabilityChart extends OverviewPageElement {
	...
	protected void onObjectChange() {
		...
		monthChart
				.updateParameter(1, 100.0 - service.getUptimeForMonth(), true);
	}
	...
	protected Control createClientArea(Composite parent) {
		...
		dayChart = createChart(clientArea,
				Messages.get().AvailabilityChart_Today);
		weekChart = createChart(clientArea,
				Messages.get().AvailabilityChart_ThisWeek);
		monthChart = createChart(clientArea,
				Messages.get().AvailabilityChart_ThisMonth);
	...
	private DataComparisonChart createChart(Composite parent, String title) {
		...
		chart.addParameter(new GraphItem(0, 0, 0, 0,
				Messages.get().AvailabilityChart_Up,
				Messages.get().AvailabilityChart_Up, "%s"), 100); //$NON-NLS-1$
		chart.addParameter(new GraphItem(0, 0, 0, 0,
				Messages.get().AvailabilityChart_Down,
	...
	private void paintLegend(GC gc) {
		final int th = gc.textExtent(Messages.get().AvailabilityChart_Uptime
				+ Messages.get().AvailabilityChart_Downtime).y;
		final Color fg = SharedColors.getColor(
				SharedColors.SERVICE_AVAILABILITY_LEGEND, getDisplay());
		...
		gc.drawText(Messages.get().AvailabilityChart_Downtime, 10 + th, 40,
				true);
	}
