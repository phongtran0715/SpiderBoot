...
public class CreateNodeLink implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateNodeLinkDialog dlg = new CreateNodeLinkDialog(
				window.getShell());
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateNodeLink_JobTitle, part,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_NODELINK, dlg.getName(), parentId);
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().CreateNodeLink_JobError,
						dlg.getName());
			}
		}.start();
	}
