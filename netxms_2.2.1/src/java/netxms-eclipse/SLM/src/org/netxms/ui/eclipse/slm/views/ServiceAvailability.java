...
public class ServiceAvailability extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		try {
			...
			object = (ServiceContainer) session.findObjectById(
					Long.parseLong(site.getSecondaryId()),
					ServiceContainer.class);
		} catch (Exception e) {
			throw new PartInitException(
					Messages.get().ServiceAvailability_InternalError, e);
		}
		if (object == null)
			throw new PartInitException(String.format(
					Messages.get().ServiceAvailability_InitError,
					site.getSecondaryId()));
		setPartName(String.format(Messages.get().ServiceAvailability_PartName,
	...
	public void createPartControl(Composite parent) {
		...
		dayChart = createChart(clientArea,
				Messages.get().ServiceAvailability_Today);
		weekChart = createChart(clientArea,
				Messages.get().ServiceAvailability_ThisWeek);
		monthChart = createChart(clientArea,
				Messages.get().ServiceAvailability_ThisMonth);
		...
		listener = new SessionListener() {
			...
			public void notificationHandler(final SessionNotification n) {
				if (!clientArea.isDisposed()
						&& (n.getCode() == SessionNotification.OBJECT_CHANGED)
	...
	private DataComparisonChart createChart(Composite parent, String title) {
		DataComparisonChart chart = ChartFactory.createPieChart(parent,
				SWT.NONE);
		...
		chart.addParameter(new GraphItem(0, 0, 0, 0,
				Messages.get().ServiceAvailability_Up,
				Messages.get().ServiceAvailability_Up, "%s"), 100); //$NON-NLS-1$
		chart.addParameter(new GraphItem(0, 0, 0, 0,
				Messages.get().ServiceAvailability_Down,
	...
	private void paintLegend(GC gc) {
		final int th = gc
				.textExtent(Messages.get().ServiceAvailability_UptimeDowntime).y;
		final Color fg = SharedColors.getColor(
				SharedColors.SERVICE_AVAILABILITY_LEGEND, Display.getCurrent());

		gc.setBackground(colors.create(127, 154, 72));
		gc.setForeground(ColorConverter.adjustColor(gc.getBackground(),
				fg.getRGB(), 0.2f, colors));
		...
		gc.setForeground(fg);
		gc.drawText(Messages.get().ServiceAvailability_Uptime, 10 + th, 10,
				true);
		gc.drawText(Messages.get().ServiceAvailability_Downtime, 10 + th, 40,
				true);
	}
