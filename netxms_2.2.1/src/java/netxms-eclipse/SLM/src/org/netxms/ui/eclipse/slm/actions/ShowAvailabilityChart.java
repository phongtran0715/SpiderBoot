...
public class ShowAvailabilityChart implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		if (service != null) {
			try {
				window.getActivePage().showView(ServiceAvailability.ID,
						Long.toString(service.getObjectId()),
						IWorkbenchPage.VIEW_ACTIVATE);
			} catch (PartInitException e) {
				MessageDialogHelper.openError(
						window.getShell(),
						Messages.get().ShowAvailabilityChart_Error,
						Messages.get().ShowAvailabilityChart_ErrorText
								+ e.getMessage());
			}
		}
	}
