...
public class CreateServiceCheckTemplate implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateServiceCheckDialog dlg = new CreateServiceCheckDialog(
				window.getShell());
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateServiceCheckTemplate_JobTitle,
				part, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_SLMCHECK, dlg.getName(), parentId);
				cd.setTemplate(true);
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().CreateServiceCheckTemplate_JobError,
						dlg.getName());
			}
		}.start();
	}
