...
public class CreateBusinessService implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateBusinessServiceDialog dlg = new CreateBusinessServiceDialog(
				window.getShell());
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateBusinessService_JobTitle, part,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				session.createObject(new NXCObjectCreationData(
						AbstractObject.OBJECT_BUSINESSSERVICE, dlg.getName(),
						parentId));
			}
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().CreateBusinessService_JobError,
						dlg.getName());
			}
		}.start();
	}
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if ((selection instanceof IStructuredSelection)
			...
			if ((object instanceof BusinessService)
					|| (object instanceof BusinessServiceRoot)) {
