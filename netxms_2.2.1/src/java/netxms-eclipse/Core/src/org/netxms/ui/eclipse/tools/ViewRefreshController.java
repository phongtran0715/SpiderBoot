...
public class ViewRefreshController implements IPartListener {
	 ...
	 * @param handler
	 */
	public ViewRefreshController(final IViewPart part, int interval,
			Runnable handler) {
	 ...
	 */
	public ViewRefreshController(final IViewPart part, int interval,
			Runnable handler, VisibilityValidator validator) {
		...
		timer = new Runnable() {
			...
			public void run() {
				if (page.isPartVisible(part)
						&& ((ViewRefreshController.this.validator == null) || ViewRefreshController.this.validator
								.isVisible())) {
					ViewRefreshController.this.handler.run();
				}
				display.timerExec(
						(ViewRefreshController.this.interval > 0) ? ViewRefreshController.this.interval * 1000
								: -1, this);
			}
		};
	...
	public void partActivated(IWorkbenchPart part) {
		if ((interval > 0)
				...
				&& compareStrings(
						((IViewSite) part.getSite()).getSecondaryId(),
						secondaryId)
				&& ((ViewRefreshController.this.validator == null) || ViewRefreshController.this.validator
						.isVisible())) {
			handler.run();
