...
public class BrandingManager {
	...
	private BrandingManager() {
		...
		IConfigurationElement[] elements = reg
				.getConfigurationElementsFor("org.netxms.ui.eclipse.branding"); //$NON-NLS-1$
		for (int i = 0; i < elements.length; i++) {
			try {
				final BrandingProvider p = (BrandingProvider) elements[i]
	 ...
	 * Get product name.
	 * 
	 * @return product name or default product name if no branding provider
	 *         defines one.
	 */
	 ...
	 * 
	 * @return default perspective ID or null if no branding provider defines
	 *         one.
	 */
	 ...
	 * 
	 * @return image descriptor for login dialog title image or null if no
	 *         branding provider defines one.
	 */
	 ...
	 * @return custom "About" dialog or null if no branding provider defines
	 *         one.
	 */
