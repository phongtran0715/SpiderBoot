...
public class AbstractSelector extends Composite {
	...
	public AbstractSelector(Composite parent, int style, int options) {
		...
		text = ((options & USE_TEXT) != 0) ? new Text(this, SWT.BORDER
				| SWT.READ_ONLY) : new CLabel(this, SWT.BORDER);
		GridData gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
	...
	private void createActions() {
		actionCopy = new Action() {
			...
			public void run() {
				...
				cb.setContents(
						new Object[] { AbstractSelector.this.getText() },
	...
	/**
	 * Fill context menu. Can be overridden by subclasses to add and/or override
	 * context menu items.
	 * 
	 * @param mgr
	 *            menu manager
	...
	/**
	 * Handler for selection button. This method intended to be overriden by
	 * subclasses. Default implementation does nothing.
	 */
	protected void selectionButtonHandler() {
	}

	/**
	 * Handler for clear button. This method intended to be overriden by
	 * subclasses. Default implementation does nothing.
	 */
	...
	/**
	 * Returns tooltip text for selection button. Can be overridden by
	 * subclasses.
	 * 
	...
	protected void setImage(final Image image) {
		...
		if (image != null) {
			...
			if ((size.width > 64) || (size.height > 64)) {
				scaledImage = new Image(getDisplay(), image.getImageData()
						.scaledTo(64, 64));
