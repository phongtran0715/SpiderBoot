...
public class ApplicationWorkbenchWindowAdvisor extends WorkbenchWindowAdvisor
	...
	public void postWindowCreate() {
		...
		configurer.setTitle(configurer.getTitle() + " - ["
				+ settings.get("Connect.Login") + "@"
				+ settings.get("Connect.Server") + "]");

		NXCSession session = ConsoleSharedData.getSession();
		Activator activator = Activator.getDefault();
		StatusLineContributionItem statusItemConnection = (StatusLineContributionItem) activator
				.getStatusLine().find("ConnectionStatus");
		statusItemConnection
				.setImage(Activator
						.getImageDescriptor(
		...
		ServerNameStatusLineItem statusItemServerName = (ServerNameStatusLineItem) activator
				.getStatusLine().find("ServerName");
		statusItemServerName.setServerInfo(session.getServerName(),
				session.getServerColor());
		...
		for (String s : Platform.getCommandLineArgs()) {
			if (s.equals("-fullscreen")) //$NON-NLS-1$
			{
				PlatformUI.getWorkbench().getActiveWorkbenchWindow().getShell()
						.setFullScreen(true);
				Action a = ((Action) ConsoleSharedData
						.getProperty("FullScreenAction")); //$NON-NLS-1$
				if (a != null) {
					a.setChecked(true);
				}
	...
	private void showDashboard(String dashboardId) {
		...
		try {
			...
			if ((object == null) || !(object instanceof Dashboard)) {
				MessageDialogHelper
						.openError(
								null,
								Messages.get().ApplicationWorkbenchWindowAdvisor_Error,
								String.format(
										Messages.get().ApplicationWorkbenchWindowAdvisor_CannotOpenDashboard,
										dashboardId));
				return;
			}
		...
		Dashboard dashboard = (Dashboard) session.findObjectById(objectId,
				Dashboard.class);
		if (dashboard == null) {
			MessageDialogHelper
					.openError(
		...
		try {
			IViewPart view = page
					.showView(
							"org.netxms.ui.eclipse.dashboard.views.DashboardView", Long.toString(objectId), IWorkbenchPage.VIEW_ACTIVATE); //$NON-NLS-1$
			page.setPartState(page.getReference(view),
			...
			MessageDialogHelper
					.openError(
							...
							Messages.get().ApplicationWorkbenchWindowAdvisor_Error,
							String.format(
									Messages.get().ApplicationWorkbenchWindowAdvisor_CannotOpenDashboardType2,
									dashboardId, e.getLocalizedMessage()));
		}
	}
	...
	private void doLogin(final Display display) {
		...
		CertificateManager certMgr = CertificateManagerProvider
				.provideCertificateManager();
		...
		for (String s : Platform.getCommandLineArgs()) {
			...
			} else if (s.startsWith("-password=")) //$NON-NLS-1$
			{
				...
				settings.put(
						"Connect.AuthMethod", AuthenticationType.PASSWORD.getValue()); //$NON-NLS-1$
		...
		while (!success) {
			...
			ConsoleSharedData.setProperty(
					"SlowLink", settings.getBoolean("Connect.SlowLink")); //$NON-NLS-1$ //$NON-NLS-2$
			...
			case CERTIFICATE:
				job.setCertificate(loginDialog.getCertificate(),
						getSignature(certMgr, loginDialog.getCertificate()));
				break;
			default:
				break;
			}
			...
			try {
				...
				if ((e.getCause() instanceof NXCException)
						&& ((((NXCException) e.getCause()).getErrorCode() == RCC.NO_ENCRYPTION_SUPPORT) || (((NXCException) e
								.getCause()).getErrorCode() == RCC.NO_CIPHERS))
						&& encrypt) {
					boolean alwaysAllow = settings
							.getBoolean("Connect.AllowUnencrypted." + settings.get("Connect.Server")); //$NON-NLS-1$ //$NON-NLS-2$
					...
					if (action != SecurityWarningDialog.NO) {
						...
						if (action == SecurityWarningDialog.ALWAYS) {
							settings.put(
									"Connect.AllowUnencrypted." + settings.get("Connect.Server"), true); //$NON-NLS-1$ //$NON-NLS-2$
						}
					}
				} else {
					...
					MessageDialog
							.openError(
									null,
									Messages.get().ApplicationWorkbenchWindowAdvisor_ConnectionError,
				...
				MessageDialog
						.openError(
								null,
								Messages.get().ApplicationWorkbenchWindowAdvisor_Exception,
		...
		if ((session.getAuthenticationMethod() == AuthenticationType.PASSWORD)
				&& session.isPasswordExpired()) {
	 ...
	 * @return
	 */
	private static Signature getSignature(CertificateManager certMgr,
			Certificate cert) {
		Signature sign;
	...
	private int getAction(IDialogSettings settings, boolean alwaysAllow) {
		...
		return SecurityWarningDialog
				.showSecurityWarning(
						null,
						String.format(
								Messages.get().ApplicationWorkbenchWindowAdvisor_NoEncryptionSupport,
								settings.get("Connect.Server")), //$NON-NLS-1$
	 ...
	 * @param session
	 */
	private void requestPasswordChange(final String currentPassword,
			final NXCSession session) {
		final PasswordExpiredDialog dlg = new PasswordExpiredDialog(null,
				session.getGraceLogins());
		...
		while (true) {
			...
			IRunnableWithProgress job = new IRunnableWithProgress() {
				...
				public void run(IProgressMonitor monitor)
					...
					try {
						...
						session.setUserPassword(session.getUserId(),
			...
			try {
				...
				MessageDialog
						.openInformation(
								null,
								Messages.get().ApplicationWorkbenchWindowAdvisor_Information,
				...
				MessageDialog
						.openError(
								null,
								Messages.get().ApplicationWorkbenchWindowAdvisor_Error,
								Messages.get().ApplicationWorkbenchWindowAdvisor_CannotChangePswd
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.netxms.certificate.request.KeyStoreLocationRequestListener#
	 * keyStoreLocationRequested()
	 */
	@Override
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.netxms.certificate.request.KeyStorePasswordRequestListener#
	 * keyStorePasswordRequested()
	 */
	@Override
	public String keyStorePasswordRequested() {
		return showPasswordRequestDialog(
				Messages.get().ApplicationWorkbenchWindowAdvisor_CertStorePassword,
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.netxms.certificate.request.KeyStoreEntryPasswordRequestListener#
	 * keyStoreEntryPasswordRequested()
	 */
	@Override
	public String keyStoreEntryPasswordRequested() {
		return showPasswordRequestDialog(
				Messages.get().ApplicationWorkbenchWindowAdvisor_CertPassword,
	...
	private void showMessageOfTheDay() {
		...
		if (!message.isEmpty()) {
			MessageDialog.openInformation(
					Display.getCurrent().getActiveShell(), "Announcement",
					message);
		}
	}
}
