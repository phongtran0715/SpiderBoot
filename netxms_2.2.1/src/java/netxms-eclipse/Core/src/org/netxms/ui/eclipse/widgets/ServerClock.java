...
public class ServerClock extends Composite {
	...
	private void createPopupMenu() {
		...
		final Action actionShowText = new Action(
				Messages.get().ServerClock_OptionShowText, Action.AS_CHECK_BOX) {
			@Override
			public void run() {
				showText = isChecked();
		...
		final Action actionShowTimeZone = new Action(
				Messages.get().ServerClock_OptionShowTimeZone,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
	...
	private void updateTimeDisplay() {
		...
		df.setTimeZone(TimeZone.getTimeZone(tz.replaceAll(
				"[A-Za-z]+([\\+\\-][0-9]+).*", "GMT$1"))); //$NON-NLS-1$ //$NON-NLS-2$
