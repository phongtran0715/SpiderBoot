...
public abstract class AbstractTraceView extends ViewPart {
	...
	protected void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.library.context.AbstractTraceView"); //$NON-NLS-1$
		}
	}
	...
	protected void createActions() {
		...
		actionClear = new Action(Messages.get().AbstractTraceView_Clear,
		...
		actionShowFilter
				.setId("org.netxms.ui.eclipse.library.actions.showSnmpFilter");
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.library.commands.show_filter");
	...
	private void createPopupMenu() {
		...
		// Create menu
		Menu menu = menuMgr.createContextMenu(traceWidget.getViewer()
				.getControl());
	...
	protected void subscribe(final String channel) {
		new ConsoleJob(String.format(
				Messages.get().AbstractTraceView_Subscribing, channel), this,
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().AbstractTraceView_CannotSubscribe,
						channel);
			}
		}.start();
	}
	...
	protected void unsubscribe(final String channel) {
		ConsoleJob job = new ConsoleJob(String.format(
				Messages.get().AbstractTraceView_Unsubscribing, channel), null,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().AbstractTraceView_CannotUnsubscribe,
						channel);
			}
		};
		job.setUser(false);
