...
public class LoginJob implements IRunnableWithProgress {
	 ...
	 */
	public LoginJob(Display display, String server, String loginName,
			boolean encryptSession, boolean ignoreProtocolVersion) {
	...
	public void run(IProgressMonitor monitor) throws InvocationTargetException,
		...
		try {
			...
			session.login(authMethod, loginName, password, certificate,
					signature);
			monitor.worked(1);
			...
			display.syncExec(new Runnable() {
				@Override
				public void run() {
					SourceProvider.getInstance().updateAccessRights(
							session.getUserSystemRights());
				}
			});
	...
	private NXCSession createSession(String hostName, int port) {
		// Read all registered extensions and create provider with minimal
		// priority
		...
		IConfigurationElement[] elements = reg
				.getConfigurationElementsFor("org.netxms.ui.eclipse.sessionproviders"); //$NON-NLS-1$
		...
		if (currentElement != null) {
			try {
				SessionProvider p = (SessionProvider) currentElement
	...
	private void callLoginListeners(NXCSession session) {
		...
		final IExtensionRegistry reg = Platform.getExtensionRegistry();
		IConfigurationElement[] elements = reg
				.getConfigurationElementsFor("org.netxms.ui.eclipse.loginlisteners"); //$NON-NLS-1$
		for (int i = 0; i < elements.length; i++) {
			try {
				final ConsoleLoginListener listener = (ConsoleLoginListener) elements[i]
						.createExecutableExtension("class"); //$NON-NLS-1$
