...
public class WidgetHelper {
	 ...
	 *            Parent composite
	 * @param flags
	 *            Flags for Text creation
	 * @param widthHint
	 *            Width hint for text control
	 * @param labelText
	 *            Label's text
	 * @param initialText
	 *            Initial text for input field (may be null)
	 * @param layoutData
	 *            Layout data for label/input pair. If null, default GridData
	 *            will be assigned.
	 * @return Created Text object
	 */
	public static Text createLabeledText(final Composite parent, int flags,
			int widthHint, final String labelText, final String initialText,
			Object layoutData) {
	 ...
	 * @return Created Text object
	 */
	public static StyledText createLabeledStyledText(final Composite parent,
			int flags, final String labelText, final String initialText,
			Object layoutData) {
		Composite group = new Composite(parent, SWT.NONE);
		GridLayout layout = new GridLayout();
		layout.verticalSpacing = INNER_SPACING;
	 ...
	 */
	public static Combo createLabeledCombo(final Composite parent, int flags,
			final String labelText, Object layoutData) {
	 ...
	 *            will be assigned.
	 * @param toolkit
	 *            form toolkit to be used for control creation. May be null.
	 * @return Created Combo object
	 */
	public static Combo createLabeledCombo(final Composite parent, int flags,
			final String labelText, Object layoutData, FormToolkit toolkit) {
		Composite group = (toolkit != null) ? toolkit.createComposite(parent)
	 ...
	 */
	public static Spinner createLabeledSpinner(final Composite parent,
			int flags, final String labelText, int minVal, int maxVal,
			Object layoutData) {
		Composite group = new Composite(parent, SWT.NONE);
		GridLayout layout = new GridLayout();
		layout.verticalSpacing = INNER_SPACING;
	 ...
	 * @return Created Text object
	 */
	public static ColorSelector createLabeledColorSelector(
			final Composite parent, final String labelText, Object layoutData) {
		Composite group = new Composite(parent, SWT.NONE);
		GridLayout layout = new GridLayout();
		layout.verticalSpacing = INNER_SPACING;
		layout.horizontalSpacing = 0;
	 ...
	 *            parent composite
	 * @param flags
	 *            flags for control being created
	 * @param factory
	 *            control factory
	 * @param labelText
	 *            Label's text
	 ...
	 */
	public static Control createLabeledControl(Composite parent, int flags,
			WidgetFactory factory, String labelText, Object layoutData) {
		Composite group = new Composite(parent, SWT.NONE);
		GridLayout layout = new GridLayout();
		layout.verticalSpacing = INNER_SPACING;
		layout.horizontalSpacing = 0;
	 ...
	 */
	public static void saveColumnSettings(Table table,
			IDialogSettings settings, String prefix) {
	...
	/**
	 * Restore settings of table viewer columns previously saved by call to
	 * WidgetHelper.saveColumnSettings
	 * 
	 * @param table
	 *            Table control
	 * @param settings
	 ...
	 *            Prefix for properties
	 */
	public static void restoreColumnSettings(Table table,
			IDialogSettings settings, String prefix) {
		TableColumn[] columns = table.getColumns();
		for (int i = 0; i < columns.length; i++) {
			try {
	 ...
	 *            Prefix for properties
	 */
	public static void saveColumnSettings(Tree tree, IDialogSettings settings,
			String prefix) {
	...
	/**
	 * Restore settings of tree viewer columns previously saved by call to
	 * WidgetHelper.saveColumnSettings
	 * 
	 * @param table
	 *            Table control
	 ...
	 *            Prefix for properties
	 */
	public static void restoreColumnSettings(Tree tree,
			IDialogSettings settings, String prefix) {
		TreeColumn[] columns = tree.getColumns();
		for (int i = 0; i < columns.length; i++) {
			try {
	 ...
	 *            Prefix for properties
	 */
	public static void saveTableViewerSettings(SortableTableViewer viewer,
			IDialogSettings settings, String prefix) {
		final Table table = viewer.getTable();
		saveColumnSettings(table, settings, prefix);
		TableColumn column = table.getSortColumn();
		settings.put(
				prefix + ".sortColumn", (column != null) ? (Integer) column.getData("ID") : -1); //$NON-NLS-1$ //$NON-NLS-2$
	 ...
	 *            Prefix for properties
	 */
	public static void restoreTableViewerSettings(SortableTableViewer viewer,
			IDialogSettings settings, String prefix) {
		final Table table = viewer.getTable();
	 ...
	 *            Prefix for properties
	 */
	public static void saveTreeViewerSettings(SortableTreeViewer viewer,
			IDialogSettings settings, String prefix) {
	 ...
	 *            Prefix for properties
	 */
	public static void restoreTreeViewerSettings(SortableTreeViewer viewer,
			IDialogSettings settings, String prefix) {
		final Tree tree = viewer.getTree();
	 ...
	 * @param prefix
	 */
	public static void saveColumnViewerSettings(ColumnViewer viewer,
			IDialogSettings settings, String prefix) {
		if (viewer instanceof SortableTableViewer) {
			saveTableViewerSettings((SortableTableViewer) viewer, settings,
					prefix);
		} else if (viewer instanceof SortableTreeViewer) {
			saveTreeViewerSettings((SortableTreeViewer) viewer, settings,
					prefix);
		}
	}
	 ...
	 * @param prefix
	 */
	public static void restoreColumnViewerSettings(ColumnViewer viewer,
			IDialogSettings settings, String prefix) {
		if (viewer instanceof SortableTableViewer) {
			restoreTableViewerSettings((SortableTableViewer) viewer, settings,
					prefix);
		} else if (viewer instanceof SortableTreeViewer) {
			restoreTreeViewerSettings((SortableTreeViewer) viewer, settings,
					prefix);
		}
	}
	...
	public static void copyToClipboard(final String text) {
		...
		cb.setContents(
				new Object[] { (text != null) ? text : "" }, new Transfer[] { transfer }); //$NON-NLS-1$
		cb.dispose();
	}
	 ...
	 * @param readOnly
	 */
	public static void addStyledTextEditorActions(final IMenuManager manager,
			final StyledText control, boolean readOnly) {
		if (!readOnly) {
			final Action cut = new Action(
					Messages.get().WidgetHelper_Action_Cut) {
				@Override
				public void run() {
					control.cut();
				}
		...
		if (!readOnly) {
			final Action paste = new Action(
					Messages.get().WidgetHelper_Action_Paste) {
				@Override
				public void run() {
					control.paste();
				}
			...
			final Action delete = new Action(
					Messages.get().WidgetHelper_Action_Delete) {
				@Override
				public void run() {
		...
		final Action selectAll = new Action(
				Messages.get().WidgetHelper_Action_SelectAll) {
			@Override
			public void run() {
				control.selectAll();
			}
	...
	/**
	 * Get best fitting font from given font list for given string and bounding
	 * rectangle. Fonts in the list must be ordered from smaller to larger.
	 * 
	 * @param gc
	 *            GC
	 * @param fonts
	 ...
	 * @return best font
	 */
	public static Font getBestFittingFont(GC gc, Font[] fonts, String text,
	 ...
	 */
	private static boolean validateTextInputInternal(Control control,
			String text, String label, TextFieldValidator validator,
			PropertyPage page) {
		if (!control.isEnabled())
			return true; // Ignore validation for disabled controls

		boolean ok = validator.validate(text);
		control.setBackground(ok ? null : SharedColors.getColor(
				SharedColors.ERROR_BACKGROUND, control.getDisplay()));
		if (ok) {
			if (page != null)
			...
			if (page != null)
				...
				MessageDialogHelper.openError(control.getShell(),
						Messages.get().WidgetHelper_InputValidationError,
	 ...
	 * @return true if text is valid
	 */
	public static boolean validateTextInput(Text text, String label,
			TextFieldValidator validator, PropertyPage page) {
		return validateTextInputInternal(text, text.getText(), label,
				validator, page);
	}
	 ...
	 * @return true if text is valid
	 */
	public static boolean validateTextInput(LabeledText text,
			TextFieldValidator validator, PropertyPage page) {
		return validateTextInputInternal(text.getTextControl(), text.getText(),
				text.getLabel(), validator, page);
	}

	/**
	 * Convert font size in pixels to platform-dependent (DPI dependent
	 * actually) points
	 * 
	 * @param device
	 * @param px
	...
	/**
	 * Get width and height of given text in pixels using settings from given
	 * control
	 * 
	 * @param control
	 * @param text
	 ...
	 * 
	 * @param table
	 *            table index to be found
	 * @param id
	 *            the id index to be found by
	...
	public static int getColumnIndexById(Table table, int id) {
		...
		for (int i = 0; i < columns.length; i++) {
			if (!columns[i].isDisposed()
					&& ((Integer) columns[i].getData("ID") == id)) //$NON-NLS-1$
			{
				index = i;
				break;
			}
