...
public class ColorConverter {
	 ...
	 * @param cache
	 *            color cache where new color should be placed
	 * @return color object
	 */
	public static Color colorFromInt(int rgb, ColorCache cache) {
		// All colors on server stored as BGR: red in less significant byte and
		// blue in most significant byte
	 ...
	 * @return Color object
	 */
	public static Color getColorFromPreferences(IPreferenceStore store,
			final String name, ColorCache cache) {
	 ...
	 */
	public static int getColorFromPreferencesAsInt(IPreferenceStore store,
			final String name) {
	...
	/**
	 * Adjust given color in the direction of another color by given amount. For
	 * example, to make color 50% lighter: adjustColor(color, new
	 * Color(color.getDevice(), 255, 255, 255), 0.5f);
	 * 
	 * @param color
	 * @param direction
	 * @param amount
	 ...
	 * @return
	 */
	public static Color adjustColor(Color color, RGB direction, float amount,
			ColorCache cache) {
		float sr = color.getRed(), sg = color.getGreen(), sb = color.getBlue();
		float dr = direction.red, dg = direction.green, db = direction.blue;
		return cache.create((int) lerp(sr, dr, amount),
				(int) lerp(sg, dg, amount), (int) lerp(sb, db, amount));
	}
	...
	public static String rgbToCss(RGB rgb) {
		...
		return String.format("#%02x%02x%02x", v & 0xFF, (v >> 8) & 0xFF,
				v >> 16);
	}
