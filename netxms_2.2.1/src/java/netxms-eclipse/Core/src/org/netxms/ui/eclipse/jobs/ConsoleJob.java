...
/**
 * Tailored Job class for NetXMS console. Callers must call start() instead of
 * schedule() for correct execution.
 */
public abstract class ConsoleJob extends Job {
	 ...
	 *            Job's name
	 * @param viewPart
	 *            Related view part or null
	 * @param pluginId
	 *            Related plugin ID
	 * @param jobFamily
	 *            Job's family or null
	 */
	public ConsoleJob(String name, IWorkbenchPart wbPart, String pluginId,
		...
		try {
			IWorkbenchPart part = (wbPart != null) ? wbPart : PlatformUI
					.getWorkbench().getActiveWorkbenchWindow().getActivePage()
					.getActivePart();
			siteService = (part != null) ? (IWorkbenchSiteProgressService) wbPart
	...
	/**
	 * Executes job. Called from within Job.run(). If job fails, this method
	 ...
	 */
	protected abstract void runInternal(IProgressMonitor monitor)
			throws Exception;
	...
	/**
	 * Called from within Job.run() if job has failed to create failure status.
	 * Subclasses may override and return IStatus.OK_STATUS to avoid standard
	 * job failure message to pop up.
	 */
	protected IStatus createFailureStatus(Exception e) {
		return new Status(
				Status.ERROR,
				pluginId,
				(e instanceof NXCException) ? ((NXCException) e).getErrorCode()
						: 0,
				getErrorMessage() + ": " + e.getMessage(), passException ? e : null); //$NON-NLS-1$
	}

	/**
	 * Called from within Job.run() if job has failed. Default implementation
	 * does nothing.
	 */
	...
	/**
	 * Called from within Job.run() if job completes, either successfully or
	 * not. Default implementation does nothing.
	 */
	...
	public boolean runInForeground() {
		...
		try {
			...
			if (cause == null)
				cause = e;
			MessageDialogHelper.openError(null,
					Messages.get().ConsoleJob_ErrorDialogTitle,
