...
public class TimePeriodSelector extends Composite {
	...
	public TimePeriodSelector(Composite parent, int style, TimePeriod period) {
		...
		radioBackFromNow
				.setSelection(period.getTimeFrameType() == GraphSettings.TIME_FRAME_BACK_FROM_NOW);
		radioBackFromNow.addSelectionListener(listener);

		radioFixedInterval = new Button(timeGroup, SWT.RADIO);
		radioFixedInterval
				.setText(Messages.get().TimePeriodSelector_FixedTimeFrame);
		radioFixedInterval
				.setSelection(period.getTimeFrameType() == GraphSettings.TIME_FRAME_FIXED);
		...
		timeRange = WidgetHelper.createLabeledSpinner(timeBackGroup,
				SWT.BORDER, Messages.get().TimePeriodSelector_TimeInterval, 1,
		...
		timeUnits = WidgetHelper.createLabeledCombo(timeBackGroup,
				SWT.READ_ONLY, Messages.get().TimePeriodSelector_TimeUnits,
		...
		timeFrom = (DateTimeSelector) WidgetHelper.createLabeledControl(
				timeFixedGroup, SWT.NONE, factory,
				Messages.get().TimePeriodSelector_8,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		timeFrom.setValue(period.getTimeFromValue());
		timeFrom.setEnabled(radioFixedInterval.getSelection());

		timeTo = (DateTimeSelector) WidgetHelper.createLabeledControl(
				timeFixedGroup, SWT.NONE, factory,
				Messages.get().TimePeriodSelector_9,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
	...
	public TimePeriod getTimePeriod() {
		...
		tp.setTimeFrameType(radioBackFromNow.getSelection() ? GraphSettings.TIME_FRAME_BACK_FROM_NOW
				: GraphSettings.TIME_FRAME_FIXED);
	...
	public int getTimeFrameType() {
		return radioBackFromNow.getSelection() ? GraphSettings.TIME_FRAME_BACK_FROM_NOW
				: GraphSettings.TIME_FRAME_FIXED;
	}
