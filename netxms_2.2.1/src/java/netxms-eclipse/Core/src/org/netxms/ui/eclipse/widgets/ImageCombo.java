...
public class ImageCombo extends Composite {
	...
	public ImageCombo(Composite parent, int style) {
		...
		this.comboComposite.setCursor(getDisplay().getSystemCursor(
				SWT.CURSOR_ARROW));
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		...
		gc.dispose();
		Point textSize = this.comboComposite.computeSize(SWT.DEFAULT,
				SWT.DEFAULT, changed);
		Point arrowSize = this.arrow.computeSize(SWT.DEFAULT, SWT.DEFAULT,
				changed);
	...
	void dropDown(boolean drop) {
		...
		itemCount = (itemCount == 0) ? this.visibleItemCount : Math.min(
		...
		this.table
				.setBounds(1, 1, Math.max(size.x - 2, listSize.x), listSize.y);
	...
	void initAccessible() {
		...
		this.comboComposite.getAccessible().addAccessibleListener(
				accessibleAdapter);
		this.table.getAccessible().addAccessibleListener(accessibleAdapter);

		this.arrow.getAccessible().addAccessibleListener(
		...
		this.text.getAccessible().addAccessibleControlListener(
				new AccessibleControlAdapter() {
					public void getRole(AccessibleControlEvent e) {
		...
		this.arrow.getAccessible().addAccessibleControlListener(
				new AccessibleControlAdapter() {
