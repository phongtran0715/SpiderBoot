 ...
 */
public class HttpProxyPrefs extends PreferencePage implements
	...
	protected Control createContents(Composite parent) {
		...
		checkUseProxy.setSelection(getPreferenceStore().getBoolean(
		...
		editProxyServer.setText(getPreferenceStore().getString(
				"HTTP_PROXY_SERVER")); //$NON-NLS-1$
		gd = new GridData();
		...
		editProxyPort
				.setText(getPreferenceStore().getString("HTTP_PROXY_PORT")); //$NON-NLS-1$
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.widthHint = 100;
		...
		editExclusions
				.setLabel(Messages.get().HttpProxyPrefs_ExcludedAddresses); //$NON-NLS-1$
		editExclusions.setText(getPreferenceStore().getString(
				"HTTP_PROXY_EXCLUSIONS").replaceAll("\\|", ",")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		gd = new GridData();
		gd.grabExcessHorizontalSpace = true;
		gd.horizontalAlignment = SWT.FILL;
		gd.horizontalSpan = 2;
		...
		checkRequireAuth
				.setText(Messages.get().HttpProxyPrefs_ProxyRequireAuth); //$NON-NLS-1$
		checkRequireAuth.setSelection(getPreferenceStore().getBoolean(
				"HTTP_PROXY_AUTH")); //$NON-NLS-1$
		gd = new GridData();
		gd.horizontalSpan = 2;
		gd.verticalIndent = WidgetHelper.DIALOG_SPACING; // additional spacing
															// before auth block
		...
		editPassword = new LabeledText(dialogArea, SWT.NONE, SWT.BORDER
				| SWT.PASSWORD);
		editPassword.setLabel(Messages.get().HttpProxyPrefs_Password); //$NON-NLS-1$
		editPassword.setText(getPreferenceStore().getString(
				"HTTP_PROXY_PASSWORD")); //$NON-NLS-1$
		gd = new GridData();
		gd.grabExcessHorizontalSpace = true;
		gd.horizontalAlignment = SWT.FILL;
		gd.horizontalSpan = 2;
	...
	protected void performDefaults() {
		...
		checkUseProxy.setSelection(getPreferenceStore().getDefaultBoolean(
				"HTTP_PROXY_ENABLED")); //$NON-NLS-1$
		editProxyServer.setText(getPreferenceStore().getDefaultString(
				"HTTP_PROXY_SERVER")); //$NON-NLS-1$
		editProxyPort.setText(getPreferenceStore().getDefaultString(
				"HTTP_PROXY_PORT")); //$NON-NLS-1$
		editExclusions.setText(getPreferenceStore().getDefaultString(
				"HTTP_PROXY_EXCLUSIONS").replaceAll("\\|", ",")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		checkRequireAuth.setSelection(getPreferenceStore().getDefaultBoolean(
				"HTTP_PROXY_AUTH")); //$NON-NLS-1$
		editLogin.setText(getPreferenceStore().getDefaultString(
				"HTTP_PROXY_LOGIN")); //$NON-NLS-1$
		editPassword.setText(getPreferenceStore().getDefaultString(
				"HTTP_PROXY_PASSWORD")); //$NON-NLS-1$
		...
		editLogin.setEnabled(checkUseProxy.getSelection()
				&& checkRequireAuth.getSelection());
		editPassword.setEnabled(checkUseProxy.getSelection()
				&& checkRequireAuth.getSelection());
	}
	...
	public boolean performOk() {
		...
		ps.setValue(
				"HTTP_PROXY_EXCLUSIONS", editExclusions.getText().replaceAll("\\,", "|")); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
		...
		if (checkUseProxy.getSelection()) {
			System.setProperty(
					"http.proxyHost", ps.getString("HTTP_PROXY_SERVER")); //$NON-NLS-1$ //$NON-NLS-2$
			System.setProperty(
					"http.proxyPort", ps.getString("HTTP_PROXY_PORT")); //$NON-NLS-1$ //$NON-NLS-2$
			System.setProperty(
					"http.noProxyHosts", ps.getString("HTTP_PROXY_EXCLUSIONS")); //$NON-NLS-1$ //$NON-NLS-2$
				...
				Authenticator.setDefault(new Authenticator() {
					...
					protected PasswordAuthentication getPasswordAuthentication() {
						return new PasswordAuthentication(
								ps.getString("HTTP_PROXY_LOGIN"), ps.getString("HTTP_PROXY_PASSWORD").toCharArray()); //$NON-NLS-1$ //$NON-NLS-2$
					}
				});
			} else {
