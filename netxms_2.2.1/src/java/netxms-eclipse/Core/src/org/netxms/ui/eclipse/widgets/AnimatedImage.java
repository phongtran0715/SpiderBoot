 ...
 */
public class AnimatedImage extends Canvas implements PaintListener,
		DisposeListener {
	...
	public AnimatedImage(Composite parent, int style) {
		...
		timer = new Runnable() {
			...
			public void run() {
				...
				final Image frameImage = new Image(getDisplay(),
						imageData[imageNumber]);
				// memGC.fillRectangle(imageData[imageNumber].x,
				// imageData[imageNumber].y, imageData[imageNumber].width,
				// imageData[imageNumber].height);
				memGC.drawImage(frameImage, imageData[imageNumber].x,
						imageData[imageNumber].y);
				frameImage.dispose();
				redraw();
	...
	/**
	 * Load animated image from URL. For loading image from plugin, use URL like
	 * platform:/plugin/<plugin_id>/<path>
	 * 
	 * @param url
	 *            image file's URL
	 */
	...
	private void scheduleAnimation() {
		...
		getDisplay()
				.timerExec(
						(imageData[imageNumber].delayTime > 0) ? imageData[imageNumber].delayTime * 10
								: 200, timer);
	}
