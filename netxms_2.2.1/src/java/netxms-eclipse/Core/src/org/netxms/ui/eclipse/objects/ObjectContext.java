	...
	public String substituteMacros(String s, Map<String, String> inputValues) {
		...
		for (int i = 0; i < s.length(); i++) {
			if (src[i] == '%') {
				...
				switch (src[i]) {
				case 'a':
					sb.append((object != null) ? object.getPrimaryIP()
							.getHostAddress()
							: Messages.get().ObjectContext_MultipleNodes);
					break;
				...
				case 'c':
					...
					break;
				case 'g':
					sb.append((object != null) ? object.getGuid().toString()
							: Messages.get().ObjectContext_MultipleNodes);
					break;
				case 'i':
					sb.append((object != null) ? String.format(
							"0x%08X", object.getObjectId()) : Messages.get().ObjectContext_MultipleNodes); //$NON-NLS-1$
					break;
				case 'I':
					sb.append((object != null) ? Long.toString(object
							.getObjectId())
							: Messages.get().ObjectContext_MultipleNodes);
					break;
				case 'm': // alarm message
					...
					break;
				case 'n':
					sb.append((object != null) ? object.getObjectName()
							: Messages.get().ObjectContext_MultipleNodes);
					break;
				case 'N':
					if (alarm != null)
						sb.append(ConsoleSharedData.getSession().getEventName(
				...
				case 'S':
					if (alarm != null)
						sb.append(StatusDisplayInfo.getStatusText(alarm
								.getCurrentSeverity()));
					break;
				case 'U':
				...
				case '{': // object's custom attribute
					...
					if ((object != null) && (attr.length() > 0)) {
						String value = object.getCustomAttributes().get(
								attr.toString());
						if (value != null)
							sb.append(value);
					}
