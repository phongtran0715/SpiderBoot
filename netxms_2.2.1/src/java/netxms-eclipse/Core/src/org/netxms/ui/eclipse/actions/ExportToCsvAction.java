...
public class ExportToCsvAction extends Action {
	 ...
	 * @param selectionOnly
	 */
	private ExportToCsvAction(IViewPart viewPart, ColumnViewer viewer,
			ViewerProvider viewerProvider, boolean selectionOnly) {
		super(selectionOnly ? Messages.get().ExportToCsvAction_ExportToCsv
				: Messages.get().ExportToCsvAction_ExportAllToCsv,
				SharedIcons.CSV);
		...
		if ((viewPart == null)
				|| viewPart
						.getViewSite()
						.getId()
						.equals("org.netxms.ui.eclipse.objectview.view.tabbed_object_view")) //$NON-NLS-1$
			return;
	 ...
	 * @param selectionOnly
	 */
	public ExportToCsvAction(IViewPart viewPart, ColumnViewer viewer,
	 ...
	 * @param selectionOnly
	 */
	public ExportToCsvAction(IViewPart viewPart, ViewerProvider viewerProvider,
			boolean selectionOnly) {
	...
	public void run() {
		...
		if (viewer instanceof TableViewer) {
			...
			TableItem[] selection = selectionOnly ? ((TableViewer) viewer)
					.getTable().getSelection() : ((TableViewer) viewer)
					.getTable().getItems();
			...
			TreeItem[] selection = selectionOnly ? ((TreeViewer) viewer)
					.getTree().getSelection() : ((TreeViewer) viewer).getTree()
					.getItems();
		...
		new ConsoleJob(String.format(Messages.get().ExportToCsvAction_SaveTo,
				fileName), viewPart, Activator.PLUGIN_ID, null) { //$NON-NLS-1$
			@Override
