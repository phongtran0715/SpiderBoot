...
/**
 * Instances of this class provide a custom-drawn border with an optional title
 * bar and buttons.
 */
public abstract class CGroup extends Canvas {
	...
	private List<DashboardElementButton> buttons = new ArrayList<DashboardElementButton>(
			0);
	...
	public CGroup(Composite parent, String text) {
		...
		borderColor = SharedColors.getColor(SharedColors.CGROUP_BORDER,
				getDisplay());
		titleColor = SharedColors.getColor(SharedColors.CGROUP_TITLE,
				getDisplay());
	...
	/**
	 * Dispose current client area and create new one by calling
	 * createClientArea
	 */
	...
	private void layoutButtons() {
		...
		for (int i = buttons.size() - 1; i >= 0; i--) {
			...
			Point cs = c.getSize();
			c.setLocation(pos - cs.x, headerSize.y / 2 + BORDER_WIDTH - cs.y
					/ 2 - 1);
			pos -= cs.x + 4;
		}
	}
