...
public class SendSMS implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		...
		new ConsoleJob(Messages.get().SendSMS_JobTitle + dlg.getPhoneNumber(),
				window.getActivePage().getActivePart(), Activator.PLUGIN_ID,
				null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
				...
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						final String message = Messages.get().SendSMS_DialogTextPrefix
								+ dlg.getPhoneNumber()
								+ Messages.get().SendSMS_DialogTextSuffix;
						MessageDialogHelper.openInformation(window.getShell(),
								Messages.get().SendSMS_DialogTitle, message);
					}
				});
			}
