...
public class AnsiConsoleAttributes implements Cloneable {
	public final static int UNDERLINE_NONE = -1; // nothing in SWT, a bit of an
													// abuse
    ...
    */
	 ...
	 * sequences to real, Eclipse console specific attributes (resolving color
	 * palette, default colors, etc.)
	 * 
	 * @param range
	 * @param attribute
	 */
	public static void updateRangeStyle(StyleRange range,
			AnsiConsoleAttributes attribute) {
		...
		if (hilite) {
			if (tempAttrib.currentFgColor == null) {
				...
				if (tempAttrib.currentFgColor < AnsiCommands.COMMAND_COLOR_INTENSITY_DELTA)
					range.foreground = new Color(
							null,
							AnsiConsoleColorPalette
									.getColor(tempAttrib.currentFgColor
											+ AnsiCommands.COMMAND_COLOR_INTENSITY_DELTA));
				else
					range.foreground = new Color(null,
							AnsiConsoleColorPalette
									.getColor(tempAttrib.currentFgColor));
			}
		} else {
		...
		if (tempAttrib.currentBgColor != null)
			range.background = new Color(null,
					AnsiConsoleColorPalette.getColor(tempAttrib.currentBgColor));

		// These two still mess with the foreground/background colors
		// We need to solve them before we use them for strike/underline/frame
		// colors
