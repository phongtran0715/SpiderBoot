 ...
 */
public class RegionalSettingsPrefPage extends PreferencePage implements
	...
	protected Control createContents(Composite parent) {
		...
		gd.horizontalSpan = 3;
		dateTimeFormat = WidgetHelper.createLabeledCombo(dialogArea, SWT.BORDER
				| SWT.READ_ONLY,
				Messages.get().RegionalSettingsPrefPage_DateTimeFormat, gd); //$NON-NLS-1$
		...
		dateTimeFormat.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				dateFormatString
						.setEnabled(format == RegionalSettings.DATETIME_FORMAT_CUSTOM);
				timeFormatString
						.setEnabled(format == RegionalSettings.DATETIME_FORMAT_CUSTOM);
		...
		dateFormatString
				.setLabel(Messages.get().RegionalSettingsPrefPage_DateFormatString); //$NON-NLS-1$
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		dateFormatString.setLayoutData(gd);
		dateFormatString.setText(getPreferenceStore().getString(
				"DATE_FORMAT_STRING")); //$NON-NLS-1$
		dateFormatString
				.setEnabled(format == RegionalSettings.DATETIME_FORMAT_CUSTOM);
		...
		timeFormatString
				.setLabel(Messages.get().RegionalSettingsPrefPage_TimeFormatString); //$NON-NLS-1$
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		timeFormatString.setLayoutData(gd);
		timeFormatString.setText(getPreferenceStore().getString(
				"TIME_FORMAT_STRING")); //$NON-NLS-1$
		timeFormatString
				.setEnabled(format == RegionalSettings.DATETIME_FORMAT_CUSTOM);
		...
		shortTimeFormatString
				.setLabel(Messages.get().RegionalSettingsPrefPage_ShortTimeFormatString);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		shortTimeFormatString.setLayoutData(gd);
		shortTimeFormatString.setText(getPreferenceStore().getString(
				"SHORT_TIME_FORMAT_STRING")); //$NON-NLS-1$
		shortTimeFormatString
				.setEnabled(format == RegionalSettings.DATETIME_FORMAT_CUSTOM);
		shortTimeFormatString.getTextControl().addModifyListener(
				new ModifyListener() {
					@Override
					public void modifyText(ModifyEvent e) {
		...
		dateTimeExample = new LabeledText(dialogArea, SWT.NONE, SWT.BORDER
				| SWT.READ_ONLY);
		dateTimeExample
				.setLabel(Messages.get().RegionalSettingsPrefPage_Example); //$NON-NLS-1$
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		gd.horizontalSpan = 2;
		...
		shortTimeExample = new LabeledText(dialogArea, SWT.NONE, SWT.BORDER
				| SWT.READ_ONLY);
		shortTimeExample
				.setLabel(Messages.get().RegionalSettingsPrefPage_ShortTimeExample);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		...
		checkServerTimeZone
				.setText(Messages.get().RegionalSettingsPrefPage_UseServerTimeZone);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		gd.horizontalSpan = layout.numColumns;
		checkServerTimeZone.setLayoutData(gd);
		checkServerTimeZone.setSelection(getPreferenceStore().getBoolean(
				"USE_SERVER_TIMEZONE")); //$NON-NLS-1$
	...
	private void updateExample(Control updatedControl) {
		...
		case RegionalSettings.DATETIME_FORMAT_CUSTOM:
			try {
				df = new SimpleDateFormat(dateFormatString.getText()
						+ " " + timeFormatString.getText()); //$NON-NLS-1$
			} catch (IllegalArgumentException e) {
				setErrorMessage(e.getLocalizedMessage());
				if (updatedControl != null)
					updatedControl.setBackground(SharedColors.getColor(
							SharedColors.ERROR_BACKGROUND, getShell()
									.getDisplay()));
				return;
			}
			break;
		...
		dateTimeExample
				.setText(df.format(new Date(System.currentTimeMillis())));
