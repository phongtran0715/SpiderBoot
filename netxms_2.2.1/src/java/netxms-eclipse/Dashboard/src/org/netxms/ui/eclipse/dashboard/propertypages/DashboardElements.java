...
public class DashboardElements extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		final String[] columnNames = { Messages.get().DashboardElements_Type,
				Messages.get().DashboardElements_Span, "Height" };
		...
		viewer.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				downButton.setEnabled((selection.size() == 1) && (index >= 0)
	 ...
	 * @return
	 */
	private static List<DashboardElement> copyElements(
			List<DashboardElement> src) {
	...
	private boolean applyChanges(final boolean isApply) {
		...
		new ConsoleJob(Messages.get().DashboardElements_JobTitle, null,
	...
	private void editElement() {
		...
		DashboardElement element = (DashboardElement) selection
				.getFirstElement();
		...
		if (config != null) {
			try {
				...
				MessageDialogHelper.openError(
						getShell(),
						Messages.get().DashboardElements_InternalErrorTitle,
						Messages.get().DashboardElements_InternalErrorText
								+ e.getMessage());
			}
		} else {
			MessageDialogHelper.openError(getShell(),
					Messages.get().DashboardElements_InternalErrorTitle,
					Messages.get().DashboardElements_InternalErrorText2);
		}
	}
