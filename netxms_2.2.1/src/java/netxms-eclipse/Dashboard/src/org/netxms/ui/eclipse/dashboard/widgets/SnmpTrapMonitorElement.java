...
public class SnmpTrapMonitorElement extends ElementWidget {
	...
	protected SnmpTrapMonitorElement(DashboardControl parent,
		...
		new ConsoleJob(String.format("Subscribing to channel ",
				NXCSession.CHANNEL_SNMP_TRAPS), viewPart, Activator.PLUGIN_ID,
				null) {
			@Override
			...
			protected String getErrorMessage() {
				return String.format("Cannot subscribe to channel ",
						NXCSession.CHANNEL_SNMP_TRAPS);
			}
		}.start();
	...
	public void dispose() {
		ConsoleJob job = new ConsoleJob(String.format(
				"Unsuscribing from channel ", NXCSession.CHANNEL_SNMP_TRAPS),
			...
			@Override
			protected String getErrorMessage() {
				return String.format("Cannot unsubscribe from channel ",
						NXCSession.CHANNEL_SNMP_TRAPS);
			}
		};
		job.setUser(false);
