...
public class CloneDashboard implements IObjectActionDelegate {
	...
	public void run(final IAction action) {
		...
		final CreateObjectDialog dlg = new CreateObjectDialog(
				window.getShell(), Messages.get().CloneDashboard_Dashboard);
		if (dlg.open() == Window.OK) {
			final NXCSession session = (NXCSession) ConsoleSharedData
					.getSession();
			new ConsoleJob(Messages.get().CloneDashboard_JobTitle, part,
				...
				protected void runInternal(IProgressMonitor monitor)
					...
					NXCObjectCreationData cd = new NXCObjectCreationData(
							AbstractObject.OBJECT_DASHBOARD,
				...
				protected String getErrorMessage() {
					return String.format(Messages.get().CloneDashboard_Error,
							dlg.getObjectName());
				}
			}.start();
		}
