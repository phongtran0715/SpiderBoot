...
public class EmbeddedDashboardElement extends ElementWidget {
	 ...
	 * @param data
	 */
	public EmbeddedDashboardElement(DashboardControl parent,
		...
		for (int i = 0; i < objects.length; i++)
			objects[i] = (Dashboard) session.findObjectById(
					config.getDashboardObjects()[i], Dashboard.class);
		...
		if (objects.length > 1) {
			...
			final ViewRefreshController refreshControler = new ViewRefreshController(
					viewPart, config.getDisplayInterval(), new Runnable() {
						@Override
						public void run() {
							if (!isDisposed())
								nextDashboard();
			...
			if ((objects != null) && (objects.length > 0)
					&& (objects[0] != null))
				new DashboardControl(this, SWT.NONE, objects[0], viewPart,
						getSelectionProvider(), true);
		}
	}
	...
	private void nextDashboard() {
		...
		if (objects[current] != null)
			control = new DashboardControl(this, SWT.NONE, objects[current],
					viewPart, getSelectionProvider(), true); /*
															 * TODO: set
															 * embedded=false if
															 * border=true
															 */
