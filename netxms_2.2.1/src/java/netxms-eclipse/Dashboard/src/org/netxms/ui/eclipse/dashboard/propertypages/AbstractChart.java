...
public class AbstractChart extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		legendPosition = WidgetHelper.createLabeledCombo(dialogArea,
				SWT.READ_ONLY, Messages.get().AbstractChart_LegendPosition,
		...
		legendPosition
				.select(positionIndexFromValue(config.getLegendPosition()));
		...
		if (config instanceof LineChartConfig) {
			...
			checkExtendedLegend
					.setText(Messages.get().AbstractChart_ExtendedLegend);
			checkExtendedLegend.setSelection(((LineChartConfig) config)
					.isExtendedLegend());
			gd = new GridData();
			gd.horizontalAlignment = SWT.FILL;
			gd.grabExcessHorizontalSpace = true;
			...
			checkLogScale
					.setText(Messages.get().AbstractChart_LogartithmicScale);
			checkLogScale.setSelection(((LineChartConfig) config)
					.isLogScaleEnabled());
			gd = new GridData();
		...
		if (config instanceof ComparisonChartConfig) {
			...
			checkShowIn3D.setSelection(((ComparisonChartConfig) config)
					.isShowIn3D());
			gd = new GridData();
			gd.horizontalAlignment = SWT.FILL;
			gd.grabExcessHorizontalSpace = true;
			...
			if ((config instanceof BarChartConfig)
					|| (config instanceof TubeChartConfig)) {
				checkTransposed = new Button(optionsGroup, SWT.CHECK);
				checkTransposed
						.setText(Messages.get().AbstractChart_Transposed);
				checkTransposed
						.setSelection((config instanceof BarChartConfig) ? ((BarChartConfig) config)
								.isTransposed() : ((TubeChartConfig) config)
								.isTransposed());
				gd = new GridData();
				gd.horizontalAlignment = SWT.FILL;
				gd.grabExcessHorizontalSpace = true;
		...
		if (config instanceof LineChartConfig) {
			...
			checkInteractive.setSelection(((LineChartConfig) config)
					.isInteractive());
			gd = new GridData();
			gd.horizontalSpan = layout.numColumns;
			...
			timeRange = WidgetHelper.createLabeledSpinner(timeRangeArea,
					SWT.BORDER, Messages.get().AbstractChart_TimeInterval, 1,
					10000, WidgetHelper.DEFAULT_LAYOUT_DATA);
			timeRange.setSelection(((LineChartConfig) config).getTimeRange());

			timeUnits = WidgetHelper.createLabeledCombo(timeRangeArea,
					SWT.READ_ONLY, Messages.get().AbstractChart_TimeUnits,
					WidgetHelper.DEFAULT_LAYOUT_DATA);
	...
	public boolean performOk() {
		...
		if (config instanceof ComparisonChartConfig) {
			((ComparisonChartConfig) config).setShowIn3D(checkShowIn3D
					.getSelection());
			if (config instanceof BarChartConfig) {
				((BarChartConfig) config).setTransposed(checkTransposed
						.getSelection());
			} else if (config instanceof TubeChartConfig) {
				((TubeChartConfig) config).setTransposed(checkTransposed
						.getSelection());
			}
		}
		if (config instanceof LineChartConfig) {
			((LineChartConfig) config).setTimeRange(timeRange.getSelection());
			((LineChartConfig) config).setTimeUnits(timeUnits
					.getSelectionIndex());
			((LineChartConfig) config)
					.setShowGrid(checkShowGrid.getSelection());
			((LineChartConfig) config).setExtendedLegend(checkExtendedLegend
					.getSelection());
			((LineChartConfig) config).setLogScaleEnabled(checkLogScale
					.getSelection());
			((LineChartConfig) config).setStacked(checkStacked.getSelection());
			((LineChartConfig) config).setArea(checkAreaChart.getSelection());
			((LineChartConfig) config).setInteractive(checkInteractive
					.getSelection());
