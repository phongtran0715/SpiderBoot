...
public class IdMatchingDialog extends Dialog {
	 ...
	 * @param dcis
	 */
	public IdMatchingDialog(Shell parentShell,
			Map<Long, ObjectIdMatchingData> objects,
			Map<Long, DciIdMatchingData> dcis) {
		super(parentShell);
	...
	protected Control createDialogArea(Composite parent) {
		...
		final String[] names = {
				Messages.get().IdMatchingDialog_ColumnOriginalID,
				Messages.get().IdMatchingDialog_ColumnName,
				Messages.get().IdMatchingDialog_ColumnMatchID,
				Messages.get().IdMatchingDialog_ColumnMatchName };
		final int[] widths = { 100, 300, 80, 300 };
		viewer = new SortableTreeViewer(dialogArea, names, widths, 0, SWT.UP,
	...
	private void createActions() {
		actionMap = new Action(Messages.get().IdMatchingDialog_MapTo,
				Activator.getImageDescriptor("icons/sync.gif")) { //$NON-NLS-1$
			@Override
			public void run() {
	...
	private void mapNode(ObjectIdMatchingData data) {
		...
		case AbstractObject.OBJECT_CLUSTER:
			classFilter = ObjectSelectionDialog
					.createNodeSelectionFilter(false);
		...
		if (dlg.open() == Window.OK) {
			...
			if (ImportDashboard.isCompatibleClasses(object.getObjectClass(),
					data.objectClass)) {
				...
				MessageDialogHelper.openWarning(getShell(),
						Messages.get().IdMatchingDialog_Warning,
						Messages.get().IdMatchingDialog_ClassMismatch);
			}
		}
	}
	...
	private void updateDciMapping(final ObjectIdMatchingData objData) {
		...
		new ConsoleJob(Messages.get().IdMatchingDialog_JobTitle, null,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final DciValue[] dciValues = session
						.getLastValues(objData.dstId);
	...
	private void mapDci(DciIdMatchingData data) {
		...
		SelectNodeDciDialog dlg = new SelectNodeDciDialog(getShell(),
				data.dstNodeId);
		if (dlg.open() == Window.OK) {
	...
	protected void okPressed() {
		...
		if (!ok) {
			if (!MessageDialogHelper.openQuestion(getShell(),
					Messages.get().IdMatchingDialog_MatchingErrors,
					Messages.get().IdMatchingDialog_ConfirmationText))
				return;
		}
