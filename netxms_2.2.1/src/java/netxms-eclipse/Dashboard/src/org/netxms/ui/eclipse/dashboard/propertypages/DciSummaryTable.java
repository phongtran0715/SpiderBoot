...
public class DciSummaryTable extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		config = (DciSummaryTableConfig) getElement().getAdapter(
				DciSummaryTableConfig.class);
		...
		tableSelector = new SummaryTableSelector(dialogArea, SWT.NONE,
				AbstractSelector.SHOW_CLEAR_BUTTON);
		...
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		refreshInterval = WidgetHelper.createLabeledSpinner(dialogArea,
				SWT.BORDER, Messages.get().DciSummaryTable_RefreshInterval, 0,
				10000, gd);
		...
		checkShowSortAndLimitFields.setSelection(config
				.isEnableSortingAndLineLimit());
		checkShowSortAndLimitFields.setLayoutData(gd);
		checkShowSortAndLimitFields
		...
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		numRowsShow = WidgetHelper.createLabeledSpinner(showTopNNodes,
				SWT.BORDER, "Number of lines to show", 0, 10000, gd);
		...
		sortTables = new SortableTableViewer(showTopNNodes, columnNames,
				columnWidths, 0, 0, SWT.BORDER | SWT.MULTI | SWT.FULL_SELECTION);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
	 ...
	 */
	private void moveUp() {
		IStructuredSelection selection = (IStructuredSelection) sortTables
				.getSelection();
		if (selection.size() != 1)
			return;
	 ...
	 */
	private void add() {
		DCISummaryTableSortColumnSelectionDialog dlg = new DCISummaryTableSortColumnSelectionDialog(
				getShell(), null, false, tableSelector.getTableId());
	...
	private void edit() {
		...
		String element = (String) selection.getFirstElement();
		DCISummaryTableSortColumnSelectionDialog dlg = new DCISummaryTableSortColumnSelectionDialog(
				getShell(), element.substring(1),
				element.charAt(0) == '>' ? true : false,
				tableSelector.getTableId());
		if (dlg.open() == Window.OK) {
			String s = dlg.getSortingColumn();
			int index = currSortingList.indexOf(element);
	...
	public boolean performOk() {
		...
		config.setEnableSortingAndLineLimit(checkShowSortAndLimitFields
				.getSelection());
		return true;
	}
}
