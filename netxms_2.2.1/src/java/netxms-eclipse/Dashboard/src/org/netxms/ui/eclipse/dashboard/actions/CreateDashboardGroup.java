...
public class CreateDashboardGroup implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateObjectDialog dlg = new CreateObjectDialog(
				window.getShell(), "Create dashboard group");
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob("Create dashboard group", part, Activator.PLUGIN_ID,
				null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_DASHBOARDGROUP,
			...
			protected String getErrorMessage() {
				return String.format("Cannot create dashboard group object %s",
						dlg.getObjectName());
			}
		}.start();
	}
