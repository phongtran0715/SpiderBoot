...
public class EmbeddedDashboard extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		config = (EmbeddedDashboardConfig) getElement().getAdapter(
				EmbeddedDashboardConfig.class);
		...
		dashboardObjects = session.findMultipleObjects(
				config.getDashboardObjects(), Dashboard.class, false);
	...
	protected void addDashboard() {
		ObjectSelectionDialog dlg = new ObjectSelectionDialog(getShell(), null,
				ObjectSelectionDialog.createDashboardSelectionFilter());
		if (dlg.open() == Window.OK) {
			dashboardObjects.addAll(Arrays.asList(dlg
					.getSelectedObjects(Dashboard.class)));
			viewer.setInput(dashboardObjects.toArray());
		}
	}
