...
public class ServiceComponentsElement extends ElementWidget {
	 ...
	 *            viewPart
	 */
	protected ServiceComponentsElement(DashboardControl parent,
		...
		AbstractObject rootObject = session
				.findObjectById(config.getObjectId());

		mapPage = new NetworkMapPage(ServiceComponents.ID
				+ rootObject.getObjectId());
		long elementId = mapPage.createElementId();
		mapPage.addElement(new NetworkMapObject(elementId, rootObject
				.getObjectId()));
	 ...
	 * @param object
	 */
	private void addServiceComponents(AbstractObject object,
		...
		while (it.hasNext()) {
			...
			if ((child != null)
					&& ((child instanceof Container)
							...
							|| (child instanceof Node) || (child instanceof Condition))) {
				long elementId = mapPage.createElementId();
