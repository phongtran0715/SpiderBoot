...
/**
 * Base class for data comparison charts based on table DCI - like bar chart,
 * pie chart, etc.
 */
public abstract class TableComparisonChartElement extends ElementWidget {
	 ...
	 * @param data
	 */
	public TableComparisonChartElement(DashboardControl parent,
	...
	protected void refreshData() {
		...
		ConsoleJob job = new ConsoleJob(
				Messages.get().TableComparisonChartElement_JobTitle, viewPart,
				Activator.PLUGIN_ID, Activator.PLUGIN_ID) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final Table data = session.getTableLastValues(
						config.getNodeId(), config.getDciId());
	...
	private void updateChart(final Table data) {
		String instanceColumn = (config.getInstanceColumn() != null) ? config
				.getInstanceColumn() : ""; // FIXME //$NON-NLS-1$
		...
		for (int i = 0; i < data.getRowCount(); i++) {
			...
			if (index == null) {
				if ((instanceMap.size() >= DataChart.MAX_CHART_ITEMS)
					...
					continue;
				index = chart.addParameter(
						new GraphItem(config.getNodeId(), config.getDciId(), 0,
								0, Long.toString(config.getDciId()), instance,
								"%s"), 0.0); //$NON-NLS-1$
