...
public class TableComparisonChart extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		config = (TableComparisonChartConfig) getElement().getAdapter(
				TableComparisonChartConfig.class);
		...
		legendPosition = WidgetHelper.createLabeledCombo(dialogArea,
				SWT.READ_ONLY,
				Messages.get().TableComparisonChart_LegendPosition,
		...
		checkTranslucent
				.setText(Messages.get().TableComparisonChart_Translucent);
		...
		if ((config instanceof TableBarChartConfig)
				|| (config instanceof TableTubeChartConfig)) {
			checkTransposed = new Button(optionsGroup, SWT.CHECK);
			checkTransposed
					.setText(Messages.get().TableComparisonChart_Transposed);
			checkTransposed
					.setSelection((config instanceof TableBarChartConfig) ? ((TableBarChartConfig) config)
							.isTransposed() : ((TableTubeChartConfig) config)
							.isTransposed());
			gd = new GridData();
			gd.horizontalAlignment = SWT.FILL;
			gd.grabExcessHorizontalSpace = true;
		...
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		refreshRate = WidgetHelper.createLabeledSpinner(dialogArea, SWT.BORDER,
				Messages.get().TableComparisonChart_RefreshInterval, 1, 10000,
				gd);
	...
	public boolean performOk() {
		...
		if (config instanceof TableBarChartConfig) {
			((TableBarChartConfig) config).setTransposed(checkTransposed
					.getSelection());
		} else if (config instanceof TableTubeChartConfig) {
			((TableTubeChartConfig) config).setTransposed(checkTransposed
					.getSelection());
		}
		return true;
	}
