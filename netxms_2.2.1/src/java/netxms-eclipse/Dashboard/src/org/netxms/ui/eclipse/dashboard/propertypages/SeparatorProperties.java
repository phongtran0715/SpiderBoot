...
public class SeparatorProperties extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		config = (SeparatorConfig) getElement().getAdapter(
		...
		new Label(fgArea, SWT.NONE)
				.setText(Messages.get().LabelProperties_TextColor);
		foreground = new ColorSelector(fgArea);
		foreground.setColorValue(ColorConverter.rgbFromInt(config
				.getForegroundColorAsInt()));
		...
		new Label(bgArea, SWT.NONE)
				.setText(Messages.get().LabelProperties_BgColor);
		background = new ColorSelector(bgArea);
		background.setColorValue(ColorConverter.rgbFromInt(config
		...
		marginLeft = WidgetHelper.createLabeledSpinner(dialogArea, SWT.BORDER,
				Messages.get().SeparatorProperties_LeftMargin, 0, 255,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		marginLeft.setSelection(config.getLeftMargin());

		marginRight = WidgetHelper.createLabeledSpinner(dialogArea, SWT.BORDER,
				Messages.get().SeparatorProperties_RightMargin, 0, 255,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		marginRight.setSelection(config.getRightMargin());

		marginTop = WidgetHelper.createLabeledSpinner(dialogArea, SWT.BORDER,
				Messages.get().SeparatorProperties_TopMargin, 0, 255,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		marginTop.setSelection(config.getTopMargin());

		marginBottom = WidgetHelper.createLabeledSpinner(dialogArea,
				SWT.BORDER, Messages.get().SeparatorProperties_BottomMargin, 0,
