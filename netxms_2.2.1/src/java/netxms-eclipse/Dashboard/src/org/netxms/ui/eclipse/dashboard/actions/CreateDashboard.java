...
public class CreateDashboard implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateObjectDialog dlg = new CreateObjectDialog(
				window.getShell(), Messages.get().CreateDashboard_Dashboard);
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateDashboard_JobTitle, part,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_DASHBOARD, dlg.getObjectName(),
						parentId);
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().CreateDashboard_Error,
						dlg.getObjectName());
			}
		}.start();
	}
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if ((selection instanceof IStructuredSelection)
			...
			if ((object instanceof Dashboard)
					...
					|| (object instanceof DashboardGroup)) {
