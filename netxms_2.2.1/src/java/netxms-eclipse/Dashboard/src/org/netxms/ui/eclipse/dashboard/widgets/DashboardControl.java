...
public class DashboardControl extends Composite {
	...
	public DashboardControl(Composite parent, int style, Dashboard dashboard,
			IViewPart viewPart,
			IntermediateSelectionProvider selectionProvider, boolean embedded) {
		super(parent, style);
	 ...
	 */
	public DashboardControl(Composite parent, int style, Dashboard dashboard,
			List<DashboardElement> elements, IViewPart viewPart,
			IntermediateSelectionProvider selectionProvider, boolean modified) {
		super(parent, style);
		this.dashboard = dashboard;
		this.embedded = false;
	...
	private void createContent() {
		setBackground(SharedColors.getColor(SharedColors.DASHBOARD_BACKGROUND,
				getDisplay()));
	...
	void editElement(DashboardElement element) {
		DashboardElementConfig config = (DashboardElementConfig) AdapterManager
				.getDefault().getAdapter(element, DashboardElementConfig.class);
		if (config != null) {
			try {
				config.setLayout(DashboardElementLayout.createFromXml(element
						.getLayout()));

				PropertyDialog dlg = PropertyDialog.createDialogOn(getShell(),
						null, config);
				...
				MessageDialogHelper.openError(
						getShell(),
						Messages.get().DashboardControl_InternalError,
						Messages.get().DashboardControl_InternalErrorPrefix
								+ e.getMessage());
			}
		} else {
			MessageDialogHelper.openError(getShell(),
					Messages.get().DashboardControl_InternalError,
					Messages.get().DashboardControl_InternalErrorText1);
		}
	}
	...
	void editElementXml(DashboardElement element) {
		EditElementXmlDlg dlg = new EditElementXmlDlg(getShell(),
				element.getData());
	...
	private void addElement(DashboardElement element) {
		...
		if (config != null) {
			...
			MessageDialogHelper.openError(getShell(),
					Messages.get().DashboardControl_InternalError,
					Messages.get().DashboardControl_InternalErrorText2);
		}
	}

	public void addSyslogMonitor() {
		DashboardElement e = new DashboardElement(
				DashboardElement.SYSLOG_MONITOR,
				DEFAULT_OBJECT_REFERENCE_CONFIG);
		addElement(e);
	}

	public void addSnmpTrapMonitor() {
		DashboardElement e = new DashboardElement(
				DashboardElement.SNMP_TRAP_MONITOR,
				DEFAULT_OBJECT_REFERENCE_CONFIG);
		addElement(e);
	}
	...
	public void addEventMonitor() {
		DashboardElement e = new DashboardElement(
				DashboardElement.EVENT_MONITOR, DEFAULT_OBJECT_REFERENCE_CONFIG);
		addElement(e);
	}
	...
	public void addAlarmBrowser() {
		DashboardElement e = new DashboardElement(
				DashboardElement.ALARM_VIEWER, DEFAULT_OBJECT_REFERENCE_CONFIG);
		addElement(e);
	}
	...
	public void addLabel() {
		DashboardElement e = new DashboardElement(DashboardElement.LABEL,
	...
	public void addPieChart() {
		DashboardElement e = new DashboardElement(DashboardElement.PIE_CHART,
	...
	public void addBarChart() {
		DashboardElement e = new DashboardElement(DashboardElement.BAR_CHART,
				DEFAULT_CHART_CONFIG);
		addElement(e);
	}
	...
	public void addTubeChart() {
		DashboardElement e = new DashboardElement(DashboardElement.TUBE_CHART,
				DEFAULT_CHART_CONFIG);
		addElement(e);
	}
	...
	public void addAvailabilityChart() {
		DashboardElement e = new DashboardElement(
				DashboardElement.AVAILABLITY_CHART,
				DEFAULT_AVAILABILITY_CHART_CONFIG);
		addElement(e);
	}
	...
	public void addLineChart() {
		DashboardElement e = new DashboardElement(DashboardElement.LINE_CHART,
	...
	public void addEmbeddedDashboard() {
		DashboardElement e = new DashboardElement(DashboardElement.DASHBOARD,
				DEFAULT_OBJECT_REFERENCE_CONFIG);
		addElement(e);
	}
	...
	public void addStatusIndicator() {
		DashboardElement e = new DashboardElement(
				DashboardElement.STATUS_INDICATOR,
				DEFAULT_OBJECT_REFERENCE_CONFIG);
		addElement(e);
	}
	...
	public void saveDashboard(IViewPart viewPart) {
		...
		new ConsoleJob(Messages.get().DashboardControl_SaveLayout, viewPart,
			...
			protected String getErrorMessage() {
				return Messages.get().DashboardControl_SaveError
