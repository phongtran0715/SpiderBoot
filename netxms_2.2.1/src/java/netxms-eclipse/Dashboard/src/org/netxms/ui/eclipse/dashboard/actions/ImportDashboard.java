...
public class ImportDashboard implements IObjectActionDelegate {
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if ((selection instanceof IStructuredSelection)
			...
			if ((object instanceof Dashboard)
					|| (object instanceof DashboardRoot)) {
	...
	public void run(IAction action) {
		final ImportDashboardDialog dlg = new ImportDashboardDialog(
				window.getShell());
		...
		new ConsoleJob(Messages.get().ImportDashboard_JobTitle, part,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				for (int i = 0; i < elementsRoot.getLength(); i++) {
					...
					NodeList elements = ((Element) elementsRoot.item(i))
							.getElementsByTagName("dashboardElement"); //$NON-NLS-1$
					for (int j = 0; j < elements.getLength(); j++) {
						Element e = (Element) elements.item(j);
						DashboardElement de = new DashboardElement(
								getNodeValueAsInt(e, "type", 0), getNodeValueAsXml(e, "element")); //$NON-NLS-1$ //$NON-NLS-2$
				...
				if (doIdMapping(display, session, dashboardElements, root)) {
					NXCObjectCreationData cd = new NXCObjectCreationData(
							AbstractObject.OBJECT_DASHBOARD, objectName,
							parentId);
					final long objectId = session.createObject(cd);

					NXCObjectModificationData md = new NXCObjectModificationData(
							objectId);
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().ImportDashboard_Error,
						dlg.getObjectName());
			}
		}.start();
	}
	 ...
	 */
	private boolean doIdMapping(final Display display,
			final NXCSession session, List<DashboardElement> dashboardElements,
		...
		for (DciIdMatchingData d : dcis.values()) {
			if (!objects.containsKey(d.srcNodeId))
				objects.put(d.srcNodeId, new ObjectIdMatchingData(d.srcNodeId,
						"", AbstractObject.OBJECT_NODE)); //$NON-NLS-1$
		}
		...
		for (ObjectIdMatchingData d : objects.values()) {
			...
			if ((object != null)
					&& isCompatibleClasses(object.getObjectClass(),
							d.objectClass)) {
		...
		UIJob job = new UIJob(display, "") { //$NON-NLS-1$
			...
			public IStatus runInUIThread(IProgressMonitor monitor) {
				IdMatchingDialog dlg = new IdMatchingDialog(window.getShell(),
						objects, dcis);
				return (dlg.open() == Window.OK) ? Status.OK_STATUS
	 ...
	 * @throws Exception
	 */
	private void updateDashboardElement(DashboardElement e,
			Map<Long, ObjectIdMatchingData> objects,
			Map<Long, DciIdMatchingData> dcis) throws Exception {
		DashboardElementConfig config = (DashboardElementConfig) Platform
				.getAdapterManager()
				.getAdapter(e, DashboardElementConfig.class);
		if (config == null)
			return;
	...
	private Map<Long, ObjectIdMatchingData> readSourceObjects(Element root) {
		...
		for (int i = 0; i < objectsRoot.getLength(); i++) {
			...
			NodeList elements = ((Element) objectsRoot.item(i))
					.getElementsByTagName("object"); //$NON-NLS-1$
			for (int j = 0; j < elements.getLength(); j++) {
				Element e = (Element) elements.item(j);
				long id = getAttributeAsLong(e, "id", 0); //$NON-NLS-1$
				objects.put(id,
						new ObjectIdMatchingData(id, e.getTextContent(),
								(int) getAttributeAsLong(e, "class", 0))); //$NON-NLS-1$
			}
		}
		return objects;
	}
	...
	private Map<Long, DciIdMatchingData> readSourceDci(Element root) {
		...
		for (int i = 0; i < objectsRoot.getLength(); i++) {
			...
			NodeList elements = ((Element) objectsRoot.item(i))
					.getElementsByTagName("dci"); //$NON-NLS-1$
			for (int j = 0; j < elements.getLength(); j++) {
				Element e = (Element) elements.item(j);
				long id = getAttributeAsLong(e, "id", 0); //$NON-NLS-1$
				dcis.put(
						id,
						new DciIdMatchingData(
								getAttributeAsLong(e, "node", 0), id, e.getTextContent())); //$NON-NLS-1$
			}
		}
		return dcis;
	}
	 ...
	 * @return
	 */
	private static int getNodeValueAsInt(Element parent, String tag,
			int defaultValue) {
		NodeList l = parent.getElementsByTagName(tag);
		if ((l.getLength() == 0)
				|| (l.item(0).getNodeType() != Node.ELEMENT_NODE))
			return defaultValue;
	 ...
	 * @return
	 */
	private static long getAttributeAsLong(Element element, String attribute,
			long defaultValue) {
		try {
	 ...
	 */
	private static String getNodeValueAsXml(Element parent, String tag)
