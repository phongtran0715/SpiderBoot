...
public class TableValueElement extends ElementWidget {
	 ...
	 * @param viewPart
	 */
	public TableValueElement(DashboardControl parent, DashboardElement element,
			IViewPart viewPart) {
		...
		if (config.getTitle().trim().isEmpty()) {
			...
			viewer = new TableValue(this, SWT.NONE, viewPart, parent
					.getDashboardObject().getGuid().toString());
		} else {
		...
		final ViewRefreshController refreshController = new ViewRefreshController(
				viewPart, config.getRefreshRate(), new Runnable() {
					@Override
					public void run() {
