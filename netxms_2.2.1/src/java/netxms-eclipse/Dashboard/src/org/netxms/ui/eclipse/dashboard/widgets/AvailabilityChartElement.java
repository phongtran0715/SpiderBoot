 ...
 */
public class AvailabilityChartElement extends ElementWidget implements
		DisposeListener, SessionListener {
	 ...
	 * @param data
	 */
	public AvailabilityChartElement(DashboardControl parent,
		...
		chart.addParameter(new GraphItem(0, 0, 0, 0,
				Messages.get().AvailabilityChartElement_Up,
				Messages.get().AvailabilityChartElement_Uptime, "%s"), 100); //$NON-NLS-1$
		chart.addParameter(new GraphItem(0, 0, 0, 0,
				Messages.get().AvailabilityChartElement_Down,
	...
	private void refreshChart() {
		...
		ServiceContainer service = (ServiceContainer) session.findObjectById(
				config.getObjectId(), ServiceContainer.class);
		if (service != null) {
	...
	public void notificationHandler(SessionNotification n) {
		if ((n.getCode() == SessionNotification.OBJECT_CHANGED)
				&& (n.getSubCode() == config.getObjectId()) && !isDisposed()) {
