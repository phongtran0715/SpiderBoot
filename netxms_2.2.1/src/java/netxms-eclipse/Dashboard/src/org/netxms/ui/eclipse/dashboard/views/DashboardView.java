...
public class DashboardView extends ViewPart implements ISaveablePart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		dashboard = (Dashboard) session.findObjectById(Long.parseLong(site
				.getSecondaryId()));
		if (dashboard == null)
			throw new PartInitException(Messages.get().DashboardView_InitError);
		setPartName(Messages.get().DashboardView_PartNamePrefix
				+ dashboard.getObjectName());
	}
	...
	public void createPartControl(Composite parent) {
		...
		ConsoleJob job = new ConsoleJob(
				Messages.get().DashboardView_GetEffectiveRights, this,
		...
		dbc = new DashboardControl(parent, SWT.NONE, dashboard, this,
		...
		clientListener = new SessionListener() {
			...
			public void notificationHandler(SessionNotification n) {
				if (n.getCode() == SessionNotification.OBJECT_CHANGED
						&& dashboard.getObjectId() == n.getSubCode()) {
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.dashboard.context.DashboardView"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
			...
			public void run() {
				if (dbc.isModified()) {
					if (!MessageDialogHelper.openConfirm(getSite().getShell(),
							Messages.get().DashboardView_Refresh,
		...
		actionExportValues = new Action(
				Messages.get().DashboardView_ExportLineChartValues,
				SharedIcons.CSV) {
			@Override
			public void run() {
		...
		actionExportValues
				.setActionDefinitionId("org.netxms.ui.eclipse.dashboard.commands.export_line_chart_values"); //$NON-NLS-1$
		handlerService
				.activateHandler(actionExportValues.getActionDefinitionId(),
						new ActionHandler(actionExportValues));

		actionEditMode = new Action(Messages.get().DashboardView_EditMode,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionAddAlarmBrowser = new Action(
				Messages.get().DashboardView_AddAlarmBrowser) {
			@Override
			public void run() {
				dbc.addAlarmBrowser();
			}
		...
		actionAddTubeChart = new Action(
				Messages.get().DashboardView_AddTubeChart) {
			@Override
			public void run() {
				dbc.addTubeChart();
			}
		...
		actionAddLineChart = new Action(
				Messages.get().DashboardView_AddLineChart) {
			@Override
			public void run() {
				dbc.addLineChart();
			}
		...
		actionAddAvailabilityChart = new Action(
				Messages.get().DashboardView_AddAvailChart) {
			@Override
			public void run() {
		...
		actionAddDashboard = new Action(
				Messages.get().DashboardView_AddDashboard) {
			@Override
			public void run() {
		...
		actionAddStatusIndicator = new Action(
				Messages.get().DashboardView_AddStatusIndicator) {
			@Override
			public void run() {
	 ...
	 * 
	 * @param reload
	 *            if true, dashboard object will be reloaded and all unsaved
	...
	private void rebuildDashboard(boolean reload) {
		...
		if (reload) {
			dashboard = (Dashboard) ((NXCSession) ConsoleSharedData
					.getSession()).findObjectById(dashboard.getObjectId(),
					Dashboard.class);

			if (dashboard != null) {
				dbc = new DashboardControl(parentComposite, SWT.NONE,
						dashboard, this, selectionProvider, false);
			...
			dbc = new DashboardControl(parentComposite, SWT.NONE, dashboard,
					dbc.getElements(), this, selectionProvider,
					dbc.isModified());
			parentComposite.layout(true, true);
			if (!readOnly) {
				dbc.setModifyListener(dbcModifyListener);
	...
	private void exportLineChartValues() {
		...
		new ConsoleJob(Messages.get().DashboardView_ExportLineChartData, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				boolean doInterpolation = session
						.getPublicServerVariableAsBoolean("DashboardDataExportEnableInterpolation"); //$NON-NLS-1$
				...
				for (DataCacheElement d : data) {
					for (DciDataRow r : d.data.getValues()) {
						...
						for (i = 0; i < combinedTimeSeries.size(); i++) {
							if (combinedTimeSeries.get(i).getTime() == r
				...
				List<Double[]> combinedData = new ArrayList<Double[]>(
						data.size());
				...
				for (DataCacheElement d : data) {
					...
					for (int i = 0; i < values.length; i++) {
						...
						long currentCombinedTimestamp = combinedTimeSeries.get(
								combinedIndex).getTime();
						while (currentCombinedTimestamp > currentTimestamp
								.getTime()) {
							...
							combinedIndex++;
							currentCombinedTimestamp = combinedTimeSeries.get(
									combinedIndex).getTime();
						}
