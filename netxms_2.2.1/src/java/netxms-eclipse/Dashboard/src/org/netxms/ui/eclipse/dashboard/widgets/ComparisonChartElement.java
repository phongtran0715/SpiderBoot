...
public abstract class ComparisonChartElement extends ElementWidget {
	 ...
	 * @param data
	 */
	public ComparisonChartElement(DashboardControl parent,
	...
	protected void refreshData(final ChartDciConfig[] dciList) {
		...
		ConsoleJob job = new ConsoleJob(
				Messages.get().ComparisonChartElement_JobTitle, viewPart,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				for (int i = 0; i < dciList.length; i++) {
					if (dciList[i].type == ChartDciConfig.ITEM)
						data[i] = session.getCollectedData(dciList[i].nodeId,
								dciList[i].dciId, null, null, 1);
					else
						data[i] = session.getCollectedTableData(
								dciList[i].nodeId, dciList[i].dciId,
								dciList[i].instance, dciList[i].column, null,
								null, 1);
				}
					...
					public void run() {
						if (!((Widget) chart).isDisposed()) {
							for (int i = 0; i < data.length; i++) {
								...
								chart.updateParameter(i,
										(lastValue != null) ? lastValue
												: new DciDataRow(new Date(),
														0.0), data[i]
												.getDataType(), false);
								if (updateThresholds)
