...
public class StatusIndicatorElement extends ElementWidget {
	 ...
	 * @param element
	 */
	protected StatusIndicatorElement(final DashboardControl parent,
	...
	protected void refreshData() {
		...
		final AbstractObject object = session.findObjectById(config
				.getObjectId());
		if (object != null) {
	...
	public void drawContent(PaintEvent e) {
		...
		if (config.isFullColorRange())
			...
			e.gc.setBackground((status == ObjectStatus.NORMAL) ? StatusDisplayInfo
					.getStatusColor(ObjectStatus.NORMAL) : StatusDisplayInfo
					.getStatusColor(ObjectStatus.CRITICAL));
		...
		e.gc.drawText(config.getTitle(), (MARGIN_X * 2) + CIRCLE_SIZE, MARGIN_Y
				+ (CIRCLE_SIZE / 2) - (textExtent.y / 2));
	}
