...
public class ImportDashboardDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		importFileSelector = new LocalFileSelector(dialogArea, SWT.NONE, false,
				SWT.OPEN);
		importFileSelector
				.setLabel(Messages.get().ImportDashboardDialog_ImportFile);
		importFileSelector.setFilterExtensions(new String[] { "*.xml", "*.*" }); //$NON-NLS-1$ //$NON-NLS-2$
		importFileSelector.setFilterNames(new String[] {
				Messages.get().ImportDashboardDialog_XMLFiles,
				Messages.get().ImportDashboardDialog_AllFiles });
			...
			public void modifyText(ModifyEvent e) {
				...
				try {
					...
					if (!root.getNodeName().equals("dashboard")) //$NON-NLS-1$
						throw new Exception(
								Messages.get().ImportDashboard_InvalidFile);
	...
	protected void okPressed() {
		...
		if (importFile == null) {
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().ImportDashboardDialog_Warning,
					Messages.get().ImportDashboardDialog_WarningSelectFile);
			return;
		}
