...
public class Members extends PropertyPage {
	...
	private HashMap<Long, AbstractUserObject> members = new HashMap<Long, AbstractUserObject>(
			0);
	...
	protected Control createContents(Composite parent) {
		...
		addButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				SelectUserDialog dlg = new SelectUserDialog(Members.this
				...
				if (dlg.open() == Window.OK) {
					...
					userList.setInput(members.values().toArray(
							new AbstractUserObject[members.size()]));
				}
			}
		});
		...
		new ConsoleJob(Messages.get().Members_JobTitle, null,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				session.modifyUserDBObject(object,
						AbstractUserObject.MODIFY_MEMBERS);
			}
