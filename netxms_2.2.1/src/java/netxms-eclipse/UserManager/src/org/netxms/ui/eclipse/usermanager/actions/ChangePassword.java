...
public class ChangePassword implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		final ChangePasswordDialog dlg = new ChangePasswordDialog(
				window.getShell(), true);
		if (dlg.open() == Window.OK) {
			final NXCSession session = ConsoleSharedData.getSession();
			ConsoleJob job = new ConsoleJob(
					Messages.get().ChangePassword_JobTitle, null,
				...
				protected void runInternal(IProgressMonitor monitor)
					...
					session.setUserPassword(session.getUserId(),
							dlg.getPassword(), dlg.getOldPassword());
					runInUIThread(new Runnable() {
						...
						public void run() {
							MessageDialogHelper
									.openInformation(
											window.getShell(),
											Messages.get().ChangePassword_Information,
											Messages.get().ChangePassword_SuccessMessage);
						}
					});
				}
			};
