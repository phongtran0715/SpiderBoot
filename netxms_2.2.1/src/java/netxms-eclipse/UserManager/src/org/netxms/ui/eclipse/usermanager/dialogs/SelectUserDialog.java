...
public class SelectUserDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		new Label(dialogArea, SWT.NONE)
				.setText(Messages.get().SelectUserDialog_AvailableUsers);
		...
		userList = new SortableTableViewer(dialogArea, columnNames,
				columnWidths, 0, SWT.UP, SWT.BORDER
						| (multiSelection ? SWT.MULTI : 0) | SWT.FULL_SELECTION);
	...
	protected void okPressed() {
		...
		if (sel.size() == 0) {
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().SelectUserDialog_Warning,
					Messages.get().SelectUserDialog_WarningText);
			return;
		}
