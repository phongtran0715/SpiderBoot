...
public class General extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		// Object ID
		WidgetHelper
				.createLabeledText(dialogArea, SWT.SINGLE | SWT.BORDER
						| SWT.READ_ONLY, SWT.DEFAULT,
						Messages.get().General_ObjectID,
						Long.toString(object.getId()),
						WidgetHelper.DEFAULT_LAYOUT_DATA);

		// Object name
		initialName = new String(object.getName());
		textName = WidgetHelper.createLabeledText(dialogArea, SWT.SINGLE
				| SWT.BORDER, SWT.DEFAULT, Messages.get().General_LoginName,
				initialName, WidgetHelper.DEFAULT_LAYOUT_DATA);

		// Full name
		if (object instanceof User) {
			initialFullName = new String(((User) object).getFullName());
			textFullName = WidgetHelper.createLabeledText(dialogArea,
					SWT.SINGLE | SWT.BORDER, SWT.DEFAULT,
					Messages.get().General_FullName, initialFullName,
					WidgetHelper.DEFAULT_LAYOUT_DATA);

			initialXmppId = new String(((User) object).getXmppId());
			textXmppId = WidgetHelper.createLabeledText(dialogArea, SWT.SINGLE
					| SWT.BORDER, SWT.DEFAULT, Messages.get().General_XMPPID,
		...
		textDescription = WidgetHelper.createLabeledText(dialogArea, SWT.SINGLE
				| SWT.BORDER, SWT.DEFAULT, Messages.get().General_Description,
	...
	protected void applyChanges(final boolean isApply) {
		...
		final String newFullName = (object instanceof User) ? textFullName
				.getText() : ""; //$NON-NLS-1$
		final String newXmppId = (object instanceof User) ? textXmppId
				.getText() : ""; //$NON-NLS-1$
		...
		new ConsoleJob(Messages.get().General_JobTitle, null,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				int fields = AbstractUserObject.MODIFY_LOGIN_NAME
						| AbstractUserObject.MODIFY_DESCRIPTION;
				object.setName(newName);
				object.setDescription(newDescription);
				if (object instanceof User) {
					((User) object).setFullName(newFullName);
					((User) object).setXmppId(newXmppId);
					fields |= AbstractUserObject.MODIFY_FULL_NAME
							| AbstractUserObject.MODIFY_XMPP_ID;
				}
