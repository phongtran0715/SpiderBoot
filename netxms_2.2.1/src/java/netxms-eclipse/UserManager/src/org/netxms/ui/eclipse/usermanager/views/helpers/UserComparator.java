...
public class UserComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		case UserManagementView.COLUMN_DESCRIPTION:
			result = ((AbstractUserObject) e1).getDescription()
					.compareToIgnoreCase(
							((AbstractUserObject) e2).getDescription());
			break;
		...
		case UserManagementView.COLUMN_GUID:
			result = ((AbstractUserObject) e1).getGuid().toString()
					.compareTo(((AbstractUserObject) e2).getGuid().toString());
			break;
		case UserManagementView.COLUMN_LDAP_DN:
			result = ((AbstractUserObject) e1).getLdapDn().compareToIgnoreCase(
					((AbstractUserObject) e2).getLdapDn());
			break;
		case UserManagementView.COLUMN_NAME:
			result = ((AbstractUserObject) e1).getName().compareToIgnoreCase(
					((AbstractUserObject) e2).getName());
			break;
		case UserManagementView.COLUMN_SOURCE:
			int s1 = ((((AbstractUserObject) e1).getFlags() & AbstractUserObject.LDAP_USER) != 0) ? 1
					: 0;
			int s2 = ((((AbstractUserObject) e1).getFlags() & AbstractUserObject.LDAP_USER) != 0) ? 1
					: 0;
			result = s1 - s2;
			break;
