...
public class CertificateComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		case CertificateView.COLUMN_ID:
			result = Long.signum(((AuthCertificate) e1).getId()
					- ((AuthCertificate) e2).getId());
			break;
		case CertificateView.COLUMN_TYPE:
			result = Integer.signum(((AuthCertificate) e1).getType()
					- ((AuthCertificate) e2).getType());
			break;
		case CertificateView.COLUMN_COMMENTS:
			result = ((AuthCertificate) e1).getComments().compareToIgnoreCase(
					((AuthCertificate) e2).getComments());
			break;
		case CertificateView.COLUMN_SUBJECT:
			result = ((AuthCertificate) e1).getSubject().compareToIgnoreCase(
					((AuthCertificate) e2).getSubject());
			break;
		default:
			result = 0;
			break;
