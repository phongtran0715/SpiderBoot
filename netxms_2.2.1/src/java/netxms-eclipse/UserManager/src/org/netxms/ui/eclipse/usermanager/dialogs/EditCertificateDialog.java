...
public class EditCertificateDialog extends Dialog {
	 ...
	 * @param parentShell
	 * @param id
	 *            Id of certificate, should be set to 0 if new certificate is
	 *            created.
	 * @param type
	 *            Type of certificate mapping. Can be USER_MAP_CERT_BY_SUBJECT
	 *            or USER_MAP_CERT_BY_PUBKEY.
	 * @param certData
	 *            Certificate in PEM format.
	 * @param subjec
	 *            Subject of the certificate
	 * @param comments
	 *            Comments for certificate
	 */
	...
	protected Control createDialogArea(Composite parent) {
		...
		comboType = WidgetHelper.createLabeledCombo(dialogArea, SWT.READ_ONLY,
				Messages.get().EditCertificateDialog_CertTypeLabel,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		String options[] = { Messages.get().EditCertificateDialog_CertType_AC,
				Messages.get().EditCertificateDialog_CertType_UC };
		...
		textSubject
				.setLabel(Messages.get().EditCertificateDialog_CertSubjectLabel);
		textSubject.setText(cert.getSubject());
		textSubject.setLayoutData(gd);

		textComments = new LabeledText(dialogArea, SWT.NONE);
		textComments
				.setLabel(Messages.get().EditCertificateDialog_CertCommentLabel);
