...
public class Authentication extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		comboAuthMethod
				.add(Messages.get().Authentication_CertificateOrPassword);
		...
		comboMappingMethod = new Combo(groupMethod, SWT.DROP_DOWN
				| SWT.READ_ONLY);
		...
		textMappingData = WidgetHelper.createLabeledText(groupMethod,
				SWT.SINGLE | SWT.BORDER, SWT.DEFAULT,
				Messages.get().Authentication_MappingData,
	...
	protected void applyChanges(final boolean isApply) {
		...
		new ConsoleJob(Messages.get().Authentication_JobTitle, null,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				session.modifyUserDBObject(object,
						AbstractUserObject.MODIFY_FLAGS
								| AbstractUserObject.MODIFY_AUTH_METHOD
								| AbstractUserObject.MODIFY_CERT_MAPPING);
			}
