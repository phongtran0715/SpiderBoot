		...
		if (adapterType == IWorkbenchAdapter.class) {
				...
				return new IWorkbenchAdapter() {
					...
					public Object[] getChildren(Object o) {
						...
						for (int i = 0; i < members.length; i++)
							childrens[i] = ConsoleSharedData.getSession()
									.findUserDBObjectById(members[i]);
						return childrens;
					}
			...
			if (adaptableObject instanceof AbstractAccessListElement) {
				return new IWorkbenchAdapter() {
					...
					@Override
					public ImageDescriptor getImageDescriptor(Object object) {
						long userId = ((AbstractAccessListElement) object)
								.getUserId();
						return Activator
								.getImageDescriptor((userId < 0x80000000L) ? "icons/user.png" : "icons/group.png"); //$NON-NLS-1$ //$NON-NLS-2$
					}
					...
					public String getLabel(Object object) {
						...
						AbstractUserObject dbo = session
								.findUserDBObjectById(userId);
						return (dbo != null) ? dbo.getName()
								: ("{" + Long.toString(userId) + "}"); //$NON-NLS-1$ //$NON-NLS-2$
					}
