...
public class CertificateView extends ViewPart implements SessionListener {
	...
	public void createPartControl(Composite parent) {
		...
		final String[] columnNames = { Messages.get().CertificateView_ID,
				Messages.get().CertificateView_Type,
				Messages.get().CertificateView_Comments,
				Messages.get().CertificateView_Subject };
		final int[] columnWidths = { 30, 100, 200, 200 };
		viewer = new SortableTableViewer(parent, columnNames, columnWidths,
				COLUMN_ID, SWT.UP, SWT.FULL_SELECTION | SWT.MULTI);
	...
	private void createActions() {
		...
		actionEdit = new Action(Messages.get().CertificateView_EditCertifData) {
			...
			public void run() {
				...
				if (selection.size() != 1)
					return;
				final long certId = ((AuthCertificate) selection
	...
	private void refreshCertificateList() {
		new ConsoleJob(Messages.get().CertificateView_RefreshCertif, this,
	...
	private void createCertificate() {
		final CreateNewCertificateDialog dlg = new CreateNewCertificateDialog(
				getSite().getShell());
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().CertificateView_CreateNewCertif,
				...
				protected void runInternal(IProgressMonitor monitor)
					...
					if (data != null) {
						...
						throw new Exception(
								Messages.get().CertificateView_ErrorIncorrectCertifFile);
					}
				}
	...
	private void deleteCertificate() {
		...
		if (!MessageDialogHelper.openConfirm(getSite().getShell(),
				Messages.get().CertificateView_Confirmation,
				Messages.get().CertificateView_AckDeleteCertif))
			return;

		final Object[] objects = selection.toArray();
		new ConsoleJob(Messages.get().CertificateView_DeleteCertif, this,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				for (int i = 0; i < objects.length; i++) {
					session.deleteCertificate(((AuthCertificate) objects[i])
							.getId());
				}
			}
		}.start();
	...
	private void saveCertificate(final AuthCertificate cert) {
		final EditCertificateDialog dlg = new EditCertificateDialog(getSite()
				.getShell(), cert);
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().CertificateView_SaveCertif, this,
					Activator.PLUGIN_ID, Activator.PLUGIN_ID) {
