...
public class GroupMembership extends PropertyPage {
	...
	private User object;
	private HashMap<Long, AbstractUserObject> groups = new HashMap<Long, AbstractUserObject>(
			0);
	...
	protected Control createContents(Composite parent) {
		...
		groupList = new SortableTableViewer(dialogArea, columnNames,
				columnWidths, 0, SWT.UP, SWT.BORDER | SWT.MULTI
						| SWT.FULL_SELECTION);
		...
		addButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				SelectUserDialog dlg = new SelectUserDialog(
						GroupMembership.this.getShell(), UserGroup.class);
				if (dlg.open() == Window.OK) {
					...
					groupList.setInput(groups.values().toArray(
							new AbstractUserObject[groups.size()]));
				}
			}
		});
		...
		deleteButton.addSelectionListener(new SelectionListener() {
			...
			@Override
			public void widgetSelected(SelectionEvent e) {
				IStructuredSelection sel = (IStructuredSelection) groupList
						.getSelection();
		...
		for (long groupId : object.getGroups()) {
			final AbstractUserObject group = session
					.findUserDBObjectById(groupId);
			if (group != null) {
