...
public class ChangePasswordDialog extends Dialog {
	 ...
	 * @param parentShell
	 *            Parent shell
	 * @param changeOwnPassword
	 *            Must be set to true if user changing it's own password. If set
	 *            to tru, additional field for old password will be created.
	 */
	...
	protected Control createDialogArea(Composite parent) {
		...
		pic.setImage(Activator
				.getImageDescriptor("icons/password.png").createImage()); //$NON-NLS-1$
		...
		if (changeOwnPassword) {
			textOldPassword = WidgetHelper.createLabeledText(editArea,
					SWT.SINGLE | SWT.BORDER | SWT.PASSWORD, SWT.DEFAULT,
					Messages.get().ChangePasswordDialog_OldPassword, "", null); //$NON-NLS-1$
		}
		textPassword1 = WidgetHelper.createLabeledText(editArea, SWT.SINGLE
				| SWT.BORDER | SWT.PASSWORD, SWT.DEFAULT,
				Messages.get().ChangePasswordDialog_NewPassword, "", null); //$NON-NLS-1$
		textPassword2 = WidgetHelper.createLabeledText(editArea, SWT.SINGLE
				| SWT.BORDER | SWT.PASSWORD, SWT.DEFAULT,
				Messages.get().ChangePasswordDialog_ConfirmNewPassword,
				"", null); //$NON-NLS-1$
	...
	/**
	 * Validate entered new password - it should match with password in "Verify"
	 * field
	 * 
	...
	/**
	 * Get content of "old password" field. Will return null if dialog was
	 * created without "changeOwndPassword" flag.
	 * 
