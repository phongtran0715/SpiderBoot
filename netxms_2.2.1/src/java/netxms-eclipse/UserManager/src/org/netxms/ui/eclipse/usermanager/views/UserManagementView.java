...
public class UserManagementView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		final String[] names = { Messages.get().UserManagementView_Name,
				...
				Messages.get().UserManagementView_GUID, "LDAP DN" };
			...
			public void selectionChanged(SelectionChangedEvent event) {
				...
				if (selection != null) {
					...
					actionChangePassword.setEnabled((selection.size() == 1)
							&& (selection.getFirstElement() instanceof User));
				...
				if (n.getCode() == SessionNotification.USER_DB_CHANGED) {
					viewer.getControl().getDisplay().asyncExec(new Runnable() {
						...
						public void run() {
							...
							if (editNewUser
									&& (n.getSubCode() == SessionNotification.USER_DB_OBJECT_CREATED)) {
								editNewUser = false;
								viewer.setSelection(
										new StructuredSelection(n.getObject()),
										true);
								actionEditUser.run();
							}
						}
		...
		new ConsoleJob(Messages.get().UserManagementView_OpenJobName, this,
			...
			protected void jobFailureHandler() {
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						UserManagementView.this.getViewSite().getPage()
								.hideView(UserManagementView.this);
					}
				});
			}
	...
	private void makeActions() {
		...
		actionAddUser = new Action(
				Messages.get().UserManagementView_CreateNewUser,
				Activator.getImageDescriptor("icons/user_add.png")) { //$NON-NLS-1$
			@Override
			public void run() {
				addUser();
			}
		...
		actionAddGroup = new Action(
				Messages.get().UserManagementView_CreateNewGroup,
				Activator.getImageDescriptor("icons/group_add.png")) { //$NON-NLS-1$
			@Override
			public void run() {
				addGroup();
			}
		...
		actionDeleteUser = new Action(Messages.get().UserManagementView_Delete,
		...
		actionChangePassword = new Action(
				Messages.get().UserManagementView_ChangePassword,
				Activator.getImageDescriptor("icons/change_password.png")) { //$NON-NLS-1$
			@Override
			public void run() {
				changePassword();
			}
		...
		actionDetachUserFromLDAP = new Action(
				Messages.get().UserManagementView_DetachFromLDAP) {
			@Override
			public void run() {
				detachLDAPUser();
			}
	...
	public void dispose() {
		...
		if (databaseLocked) {
			new ConsoleJob(Messages.get().UserManagementView_UnlockJobName,
	...
	private void addUser() {
		final CreateObjectDialog dlg = new CreateObjectDialog(getViewSite()
				.getShell(), true);
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().UserManagementView_CreateUserJobName,
	...
	private void addGroup() {
		...
		if (dlg.open() == Window.OK) {
			new ConsoleJob(
					Messages.get().UserManagementView_CreateGroupJobName, this,
					Activator.PLUGIN_ID, null) {
				@Override
				protected void runInternal(IProgressMonitor monitor)
						throws Exception {
	...
	private void deleteUser() {
		...
		final String message = (selection.size() == 1) ? Messages.get().UserManagementView_ConfirmDeleteSingular
				: Messages.get().UserManagementView_ConfirmDeletePlural;
		final Shell shell = getViewSite().getShell();
		if (!MessageDialogHelper.openQuestion(shell,
				Messages.get().UserManagementView_ConfirmDeleteTitle, message)) {
			return;
		}

		new ConsoleJob(Messages.get().UserManagementView_DeleteJobName, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				for (Object object : selection.toList()) {
					session.deleteUserDBObject(((AbstractUserObject) object)
							.getId());
				}
			}
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				for (Object object : selection.toList()) {
					((AbstractUserObject) object)
							.setFlags(((AbstractUserObject) object).getFlags()
									& ~AbstractUserObject.DISABLED);
					session.modifyUserDBObject(((AbstractUserObject) object),
							AbstractUserObject.MODIFY_FLAGS);
				}
			}
	...
	private void changePassword() {
		...
		if (firstElement instanceof User) {
			...
			final ChangePasswordDialog dialog = new ChangePasswordDialog(
					getSite().getShell(), user.getId() == session.getUserId());
			if (dialog.open() == Window.OK) {
				try {
					...
					MessageDialogHelper
							.openError(
									getSite().getShell(),
									Messages.get().UserManagementView_CannotChangePassword,
									e.getMessage());
				}
			}
		}
