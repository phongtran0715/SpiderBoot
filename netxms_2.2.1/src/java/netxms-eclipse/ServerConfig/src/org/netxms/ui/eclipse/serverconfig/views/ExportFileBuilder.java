...
public class ExportFileBuilder extends ViewPart implements ISaveablePart {
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.serverconfig.context.ExportFileBuilder"); //$NON-NLS-1$
		}
	}
	...
	private void createFileSection() {
		...
		description = new LabeledText(clientArea, SWT.NONE, SWT.MULTI
	...
	private void createTemplateSection() {
		...
		templateViewer = new TableViewer(clientArea, SWT.FULL_SELECTION
		...
		templateViewer.setComparator(new ObjectLabelComparator(
				(ILabelProvider) templateViewer.getLabelProvider()));
	...
	private void createEventSection() {
		...
		eventViewer = new TableViewer(clientArea, SWT.FULL_SELECTION
				| SWT.MULTI | SWT.BORDER);
		...
		eventViewer.setComparator(new ObjectLabelComparator(
				(ILabelProvider) eventViewer.getLabelProvider()));
	...
	private void createTrapSection() {
		...
		trapViewer.setComparator(new ObjectLabelComparator(
				(ILabelProvider) eventViewer.getLabelProvider()));
		...
		linkAdd.addHyperlinkListener(new HyperlinkAdapter() {
			...
			public void linkActivated(HyperlinkEvent e) {
				if (snmpTrapCache == null) {
					new ConsoleJob(
							Messages.get().ExportFileBuilder_TrapsLoadJobName,
							ExportFileBuilder.this, Activator.PLUGIN_ID, null) {
						@Override
						protected void runInternal(IProgressMonitor monitor)
								throws Exception {
							snmpTrapCache = session
	...
	private void createRuleSection() {
		...
		linkAdd.addHyperlinkListener(new HyperlinkAdapter() {
			...
			public void linkActivated(HyperlinkEvent e) {
				if (rulesCache == null) {
					new ConsoleJob(
							Messages.get().ExportFileBuilder_EPPLoadJobName,
							ExportFileBuilder.this, Activator.PLUGIN_ID, null) {
						@Override
						protected void runInternal(IProgressMonitor monitor)
								throws Exception {
							rulesCache = session.getEventProcessingPolicy()
									.getRules();
							runInUIThread(new Runnable() {
								@Override
								public void run() {
	...
	private void createScriptSection() {
		...
		scriptViewer = new TableViewer(clientArea, SWT.FULL_SELECTION
				| SWT.MULTI | SWT.BORDER);
		toolkit.adapt(ruleViewer.getTable());
		GridData gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
	...
	private void createToolsSection() {
		...
		toolsViewer = new TableViewer(clientArea, SWT.FULL_SELECTION
				| SWT.MULTI | SWT.BORDER);
		toolkit.adapt(ruleViewer.getTable());
		GridData gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
	...
	private void createSummaryTablesSection() {
		...
		summaryTableViewer = new TableViewer(clientArea, SWT.FULL_SELECTION
				| SWT.MULTI | SWT.BORDER);
		toolkit.adapt(ruleViewer.getTable());
		GridData gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
	...
	private void createActionsSection() {
		...
		actionViewer = new TableViewer(clientArea, SWT.FULL_SELECTION
				| SWT.MULTI | SWT.BORDER);
		toolkit.adapt(ruleViewer.getTable());
		GridData gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
	...
	private void createActions() {
		...
		actionSave
				.setActionDefinitionId("org.netxms.ui.eclipse.serverconfig.commands.save_exported_config"); //$NON-NLS-1$
		handlerService.activateHandler(actionSave.getActionDefinitionId(),
				new ActionHandler(actionSave));

		actionPublish = new Action("&Publish...",
				Activator.getImageDescriptor("icons/publish.gif")) {
			@Override
			public void run() {
				publish();
			}
		...
		actionPublish
				.setActionDefinitionId("org.netxms.ui.eclipse.serverconfig.commands.publish_config"); //$NON-NLS-1$
		handlerService.activateHandler(actionPublish.getActionDefinitionId(),
				new ActionHandler(actionPublish));
	}
	...
	private void publish() {
		new ConsoleJob("Get list of configured repositories", this,
	...
	private void publishStage2(List<Repository> repositories) {
		RepositorySelectionDialog dlg = new RepositorySelectionDialog(getSite()
				.getShell(), repositories);
		...
		doExport(new ExportCompletionHandler() {
			...
			public void exportCompleted(final String xml) {
				new ConsoleJob("Publish configuration", ExportFileBuilder.this,
						Activator.PLUGIN_ID, null) {
					@Override
					protected void runInternal(IProgressMonitor monitor)
							throws Exception {
						URL url = new URL(repository.getUrl()
								+ "/rest-api/push-export?accessToken="
						...
						try {
							...
							int responseCode = ((HttpURLConnection) conn)
									.getResponseCode();
							Activator.logInfo("Publish config: url="
									+ url.toString() + " response="
									+ responseCode);
							if (responseCode != 200) {
								throw new Exception(String.format(
										"HTTP error %d", responseCode));
							}
						} finally {
							out.close();
						}
	...
	private void doExport(final ExportCompletionHandler completionHandler) {
		...
		new ConsoleJob(Messages.get().ExportFileBuilder_ExportJobName, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final String xml = session.exportConfiguration(descriptionText,
						eventList, trapList, templateList, ruleList,
						scriptList, toolList, summaryTableList, actionList);
				runInUIThread(new Runnable() {
					@Override
					public void run() {
		...
		doExport(new ExportCompletionHandler() {
			...
			public void exportCompleted(final String xml) {
				...
				dlg.setFilterNames(new String[] {
						Messages.get().ConfigurationImportDialog_FileTypeXML,
						Messages.get().ConfigurationImportDialog_FileTypeAll });
				dlg.setOverwrite(true);
				dlg.setFileName(exportFileName);
				final String fileName = dlg.open();
				if (fileName != null) {
					exportFileName = fileName;
					new ConsoleJob("Save exported configuration",
							ExportFileBuilder.this, Activator.PLUGIN_ID, null) {
						@Override
						protected void runInternal(IProgressMonitor monitor)
								throws Exception {
	...
	private void addEvents() {
		EventSelectionDialog dlg = new EventSelectionDialog(getSite()
				.getShell());
	...
	private void addTemplates() {
		...
		if (dlg.open() == Window.OK) {
			...
			setModified();
			new ConsoleJob(Messages.get().ExportFileBuilder_ResolveJobName,
				...
				protected void runInternal(IProgressMonitor monitor)
					...
					runInUIThread(new Runnable() {
						@Override
						public void run() {
							for (EventTemplate e : session
									.findMultipleEventTemplates(eventCodes
											.toArray(new Long[eventCodes.size()]))) {
	...
	private void addTraps() {
		SelectSnmpTrapDialog dlg = new SelectSnmpTrapDialog(getSite()
				.getShell(), snmpTrapCache);
		if (dlg.open() == Window.OK) {
			final Set<Long> eventCodes = new HashSet<Long>();
	...
	private void addRules() {
		RuleSelectionDialog dlg = new RuleSelectionDialog(getSite().getShell(),
				rulesCache);
		if (dlg.open() == Window.OK) {
			final Set<Long> eventCodes = new HashSet<Long>();
	...
	private void addObjectTools() {
		ObjectToolSelectionDialog dlg = new ObjectToolSelectionDialog(getSite()
				.getShell());
		if (dlg.open() == Window.OK) {
	...
	private void addSummaryTables() {
		SummaryTableSelectionDialog dlg = new SummaryTableSelectionDialog(
				getSite().getShell());
		if (dlg.open() == Window.OK) {
	...
	private void addActions() {
		ActionSelectionDialog dlg = new ActionSelectionDialog(getSite()
				.getShell());
		if (dlg.open() == Window.OK) {
