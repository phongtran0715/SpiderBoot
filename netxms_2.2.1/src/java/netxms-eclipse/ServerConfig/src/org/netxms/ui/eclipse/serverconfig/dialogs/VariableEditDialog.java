...
public class VariableEditDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		case CHOICE:
			...
			gd.horizontalAlignment = SWT.FILL;
			gd.grabExcessHorizontalSpace = true;
			comboValue = WidgetHelper.createLabeledCombo(dialogArea, SWT.BORDER
					| SWT.READ_ONLY, Messages.get().VariableEditDialog_Value,
					gd);
			for (Entry<String, String> e : variable.getPossibleValues()
					.entrySet()) {
			...
			break;
		case COLOR:
			colorSelector = WidgetHelper.createLabeledColorSelector(dialogArea,
					Messages.get().VariableEditDialog_Value,
					WidgetHelper.DEFAULT_LAYOUT_DATA);
			if (!variable.getValue().isEmpty())
				colorSelector.setColorValue(ColorConverter
						.parseColorDefinition(variable.getValue()));
			break;
		case INTEGER:
			spinnerValue = WidgetHelper
					.createLabeledSpinner(
							dialogArea,
							SWT.BORDER,
							Messages.get().VariableEditDialog_Value,
							0,
							(variable.getPossibleValues().isEmpty()) ? 0xffffff
									: Integer.parseInt((String) variable
											.getPossibleValues().keySet()
											.toArray()[0]),
							WidgetHelper.DEFAULT_LAYOUT_DATA);
			...
			break;
		case STRING:
			textValue = WidgetHelper.createLabeledText(dialogArea, SWT.BORDER,
					300, Messages.get().VariableEditDialog_Value,
	...
	protected void configureShell(Shell newShell) {
		...
		newShell.setText((variable.getName() == null) ? Messages.get().VariableEditDialog_TitleCreate
				: Messages.get().VariableEditDialog_TitleEdit);
	}
		...
		case CHOICE:
			if (comboValue.getSelectionIndex() == -1) {
				MessageDialogHelper.openWarning(getShell(), "Warning",
						"Please select valid value and then press OK");
				return;
			}
