...
public class LogParserRuleEditor extends DashboardComposite {
	private static final Color CONDITION_BORDER_COLOR = new Color(
			Display.getCurrent(), 198, 214, 172);
	private static final Color ACTION_BORDER_COLOR = new Color(
			Display.getCurrent(), 186, 176, 201);
	private static final Color TITLE_COLOR = new Color(Display.getCurrent(), 0,
			0, 0);
	...
	public LogParserRuleEditor(Composite parent, FormToolkit toolkit,
			LogParserRule rule, LogParserEditor editor) {
		...
		final CGroup condition = new CGroup(this,
				Messages.get().LogParserRuleEditor_Condition) {
			@Override
		...
		final CGroup action = new CGroup(this,
				Messages.get().LogParserRuleEditor_Action) {
			@Override
			protected Control createClientArea(Composite parent) {
	...
	private Composite createMatchingArea(Composite parent) {
		...
		repeatCount = (Spinner) WidgetHelper.createLabeledControl(
				matcherRepeatConf, SWT.BORDER, factory, "Repeat count",
		...
		timeRange = WidgetHelper.createLabeledSpinner(timeBackGroup,
				SWT.BORDER, "Repeat interval", 1, 10000,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		timeRange.setSelection(rule.getMatch().getTimeRagne());
		toolkit.adapt(timeRange);

		timeUnits = WidgetHelper.createLabeledCombo(timeBackGroup,
				SWT.READ_ONLY, "", WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		checkboxReset = toolkit.createButton(matcherRepeatConf,
				"Reset repeat count", SWT.CHECK);
		...
		severity.setLabel(editor.isSyslogParser() ? Messages.get().LogParserRuleEditor_Severity
				: "Level");
		...
		facility.setLabel(editor.isSyslogParser() ? Messages.get().LogParserRuleEditor_Facility
				: "Id");
		...
		toolkit.adapt(tag);
		tag.setLabel(editor.isSyslogParser() ? Messages.get().LogParserRuleEditor_SyslogTag
				: "Source");
		...
		activeContext
				.setLabel(Messages.get().LogParserRuleEditor_ActiveContext);
		...
		description.setText((rule.getDescription() != null) ? rule
	...
	private Composite createActionArea(Composite parent) {
		...
		if (rule.getEvent() != null) {
			try {
				...
				EventTemplate t = ((NXCSession) ConsoleSharedData.getSession())
						.findEventTemplateByName(rule.getEvent().getEvent());
				if (t != null)
					eventCode = t.getCode();
			}
		}
		...
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		eventParamCount = (Spinner) WidgetHelper.createLabeledControl(area,
				SWT.BORDER, factory,
				Messages.get().LogParserRuleEditor_Parameters, gd);
		...
		context.setText((contextDefinition != null) ? contextDefinition
				.getData() : ""); //$NON-NLS-1$
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		...
		context.getTextControl().addModifyListener(new ModifyListener() {
			...
			public void modifyText(ModifyEvent e) {
				...
				contextResetMode.setEnabled(contextSet
						&& (contextAction.getSelectionIndex() == 0));
				editor.fireModifyListeners();
			}
		});
		...
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		contextAction = WidgetHelper.createLabeledCombo(contextOptions,
				SWT.READ_ONLY,
				Messages.get().LogParserRuleEditor_ContextAction, gd);
		...
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		contextResetMode = WidgetHelper.createLabeledCombo(contextOptions,
				SWT.READ_ONLY,
				Messages.get().LogParserRuleEditor_ContextResetMode, gd);
	...
	public void save() {
		...
		rule.setMatch(new LogParserMatch(regexp.getText(), checkboxInvert
				.getSelection(), intOrNull(repeatCount.getText()), Integer
				.parseInt(timeRange.getText())
				* (timeUnits.getSelectionIndex() * 60), checkboxReset
				.getSelection()));
		...
		rule.setContext(activeContext.getText().trim().isEmpty() ? null
				: activeContext.getText());
		rule.setBreakProcessing(checkboxBreak.getSelection());
		rule.setDescription(description.getText());
		if (event.getEventCode() != 0) {
			rule.setEvent(new LogParserEvent(
					Long.toString(event.getEventCode()), eventParamCount
							.getSelection()));
		} else {
			rule.setEvent(null);
		}
	...
	/**
	 * Return integer object created from input string or null if input text
