 ...
 */
public class NetworkDiscoveryConfigurator extends ViewPart implements
		ISaveablePart {
	...
	public void createPartControl(Composite parent) {
		...
		new ConsoleJob(Messages.get().NetworkDiscoveryConfigurator_LoadJobName,
	...
	private void createActions() {
		actionSave = new Action(
				Messages.get().NetworkDiscoveryConfigurator_Save,
				SharedIcons.SAVE) {
			@Override
			public void run() {
				save();
	...
	private void createGeneralSection() {
		...
		radioDiscoveryOff = toolkit
				.createButton(clientArea,
						Messages.get().NetworkDiscoveryConfigurator_Disabled,
						SWT.RADIO);
		radioDiscoveryOff.addSelectionListener(listener);
		radioDiscoveryPassive = toolkit.createButton(clientArea,
				Messages.get().NetworkDiscoveryConfigurator_PassiveDiscovery,
				SWT.RADIO);
		radioDiscoveryPassive.addSelectionListener(listener);
		radioDiscoveryActive = toolkit.createButton(clientArea,
				Messages.get().NetworkDiscoveryConfigurator_ActiveDiscovery,
				SWT.RADIO);
		radioDiscoveryActive.addSelectionListener(listener);

		checkUseSnmpTraps = toolkit
				.createButton(
						clientArea,
						Messages.get().NetworkDiscoveryConfigurator_UseSNMPTrapsForDiscovery,
						SWT.CHECK);
		...
		checkUseSyslog = toolkit
				.createButton(
						clientArea,
						Messages.get().NetworkDiscoveryConfigurator_UseSyslogForDiscovery,
						SWT.CHECK);
	...
	private void createFilterSection() {
		...
		radioFilterOff = toolkit.createButton(clientArea,
				Messages.get().NetworkDiscoveryConfigurator_NoFiltering,
				SWT.RADIO);
		radioFilterOff.addSelectionListener(radioButtonListener);
		radioFilterCustom = toolkit.createButton(clientArea,
				Messages.get().NetworkDiscoveryConfigurator_CustomScript,
				SWT.RADIO);
		...
		radioFilterAuto = toolkit.createButton(clientArea,
				Messages.get().NetworkDiscoveryConfigurator_AutoScript,
				SWT.RADIO);
		radioFilterAuto.addSelectionListener(radioButtonListener);

		checkAgentOnly = toolkit.createButton(clientArea,
				Messages.get().NetworkDiscoveryConfigurator_AcceptAgent,
				SWT.CHECK);
		...
		checkSnmpOnly = toolkit.createButton(clientArea,
				Messages.get().NetworkDiscoveryConfigurator_AcceptSNMP,
				SWT.CHECK);
		...
		checkRangeOnly = toolkit.createButton(clientArea,
				Messages.get().NetworkDiscoveryConfigurator_AcceptRange,
				SWT.CHECK);
	...
	private void createActiveDiscoverySection() {
		...
		activeDiscoveryAddressList = new TableViewer(clientArea, SWT.BORDER
		...
		activeDiscoveryAddressList
				.setContentProvider(new ArrayContentProvider());
		activeDiscoveryAddressList
				.setComparator(new AddressListElementComparator());
	...
	private void createSubnetFilterSection() {
		...
		filterAddressList = new TableViewer(clientArea, SWT.BORDER | SWT.MULTI
				| SWT.FULL_SELECTION);
	...
	public void setConfig(DiscoveryConfig config) {
		...
		radioDiscoveryPassive.setSelection(config.isEnabled()
				&& !config.isActive());
		radioDiscoveryActive.setSelection(config.isEnabled()
				&& config.isActive());
		checkUseSnmpTraps.setSelection(config.isUseSnmpTraps());
		checkUseSyslog.setSelection(config.isUseSyslog());

		if (config.getFilter().equalsIgnoreCase(NetworkDiscovery.FILTER_NONE)
				|| config.getFilter().isEmpty()) {
			radioFilterOff.setSelection(true);
		} else if (config.getFilter().equalsIgnoreCase(
				NetworkDiscovery.FILTER_AUTO)) {
		...
		checkAgentOnly
				.setSelection((config.getFilterFlags() & NetworkDiscovery.FILTER_ALLOW_AGENT) != 0);
		checkSnmpOnly
				.setSelection((config.getFilterFlags() & NetworkDiscovery.FILTER_ALLOW_SNMP) != 0);
		checkRangeOnly
				.setSelection((config.getFilterFlags() & NetworkDiscovery.FILTER_LIMIT_BY_RANGE) != 0);
	...
	public void doSave(IProgressMonitor monitor) {
		try {
			...
			MessageDialogHelper
					.openError(
							getSite().getShell(),
							Messages.get().NetworkDiscoveryConfigurator_Error,
							String.format(
									Messages.get().NetworkDiscoveryConfigurator_SaveErrorText,
	 ...
	 */
	private void save() {
		new ConsoleJob(Messages.get().NetworkDiscoveryConfigurator_SaveJobName,
				this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
	...
	private void addTargetAddressListElement() {
		AddAddressListElementDialog dlg = new AddAddressListElementDialog(
				getSite().getShell());
	...
	private void removeTargetAddressListElements() {
		final List<InetAddressListElement> list = config.getTargets();
		IStructuredSelection selection = (IStructuredSelection) activeDiscoveryAddressList
				.getSelection();
		if (selection.size() > 0) {
	...
	private void removeAddressFilterElements() {
		final List<InetAddressListElement> list = config.getAddressFilter();
		IStructuredSelection selection = (IStructuredSelection) filterAddressList
				.getSelection();
		if (selection.size() > 0) {
			for (Object o : selection.toList()) {
				list.remove(o);
