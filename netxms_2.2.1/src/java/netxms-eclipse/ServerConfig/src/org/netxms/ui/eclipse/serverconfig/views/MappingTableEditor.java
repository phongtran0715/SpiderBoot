...
public class MappingTableEditor extends ViewPart implements ISaveablePart2 {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		setPartName(String.format(
				Messages.get().MappingTableEditor_InitialPartName,
				mappingTableId));
	...
	public void createPartControl(Composite parent) {
		...
		final String[] names = { Messages.get().MappingTableEditor_ColKey,
				Messages.get().MappingTableEditor_ColValue,
				Messages.get().MappingTableEditor_ColComments };
		viewer = new SortableTableViewer(parent, names, widths, COLUMN_KEY,
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.serverconfig.context.MappingTableEditor"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
			...
			public void run() {
				if (modified)
					if (!MessageDialogHelper
							.openQuestion(
									getSite().getShell(),
									Messages.get().MappingTableEditor_RefreshConfirmation,
									Messages.get().MappingTableEditor_RefreshConfirmationText))
						return;
				refresh();
			}
		};

		actionNewRow = new Action(Messages.get().MappingTableEditor_NewRow,
		...
		actionNewRow
				.setActionDefinitionId("org.netxms.ui.eclipse.serverconfig.commands.add_new_row"); //$NON-NLS-1$
		handlerService.activateHandler(actionNewRow.getActionDefinitionId(),
				new ActionHandler(actionNewRow));

		actionDelete = new Action(Messages.get().MappingTableEditor_Delete,
		...
		actionDelete
				.setActionDefinitionId("org.netxms.ui.eclipse.serverconfig.commands.delete_rows"); //$NON-NLS-1$
		handlerService.activateHandler(actionDelete.getActionDefinitionId(),
				new ActionHandler(actionDelete));

		actionSave = new Action(Messages.get().MappingTableEditor_Save,
				SharedIcons.SAVE) {
			@Override
			public void run() {
				new SaveJob().start();
	...
	private void refresh() {
		new ConsoleJob(Messages.get().MappingTableEditor_LoadJobName, this,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						mappingTable = t;
						setPartName(String.format(
								Messages.get().MappingTableEditor_PartName,
	...
	private class SaveJob extends ConsoleJob {
		public SaveJob() {
			super(Messages.get().MappingTableEditor_SaveJobName,
					MappingTableEditor.this, Activator.PLUGIN_ID, null);
		}
		 ...
		 * 
		 * @see
		 * org.netxms.ui.eclipse.jobs.ConsoleJob#runInternal(org.eclipse.core
