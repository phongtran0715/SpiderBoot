...
public class MappingTables extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		final String[] names = { Messages.get().MappingTables_ColID,
				Messages.get().MappingTables_ColName,
				Messages.get().MappingTables_ColFlags,
				Messages.get().MappingTables_ColDescription };
		viewer = new SortableTableViewer(parent, names, widths, COLUMN_NAME,
	...
	private void createActions() {
		...
		actionNewTable = new Action(Messages.get().MappingTables_NewTable,
		...
		actionEditTable = new Action(Messages.get().MappingTables_Edit,
				SharedIcons.EDIT) {
			@Override
			public void run() {
				editTable();
		...
		actionDeleteTables = new Action(Messages.get().MappingTables_Delete,
	...
	private void refresh(final int tableId) {
		new ConsoleJob(Messages.get().MappingTables_ReloadJobName, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<MappingTableDescriptor> tables = session
						.listMappingTables();
					...
					public void run() {
						if (tableId > 0) {
							for (MappingTableDescriptor d : tables) {
								if (d.getId() == tableId) {
									MappingTableDescriptor od = mappingTables
											.get(tableId);
									if (!d.equals(od))
	...
	private void createNewTable() {
		final CreateMappingTableDialog dlg = new CreateMappingTableDialog(
				getSite().getShell());
		if (dlg.open() != Window.OK)
			return;

		new ConsoleJob(Messages.get().MappingTables_CreateJobName, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final Integer assignedId = session.createMappingTable(
						dlg.getName(), dlg.getDescription(), 0);
				runInUIThread(new Runnable() {
					@Override
					public void run() {
						// descriptor may already be presented if creation
						// notification
						// was already processed
						MappingTableDescriptor d = mappingTables
								.get(assignedId);
						if (d == null) {
							d = new MappingTableDescriptor(assignedId,
									dlg.getName(), dlg.getDescription(), 0);
	...
	private void editTable() {
		...
		MappingTableDescriptor d = (MappingTableDescriptor) selection
				.getFirstElement();
		try {
			getSite().getPage().showView(MappingTableEditor.ID,
					Integer.toString(d.getId()), IWorkbenchPage.VIEW_ACTIVATE);
		} catch (PartInitException e) {
			MessageDialogHelper.openError(getSite().getShell(),
					Messages.get().MappingTables_Error, String.format(
							Messages.get().MappingTables_ErrorOpeningView,
	...
	private void deleteTables() {
		...
		if (!MessageDialogHelper.openQuestion(getSite().getShell(),
				Messages.get().MappingTables_DeleteConfirmation,
				Messages.get().MappingTables_DeleteConfirmationText))
			return;
		...
		new ConsoleJob(Messages.get().MappingTables_DeleteJobName, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
