...
public class ScheduledTaskView extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		...
		final String[] names = { "Id", "Schedule Type", "Object", "Parameters",
				"Execution time", "Last execution time", "Execution status",
				"Administrative status", "Owner", "Comments" };
		viewer = new SortableTableViewer(parent, names, widths, SCHEDULE_ID,
		...
		WidgetHelper.restoreTableViewerSettings(viewer, Activator.getDefault()
				.getDialogSettings(), "ScheduledTasks");
		viewer.getControl().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveTableViewerSettings(viewer, Activator
						.getDefault().getDialogSettings(), "ScheduledTasks");
			}
		});
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.serverconfig.context.ScheduledTaskView"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionCreate = new Action("New scheduled task...",
		...
		actionCreate
				.setActionDefinitionId("org.netxms.ui.eclipse.serverconfig.commands.new_task"); //$NON-NLS-1$
		handlerService.activateHandler(actionCreate.getActionDefinitionId(),
				new ActionHandler(actionCreate));
		...
		actionEdit
				.setActionDefinitionId("org.netxms.ui.eclipse.serverconfig.commands.edit_task"); //$NON-NLS-1$
		...
		actionDisable
				.setActionDefinitionId("org.netxms.ui.eclipse.serverconfig.commands.disable_task"); //$NON-NLS-1$
		handlerService.activateHandler(actionDisable.getActionDefinitionId(),
				new ActionHandler(actionDisable));
		...
		actionEnable
				.setActionDefinitionId("org.netxms.ui.eclipse.serverconfig.commands.enable_task"); //$NON-NLS-1$
		handlerService.activateHandler(actionEnable.getActionDefinitionId(),
				new ActionHandler(actionEnable));
		...
		actionReschedule
				.setActionDefinitionId("org.netxms.ui.eclipse.serverconfig.commands.reschedule_task"); //$NON-NLS-1$
		handlerService.activateHandler(
				actionReschedule.getActionDefinitionId(), new ActionHandler(
						actionReschedule));
	}
	...
	protected void rescheduleTask() {
		...
		final RerunTimeDialog dialog = new RerunTimeDialog(
				getSite().getShell(), origin.getExecutionTime());
	...
	protected void deleteTask() {
		...
		if (!MessageDialogHelper
				.openQuestion(
						getSite().getShell(),
						"Confirm Task Delete",
						(selection.size() == 1) ? "Selected task will be deleted. Are you sure?"
								: "Selected tasks will be deleted. Are you sure?"))
			return;
		...
		new ConsoleJob("Update scheduled task", null, Activator.PLUGIN_ID, null) {
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				getDisplay().syncExec(new Runnable() {
					@Override
					public void run() {
						ScheduledTaskEditor dialog = new ScheduledTaskEditor(
								getSite().getShell(), origin, taskList);
						if (dialog.open() == Window.OK) {
		...
		new ConsoleJob("Create scheduled task", null, Activator.PLUGIN_ID, null) {
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				getDisplay().syncExec(new Runnable() {
					...
					public void run() {
						ScheduledTaskEditor dialog = new ScheduledTaskEditor(
								getSite().getShell(), null, taskList);
						if (dialog.open() == Window.OK) {
							task = dialog.getScheduledTask();
						}
					}
	...
	private void refresh() {
		new ConsoleJob("Reloading scheduled task list", this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<ScheduledTask> schedules = session
						.listScheduleTasks();
