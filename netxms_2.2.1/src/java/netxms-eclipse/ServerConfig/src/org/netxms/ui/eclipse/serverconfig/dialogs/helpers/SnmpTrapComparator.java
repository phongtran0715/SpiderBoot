...
public class SnmpTrapComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		case SelectSnmpTrapDialog.COLUMN_EVENT:
			EventTemplate evt1 = session.findEventTemplateByCode(trap1
					.getEventCode());
			EventTemplate evt2 = session.findEventTemplateByCode(trap2
					.getEventCode());
			String name1 = (evt1 != null) ? evt1.getName()
					: ("[" + Integer.toString(trap1.getEventCode()) + "]"); //$NON-NLS-1$ //$NON-NLS-2$
			String name2 = (evt2 != null) ? evt2.getName()
					: ("[" + Integer.toString(trap2.getEventCode()) + "]"); //$NON-NLS-1$ //$NON-NLS-2$
			rc = name1.compareToIgnoreCase(name2);
			break;
		case SelectSnmpTrapDialog.COLUMN_DESCRIPTION:
			rc = trap1.getDescription().compareToIgnoreCase(
					trap2.getDescription());
			break;
		default:
			rc = 0;
			break;
