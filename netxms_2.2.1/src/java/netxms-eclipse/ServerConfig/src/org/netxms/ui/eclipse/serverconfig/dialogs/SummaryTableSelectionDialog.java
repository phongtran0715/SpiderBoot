...
public class SummaryTableSelectionDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		viewer = new TableViewer(dialogArea, SWT.FULL_SELECTION | SWT.MULTI
				| SWT.BORDER);
		...
		viewer.setLabelProvider(new LabelProvider() {
			...
			public String getText(Object element) {
				return ((DciSummaryTableDescriptor) element).getTitle()
						+ " ("
						+ ((DciSummaryTableDescriptor) element).getMenuPath()
								.replace("&", "") + ")";
			}
		});
		viewer.setComparator(new ViewerComparator() {
			...
			public int compare(Viewer viewer, Object e1, Object e2) {
				String n1 = ((DciSummaryTableDescriptor) e1).getTitle()
						+ " ("
						+ ((DciSummaryTableDescriptor) e1).getMenuPath()
								.replace("&", "") + ")";
				String n2 = ((DciSummaryTableDescriptor) e2).getTitle()
						+ " ("
						+ ((DciSummaryTableDescriptor) e2).getMenuPath()
								.replace("&", "") + ")";
