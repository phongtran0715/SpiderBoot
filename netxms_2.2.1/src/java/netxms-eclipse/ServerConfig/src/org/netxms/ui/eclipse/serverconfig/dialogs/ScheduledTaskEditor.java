...
public class ScheduledTaskEditor extends Dialog {
	 ...
	 * @param scheduleType
	 */
	public ScheduledTaskEditor(Shell shell, ScheduledTask task,
			List<String> scheduleType) {
		super(shell);
		if (task != null)
			scheduledTask = task;
		else
	...
	protected Control createDialogArea(Composite parent) {
		...
		int taskId = scheduleTypeList.indexOf(scheduledTask
				.getScheduledTaskId());
		...
		textComments = WidgetHelper.createLabeledText(dialogArea, SWT.MULTI
	...
	protected void okPressed() {
		...
		if ((scheduledTask.getFlags() & ScheduledTask.SYSTEM) == 0) {
			scheduledTask.setScheduledTaskId(scheduleTypeList.get(scheduleType
					.getSelectionIndex()));
