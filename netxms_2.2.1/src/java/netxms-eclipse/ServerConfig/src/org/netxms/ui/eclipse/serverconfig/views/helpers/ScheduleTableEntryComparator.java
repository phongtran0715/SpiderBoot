...
public class ScheduleTableEntryComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		case ScheduledTaskView.CALLBACK_ID:
			result = task1.getScheduledTaskId().compareToIgnoreCase(
					task2.getScheduledTaskId());
			break;
		case ScheduledTaskView.OBJECT:
			AbstractObject obj1 = ((NXCSession) ConsoleSharedData.getSession())
					.findObjectById(task1.getObjectId());
			AbstractObject obj2 = ((NXCSession) ConsoleSharedData.getSession())
					.findObjectById(task2.getObjectId());
		...
		case ScheduledTaskView.PARAMETERS:
			result = task1.getParameters().compareToIgnoreCase(
					task2.getParameters());
			break;
		case ScheduledTaskView.EXECUTION_TIME:
			if (task1.getSchedule().isEmpty()) {
				if (task2.getSchedule().isEmpty()) {
					result = task1.getExecutionTime().compareTo(
							task2.getExecutionTime());
					break;
				} else {
					result = 1;
					break;
			...
			result = task1.getSchedule().compareToIgnoreCase(
					task2.getSchedule());
			break;
		case ScheduledTaskView.LAST_EXECUTION_TIME:
			result = task1.getLastExecutionTime().compareTo(
					task2.getLastExecutionTime());
			break;
		...
		case ScheduledTaskView.OWNER:
			...
			if ((task1.getFlags() & ScheduledTask.SYSTEM) > 0)
				...
				user1 = ((NXCSession) ConsoleSharedData.getSession())
						.findUserDBObjectById(task1.getOwner()).getName();

			if ((task2.getFlags() & ScheduledTask.SYSTEM) > 0)
				user2 = "Internal";
			else
				user2 = ((NXCSession) ConsoleSharedData.getSession())
						.findUserDBObjectById(task2.getOwner()).getName();

			result = user1.compareTo(user2);
			break;
		case ScheduledTaskView.COMMENTS:
			result = task1.getComments().compareToIgnoreCase(
					task2.getComments());
			break;
		default:
			result = 0;
