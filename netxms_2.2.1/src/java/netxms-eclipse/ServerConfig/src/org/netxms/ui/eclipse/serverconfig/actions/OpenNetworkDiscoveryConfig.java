 ...
 */
public class OpenNetworkDiscoveryConfig implements
	...
	public void run(IAction action) {
		...
		new ConsoleJob(Messages.get().OpenNetworkDiscoveryConfig_JobName, null,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				runInUIThread(new Runnable() {
					...
					public void run() {
						try {
							NetworkDiscoveryConfigurator view = (NetworkDiscoveryConfigurator) window
									.getActivePage().showView(
											NetworkDiscoveryConfigurator.ID);
							view.setConfig(config);
						} catch (PartInitException e) {
							MessageDialogHelper
									.openError(
											window.getShell(),
											Messages.get().OpenNetworkDiscoveryConfig_Error,
											Messages.get().OpenNetworkDiscoveryConfig_ErrorOpeningView
													+ e.getMessage());
						}
					}
				});
