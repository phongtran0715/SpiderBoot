...
public class ScheduleTableEntryLabelProvider extends LabelProvider implements
	...
	public ScheduleTableEntryLabelProvider() {
		...
		statusImages[EXECUTED] = Activator.getImageDescriptor(
				"icons/active.gif").createImage(); //$NON-NLS-1$
		statusImages[PENDING] = Activator.getImageDescriptor(
				"icons/pending.gif").createImage(); //$NON-NLS-1$
		statusImages[DISBALED] = Activator.getImageDescriptor(
				"icons/disabled.gif").createImage(); //$NON-NLS-1$
	}
	...
	public Image getColumnImage(Object element, int columnIndex) {
		...
		case ScheduledTaskView.SCHEDULE_ID:
			...
			if ((task.getFlags() & ScheduledTask.EXECUTED) != 0
					|| (task.getFlags() & ScheduledTask.RUNNING) != 0)
		...
		case ScheduledTaskView.OBJECT:
			...
			AbstractObject object = session
					.findObjectById(((ScheduledTask) element).getObjectId());
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		case ScheduledTaskView.OBJECT:
			...
			return (object != null) ? object.getObjectName()
					: ("[" + Long.toString(task.getObjectId()) + "]"); //$NON-NLS-1$ //$NON-NLS-2$
		case ScheduledTaskView.PARAMETERS:
			return task.getParameters();
		case ScheduledTaskView.EXECUTION_TIME:
			return task.getSchedule().isEmpty() ? RegionalSettings
					.getDateTimeFormat().format(task.getExecutionTime()) : task
					.getSchedule();
		case ScheduledTaskView.LAST_EXECUTION_TIME:
			return (task.getLastExecutionTime().getTime() == 0) ? ""
		...
		case ScheduledTaskView.OWNER:
			if ((task.getFlags() & ScheduledTask.SYSTEM) != 0)
				return "system";
			AbstractUserObject user = session.findUserDBObjectById(task
					.getOwner());
			return (user != null) ? user.getName()
					: ("[" + Long.toString(task.getOwner()) + "]"); //$NON-NLS-1$ //$NON-NLS-2$
