...
public class SyslogParserConfigurator extends ViewPart implements ISaveablePart {
	...
	private void createActions() {
		...
		actionSave = new Action(Messages.get().SyslogParserConfigurator_Save,
				SharedIcons.SAVE) {
			@Override
			public void run() {
				save();
	...
	public void doSave(IProgressMonitor monitor) {
		...
		try {
			...
			MessageDialogHelper
					.openError(
							getSite().getShell(),
							Messages.get().SyslogParserConfigurator_Error,
							String.format(
									Messages.get().SyslogParserConfigurator_ErrorSaveConfig,
	...
	private void refresh() {
		if (modified) {
			if (!MessageDialogHelper.openQuestion(getSite().getShell(),
					Messages.get().SyslogParserConfigurator_ConfirmRefresh,
					Messages.get().SyslogParserConfigurator_ConfirmRefreshText))
				return;
		}

		actionSave.setEnabled(false);
		new ConsoleJob(Messages.get().SyslogParserConfigurator_LoadJobName,
	...
	private void save() {
		...
		actionSave.setEnabled(false);
		new ConsoleJob(Messages.get().SyslogParserConfigurator_SaveJobName,
				this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
