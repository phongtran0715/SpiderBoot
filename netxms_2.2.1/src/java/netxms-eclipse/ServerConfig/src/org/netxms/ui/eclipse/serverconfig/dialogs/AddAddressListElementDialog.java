...
public class AddAddressListElementDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		radioRange
				.setText(Messages.get().AddAddressListElementDialog_AddrRange);
	...
	protected void okPressed() {
		try {
			if (radioSubnet.getSelection()) {
				InetAddress baseAddress = InetAddress.getByName(textAddr1
						.getText().trim());
				...
				element = new InetAddressListElement(
						InetAddress.getByName(textAddr1.getText().trim()),
						InetAddress.getByName(textAddr2.getText().trim()));
			}
		} catch (UnknownHostException e) {
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().AddAddressListElementDialog_Warning,
					Messages.get().AddAddressListElementDialog_EnterValidData);
			return;
