...
public class ImportConfiguration implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		final ConfigurationImportDialog dlg = new ConfigurationImportDialog(
				shell);
		if (dlg.open() == Window.OK) {
			final NXCSession session = (NXCSession) ConsoleSharedData
					.getSession();
			ConsoleJob job = new ConsoleJob(
					Messages.get().ImportConfiguration_JobName, null,
					...
					runInUIThread(new Runnable() {
						...
						public void run() {
							MessageDialogHelper
									.openInformation(
											shell,
											Messages.get().ImportConfiguration_Information,
											String.format(
													Messages.get().ImportConfiguration_SuccessMessage,
													dlg.getFileName()));
						}
					});
				}
