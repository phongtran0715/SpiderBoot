...
public class RepositorySelectionDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		viewer = new TableViewer(dialogArea, SWT.BORDER | SWT.SINGLE
				| SWT.FULL_SELECTION);
		...
		viewer.setComparator(new ViewerComparator() {
			...
			public int compare(Viewer viewer, Object e1, Object e2) {
				return ((Repository) e1).getDescription().compareToIgnoreCase(
						((Repository) e2).getDescription());
			}
		});
	...
	protected void okPressed() {
		...
		if (selection.size() != 1) {
			MessageDialogHelper.openWarning(getShell(), "Warning",
					"Plese select repository from the list then press OK");
			return;
		}
