...
public class ZoneSelectionDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		zoneList = new SortableTableViewer(dialogArea, names, widths, 0,
				SWT.UP, SWT.BORDER | SWT.FULL_SELECTION | SWT.SINGLE
						| SWT.V_SCROLL);
	...
	protected void okPressed() {
		final IStructuredSelection selection = (IStructuredSelection) zoneList
				.getSelection();
		if (selection.size() > 0) {
			zone = (Zone) selection.getFirstElement();
		} else {
			MessageDialogHelper.openWarning(getShell(), "Warning",
					"Please select zone object");
			return;
		}
		super.okPressed();
	}
