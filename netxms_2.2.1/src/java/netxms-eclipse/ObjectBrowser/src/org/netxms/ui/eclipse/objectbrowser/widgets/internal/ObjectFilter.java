...
public class ObjectFilter extends ViewerFilter {
	 ...
	 * Constructor
	 */
	public ObjectFilter(long[] rootObjects, AbstractObject[] sourceObjects,
			Set<Integer> classFilter) {
		if (rootObjects != null) {
			this.rootObjects = new long[rootObjects.length];
			System.arraycopy(rootObjects, 0, this.rootObjects, 0,
					rootObjects.length);
		}
		this.sourceObjects = (sourceObjects != null) ? Arrays
				.asList(sourceObjects) : null;
		...
		case IP_ADDRESS:
			if (object instanceof AbstractNode) {
				if (!((AbstractNode) object).getPrimaryIP().isValidAddress())
					return false;
				return ((AbstractNode) object).getPrimaryIP().getHostAddress()
						.startsWith(filterString);
			} else if (object instanceof Subnet) {
				return ((Subnet) object).getSubnetAddress().getHostAddress()
						.startsWith(filterString);
				...
				if (!((AccessPoint) object).getIpAddress().isValidAddress())
					return false;
				return ((AccessPoint) object).getIpAddress().getHostAddress()
						.startsWith(filterString);
			}
			return false;
		case NAME:
			return usePatternMatching ? Glob.matchIgnoreCase(filterString,
	...
	public boolean select(Viewer viewer, Object parentElement, Object element) {
		if (classFilter != null) {
			if (!classFilter.contains(((AbstractObject) element)
					.getObjectClass()))
				return false;
		}

		if (hideUnmanaged
				&& (((AbstractObject) element).getStatus() == ObjectStatus.UNMANAGED))
			return false;

		if (hideTemplateChecks && (element instanceof ServiceCheck)
				&& ((ServiceCheck) element).isTemplate())
			return false;

		if (hideSubInterfaces && (element instanceof Interface)
				&& (((Interface) element).getParentInterfaceId() != 0))
			return false;

		if (objectList == null)
			return true;

		boolean pass = objectList.containsKey(((AbstractObject) element)
				.getObjectId());
	...
	public void setFilterString(final String filterString) {
		...
		if ((filterString != null) && !filterString.isEmpty()) {
			if (filterString.charAt(0) == '/') {
				mode = COMMENTS;
				String newFilterString = filterString.substring(1)
						.toLowerCase();
				if ((this.filterString != null)
						&& newFilterString.startsWith(this.filterString))
					fullSearch = false;
	...
	private void updateObjectList(boolean doFullSearch) {
		if (filterString != null) {
			if (doFullSearch) {
				List<AbstractObject> fullList = (sourceObjects != null) ? sourceObjects
						: ConsoleSharedData.getSession().getAllObjects();
				objectList = new HashMap<Long, AbstractObject>();
				for (AbstractObject o : fullList)
					if (matchFilterString(o)
							&& ((rootObjects == null) || o
									.isChildOf(rootObjects))) {
	...
	/**
	 * @param hideTemplateChecks
	 *            the hideTemplateChecks to set
	 */
