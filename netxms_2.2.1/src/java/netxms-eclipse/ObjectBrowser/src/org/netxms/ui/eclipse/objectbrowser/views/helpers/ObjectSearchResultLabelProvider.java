 ...
 */
public class ObjectSearchResultLabelProvider extends LabelProvider implements
		ITableLabelProvider {
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		case ObjectFinder.COL_IP_ADDRESS:
			if (object instanceof AbstractNode) {
				...
				return addr.isValidUnicastAddress() ? addr.getHostAddress()
						: null;
			}
		...
		case ObjectFinder.COL_ZONE:
			if (object instanceof AbstractNode) {
				...
				return (zone != null) ? zone.getObjectName() + " ("
						+ Long.toString(zoneId) + ")" : Long.toString(zoneId);
	...
	private static String getParentNames(AbstractObject object) {
		...
		for (AbstractObject o : object.getParentsAsArray()) {
			if ((o instanceof AbstractNode) || (o instanceof Container)
					|| (o instanceof Rack) || (o instanceof Cluster)) {
				if (sb.length() > 0)
					sb.append(", ");
