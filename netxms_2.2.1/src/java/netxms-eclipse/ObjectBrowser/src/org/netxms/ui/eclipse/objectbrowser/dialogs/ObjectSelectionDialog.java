...
public class ObjectSelectionDialog extends Dialog {
	...
	public static Set<Integer> createNodeSelectionFilter(
		...
		return ObjectSelectionFilterFactory.getInstance()
				.createNodeSelectionFilter(allowMobileDevices);
	}

	/**
	 * Create filter for zone selection - it allows only zone and entire network
	 * obejcts.
	 * 
	...
	public static Set<Integer> createNetworkMapSelectionFilter() {
		return ObjectSelectionFilterFactory.getInstance()
				.createNetworkMapSelectionFilter();
	}
	...
	/**
	 * Create filter for template selection - it allows template objects and
	 * possible parents - template groups.
	 * 
	 * @return Class filter for node selection
	 */
	public static Set<Integer> createTemplateSelectionFilter() {
		return ObjectSelectionFilterFactory.getInstance()
				.createTemplateSelectionFilter();
	}
	...
	public static Set<Integer> createNodeAndTemplateSelectionFilter(
			boolean allowMobileDevices) {
		return ObjectSelectionFilterFactory.getInstance()
				.createNodeAndTemplateSelectionFilter(allowMobileDevices);
	}
	...
	public static Set<Integer> createDashboardAndNetworkMapSelectionFilter() {
		return ObjectSelectionFilterFactory.getInstance()
				.createDashboardAndNetworkMapSelectionFilter();
	}
	 ...
	 *            parent shell
	 * @param rootObjects
	 *            list of root objects (set to null to show entire object tree)
	 * @param classFilter
	 *            set of allowed object classes (set to null to show all
	 *            classes)
	 */
	 ...
	 *            classes)
	 * @param currentObject
	 *            object list selected for move (the same object can't be
	 *            selected as a move terget)
	 */
	public ObjectSelectionDialog(Shell parentShell, long[] rootObjects,
			Set<Integer> classFilter, List<Object> currentObject) {
		super(parentShell);
		setShellStyle(getShellStyle() | SWT.RESIZE);
		this.rootObjects = rootObjects;
		this.classFilter = classFilter;
	...
	protected void configureShell(Shell newShell) {
		...
		try {
			newShell.setSize(
					settings.getInt("SelectObject.cx"), settings.getInt("SelectObject.cy")); //$NON-NLS-1$ //$NON-NLS-2$
	...
	protected Control createDialogArea(Composite parent) {
		...
		objectTree = new ObjectTree(dialogArea, SWT.NONE,
				multiSelection ? ObjectTree.MULTI : 0, rootObjects,
				classFilter, showFilterToolTip, showFilterCloseButton);
		...
		if (multiSelection) {
			...
			if (objectId != 0) {
				...
				for (int i = 0; i < currentObject.size(); i++) {
					if (((AbstractObject) currentObject.get(i)).getObjectId() == objectId) {
						MessageDialogHelper
								.openWarning(
										getShell(),
										Messages.get().ObjectSelectionDialog_Warning,
				...
				MessageDialogHelper.openWarning(getShell(),
						Messages.get().ObjectSelectionDialog_Warning,
						Messages.get().ObjectSelectionDialog_WarningText);
				return;
			}
		}
	...
	public AbstractObject[] getSelectedObjects(
		...
		final Set<AbstractObject> resultSet = new HashSet<AbstractObject>(
	...
	/**
	 * Enable or disable selection of multiple objects. If multiselection is
	 * enabled, object tree will appear with check boxes, and objects can be
	 * selected by checking them.
	 * 
	 * @param enable
