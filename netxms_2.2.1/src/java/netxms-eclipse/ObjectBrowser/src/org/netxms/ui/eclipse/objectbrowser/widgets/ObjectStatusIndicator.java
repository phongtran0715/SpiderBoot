...
public class ObjectStatusIndicator extends Canvas implements PaintListener {
	...
	public ObjectStatusIndicator(Composite parent, int style) {
		...
		addDisposeListener(new DisposeListener() {
			...
			public void widgetDisposed(DisposeEvent e) {
				...
				ps.setValue(
						"ObjectStatusIndicator.hideUnmanaged", hideUnmanaged); //$NON-NLS-1$
	...
	private void createActions() {
		actionShowIcons = new Action(
				Messages.get().ObjectStatusIndicator_ShowIcons,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionHideDisabled = new Action(
				Messages.get().ObjectStatusIndicator_HideDisabled,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionHideNormal = new Action(
				Messages.get().ObjectStatusIndicator_HideNormal,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionHideUnknown = new Action(
				Messages.get().ObjectStatusIndicator_HideUnknown,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionHideUnmanaged = new Action(
				Messages.get().ObjectStatusIndicator_HideUnmanaged,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
	...
	public void paintControl(PaintEvent e) {
		...
		gc.setForeground(SharedColors.getColor(SharedColors.BORDER,
				getDisplay()));
	 ...
	 * @param y
	 */
	private void drawObject(GC gc, AbstractObject object, int y, int width,
			int height) {
		...
		if (showIcons) {
			gc.drawImage(StatusDisplayInfo.getStatusImage(status),
					(width - 16) / 2, y + (height - 16) / 2);
		} else {
