...
public class ObjectBrowser extends ViewPart {
	...
	private List<OpenHandlerData> openHandlers = new ArrayList<OpenHandlerData>(
			0);
	...
	public void init(IViewSite site, IMemento memento) throws PartInitException {
		...
		if (memento != null) {
			initHideUnmanaged = safeCast(
					memento.getBoolean("ObjectBrowser.hideUnmanaged"), false); //$NON-NLS-1$
			initHideTemplateChecks = safeCast(
					memento.getBoolean("ObjectBrowser.hideTemplateChecks"), false); //$NON-NLS-1$
			initHideSubInterfaces = safeCast(
					memento.getBoolean("ObjectBrowser.hideSubInterfaces"), false); //$NON-NLS-1$
			initShowStatus = safeCast(
					memento.getBoolean("ObjectBrowser.showStatusIndicator"), false); //$NON-NLS-1$
			initialObjectSelection = memento
					.getString("ObjectBrowser.selectedObject"); //$NON-NLS-1$
		}
		registerOpenHandlers();
		registerActionValidators();

		IDialogSettings settings = Activator.getDefault().getDialogSettings();
		initShowFilter = safeCast(settings.get("ObjectBrowser.showFilter"),
				settings.getBoolean("ObjectBrowser.showFilter"), initShowFilter);
	}
	...
	public void saveState(IMemento memento) {
		...
		memento.putBoolean(
				"ObjectBrowser.hideUnmanaged", objectTree.isHideUnmanaged()); //$NON-NLS-1$
		memento.putBoolean(
				"ObjectBrowser.hideTemplateChecks", objectTree.isHideTemplateChecks()); //$NON-NLS-1$
		memento.putBoolean(
				"ObjectBrowser.hideSubInterfaces", objectTree.isHideSubInterfaces()); //$NON-NLS-1$
		memento.putBoolean(
				"ObjectBrowser.showStatusIndicator", objectTree.isStatusIndicatorEnabled()); //$NON-NLS-1$
	...
	public void createPartControl(Composite parent) {
		...
		Object value = ConsoleSharedData
				.getProperty("ObjectBrowser.rootObjects"); //$NON-NLS-1$
		if ((value != null) && (value instanceof long[])) {
			rootObjects = (long[]) value;
		}

		objectTree = new ObjectTree(parent, SWT.NONE, ObjectTree.MULTI,
				rootObjects, null, true, true);
		...
		objectTree.getTreeViewer().addSelectionChangedListener(
				new ISelectionChangedListener() {
					@Override
					public void selectionChanged(SelectionChangedEvent event) {
						int size = ((IStructuredSelection) objectTree
								.getTreeViewer().getSelection()).size();
		...
		TreeViewerEditor.create(tree, new ColumnViewerEditorActivationStrategy(
				tree) {
			@Override
		...
		tree.setCellModifier(new ICellModifier() {
			...
			public void modify(Object element, String property, Object value) {
				final Object data = (element instanceof Item) ? ((Item) element)
						.getData() : element;
				...
				if (property.equals("name")) //$NON-NLS-1$
				{
					if (data instanceof AbstractObject) {
						...
						new ConsoleJob(
								Messages.get().ObjectBrowser_RenameJobName,
								null, Activator.PLUGIN_ID, null) {
							@Override
							protected void runInternal(IProgressMonitor monitor)
									throws Exception {
								session.setObjectName(
										((AbstractObject) data).getObjectId(),
										newName);
							}
							...
							protected String getErrorMessage() {
								return String.format(
										Messages.get().ObjectBrowser_RenameJobError,
	...
	private void saveSelection(IMemento memento) {
		ITreeSelection selection = (ITreeSelection) objectTree.getTreeViewer()
				.getSelection();
		if (selection.size() == 1) {
	...
	private void restoreSelection() {
		if ((initialObjectSelection == null)
				|| initialObjectSelection.isEmpty()
				|| !initialObjectSelection.startsWith("/")) //$NON-NLS-1$
			return;
		...
		objectTree.getTreeViewer().setSelection(
				new TreeSelection(new TreePath(elements)), true);
		...
		display.asyncExec(new Runnable() {
			...
			public void run() {
				...
				CommandBridge
						.getInstance()
						.execute(
								"TabbedObjectView/changeObject", ((AbstractObject) elements[elements.length - 1]).getObjectId()); //$NON-NLS-1$
			}
		});
	}
	 ...
	 * 
	 * @param objectId
	 *            ID of object to be selected
	 * @param tabId
	 *            tab ID to be selected or null if tab selection not needed
	 */
	public void setSelection(long objectId, String tabId) {
		...
		if (object != null) {
			...
			CommandBridge.getInstance().execute(
					"TabbedObjectView/changeObject", objectId); //$NON-NLS-1$
			if (tabId != null) {
				CommandBridge.getInstance().execute(
						"TabbedObjectView/selectTab", tabId); //$NON-NLS-1$
			}
		}
	}
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.objectbrowser.context.ObjectBrowser"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionMoveObject
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.moveObject"); //$NON-NLS-1$

		actionMoveTemplate = new Action(
				Messages.get().ObjectBrowser_MoveTemplate) {
			@Override
			public void run() {
		...
		actionMoveTemplate
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.moveTemplate"); //$NON-NLS-1$

		actionMoveBusinessService = new Action(
				Messages.get().ObjectBrowser_MoveService) {
			@Override
			public void run() {
				moveObject(SubtreeType.BUSINESS_SERVICES);
			}
		};
		actionMoveBusinessService
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.moveBusinessService"); //$NON-NLS-1$

		actionMoveDashboard = new Action(
				Messages.get().ObjectBrowser_MoveDashboard) {
			@Override
			public void run() {
		...
		actionMoveDashboard
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.moveDashboard"); //$NON-NLS-1$
		...
		actionMoveMap
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.moveMap"); //$NON-NLS-1$
		...
		actionMovePolicy
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.movePolicy"); //$NON-NLS-1$

		actionHideUnmanaged = new Action(
				Messages.get().ObjectBrowser_HideUnmanaged, Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionHideUnmanaged
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.hideUnmanaged"); //$NON-NLS-1$
		actionHideUnmanaged.setChecked(objectTree.isHideUnmanaged());

		actionHideTemplateChecks = new Action(
				Messages.get().ObjectBrowser_HideCheckTemplates,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
				objectTree.setHideTemplateChecks(actionHideTemplateChecks
						.isChecked());
			}
		};
		actionHideTemplateChecks
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.hideTemplateChecks"); //$NON-NLS-1$
		actionHideTemplateChecks.setChecked(objectTree.isHideTemplateChecks());

		actionHideSubInterfaces = new Action(
				Messages.get().ObjectBrowser_HideSubInterfaces,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
				objectTree.setHideSubInterfaces(actionHideSubInterfaces
						.isChecked());
			}
		};
		actionHideSubInterfaces
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.hideSubInterfaces"); //$NON-NLS-1$
		actionHideSubInterfaces.setChecked(objectTree.isHideSubInterfaces());

		actionShowFilter = new Action(Messages.get().ObjectBrowser_ShowFilter,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionShowFilter
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.showFilter"); //$NON-NLS-1$
		actionShowFilter.setChecked(objectTree.isFilterEnabled());
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.objectbrowser.commands.show_object_filter"); //$NON-NLS-1$
		final ActionHandler showFilterHandler = new ActionHandler(
				actionShowFilter);
		handlerService.activateHandler(
				actionShowFilter.getActionDefinitionId(), showFilterHandler);

		actionShowStatusIndicator = new Action(
				Messages.get().ObjectBrowser_ShowStatusIndicator,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
				objectTree.enableStatusIndicator(actionShowStatusIndicator
						.isChecked());
			}
		};
		actionShowStatusIndicator
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.showStatusIndicator"); //$NON-NLS-1$
		actionShowStatusIndicator.setChecked(objectTree
				.isStatusIndicatorEnabled());
		actionShowStatusIndicator
				.setActionDefinitionId("org.netxms.ui.eclipse.objectbrowser.commands.show_status_indicator"); //$NON-NLS-1$
		final ActionHandler showStatusIndicatorHandler = new ActionHandler(
				actionShowStatusIndicator);
		handlerService.activateHandler(
				actionShowStatusIndicator.getActionDefinitionId(),
				showStatusIndicatorHandler);

		actionRenameObject = new Action(Messages.get().ObjectBrowser_Rename) {
			@Override
			public void run() {
				TreeItem[] selection = objectTree.getTreeControl()
						.getSelection();
				if (selection.length != 1)
					return;
				objectTree.getTreeViewer().editElement(selection[0].getData(),
						0);
			}
		};
		actionRenameObject
				.setId("org.netxms.ui.eclipse.objectbrowser.actions.rename"); //$NON-NLS-1$
		actionRenameObject
				.setActionDefinitionId("org.netxms.ui.eclipse.objectbrowser.commands.rename_object"); //$NON-NLS-1$
		final ActionHandler renameObjectHandler = new ActionHandler(
				actionRenameObject);
		handlerService
				.activateHandler(actionRenameObject.getActionDefinitionId(),
						renameObjectHandler);
	}
	...
	private void createMenu() {
		...
		FilteringMenuManager
				.add(manager, actionShowFilter, Activator.PLUGIN_ID);
		FilteringMenuManager.add(manager, actionShowStatusIndicator,
				Activator.PLUGIN_ID);
		FilteringMenuManager.add(manager, actionHideSubInterfaces,
				Activator.PLUGIN_ID);
		FilteringMenuManager.add(manager, actionHideUnmanaged,
				Activator.PLUGIN_ID);
		FilteringMenuManager.add(manager, actionHideTemplateChecks,
				Activator.PLUGIN_ID);
	...
	protected void fillContextMenu(IMenuManager manager) {
		...
		if (isValidSelectionForMove(SubtreeType.INFRASTRUCTURE))
			manager.insertAfter(GroupMarkers.MB_OBJECT_BINDING,
					actionMoveObject);
	...
	public boolean isValidSelectionForMove(SubtreeType subtree) {
		...
		for (int i = 0; i < selection.length; i++) {
			if (!isValidObjectForMove(selection, i, subtree)
					|| ((AbstractObject) selection[0].getParentItem().getData())
							.getObjectId() != ((AbstractObject) selection[i]
							.getParentItem().getData()).getObjectId())
				return false;
		}
		return true;
	}
	 ...
	 */
	public boolean isValidObjectForMove(TreeItem[] selection, int i,
			SubtreeType subtree) {
		if (selection[i].getParentItem() == null)
			return false;

		final AbstractObject currentObject = (AbstractObject) selection[i]
				.getData();
		final AbstractObject parentObject = (AbstractObject) selection[i]
				.getParentItem().getData();

		for (ObjectActionValidator v : actionValidators) {
			int result = v.isValidSelectionForMove(subtree, currentObject,
					parentObject);
	...
	private void moveObject(SubtreeType subtree) {
		...
		case BUSINESS_SERVICES:
			filter = ObjectSelectionDialog
					.createBusinessServiceSelectionFilter();
			break;
		case DASHBOARDS:
			filter = ObjectSelectionDialog
					.createDashboardGroupSelectionFilter();
			break;
		case MAPS:
			filter = ObjectSelectionDialog
					.createNetworkMapGroupsSelectionFilter();
			break;
		case POLICIES:
		...
		ObjectSelectionDialog dlg = new ObjectSelectionDialog(getSite()
				.getShell(), null, filter, currentObject);
		dlg.enableMultiSelection(false);
		if (dlg.open() == Window.OK) {
			final AbstractObject target = dlg.getSelectedObjects().get(0);
			for (int i = 0; i < selection.length; i++) {
				performObjectMove(target, parentObject.get(i),
						currentObject.get(i));
			}
		}
	}

	public void performObjectMove(final AbstractObject target,
			final Object parentObject, final Object currentObject) {
		if (target.getObjectId() != ((AbstractObject) parentObject)
				.getObjectId()) {
			final NXCSession session = (NXCSession) ConsoleSharedData
					.getSession();

			new ConsoleJob(Messages.get().ObjectBrowser_MoveJob_Title
					+ ((AbstractObject) currentObject).getObjectName(), this,
					Activator.PLUGIN_ID, null) {
				@Override
				protected void runInternal(IProgressMonitor monitor)
						throws Exception {
					long objectId = ((AbstractObject) currentObject)
							.getObjectId();
					session.bindObject(target.getObjectId(), objectId);
					session.unbindObject(
							((AbstractObject) parentObject).getObjectId(),
							objectId);
				}

				@Override
				protected String getErrorMessage() {
					return Messages.get().ObjectBrowser_MoveJob_Error
							+ ((AbstractObject) currentObject).getObjectName();
				}
			}.start();
		}
	}
	...
	private void registerOpenHandlers() {
		...
		IConfigurationElement[] elements = reg
				.getConfigurationElementsFor("org.netxms.ui.eclipse.objectbrowser.objectOpenHandlers"); //$NON-NLS-1$
		for (int i = 0; i < elements.length; i++) {
			try {
				final OpenHandlerData h = new OpenHandlerData();
				h.handler = (ObjectOpenHandler) elements[i]
	...
	private void registerActionValidators() {
		...
		final IExtensionRegistry reg = Platform.getExtensionRegistry();
		IConfigurationElement[] elements = reg
				.getConfigurationElementsFor("org.netxms.ui.eclipse.objectbrowser.objectActionValidators"); //$NON-NLS-1$
		for (int i = 0; i < elements.length; i++) {
			try {
				final ActionValidatorData v = new ActionValidatorData();
				v.validator = (ObjectActionValidator) elements[i]
						.createExecutableExtension("class"); //$NON-NLS-1$
		...
		Collections.sort(list, new Comparator<ActionValidatorData>() {
			@Override
			public int compare(ActionValidatorData arg0,
					ActionValidatorData arg1) {
				return arg0.priority - arg1.priority;
			}
		});
		...
		actionValidators[i] = new ObjectActionValidator() {
			@Override
			public int isValidSelectionForMove(SubtreeType subtree,
					AbstractObject currentObject, AbstractObject parentObject) {
				switch (subtree) {
				case INFRASTRUCTURE:
					return ((currentObject instanceof Node)
							...
							|| (currentObject instanceof MobileDevice) || (currentObject instanceof Container))
							&& ((parentObject instanceof Container) || (parentObject instanceof ServiceRoot)) ? APPROVE
							: REJECT;
				case TEMPLATES:
					return ((currentObject instanceof Template) || (currentObject instanceof TemplateGroup))
							&& ((parentObject instanceof TemplateGroup) || (parentObject instanceof TemplateRoot)) ? APPROVE
							: REJECT;
				case BUSINESS_SERVICES:
					return (currentObject instanceof BusinessService)
							&& ((parentObject instanceof BusinessService) || (parentObject instanceof BusinessServiceRoot)) ? APPROVE
							: REJECT;
				case MAPS:
					return ((currentObject instanceof NetworkMap) || (currentObject instanceof NetworkMapGroup))
							&& ((parentObject instanceof NetworkMapGroup) || (parentObject instanceof NetworkMapRoot)) ? APPROVE
							: REJECT;
				case DASHBOARDS:
					return (((currentObject instanceof Dashboard) || (currentObject instanceof DashboardGroup)) && ((parentObject instanceof DashboardRoot)
							|| (parentObject instanceof DashboardGroup) || (parentObject instanceof Dashboard))) ? APPROVE
							: REJECT;
				case POLICIES:
					return ((currentObject instanceof AgentPolicy) || (currentObject instanceof PolicyGroup))
							&& ((parentObject instanceof PolicyGroup) || (parentObject instanceof PolicyRoot)) ? APPROVE
							: REJECT;
				default:
					return REJECT;
				}
