...
public class ActionComparator extends ViewerComparator {
	...
	public int compare(Viewer viewer, Object e1, Object e2) {
		...
		case ActionManager.COLUMN_NAME:
			rc = ((ServerAction) e1).getName().compareToIgnoreCase(
					((ServerAction) e2).getName());
			break;
		case ActionManager.COLUMN_TYPE:
			rc = ((ServerAction) e1).getType() - ((ServerAction) e2).getType();
			break;
		case ActionManager.COLUMN_RCPT:
			rc = ((ServerAction) e1).getRecipientAddress().compareToIgnoreCase(
					((ServerAction) e2).getRecipientAddress());
			break;
		case ActionManager.COLUMN_SUBJ:
			rc = ((ServerAction) e1).getEmailSubject().compareToIgnoreCase(
					((ServerAction) e2).getEmailSubject());
			break;
		case ActionManager.COLUMN_DATA:
			rc = ((ServerAction) e1).getData().compareToIgnoreCase(
					((ServerAction) e2).getData());
			break;
		default:
			rc = 0;
			break;
