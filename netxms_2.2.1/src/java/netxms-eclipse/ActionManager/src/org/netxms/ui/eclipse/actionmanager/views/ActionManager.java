...
public class ActionManager extends ViewPart implements SessionListener {
	...
	public void createPartControl(Composite parent) {
		...
		final String[] columnNames = { Messages.get().ActionManager_ColumnName,
				Messages.get().ActionManager_ColumnType,
				Messages.get().ActionManager_ColumnRcpt,
				Messages.get().ActionManager_ColumnSubj,
				Messages.get().ActionManager_ColumnData };
	...
	private void refreshActionList() {
		new ConsoleJob(Messages.get().ActionManager_LoadJobName, this,
	...
	private void createAction() {
		...
		final EditActionDlg dlg = new EditActionDlg(getSite().getShell(),
				action, true);
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().ActionManager_CreateJobName, this,
					Activator.PLUGIN_ID, Activator.PLUGIN_ID) {
				@Override
				protected String getErrorMessage() {
	...
	private void editAction() {
		...
		final EditActionDlg dlg = new EditActionDlg(getSite().getShell(),
				action, false);
		if (dlg.open() == Window.OK) {
			new ConsoleJob(Messages.get().ActionManager_UpdateJobName, this,
					Activator.PLUGIN_ID, Activator.PLUGIN_ID) {
				@Override
				protected String getErrorMessage() {
	...
	private void deleteActions() {
		...
		if (!MessageDialogHelper.openConfirm(getSite().getShell(),
				Messages.get().ActionManager_Confirmation,
				Messages.get().ActionManager_ConfirmDelete))
			return;

		final Object[] objects = selection.toArray();
		new ConsoleJob(Messages.get().ActionManager_DeleteJobName, this,
				Activator.PLUGIN_ID, Activator.PLUGIN_ID) {
			@Override
			protected String getErrorMessage() {
	...
	private void updateActionsList() {
		viewer.getControl().getDisplay().asyncExec(new Runnable() {
			...
			public void run() {
				synchronized (ActionManager.this.actions) {
					viewer.setInput(ActionManager.this.actions.values()
							.toArray());
				}
			}
		});
	 ...
	 * 
	 * @see
	 * org.netxms.api.client.ISessionListener#notificationHandler(org.netxms
