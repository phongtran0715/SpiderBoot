...
public class EditPolicy implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		if (currentSelection != null) {
			if (currentSelection instanceof AgentPolicyConfig) {
				try {
					ConfigPolicyEditor view = (ConfigPolicyEditor) window
							.getActivePage().showView(ConfigPolicyEditor.ID);
				...
				try {
					LogParserPolicyEditor view = (LogParserPolicyEditor) window
							.getActivePage().showView(LogParserPolicyEditor.ID);
					view.setPolicy(currentSelection);
				} catch (PartInitException e) {
					MessageDialogHelper.openError(window.getShell(), "", "");
				}
	...
	public void selectionChanged(IAction action, ISelection selection) {
		...
		if (selection instanceof TreeSelection) {
			currentSelection = (AbstractObject) ((TreeSelection) selection)
					.getFirstElement();
			action.setEnabled((currentSelection instanceof AgentPolicyConfig)
					|| (currentSelection instanceof AgentPolicyLogParser));
		}
	}
}
