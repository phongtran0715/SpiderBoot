...
public class SelectInstallTargetDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		radioInstallOnCurrent
				.setText(Messages.get().SelectInstallTargetDialog_InstallOnAlreadyInstalled);
		...
		radioInstallOnSelected
				.setText(Messages.get().SelectInstallTargetDialog_InstallOnSelected);
		...
		Object value = ConsoleSharedData
				.getProperty("PolicyManager.rootObjects"); //$NON-NLS-1$
		if ((value != null) && (value instanceof long[])) {
			rootObjects = (long[]) value;
		}

		objectTree = new ObjectTree(dialogArea, SWT.BORDER,
				ObjectTree.CHECKBOXES, rootObjects,
				ObjectSelectionDialog.createNodeSelectionFilter(false), true,
				false);
	...
	protected void okPressed() {
		installMode = radioInstallOnCurrent.getSelection() ? INSTALL_ON_CURRENT
				: INSTALL_ON_SELECTED;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		List<AbstractObject> objects = session.findMultipleObjects(
				objectTree.getCheckedObjects(), false);
