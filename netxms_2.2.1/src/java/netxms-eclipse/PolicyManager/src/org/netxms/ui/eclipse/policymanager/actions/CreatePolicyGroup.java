...
public class CreatePolicyGroup implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateObjectDialog dlg = new CreateObjectDialog(targetPart
				.getSite().getShell(),
				Messages.get().CreatePolicyGroup_PolicyGroup);
		if (dlg.open() == Window.OK) {
			final NXCSession session = (NXCSession) ConsoleSharedData
					.getSession();
			new ConsoleJob(Messages.get().CreatePolicyGroup_JobName,
					targetPart, Activator.PLUGIN_ID, null) {
				@Override
				protected void runInternal(IProgressMonitor monitor)
						throws Exception {
					NXCObjectCreationData cd = new NXCObjectCreationData(
							AbstractObject.OBJECT_POLICYGROUP,
							dlg.getObjectName(), currentObject.getObjectId());
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if (selection instanceof TreeSelection) {
			currentObject = (AbstractObject) ((TreeSelection) selection)
					.getFirstElement();
			action.setEnabled((currentObject instanceof PolicyRoot)
