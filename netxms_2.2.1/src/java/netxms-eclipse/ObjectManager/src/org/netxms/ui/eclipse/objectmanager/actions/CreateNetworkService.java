...
public class CreateNetworkService implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateNetworkServiceDialog dlg = new CreateNetworkServiceDialog(
				window.getShell());
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateNetworkService_JobTitle, part,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_NETWORKSERVICE, dlg.getName(),
						parentId);
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().CreateNetworkService_JobError,
						dlg.getName());
			}
		}.start();
	}
