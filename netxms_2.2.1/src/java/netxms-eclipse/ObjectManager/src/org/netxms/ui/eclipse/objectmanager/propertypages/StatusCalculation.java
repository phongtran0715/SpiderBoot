...
public class StatusCalculation extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		currentState.setStatusCalculationMethod(object
				.getStatusCalculationMethod());
		currentState.setStatusPropagationMethod(object
				.getStatusPropagationMethod());
		currentState
				.setFixedPropagatedStatus(object.getFixedPropagatedStatus());
		currentState.setStatusShift(object.getStatusShift());
		currentState.setStatusTransformation(object.getStatusTransformation());
		currentState
				.setStatusSingleThreshold(object.getStatusSingleThreshold());
		...
		createCalcSelectionListener(radioCalcDefault,
				AbstractObject.CALCULATE_DEFAULT);
		createCalcSelectionListener(radioCalcMostCritical,
				AbstractObject.CALCULATE_MOST_CRITICAL);
		createCalcSelectionListener(radioCalcSingle,
				AbstractObject.CALCULATE_SINGLE_THRESHOLD);
		createCalcSelectionListener(radioCalcMultiple,
				AbstractObject.CALCULATE_MULTIPLE_THRESHOLDS);

		createPropSelectionListener(radioPropDefault,
				AbstractObject.PROPAGATE_DEFAULT);
		createPropSelectionListener(radioPropUnchanged,
				AbstractObject.PROPAGATE_UNCHANGED);
		createPropSelectionListener(radioPropFixed,
				AbstractObject.PROPAGATE_FIXED);
		createPropSelectionListener(radioPropRelative,
				AbstractObject.PROPAGATE_RELATIVE);
		createPropSelectionListener(radioPropTranslated,
				AbstractObject.PROPAGATE_TRANSLATED);
	...
	private void createLeftPanel(Composite parent) {
		...
		radioPropDefault
				.setSelection(object.getStatusPropagationMethod() == AbstractObject.PROPAGATE_DEFAULT);

		radioPropUnchanged = new Button(group, SWT.RADIO);
		radioPropUnchanged.setText(Messages.get().StatusCalculation_Unchanged);
		radioPropUnchanged
				.setSelection(object.getStatusPropagationMethod() == AbstractObject.PROPAGATE_UNCHANGED);

		radioPropFixed = new Button(group, SWT.RADIO);
		radioPropFixed.setText(Messages.get().StatusCalculation_FixedTo);
		radioPropFixed
				.setSelection(object.getStatusPropagationMethod() == AbstractObject.PROPAGATE_FIXED);
		...
		radioPropRelative
				.setSelection(object.getStatusPropagationMethod() == AbstractObject.PROPAGATE_RELATIVE);
		...
		radioPropTranslated
				.setText(Messages.get().StatusCalculation_SeverityBased);
		radioPropTranslated
				.setSelection(object.getStatusPropagationMethod() == AbstractObject.PROPAGATE_TRANSLATED);
		...
		for (int i = 0; i < 4; i++) {
			...
			comboTranslatedStatus[i] = new Combo(translationGroup, SWT.BORDER
					| SWT.READ_ONLY);
			gd = new GridData();
			gd.horizontalAlignment = SWT.FILL;
			gd.grabExcessHorizontalSpace = true;
			...
			for (int j = 0; j < 5; j++)
				comboTranslatedStatus[i]
						.add(StatusDisplayInfo.getStatusText(j));
	...
	private void createRightPanel(Composite parent) {
		...
		radioCalcDefault
				.setSelection(object.getStatusCalculationMethod() == AbstractObject.CALCULATE_DEFAULT);

		radioCalcMostCritical = new Button(group, SWT.RADIO);
		radioCalcMostCritical
				.setText(Messages.get().StatusCalculation_MostCritical);
		radioCalcMostCritical
				.setSelection(object.getStatusCalculationMethod() == AbstractObject.CALCULATE_MOST_CRITICAL);

		radioCalcSingle = new Button(group, SWT.RADIO);
		radioCalcSingle
				.setText(Messages.get().StatusCalculation_SingleThreshold);
		radioCalcSingle
				.setSelection(object.getStatusCalculationMethod() == AbstractObject.CALCULATE_SINGLE_THRESHOLD);
		...
		textSingleThreshold.setText(Integer.toString(object
				.getStatusSingleThreshold()));

		radioCalcMultiple = new Button(group, SWT.RADIO);
		radioCalcMultiple
				.setText(Messages.get().StatusCalculation_MultipleThresholds);
		radioCalcMultiple
				.setSelection(object.getStatusCalculationMethod() == AbstractObject.CALCULATE_MULTIPLE_THRESHOLDS);
		...
		for (int i = 0; i < 4; i++) {
			...
			textThresholds[i].setText(Integer.toString(object
					.getStatusThresholds()[i]));
		}
	}
	...
	private void changeCalculationMethod(int method) {
		...
		textSingleThreshold
				.setEnabled(method == AbstractObject.CALCULATE_SINGLE_THRESHOLD);
		for (int i = 0; i < 4; i++)
			textThresholds[i]
					.setEnabled(method == AbstractObject.CALCULATE_MULTIPLE_THRESHOLDS);
	}
	...
	private void changePropagationMethod(int method) {
		...
		textRelativeStatus
				.setEnabled(method == AbstractObject.PROPAGATE_RELATIVE);
		for (int i = 0; i < 4; i++)
			comboTranslatedStatus[i]
					.setEnabled(method == AbstractObject.PROPAGATE_TRANSLATED);
	}
	...
	private boolean hasChanges(NXCObjectModificationData md) {
		if ((currentState.getStatusCalculationMethod() != md
				.getStatusCalculationMethod())
				|| (currentState.getStatusPropagationMethod() != md
						.getStatusPropagationMethod()))
			return true;

		switch (md.getStatusCalculationMethod()) {
		case AbstractObject.CALCULATE_SINGLE_THRESHOLD:
			if (currentState.getStatusSingleThreshold() != md
					.getStatusSingleThreshold())
				return true;
			break;
		case AbstractObject.CALCULATE_MULTIPLE_THRESHOLDS:
			if (!Arrays.equals(currentState.getStatusThresholds(),
					md.getStatusThresholds()))
				return true;
			break;
		default:
			break;
		...
		case AbstractObject.PROPAGATE_FIXED:
			if (currentState.getFixedPropagatedStatus() != md
		...
		case AbstractObject.PROPAGATE_TRANSLATED:
			if (!Arrays.equals(currentState.getStatusTransformation(),
					md.getStatusTransformation()))
				return true;
			break;
		default:
			break;
	...
	protected boolean applyChanges(final boolean isApply) {
		if (!WidgetHelper.validateTextInput(textRelativeStatus,
				Messages.get().StatusCalculation_Validate_RelativeStatus,
				new NumericTextFieldValidator(-4, 4), this)
				|| !WidgetHelper
						.validateTextInput(
								textSingleThreshold,
								Messages.get().StatusCalculation_Validate_SingleThreshold,
								new NumericTextFieldValidator(0, 100), this)
				|| !WidgetHelper
						.validateTextInput(
								textThresholds[0],
								String.format(
										Messages.get().StatusCalculation_Validate_Threshold,
										StatusDisplayInfo
												.getStatusText(Severity.WARNING)),
								new NumericTextFieldValidator(0, 100), this)
				|| !WidgetHelper
						.validateTextInput(
								textThresholds[1],
				...
				|| !WidgetHelper
						.validateTextInput(
								...
								String.format(
										Messages.get().StatusCalculation_Validate_Threshold,
										StatusDisplayInfo
												.getStatusText(Severity.CRITICAL)),
								new NumericTextFieldValidator(0, 100), this))
			return false;
		...
		md.setFixedPropagatedStatus(ObjectStatus.getByValue(comboFixedStatus
				.getSelectionIndex()));
		md.setStatusShift(Integer.parseInt(textRelativeStatus.getText()));
		ObjectStatus[] transformations = new ObjectStatus[4];
		for (int i = 0; i < 4; i++)
			transformations[i] = ObjectStatus
					.getByValue(comboTranslatedStatus[i].getSelectionIndex());
		md.setStatusTransformation(transformations);
		md.setStatusSingleThreshold(Integer.parseInt(textSingleThreshold
				.getText()));
		...
		new ConsoleJob(String.format(Messages.get().StatusCalculation_JobName,
