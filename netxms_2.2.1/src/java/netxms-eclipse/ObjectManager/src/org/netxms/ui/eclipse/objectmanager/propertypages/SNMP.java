...
public class SNMP extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		snmpVersion = WidgetHelper.createLabeledCombo(dialogArea, SWT.BORDER
				| SWT.READ_ONLY, Messages.get().Communication_Version, fd);
		...
		snmpAuth = WidgetHelper.createLabeledCombo(dialogArea, SWT.BORDER
				| SWT.READ_ONLY, Messages.get().Communication_Authentication,
				fd);
		...
		fd.top = new FormAttachment(snmpVersion.getParent(), 0, SWT.BOTTOM);
		snmpPriv = WidgetHelper.createLabeledCombo(dialogArea, SWT.BORDER
				| SWT.READ_ONLY, Messages.get().Communication_Encryption, fd);
		...
		snmpAuthName
				.setLabel(node.getSnmpVersion() == AbstractNode.SNMP_VERSION_3 ? Messages
						.get().Communication_UserName
		...
		snmpAuthPassword.getTextControl().setEnabled(
				node.getSnmpVersion() == AbstractNode.SNMP_VERSION_3);
		...
		snmpPrivPassword.getTextControl().setEnabled(
				node.getSnmpVersion() == AbstractNode.SNMP_VERSION_3);
	...
	private int snmpIndexToVersion(int index) {
		final int[] versions = { AbstractNode.SNMP_VERSION_1,
				AbstractNode.SNMP_VERSION_2C, AbstractNode.SNMP_VERSION_3 };
	...
	private void onSnmpVersionChange() {
		...
		snmpAuthName.setLabel(isV3 ? Messages.get().Communication_UserName
				: Messages.get().Communication_Community);
	...
	protected boolean applyChanges(final boolean isApply) {
		...
		try {
			...
			MessageDialog.openWarning(getShell(),
					...
					Messages.get().Communication_WarningInvalidSNMPPort);
			if (isApply)
				setValid(true);
			return false;
		}
		...
		new ConsoleJob(String.format("Updating SNMP settings for node %s",
			...
			protected String getErrorMessage() {
				return String.format("Cannot update SNMP settings for node %s",
						node.getObjectName());
			}
