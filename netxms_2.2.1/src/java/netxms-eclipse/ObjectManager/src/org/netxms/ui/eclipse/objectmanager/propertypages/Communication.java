...
public class Communication extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		agentIsRemote
				.setSelection((node.getFlags() & AbstractNode.NF_REMOTE_AGENT) != 0);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
	...
	protected boolean applyChanges(final boolean isApply) {
					...
					.matches("^(([A-Za-z0-9\\-]+\\.)*[A-Za-z0-9\\-]+|[A-Fa-f0-9:]+)$")) //$NON-NLS-1$
			{
				MessageDialogHelper
						.openWarning(
								...
								String.format(
										Messages.get().Communication_WarningInvalidHostname,
										hostName));
				return false;
			}
		...
		new ConsoleJob(String.format(Messages.get().Communication_JobName,
