...
public class CreateRack implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateObjectDialog dlg = new CreateObjectDialog(
				window.getShell(), Messages.get().CreateRack_Rack);
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateRack_JobTitle, part,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_RACK, dlg.getObjectName(),
						parentId);
				cd.setHeight(42);
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().CreateRack_JobError,
						dlg.getObjectName());
			}
		}.start();
	}
