...
public class CreateMobileDevice implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateMobileDeviceDialog dlg = new CreateMobileDeviceDialog(
				window.getShell());
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateMobileDevice_JobTitle, part,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_MOBILEDEVICE, dlg.getName(),
						parentId);
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().CreateMobileDevice_JobError,
						dlg.getName());
			}
		}.start();
	}
