...
public class RemoveClusterNode implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final ChildObjectListDialog dlg = new ChildObjectListDialog(shell,
				clusterId,
				ChildObjectListDialog
						.createClassFilter(AbstractObject.OBJECT_NODE));
		dlg.open();
		if (dlg.getReturnCode() == Window.OK) {
			final NXCSession session = (NXCSession) ConsoleSharedData
					.getSession();
			new ConsoleJob(Messages.get().RemoveClusterNode_JobTitle, viewPart,
				...
				protected void runInternal(IProgressMonitor monitor)
					...
					for (int i = 0; i < objects.size(); i++)
						session.removeClusterNode(clusterId, objects.get(i)
								.getObjectId());
				}
			}.start();
		}
