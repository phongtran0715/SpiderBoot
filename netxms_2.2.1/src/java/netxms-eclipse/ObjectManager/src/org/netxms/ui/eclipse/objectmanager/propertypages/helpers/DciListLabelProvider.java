 ...
 */
public class DciListLabelProvider extends LabelProvider implements
		ITableLabelProvider {
	private static final String[] functions = {
			"last()", "average(", "deviation(", "diff()", "error(", "sum(" }; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$ //$NON-NLS-5$ //$NON-NLS-6$
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		case ConditionData.COLUMN_NODE:
			...
			return (object != null) ? object.getObjectName()
					: ("[" + Long.toString(dci.getNodeId()) + "]"); //$NON-NLS-1$ //$NON-NLS-2$
		case ConditionData.COLUMN_METRIC:
			String name = dciNameCache.get(new NodeItemPair(dci.getNodeId(),
					dci.getDciId()));
			return (name != null) ? name
					: Messages.get().DciListLabelProvider_Unresolved;
	...
	public void resolveDciNames(final Collection<ConditionDciInfo> dciList) {
		new ConsoleJob(Messages.get().DciListLabelProvider_JobName, null,
				...
				getDisplay().asyncExec(new Runnable() {
					...
					public void run() {
						...
						for (ConditionDciInfo dci : dciList) {
							dciNameCache.put(new NodeItemPair(dci.getNodeId(),
									dci.getDciId()), names[i++]);
						}
					}
				});
			}
