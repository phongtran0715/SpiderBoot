...
public class CreateNetworkServiceDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		serviceTypeField = WidgetHelper.createLabeledCombo(dialogArea,
				SWT.READ_ONLY,
				Messages.get().CreateNetworkServiceDialog_ServiceType, gd);
		serviceTypeField
				.add(Messages.get().CreateNetworkServiceDialog_TypeUserDef);
		serviceTypeField.add(Messages.get().CreateNetworkServiceDialog_TypeSSH);
		serviceTypeField
				.add(Messages.get().CreateNetworkServiceDialog_TypePOP3);
		serviceTypeField
				.add(Messages.get().CreateNetworkServiceDialog_TypeSMTP);
		serviceTypeField.add(Messages.get().CreateNetworkServiceDialog_TypeFTP);
		serviceTypeField
				.add(Messages.get().CreateNetworkServiceDialog_TypeHTTP);
		serviceTypeField
				.add(Messages.get().CreateNetworkServiceDialog_TypeHTTPS);
		serviceTypeField
				.add(Messages.get().CreateNetworkServiceDialog_TypeTelnet);
		...
		requestField
				.setLabel(Messages.get().CreateNetworkServiceDialog_Request);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		gd.horizontalSpan = 2;
		...
		responseField
				.setLabel(Messages.get().CreateNetworkServiceDialog_Response);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		gd.horizontalSpan = 2;
		...
		checkCreateDci
				.setText(Messages.get().CreateNetworkServiceDialog_CreateStatusDCI);
		checkCreateDci.setSelection(settings
				.getBoolean("CreateNetworkServiceDialog.checkCreateDci")); //$NON-NLS-1$
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		gd.horizontalSpan = 2;
	...
	protected void okPressed() {
		...
		if (name.isEmpty()) {
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().CreateNetworkServiceDialog_Warning,
					Messages.get().CreateNetworkServiceDialog_WarningEmptyName);
			return;
		}
		...
		try {
			...
			MessageDialogHelper
					.openWarning(
							getShell(),
							Messages.get().CreateNetworkServiceDialog_Warning,
							Messages.get().CreateNetworkServiceDialog_WarningInvalidPort);
			return;
		}
