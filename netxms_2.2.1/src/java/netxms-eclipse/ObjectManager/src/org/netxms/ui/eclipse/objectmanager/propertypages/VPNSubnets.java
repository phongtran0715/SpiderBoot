...
public class VPNSubnets extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		// networks lists
		localNetworksElements = new ArrayList<InetAddressEx>(
				connector.getLocalSubnets());
		remoteNetworksElements = new ArrayList<InetAddressEx>(
				connector.getRemoteSubnets());
		createNetworkList(clientArea, Messages.get().VPNSubnets_LocalNetworks,
				localNetworksList, localNetworksElements);
		createNetworkList(clientArea, Messages.get().VPNSubnets_RemoteNetworks,
				remoteNetworksList, remoteNetworksElements);
	 ...
	 * @param dialogArea
	 * @param viewList
	 *            viewer to be created and added to view
	 * @param data
	 *            elements that should be added as a content of this viewer
	 */
	private void createNetworkList(Composite dialogArea, String title,
			TableViewer viewList, final List<InetAddressEx> data) {
		...
		viewList = new TableViewer(clientArea, SWT.BORDER | SWT.MULTI
				| SWT.FULL_SELECTION);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		...
		final ImageHyperlink linkRemove = new ImageHyperlink(clientArea,
				SWT.NONE);
	 ...
	 */
	private void addTargetAddressListElement(TableViewer elementList,
			List<InetAddressEx> elements) {
	 ...
	 */
	private void removeTargetAddressListElements(TableViewer elementList,
			List<InetAddressEx> elements) {
		IStructuredSelection selection = (IStructuredSelection) elementList
				.getSelection();
		if (selection.size() > 0) {
