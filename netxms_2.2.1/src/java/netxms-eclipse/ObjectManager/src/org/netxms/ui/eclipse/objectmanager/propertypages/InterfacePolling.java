...
public class InterfacePolling extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		pollCount = WidgetHelper.createLabeledSpinner(dialogArea, SWT.BORDER,
				Messages.get().InterfacePolling_RequiredPollCount, 0, 1000,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		pollCount.setSelection(object.getRequiredPollCount());

		expectedState = WidgetHelper.createLabeledCombo(dialogArea, SWT.BORDER
				| SWT.READ_ONLY, Messages.get().InterfacePolling_ExpectedState,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		...
		checkExcludeFromTopology
				.setText(Messages.get().InterfacePolling_ExcludeFromTopology);
		checkExcludeFromTopology.setSelection(object.isExcludedFromTopology());
		checkExcludeFromTopology.setLayoutData(new GridData(SWT.FILL,
	...
	protected void applyChanges(final boolean isApply) {
		...
		final NXCObjectModificationData data = new NXCObjectModificationData(
				object.getObjectId());
		data.setExpectedState(expectedState.getSelectionIndex());
		data.setRequiredPolls(pollCount.getSelection());
		data.setObjectFlags(
				checkExcludeFromTopology.getSelection() ? Interface.IF_EXCLUDE_FROM_TOPOLOGY
						: 0, Interface.IF_EXCLUDE_FROM_TOPOLOGY);
		new ConsoleJob(Messages.get().InterfacePolling_JobName, null,
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().InterfacePolling_JobError,
