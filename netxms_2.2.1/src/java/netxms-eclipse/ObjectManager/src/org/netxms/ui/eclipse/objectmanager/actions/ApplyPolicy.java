...
public class ApplyPolicy implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final ObjectSelectionDialog dlg = new ObjectSelectionDialog(shell,
				null, ObjectSelectionDialog.createPolicySelectionFilter());
		...
		if (dlg.open() == Window.OK) {
			...
			new ConsoleJob("Deploy agent policy", null, Activator.PLUGIN_ID,
					null) {
				...
				protected void runInternal(IProgressMonitor monitor)
					...
					for (int i = 0; i < selectedObjects.size(); i++) {
						for (int n = 0; n < dlg.getSelectedObjects().size(); n++) {
							session.deployAgentPolicy(dlg.getSelectedObjects()
									.get(n).getObjectId(),
									selectedObjects.get(i).getObjectId());
						}
					}
				}
