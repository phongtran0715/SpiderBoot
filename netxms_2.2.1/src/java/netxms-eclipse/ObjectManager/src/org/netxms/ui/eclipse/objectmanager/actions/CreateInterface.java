...
import org.netxms.ui.eclipse.objectmanager.dialogs.CreateInterfaceDialog;
...
public class CreateInterface implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateInterfaceDialog dlg = new CreateInterfaceDialog(
				window.getShell());
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateInterface_JobTitle, part,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_INTERFACE, dlg.getName(),
						parentId);
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().CreateInterface_JobError,
						dlg.getName());
			}
		}.start();
	}
