...
public class NodePollerView extends ViewPart {
	...
	private static final Color COLOR_ERROR = new Color(Display.getCurrent(),
			192, 0, 0);
	private static final Color COLOR_WARNING = new Color(Display.getCurrent(),
			255, 128, 0);
	private static final Color COLOR_INFO = new Color(Display.getCurrent(), 0,
			128, 0);
	private static final Color COLOR_LOCAL = new Color(Display.getCurrent(), 0,
			0, 192);
	...
	public void init(IViewSite site) throws PartInitException {
		...
		node = ((obj != null) && (obj instanceof AbstractNode)) ? (AbstractNode) obj
				: null;
		if (node == null)
			throw new PartInitException(
					Messages.get().NodePollerView_InvalidObjectID);
		pollType = NodePollType.valueOf(parts[1]);

		setPartName(POLL_NAME[pollType.getValue()]
				+ " - " + node.getObjectName()); //$NON-NLS-1$
	}
	...
	public void createPartControl(Composite parent) {
		textArea = new StyledText(parent, SWT.MULTI | SWT.H_SCROLL
				| SWT.V_SCROLL);
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.objectmanager.contexts.NodePollerView"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionRestart = new Action(Messages.get().NodePollerView_ActionRestart,
				SharedIcons.RESTART) {
			@Override
			public void run() {
				startPoll();
		...
		actionRestart
				.setActionDefinitionId("org.netxms.ui.eclipse.objectmanager.commands.restart_poller"); //$NON-NLS-1$
		handlerService.activateHandler(actionRestart.getActionDefinitionId(),
				new ActionHandler(actionRestart));

		actionClearOutput = new Action(
				Messages.get().NodePollerView_ActionClear,
		...
		actionClearOutput
				.setActionDefinitionId("org.netxms.ui.eclipse.objectmanager.commands.clear_output"); //$NON-NLS-1$
	...
	public void startPoll() {
		...
		Job job = new Job(String.format(Messages.get().NodePollerView_JobName,
				node.getObjectName(), node.getObjectId())) {
			@Override
		...
		display.asyncExec(new Runnable() {
			...
			public void run() {
				...
				if (success) {
					...
					addPollerMessage(String.format(
							"\u007FePOLL ERROR: %s", errorMessage)); //$NON-NLS-1$
