...
public abstract class AbstractNodePoll implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		if (msg != null) {
			if (!MessageDialogHelper.openQuestion(window.getShell(),
					Messages.get().AbstractNodePoll_Warning, msg))
				return;
		}

		try {
			NodePollerView view = (NodePollerView) window
					.getActivePage()
					.showView(
							NodePollerView.ID,
							Long.toString(node.getObjectId())
									+ "&" + getPollType(), IWorkbenchPage.VIEW_ACTIVATE); //$NON-NLS-1$
			view.startPoll();
		} catch (PartInitException e) {
			MessageDialogHelper.openError(
					window.getShell(),
					Messages.get().AbstractNodePoll_Error,
					String.format(Messages.get().AbstractNodePoll_ErrorText,
