...
public class CreateCluster implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		final CreateObjectDialog dlg = new CreateObjectDialog(
				window.getShell(), Messages.get().CreateCluster_Cluster);
		if (dlg.open() != Window.OK)
			return;

		final NXCSession session = (NXCSession) ConsoleSharedData.getSession();
		new ConsoleJob(Messages.get().CreateCluster_JobTitle, part,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				NXCObjectCreationData cd = new NXCObjectCreationData(
						AbstractObject.OBJECT_CLUSTER, dlg.getObjectName(),
						parentId);
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().CreateCluster_JobError,
						dlg.getObjectName());
			}
		}.start();
	}
