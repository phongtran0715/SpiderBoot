...
public class ConditionData extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		final String[] columnNames = {
				Messages.get().ConditionData_ColPosition,
				Messages.get().ConditionData_ColNode,
				Messages.get().ConditionData_ColParameter,
				Messages.get().ConditionData_ColFunction };
	...
	private void addItem() {
		...
		if (dlg.open() == Window.OK) {
			...
			for (DciValue item : selection) {
				ConditionDciInfo dci = new ConditionDciInfo(item.getNodeId(),
						item.getId(), item.getDcObjectType(), Threshold.F_LAST,
						1);
	...
	private void editItem() {
		...
		ConditionDciEditDialog dlg = new ConditionDciEditDialog(getShell(),
				dci, labelProvider.getColumnText(dci, COLUMN_NODE),
				labelProvider.getColumnText(dci, COLUMN_METRIC));
		if (dlg.open() == Window.OK) {
	...
	protected void applyChanges(final boolean isApply) {
		...
		new ConsoleJob(Messages.get().ConditionData_JobName, null,
	...
	private void moveUp() {
		final IStructuredSelection selection = (IStructuredSelection) viewer
				.getSelection();
		if (selection.size() == 1) {
			ConditionDciInfo element = (ConditionDciInfo) selection
					.getFirstElement();
