...
public class NodePolling extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		pollerNode
				.setEmptySelectionName(Messages.get().NodePolling_EmptySelectionServer);
		...
		if (object instanceof AbstractNode) {
			...
			pollCount.setSelection(((AbstractNode) object)
					.getRequredPollCount());
		}
		...
		addFlag(optionsGroup, AbstractNode.NF_DISABLE_NXCP,
				Messages.get().NodePolling_OptDisableAgent);
		addFlag(optionsGroup, AbstractNode.NF_DISABLE_SNMP,
				Messages.get().NodePolling_OptDisableSNMP);
		addFlag(optionsGroup, AbstractNode.NF_DISABLE_ICMP,
				Messages.get().NodePolling_OptDisableICMP);
		addFlag(optionsGroup, AbstractNode.NF_DISABLE_STATUS_POLL,
				Messages.get().NodePolling_OptDisableStatusPoll);
		addFlag(optionsGroup, AbstractNode.NF_DISABLE_CONF_POLL,
				Messages.get().NodePolling_OptDisableConfigPoll);
		addFlag(optionsGroup, AbstractNode.NF_DISABLE_ROUTE_POLL,
				Messages.get().NodePolling_OptDisableRTPoll);
		addFlag(optionsGroup, AbstractNode.NF_DISABLE_TOPOLOGY_POLL,
				Messages.get().NodePolling_OptDisableTopoPoll);
		addFlag(optionsGroup, AbstractNode.NF_DISABLE_DISCOVERY_POLL,
				Messages.get().NodePolling_OptDisableDiscoveryPoll);
		addFlag(optionsGroup, AbstractNode.NF_DISABLE_DATA_COLLECT,
				Messages.get().NodePolling_OptDisableDataCollection);
		...
		radioIfXTableDefault
				.setSelection(object.getIfXTablePolicy() == AbstractNode.IFXTABLE_DEFAULT);

		radioIfXTableEnable = new Button(ifXTableGroup, SWT.RADIO);
		radioIfXTableEnable.setText(Messages.get().NodePolling_Enable);
		radioIfXTableEnable
				.setSelection(object.getIfXTablePolicy() == AbstractNode.IFXTABLE_ENABLED);

		radioIfXTableDisable = new Button(ifXTableGroup, SWT.RADIO);
		radioIfXTableDisable.setText(Messages.get().NodePolling_Disable);
		radioIfXTableDisable
				.setSelection(object.getIfXTablePolicy() == AbstractNode.IFXTABLE_DISABLED);
	...
	protected boolean applyChanges(final boolean isApply) {
		...
		new ConsoleJob(Messages.get().NodePolling_JobName, null,
