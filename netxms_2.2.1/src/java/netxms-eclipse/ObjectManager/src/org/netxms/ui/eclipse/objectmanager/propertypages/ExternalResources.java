...
public class ExternalResources extends PropertyPage {
	 ...
	 */
	private void addUrl() {
		ObjectUrlEditDialog dlg = new ObjectUrlEditDialog(getShell(), null,
				null);
	...
	private void editUrl() {
		...
		ObjectUrlEditDialog dlg = new ObjectUrlEditDialog(getShell(),
				url.getUrl(), url.getDescription());
		if (dlg.open() != Window.OK)
			return;

		int index = urls.indexOf(url);
		urls.set(index,
				new ObjectUrl(url.getId(), dlg.getUrl(), dlg.getDescription()));
		viewer.refresh();
	...
	private void moveSelection(boolean up) {
		...
		urls.set(
				index,
				new ObjectUrl(index, swapUrl.getUrl(), swapUrl.getDescription()));
		urls.set(swapIndex,
				new ObjectUrl(index, url.getUrl(), url.getDescription()));
