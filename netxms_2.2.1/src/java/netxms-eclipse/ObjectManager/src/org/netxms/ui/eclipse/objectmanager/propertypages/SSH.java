...
public class SSH extends PropertyPage {
	...
	protected Control createContents(Composite parent) {
		...
		sshPassword.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false));
		...
		sshProxy.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true, false,
				2, 1));
	...
	protected boolean applyChanges(final boolean isApply) {
		...
		new ConsoleJob(String.format("Updating SSH settings for node %s",
			...
			protected String getErrorMessage() {
				return String.format("Cannot update SSH settings for node %s",
						node.getObjectName());
			}
