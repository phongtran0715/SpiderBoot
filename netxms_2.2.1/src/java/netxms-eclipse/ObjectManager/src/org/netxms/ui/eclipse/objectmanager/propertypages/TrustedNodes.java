...
public class TrustedNodes extends PropertyPage {
	...
	private HashMap<Long, AbstractObject> trustedNodes = new HashMap<Long, AbstractObject>(
			0);
	...
	protected Control createContents(Composite parent) {
		...
		viewer.setComparator(new ObjectLabelComparator((ILabelProvider) viewer
				.getLabelProvider()));
		...
		addButton.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				ObjectSelectionDialog dlg = new ObjectSelectionDialog(
						getShell(), null, ObjectSelectionDialog
								.createNodeSelectionFilter(true));
				if (dlg.open() == Window.OK) {
					AbstractObject[] nodes = dlg
	...
	protected void applyChanges(final boolean isApply) {
		...
		new ConsoleJob(String.format(Messages.get().TrustedNodes_JobName,
