...
public class CreateNodeDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		agentPortField = WidgetHelper.createLabeledSpinner(dialogArea,
				SWT.BORDER, Messages.get().CreateNodeDialog_AgentPort, 1,
				65535, WidgetHelper.DEFAULT_LAYOUT_DATA);
		agentPortField.setSelection(agentPort);

		snmpPortField = WidgetHelper.createLabeledSpinner(dialogArea,
				SWT.BORDER, Messages.get().CreateNodeDialog_SNMPPort, 1, 65535,
				WidgetHelper.DEFAULT_LAYOUT_DATA);
		snmpPortField.setSelection(snmpPort);

		sshLoginField = new LabeledText(dialogArea, SWT.NONE);
		sshLoginField.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false));
		sshLoginField.setLabel("SSH Login");
		sshLoginField.setText(sshLogin);

		sshPasswordField = new LabeledText(dialogArea, SWT.NONE);
		sshPasswordField.setLayoutData(new GridData(SWT.FILL, SWT.CENTER, true,
				false));
		...
		if (enableShowAgainFlag) {
			...
			checkCreateAnother
					.setText(Messages.get().CreateNodeDialog_ShowAgain);
	...
	protected void okPressed() {
		...
		if (!hostName
				.matches("^(([A-Za-z0-9\\-]+\\.)*[A-Za-z0-9\\-]+|[A-Fa-f0-9:]+)$")) //$NON-NLS-1$
		{
			MessageDialogHelper
					.openWarning(
							getShell(),
							Messages.get().CreateNodeDialog_Warning,
							String.format(
									Messages.get().CreateNodeDialog_WarningInvalidHostname,
									hostName));
			return;
		}
		...
		showAgain = enableShowAgainFlag ? checkCreateAnother.getSelection()
				: false;
		super.okPressed();
	}
	...
	/**
	 * @param enableShowAgainFlag
	 *            the enableShowAgainFlag to set
	 */
