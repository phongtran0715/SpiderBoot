...
public class Dot1xStatusView extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		setPartName(String.format(Messages.get().Dot1xStatusView_PartName,
	...
	public void createPartControl(Composite parent) {
		final String[] names = { Messages.get().Dot1xStatusView_ColDevice,
				Messages.get().Dot1xStatusView_ColSlotPort,
				Messages.get().Dot1xStatusView_ColInterface,
				Messages.get().Dot1xStatusView_ColPAE,
				Messages.get().Dot1xStatusView_ColBackend };
		...
		WidgetHelper.restoreTableViewerSettings(viewer, Activator.getDefault()
				.getDialogSettings(), "Dot1xStatusView"); //$NON-NLS-1$
		viewer.getTable().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveTableViewerSettings(viewer, Activator
						.getDefault().getDialogSettings(), "Dot1xStatusView"); //$NON-NLS-1$
			}
		});
	 ...
	 * @param rootObject2
	 */
	private void fillPortList(List<Dot1xPortSummary> portList,
			Set<Long> nodeList, long root) {
		AbstractObject object = session.findObjectById(root);
		if (object == null)
			return;

		if ((object instanceof Node)
				&& !nodeList.contains(object.getObjectId())) {
			for (AbstractObject o : object
				...
				if ((((Interface) o).getFlags() & Interface.IF_PHYSICAL_PORT) != 0)
					portList.add(new Dot1xPortSummary((Node) object,
							(Interface) o));
			}
