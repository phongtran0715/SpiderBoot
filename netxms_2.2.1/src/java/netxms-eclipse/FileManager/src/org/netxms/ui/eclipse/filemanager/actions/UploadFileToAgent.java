...
public class UploadFileToAgent implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		final StartServerToAgentFileUploadDialog dlg = new StartServerToAgentFileUploadDialog(
				shell, canScheduleFileUpload);
		if (dlg.open() == Window.OK) {
			final Long[] nodeIdList = nodes.toArray(new Long[nodes.size()]);
			new ConsoleJob(Messages.get().UploadFileToAgent_JobTitle, viewPart,
					...
					for (ServerFile sf : dlg.getServerFiles()) {
						...
						for (int i = 0; i < nodeIdList.length; i++) {
							if (dlg.isScheduled()) {
								...
								String parameters = sf.getName() + ","
										+ remoteFileName; // $NON-NLS-1$
								task.setParameters(parameters);
								task.setObjectId(nodeIdList[i]);
								session.addSchedule(task);
							} else {
								session.uploadFileToAgent(nodeIdList[i],
										sf.getName(), remoteFileName,
										dlg.isCreateJobOnHold());
							}
						}
					}
				}
	...
	public void selectionChanged(IAction action, ISelection selection) {
		...
		while (it.hasNext()) {
			...
			if (object instanceof Node) {
				...
				Set<AbstractObject> set = ((AbstractObject) object)
