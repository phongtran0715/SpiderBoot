...
public class ServerFileManager extends ViewPart implements SessionListener {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		initShowFilter = safeCast(settings.get("ServerFileManager.showFilter"),
				settings.getBoolean("ServerFileManager.showFilter"),
				initShowFilter);
	}
	...
	public void createPartControl(Composite parent) {
		...
		final String[] columnNames = { Messages.get().ViewServerFile_FileName,
				Messages.get().ViewServerFile_FileType,
				Messages.get().ViewServerFile_FileSize,
				Messages.get().ViewServerFile_ModificationDate };
		final int[] columnWidths = { 300, 150, 300, 300 };
		viewer = new SortableTableViewer(content, columnNames, columnWidths, 0,
	...
	private void createActions() {
		...
		actionNew = new Action(
				Messages.get().ViewServerFile_UploadFileOnServerAction) {
			@Override
			public void run() {
				createFile();
			}
		...
		actionDelete = new Action(
				Messages.get().ViewServerFile_DeleteFileOnServerAction) {
			@Override
			public void run() {
				deleteFile();
			}
	...
	private void createFile() {
		...
		if (dlg.open() == Window.OK) {
			final NXCSession session = (NXCSession) ConsoleSharedData
					.getSession();
			new ConsoleJob(Messages.get().UploadFileToServer_JobTitle, null,
					Activator.PLUGIN_ID, null) {
				@Override
				protected void runInternal(final IProgressMonitor monitor)
					...
					for (int i = 0; i < fileList.size(); i++) {
						...
						session.uploadFileToServer(localFile, remoteFile,
		...
		new ConsoleJob(Messages.get().ViewServerFile_DeletFileFromServerJob,
			...
			protected void runInternal(IProgressMonitor monitor)
				...
				for (int i = 0; i < objects.length; i++) {
					session.deleteServerFile(((ServerFile) objects[i])
							.getName());
				}
			}
		}.start();
