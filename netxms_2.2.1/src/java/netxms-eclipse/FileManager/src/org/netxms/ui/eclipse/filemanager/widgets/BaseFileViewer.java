...
public class BaseFileViewer extends Composite {
	...
	public BaseFileViewer(Composite parent, int style, IViewPart viewPart) {
		...
		messageBar.setBackground(SharedColors.getColor(
				SharedColors.MESSAGE_BAR_BACKGROUND, getDisplay()));
		messageBar.setForeground(SharedColors.getColor(
		...
		messageCloseButton.setCursor(getDisplay().getSystemCursor(
				SWT.CURSOR_HAND));
		messageCloseButton.setImage(SharedIcons.IMG_CLOSE);
		messageCloseButton
				.setToolTipText(Messages.get().BaseFileViewer_HideMessage);
		gd = new GridData();
		...
		searchCloseButton.setCursor(getDisplay().getSystemCursor(
				SWT.CURSOR_HAND));
	...
	public void showFile(final File file) {
		ConsoleJob job = new ConsoleJob(
				Messages.get().BaseFileViewer_LoadJobName, viewPart,
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().BaseFileViewer_LoadJobError,
	...
	public void showMessage(int severity, String messageText) {
		...
		((FormData) text.getLayoutData()).top = new FormAttachment(messageBar,
				0, SWT.BOTTOM);
		layout(true, true);
	}
	...
	public void showSearchBar() {
		...
		((FormData) text.getLayoutData()).bottom = new FormAttachment(
				searchBar, 0, SWT.TOP);
		layout(true, true);
	...
	private void doSearch(boolean typing) {
		...
		if (index > -1) {
			...
			searchBarText.setBackground(SharedColors.getColor(
					SharedColors.ERROR_BACKGROUND, text.getDisplay()));
		}
	}
	...
	private void doReverseSearch() {
		...
		if ((text.getSelectionCount() > 0)
				&& text.getSelectionText().toLowerCase().equals(searchString)) {
			Point p = text.getSelectionRange();
