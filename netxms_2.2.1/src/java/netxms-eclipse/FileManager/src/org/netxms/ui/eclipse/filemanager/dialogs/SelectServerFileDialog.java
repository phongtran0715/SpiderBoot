...
public class SelectServerFileDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		final String[] names = { Messages.get().SelectServerFileDialog_ColName,
				Messages.get().SelectServerFileDialog_ColSize,
				Messages.get().SelectServerFileDialog_ColModTime };
		final int[] widths = { 200, 100, 150 };
		viewer = new SortableTableViewer(dialogArea, names, widths, 0,
				SWT.DOWN, SWT.FULL_SELECTION | SWT.BORDER
						| (multiSelect ? SWT.MULTI : 0));
	...
	protected void okPressed() {
		...
		if (selection.isEmpty()) {
			MessageDialogHelper.openWarning(getShell(),
					Messages.get().SelectServerFileDialog_Warning,
					Messages.get().SelectServerFileDialog_WarningText);
			return;
		}
