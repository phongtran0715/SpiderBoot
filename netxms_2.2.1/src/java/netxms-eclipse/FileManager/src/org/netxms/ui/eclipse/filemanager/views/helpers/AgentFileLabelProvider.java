 ...
 */
public class AgentFileLabelProvider extends LabelProvider implements
	...
	public String getColumnText(Object element, int columnIndex) {
		...
		case AgentFileManager.COLUMN_SIZE:
			if (((AgentFile) element).isDirectory()) {
				if (((AgentFile) element).getFileInfo() != null)
					return getValue(((AgentFile) element).getFileInfo()
							.getSize())
							+ " ("
							+ ((AgentFile) element).getFileInfo()
									.getItemCount() + " files)";
				else
					return "";
			}
		...
		case AgentFileManager.COLUMN_MODIFYED:
			return (((AgentFile) element).isPlaceholder() || ((AgentFile) element)
					.getModifyicationTime().getTime() == 0) ? "" : RegionalSettings.getDateTimeFormat().format(((AgentFile) element).getModifyicationTime()); //$NON-NLS-1$
