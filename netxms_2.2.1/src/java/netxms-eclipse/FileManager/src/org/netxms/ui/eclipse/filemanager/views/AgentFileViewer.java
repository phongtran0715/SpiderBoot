...
public class AgentFileViewer extends ViewPart {
	...
	private final NXCSession session = (NXCSession) ConsoleSharedData
			.getSession();
	...
	public void init(IViewSite site) throws PartInitException {
		...
		if ((object == null)
				|| (object.getObjectClass() != AbstractObject.OBJECT_NODE))
			throw new PartInitException(
					Messages.get().FileViewer_InvalidObjectID);
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.filemanager.context.AgentFileViewer"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionClear = new Action(Messages.get().FileViewer_ClearOutput,
		...
		actionClear
				.setActionDefinitionId("org.netxms.ui.eclipse.filemanager.commands.clear_output"); //$NON-NLS-1$
		handlerService.activateHandler(actionClear.getActionDefinitionId(),
				new ActionHandler(actionClear));

		actionScrollLock = new Action(Messages.get().FileViewer_ScrollLock,
				Action.AS_CHECK_BOX) {
			@Override
			public void run() {
		...
		actionScrollLock.setImageDescriptor(Activator
				.getImageDescriptor("icons/scroll_lock.gif")); //$NON-NLS-1$
		actionScrollLock.setChecked(false);
		actionScrollLock
				.setActionDefinitionId("org.netxms.ui.eclipse.objecttools.commands.scroll_lock"); //$NON-NLS-1$
		handlerService.activateHandler(
				actionScrollLock.getActionDefinitionId(), new ActionHandler(
						actionScrollLock));
		...
		actionCopy
				.setActionDefinitionId("org.netxms.ui.eclipse.filemanager.commands.copy"); //$NON-NLS-1$
		...
		actionSelectAll
				.setActionDefinitionId("org.netxms.ui.eclipse.filemanager.commands.select_all"); //$NON-NLS-1$
		handlerService.activateHandler(actionSelectAll.getActionDefinitionId(),
				new ActionHandler(actionSelectAll));

		actionFind = new Action(Messages.get().AgentFileViewer_Find,
				SharedIcons.FIND) {
			@Override
			public void run() {
		...
		actionFind
				.setActionDefinitionId("org.netxms.ui.eclipse.filemanager.commands.find"); //$NON-NLS-1$
		handlerService.activateHandler(actionFind.getActionDefinitionId(),
				new ActionHandler(actionFind));
	}
	...
	/**
	 * Create new file viewer view. Checks that file does not exceed allowed
	 * size. In case if file is too large asks if it should be opened partly.
	 * 
	 * @throws PartInitException
	 */
	public static boolean createView(String secondaryId, final long nodeId,
			final AgentFileData file, boolean followChanges)
			throws PartInitException {
		return createView(secondaryId, nodeId, file, followChanges, null);
	}

	/**
	 * Create new file viewer view with custom line styler. Checks that file
	 * does not exceed allowed size. In case if file is too large asks if it
	 * should be opened partly.
	 * 
	 * @throws PartInitException
	 */
	public static boolean createView(String secondaryId, final long nodeId,
			final AgentFileData file, boolean followChanges,
			BaseFileViewer.LineStyler lineStyler) throws PartInitException {
		...
		if (exceedSize
				&& !MessageDialogHelper
						.openConfirm(
								window.getShell(),
								Messages.get().AgentFileViewer_FileIsTooLarge,
								Messages.get().AgentFileViewer_FileIsTooLargeMessageText)) {
			if (followChanges) {
				final NXCSession session = (NXCSession) ConsoleSharedData
						.getSession();

				final ConsoleJob job = new ConsoleJob(
						Messages.get().FileViewer_Stop_File_Monitoring, null,
		...
		AgentFileViewer view = (AgentFileViewer) window.getActivePage()
				.showView(AgentFileViewer.ID, secondaryId,
						IWorkbenchPage.VIEW_ACTIVATE);
		view.viewer.setLineStyler(lineStyler);
		view.followChanges = followChanges;
		view.viewer.showFile(file.getFile());
		if (followChanges) {
			view.viewer.startTracking(nodeId, file.getId(),
					file.getRemoteName());
		}
		return true;
	}
