...
public class StartServerToAgentFileUploadDialog extends Dialog {
	...
	protected Control createDialogArea(Composite parent) {
		...
		fileList = new TableViewer(dialogArea, SWT.BORDER | SWT.MULTI
				| SWT.FULL_SELECTION);
		...
		fileList.setComparator(new ViewerComparator() {
			...
			public int compare(Viewer viewer, Object e1, Object e2) {
				return ((ServerFile) e1).getName().compareToIgnoreCase(
		...
		buttonAddFile.addSelectionListener(new SelectionAdapter() {
			...
			public void widgetSelected(SelectionEvent e) {
				SelectServerFileDialog dlg = new SelectServerFileDialog(
						getShell(), true);
		...
		buttonRemoveFile.addSelectionListener(new SelectionAdapter() {
			@Override
			public void widgetSelected(SelectionEvent e) {
				IStructuredSelection selection = (IStructuredSelection) fileList
						.getSelection();
		...
		textRemoteFile
				.setLabel(Messages.get().StartServerToAgentFileUploadDialog_RemoteFileName);
		gd = new GridData();
		gd.horizontalAlignment = SWT.FILL;
		gd.grabExcessHorizontalSpace = true;
		...
		checkJobOnHold
				.setText(Messages.get().StartServerToAgentFileUploadDialog_CreateJobOnHold);
		...
		if (canScheduleFileUpload) {
			...
			checkIsSchedule
					.setText(Messages.get().StartServerToAgentFileUploadDialog_ScheduleTask);
	...
	protected void okPressed() {
		...
		if (serverFiles.isEmpty()) {
			MessageDialogHelper
					.openWarning(
							getShell(),
							Messages.get().StartServerToAgentFileUploadDialog_Warning,
							Messages.get().StartServerToAgentFileUploadDialog_WarningText);
			return;
		}
