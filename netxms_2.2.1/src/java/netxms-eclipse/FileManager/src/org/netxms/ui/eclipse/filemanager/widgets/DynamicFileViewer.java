...
public class DynamicFileViewer extends BaseFileViewer {
	...
	public DynamicFileViewer(Composite parent, int style, IViewPart viewPart) {
		...
		final SessionListener sessionListener = new SessionListener() {
			...
			public void notificationHandler(SessionNotification n) {
				if ((n.getCode() == SessionNotification.FILE_MONITORING_FAILED)
						&& (n.getSubCode() == nodeId)) {
	 ...
	 * @param fileId
	 */
	public void startTracking(final long nodeId, final String fileId,
		...
		monitoringJob = new ConsoleJob(
				Messages.get().DynamicFileViewer_TrackFileChanges, null,
	...
	private void restartTracking() {
		...
		restartJob = new ConsoleJobCallingServerJob(
				Messages.get().DynamicFileViewer_RestartFileTracking, null,
				Activator.PLUGIN_ID, null) {
			...
			protected void runInternal(final IProgressMonitor monitor)
				...
				while (running) {
					try {
						final AgentFileData file = session
								.downloadFileFromAgent(nodeId, remoteFileName,
										1024, true, new ProgressListener() {
											@Override
											public void setTotalWorkAmount(
													long workTotal) {
												monitor.beginTask("Track file "
														+ remoteFileName,
														(int) workTotal);
											}
						...
						// When successfully connected - display notification to
						// client.
						runInUIThread(new Runnable() {
							@Override
							public void run() {
