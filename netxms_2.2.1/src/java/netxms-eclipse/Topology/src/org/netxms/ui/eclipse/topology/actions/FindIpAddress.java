...
public class FindIpAddress implements IWorkbenchWindowActionDelegate {
	...
	public void run(IAction action) {
		...
		new ConsoleJob(String.format(Messages.get().FindIpAddress_JobTitle, dlg
				.getIpAddress().getHostAddress()), null, Activator.PLUGIN_ID,
				null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final ConnectionPoint cp = session.findConnectionPoint(
						(int) dlg.getZoneId(), dlg.getIpAddress());
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().FindIpAddress_JobError, dlg
