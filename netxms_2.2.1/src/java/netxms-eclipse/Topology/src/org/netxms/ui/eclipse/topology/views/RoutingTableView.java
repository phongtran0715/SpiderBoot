...
public class RoutingTableView extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		setPartName(String.format(Messages.get().RoutingTableView_Title,
	...
	public void createPartControl(Composite parent) {
		final String[] names = { Messages.get().RoutingTableView_Destination,
				Messages.get().RoutingTableView_NextHop,
				Messages.get().RoutingTableView_Interface,
				Messages.get().RoutingTableView_Type };
		final int[] widths = { 180, 140, 200, 140 };
		viewer = new SortableTableViewer(parent, names, widths,
				COLUMN_DESTINATION, SWT.DOWN, SWT.FULL_SELECTION | SWT.MULTI);
		...
		WidgetHelper.restoreTableViewerSettings(viewer, Activator.getDefault()
				.getDialogSettings(), "RoutingTable"); //$NON-NLS-1$
		viewer.getTable().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveTableViewerSettings(viewer, Activator
						.getDefault().getDialogSettings(), "RoutingTable"); //$NON-NLS-1$
			}
		});
	...
	private void refresh() {
		new ConsoleJob(Messages.get().RoutingTableView_JobTitle, this,
			...
			protected String getErrorMessage() {
				return String.format(Messages.get().RoutingTableView_JobError,
						session.getObjectName(rootObject));
			}
		}.start();
	}
