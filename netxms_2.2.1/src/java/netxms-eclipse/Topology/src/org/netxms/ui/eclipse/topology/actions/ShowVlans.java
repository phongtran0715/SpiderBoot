...
public class ShowVlans implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		IViewReference vr = window.getActivePage().findViewReference(
				VlanView.ID, Long.toString(objectId));
		if (vr != null) {
			...
			final NXCSession session = (NXCSession) ConsoleSharedData
					.getSession();
			new ConsoleJob(Messages.get().ShowVlans_JobTitle, null,
				...
				protected void runInternal(IProgressMonitor monitor)
					...
					runInUIThread(new Runnable() {
						...
						public void run() {
							try {
								VlanView view = (VlanView) window
										.getActivePage().showView(VlanView.ID,
								...
								MessageDialogHelper.openError(
										window.getShell(),
										Messages.get().ShowVlans_Error,
										Messages.get().ShowVlans_CannotOpenView
