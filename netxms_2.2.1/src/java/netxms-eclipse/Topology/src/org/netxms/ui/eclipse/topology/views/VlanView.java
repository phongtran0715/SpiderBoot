...
public class VlanView extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		setPartName(String.format(
				Messages.get().VlanView_PartName,
				(object != null) ? object.getObjectName()
						: "<" + site.getSecondaryId() + ">")); //$NON-NLS-1$ //$NON-NLS-2$
	}
	...
	public void createPartControl(Composite parent) {
		...
		final String[] names = { Messages.get().VlanView_ColumnID,
				Messages.get().VlanView_ColumnName,
				Messages.get().VlanView_ColumnPorts };
		final int[] widths = { 80, 180, 400 };
		vlanList = new SortableTableViewer(parent, names, widths, 0, SWT.DOWN,
		...
		vlanList.addSelectionChangedListener(new ISelectionChangedListener() {
			...
			public void selectionChanged(SelectionChangedEvent event) {
				IStructuredSelection selection = (IStructuredSelection) vlanList
						.getSelection();
		...
		scroller = new ScrolledComposite(deviceViewArea, SWT.H_SCROLL
				| SWT.V_SCROLL);
		...
		scroller.addControlListener(new ControlAdapter() {
			public void controlResized(ControlEvent e) {
				scroller.setMinSize(deviceView.computeSize(SWT.DEFAULT,
						SWT.DEFAULT));
			}
		});
	...
	private void showVlanMap() {
		...
		for (final Object o : selection.toList()) {
			...
			try {
				page.showView(
						"org.netxms.ui.eclipse.networkmaps.views.VlanMap", Long.toString(nodeId) + "&" + Integer.toString(vlan.getVlanId()), IWorkbenchPage.VIEW_ACTIVATE); //$NON-NLS-1$ //$NON-NLS-2$
			} catch (PartInitException e) {
				MessageDialogHelper.openError(getSite().getShell(), Messages
						.get().VlanView_Error, String.format(
						Messages.get().VlanView_OpenMapError, vlan.getVlanId(),
	...
	private void refreshVlanList() {
		new ConsoleJob(Messages.get().VlanView_JobTitle, this,
