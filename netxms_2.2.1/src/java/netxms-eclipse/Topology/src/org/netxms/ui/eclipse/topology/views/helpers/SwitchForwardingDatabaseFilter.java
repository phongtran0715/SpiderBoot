...
public class SwitchForwardingDatabaseFilter extends ViewerFilter {
	...
	private boolean containsType(FdbEntry en) {
		switch (en.getType()) {
		case (3):
			if (Messages.get().FDBLabelProvider_Dynamic.toLowerCase().contains(
					filterString))
				return true;
			break;
		case (5):
			if (Messages.get().FDBLabelProvider_Static.toLowerCase().contains(
					filterString))
				return true;
			break;
		default:
			if (Messages.get().FDBLabelProvider_Unknown.toLowerCase().contains(
					filterString))
				return true;
			return false;
		}
	...
	private boolean containsNode(FdbEntry en) {
		...
		if (session.getObjectName(en.getNodeId()).toLowerCase()
	...
	private boolean containsVlan(FdbEntry en) {
		if (Integer.toString(en.getVlanId()).toLowerCase()
				.contains(filterString))
			return true;
		return false;
	}
