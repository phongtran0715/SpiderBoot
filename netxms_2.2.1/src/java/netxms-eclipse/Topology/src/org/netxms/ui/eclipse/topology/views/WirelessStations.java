...
public class WirelessStations extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		setPartName(String.format(Messages.get().WirelessStations_PartName,
	...
	public void createPartControl(Composite parent) {
		final String[] names = { Messages.get().WirelessStations_ColMacAddr,
				Messages.get().WirelessStations_ColIpAddr,
				Messages.get().WirelessStations_ColNode,
				Messages.get().WirelessStations_ColAp,
				Messages.get().WirelessStations_ColRadio,
				Messages.get().WirelessStations_ColSSID };
		...
		WidgetHelper.restoreTableViewerSettings(viewer, Activator.getDefault()
				.getDialogSettings(), "WirelessStations"); //$NON-NLS-1$
		viewer.getTable().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveTableViewerSettings(viewer, Activator
						.getDefault().getDialogSettings(), "WirelessStations"); //$NON-NLS-1$
			}
		});
	...
	private void refresh() {
		new ConsoleJob(Messages.get().WirelessStations_JobTitle, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<WirelessStation> stations = session
						.getWirelessStations(rootObject);
