 ...
 */
public class VlanLabelProvider extends LabelProvider implements
		ITableLabelProvider, IColorProvider {
	private static final Color HIGHLIGHT = new Color(Display.getDefault(), 255,
			216, 0);
	...
	private String buildPortList(VlanInfo vlan) {
		...
		while (it.hasNext()) {
			...
			Interface iface = (Interface) session.findObjectById(
					p.getObjectId(), Interface.class);
			if (iface != null) {
				if (!iface.isPhysicalPort()) {
					Interface parent = iface.getParentInterface();
					if ((parent != null) && parent.isPhysicalPort()) {
						physicalPorts.add(new Port(parent.getObjectId(), parent
								.getIfIndex(), parent.getSlot(), parent
								.getPort()));
					} else {
						if (sb.length() > 0)
							sb.append(',');
		...
		Collections.sort(physicalPorts, new Comparator<Port>() {
			...
			public int compare(Port p1, Port p2) {
				return (p1.getSlot() == p2.getSlot()) ? p1.getPort()
						- p2.getPort() : p1.getSlot() - p2.getSlot();
			}
		});
		...
		for (i = 1; i < physicalPorts.size(); i++) {
			if ((physicalPorts.get(i).getSlot() == slot)
	...
	public Color getBackground(Object element) {
		return (selectedPort != null)
				&& ((VlanInfo) element).containsPort(selectedPort.getSlot(),
						selectedPort.getPort()) ? HIGHLIGHT : null;
	}
}
