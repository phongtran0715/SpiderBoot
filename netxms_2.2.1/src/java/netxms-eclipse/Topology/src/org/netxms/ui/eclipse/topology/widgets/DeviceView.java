...
public class DeviceView extends DashboardComposite {
	...
	public void refresh() {
		...
		for (Interface iface : interfaces) {
			...
			if (sv == null) {
				sv = new SlotView(this, SWT.NONE, String.format(
						Messages.get().DeviceView_SlotName, slot),
						((Node) object).getPortRowCount(),
						((Node) object).getPortNumberingScheme());
	...
	public void setHighlight(Port[] ports) {
		...
		for (Port p : ports) {
			...
			if ((p.getSlot() == 0) && (p.getPort() == 0)) {
				Interface iface = session.findObjectById(p.getObjectId(),
						Interface.class);
				if (iface == null)
					continue;
				if (!iface.isPhysicalPort()) {
					...
					if ((parent == null) || !parent.isPhysicalPort())
						continue;
					p = new Port(parent.getObjectId(), parent.getIfIndex(),
							parent.getSlot(), parent.getPort());
				}
			}
	 ...
	 * 
	 * @param doRedraw
	 *            if true, control will be redrawn
	 */
