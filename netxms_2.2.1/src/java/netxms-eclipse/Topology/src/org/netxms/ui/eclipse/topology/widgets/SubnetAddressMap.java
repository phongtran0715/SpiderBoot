 ...
 * Subnet address map
 */
public class SubnetAddressMap extends Canvas implements PaintListener,
		MouseTrackListener {
	...
	public void setSubnet(Subnet subnet) {
		...
		this.subnet = subnet;
		if ((subnet != null)
				&& (subnet.getSubnetAddress() instanceof Inet4Address)) {
			rowCount = ((1 << (32 - subnet.getSubnetMask())) + ELEMENTS_PER_ROW - 1)
					/ ELEMENTS_PER_ROW;
	...
	public void refresh() {
		...
		new ConsoleJob(Messages.get().SubnetAddressMap_JobTitle, viewPart,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				try {
					final long[] map = session.getSubnetAddressMap(subnet
							.getObjectId());
					runInUIThread(new Runnable() {
						@Override
						public void run() {
							addressMap = map;
							rowCount = (map.length + ELEMENTS_PER_ROW - 1)
									/ ELEMENTS_PER_ROW;
							if (rowCount > MAX_ROWS)
								rowCount = MAX_ROWS;
							redraw();
	...
	private int getElementFromPoint(int x, int y) {
		...
		if (x > xOffset + (column * (ELEMENT_WIDTH + HORIZONTAL_SPACING))
				+ ELEMENT_WIDTH)
		...
		if (y > VERTICAL_MARGIN + (row * (ELEMENT_HEIGHT + VERTICAL_SPACING))
				+ ELEMENT_HEIGHT)
	...
	private void drawAddress(byte[] address, long nodeId, int x, int y, GC gc) {
		final String label = (subnet.getSubnetMask() < 24) ? ("." + ((int) address[address.length - 2] & 0xFF) + "." + ((int) address[address.length - 1] & 0xFF)) : //$NON-NLS-1$ //$NON-NLS-2$
		...
		gc.drawText(label, x + (ELEMENT_WIDTH - ext.x) / 2, y
				+ (ELEMENT_HEIGHT - ext.y) / 2);
	}
	...
	public Point computeSize(int wHint, int hHint, boolean changed) {
		return new Point(ELEMENTS_PER_ROW * ELEMENT_WIDTH
				+ (ELEMENTS_PER_ROW - 1) * HORIZONTAL_SPACING
				+ HORIZONTAL_MARGIN * 2, rowCount * ELEMENT_HEIGHT
				+ (rowCount - 1) * VERTICAL_SPACING + VERTICAL_MARGIN * 2);
	}
		...
		if (index == -1) {
			...
			if (addressMap[index] == 0) {
				...
				AbstractNode node = (AbstractNode) session.findObjectById(
						addressMap[index], AbstractNode.class);
				if (node != null) {
