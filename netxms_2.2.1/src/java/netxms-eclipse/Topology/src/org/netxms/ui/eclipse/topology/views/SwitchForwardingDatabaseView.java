...
public class SwitchForwardingDatabaseView extends ViewPart {
	...
	public void init(IViewSite site) throws PartInitException {
		...
		setPartName(String.format(
				Messages.get().SwitchForwardingDatabaseView_Title,
				session.getObjectName(rootObject)));
		IDialogSettings settings = Activator.getDefault().getDialogSettings();
		initShowFilter = safeCast(
				settings.get("SwitchForwardingDatabase.showFilter"),
				settings.getBoolean("SwitchForwardingDatabase.showFilter"),
				initShowFilter);
	}
	...
	public void createPartControl(Composite parent) {
		...
		viewer = new SortableTableViewer(resultArea, names, widths,
				COLUMN_MAC_ADDRESS, SWT.DOWN, SWT.FULL_SELECTION | SWT.MULTI);
		...
		WidgetHelper.restoreTableViewerSettings(viewer, Activator.getDefault()
				.getDialogSettings(), "SwitchForwardingDatabase"); //$NON-NLS-1$
		viewer.getTable().addDisposeListener(new DisposeListener() {
			@Override
			public void widgetDisposed(DisposeEvent e) {
				WidgetHelper.saveTableViewerSettings(viewer, Activator
						.getDefault().getDialogSettings(),
						"SwitchForwardingDatabase"); //$NON-NLS-1$
			}
		});
	...
	private void activateContext() {
		...
		if (contextService != null) {
			contextService
					.activateContext("org.netxms.ui.eclipse.topology.context.Topology"); //$NON-NLS-1$
		}
	}
	...
	private void createActions() {
		...
		actionShowFilter.setChecked(initShowFilter);
		actionShowFilter
				.setActionDefinitionId("org.netxms.ui.eclipse.topology.commands.show_filter"); //$NON-NLS-1$
	...
	private void refresh() {
		new ConsoleJob(Messages.get().SwitchForwardingDatabaseView_JobTitle,
				this, Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final List<FdbEntry> fdb = session
						.getSwitchForwardingDatabase(rootObject);
			...
			protected String getErrorMessage() {
				return String.format(
						Messages.get().SwitchForwardingDatabaseView_JobError,
						session.getObjectName(rootObject));
			}
		}.start();
	}
