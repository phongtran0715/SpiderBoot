...
public class FindConnectionPoint implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		...
		new ConsoleJob(Messages.get().FindConnectionPoint_JobTitle, wbPart,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
					throws Exception {
				final ConnectionPoint[] cps = new ConnectionPoint[objects
						.size()];
				for (int i = 0; i < objects.size(); i++) {
					...
					if (cps[i] == null) {
						if (object instanceof Node)
							cps[i] = new ConnectionPoint(object.getObjectId(),
									0, false);
						else
							cps[i] = new ConnectionPoint(
									object.getParentIdList()[0],
									object.getObjectId(), false);
					}
				}
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if ((selection instanceof IStructuredSelection) && !selection.isEmpty()) {
			...
			for (Object s : ((IStructuredSelection) selection).toList()) {
				if ((s instanceof Node) || (s instanceof Interface)
						|| (s instanceof AccessPoint)) {
