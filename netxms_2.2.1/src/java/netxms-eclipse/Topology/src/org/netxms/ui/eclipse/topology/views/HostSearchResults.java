...
public class HostSearchResults extends ViewPart {
	...
	public void createPartControl(Composite parent) {
		final String[] names = { Messages.get().HostSearchResults_ColSeq,
				Messages.get().HostSearchResults_ColNode,
				Messages.get().HostSearchResults_ColIface,
				Messages.get().HostSearchResults_ColMac,
				Messages.get().HostSearchResults_ColIp,
				Messages.get().HostSearchResults_ColSwitch,
				Messages.get().HostSearchResults_ColPort,
				Messages.get().HostSearchResults_ColType };
		final int[] widths = { 70, 120, 120, 90, 90, 120, 120, 60 };
		viewer = new SortableTableViewer(parent, names, widths,
				COLUMN_SEQUENCE, SWT.UP, SWT.MULTI | SWT.FULL_SELECTION);
	...
	private void createActions() {
		...
		actionCopyRecord = new Action(Messages.get().HostSearchResults_Copy,
				SharedIcons.COPY) {
			@Override
			public void run() {
				copyToClipboard(-1);
	...
	public static void showConnection(ConnectionPoint cp) {
		...
		if (cp == null) {
			MessageDialogHelper.openWarning(shell,
					Messages.get().HostSearchResults_Warning,
		...
		try {
			...
			if ((bridge != null) && (iface != null)) {
				if (cp.getType() == ConnectionPointType.WIRELESS) {
					if (host != null) {
						MessageDialogHelper
								.openInformation(
										shell,
										Messages.get().HostSearchResults_ConnectionPoint,
										String.format(
												Messages.get().HostSearchResults_NodeConnectedToWiFi,
												host.getObjectName(),
												bridge.getObjectName(),
												iface.getObjectName()));
					} else {
						if (cp.getLocalIpAddress() != null) {
							MessageDialogHelper
									.openInformation(
											...
											Messages.get().HostSearchResults_ConnectionPoint,
											String.format(
													Messages.get().HostSearchResults_NodeIpMacConnectedToWiFi,
													cp.getLocalIpAddress()
															.getHostAddress(),
							...
							MessageDialogHelper
									.openInformation(
											...
											Messages.get().HostSearchResults_ConnectionPoint,
											String.format(
													Messages.get().HostSearchResults_NodeMacConnectedToWiFi,
													cp.getLocalMacAddress(),
													bridge.getObjectName(),
													iface.getObjectName()));
						}
					...
					if (host != null) {
						MessageDialogHelper
								.openInformation(
										...
										Messages.get().HostSearchResults_ConnectionPoint,
										String.format(
												Messages.get().HostSearchResults_NodeConnected,
												host.getObjectName(),
												cp.getType() == ConnectionPointType.DIRECT ? Messages
														.get().HostSearchResults_ModeDirectly
														: Messages.get().HostSearchResults_ModeIndirectly,
												bridge.getObjectName(), iface
														.getObjectName()));
					} else {
						if (cp.getLocalIpAddress() != null) {
							MessageDialogHelper
									.openInformation(
											...
											Messages.get().HostSearchResults_ConnectionPoint,
											String.format(
													Messages.get().HostSearchResults_NodeIpMacConnected,
													cp.getLocalIpAddress()
															.getHostAddress(),
													cp.getLocalMacAddress(),
													cp.getType() == ConnectionPointType.DIRECT ? Messages
							...
							MessageDialogHelper
									.openInformation(
											...
											Messages.get().HostSearchResults_ConnectionPoint,
											String.format(
													Messages.get().HostSearchResults_NodeMacConnected,
													cp.getLocalMacAddress(),
													cp.getType() == ConnectionPointType.DIRECT ? Messages
															.get().HostSearchResults_ModeDirectly
															: Messages.get().HostSearchResults_ModeIndirectly,
				...
				((HostSearchResults) part).addResult(cp);
					...
					&& (cp.getType() == ConnectionPointType.UNKNOWN)) {
				MessageDialogHelper
						.openWarning(
								shell,
								Messages.get().HostSearchResults_Warning,
								String.format(
										"Found node %s but it's connection point is unknown",
										host.getObjectName()));
			...
			MessageDialogHelper.openWarning(
					shell,
					Messages.get().HostSearchResults_Warning,
					String.format(Messages.get().HostSearchResults_ShowError,
	...
	public static void showConnection(ConnectionPoint[] cps) {
		...
		try {
			...
			if (counter > 0) {
				MessageDialogHelper.openWarning(shell,
						Messages.get().HostSearchResults_Warning,
						Messages.get().HostSearchResults_NotFound + " for "
								+ counter + " interfaces!");
			}
