...
public class ShowWirelessStations implements IObjectActionDelegate {
	...
	public void run(IAction action) {
		try {
			window.getActivePage().showView(WirelessStations.ID,
					Long.toString(objectId), IWorkbenchPage.VIEW_ACTIVATE);
		} catch (PartInitException e) {
			MessageDialogHelper.openError(
					window.getShell(),
					Messages.get().ShowWirelessStations_Error,
					Messages.get().ShowWirelessStations_CannotOpenView
	...
	public void selectionChanged(IAction action, ISelection selection) {
		if ((selection instanceof IStructuredSelection)
			...
			if ((obj instanceof AbstractNode)
					&& ((AbstractNode) obj).isWirelessController()) {
