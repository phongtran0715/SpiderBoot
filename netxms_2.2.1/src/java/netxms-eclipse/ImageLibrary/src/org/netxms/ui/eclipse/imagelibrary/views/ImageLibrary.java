...
public class ImageLibrary extends ViewPart implements ImageUpdateListener {
	...
	private void createActions() {
		actionNew = new Action(Messages.get().ImageLibrary_ActionUpload) {
			@Override
			public void run() {
				final ImagePropertiesDialog dialog = new ImagePropertiesDialog(
						getSite().getShell(), knownCategories, imageLibrary,
						false);
				...
				if (dialog.open() == Dialog.OK) {
					uploadNewImage(dialog.getName(), dialog.getCategory(),
							dialog.getFileName());
				}
			}
		};
		...
		actionEdit = new Action(Messages.get().ImageLibrary_ActionEdit) {
			...
			public void run() {
				...
				if (selection.length == 1) {
					final ImagePropertiesDialog dialog = new ImagePropertiesDialog(
							getSite().getShell(), knownCategories,
							imageLibrary, true);
					LibraryImage image = (LibraryImage) selection[0].getData();
					dialog.setName(image.getName());
					dialog.setDefaultCategory(image.getCategory());
					if (dialog.open() == Dialog.OK) {
						editImage(selection[0], dialog.getName(),
								dialog.getCategory(), dialog.getFileName());
					}
				}
			}
		};
		...
		actionRefresh = new RefreshAction(this) {
			...
			public void run() {
				try {
					...
					Activator
							.logError(
									"ImageLibrary view: Exception in refresh action", e); //$NON-NLS-1$
				}
			}
		};
		...
		actionZoomIn = new Action(Messages.get().ImageLibrary_ActionZoomIn) {
			@Override
			public void run() {
				final DefaultGalleryGroupRenderer groupRenderer = (DefaultGalleryGroupRenderer) gallery
						.getGroupRenderer();
	 ...
	 * @param fileName
	 */
	protected void editImage(final GalleryItem galleryItem, final String name,
			final String category, final String fileName) {
		final LibraryImage image = (LibraryImage) galleryItem.getData();

		new ConsoleJob(Messages.get().ImageLibrary_UpdateJob, this,
			...
			protected void runInternal(final IProgressMonitor monitor)
				...
				session.modifyImage(image, new ProgressListener() {
					...
					public void setTotalWorkAmount(long workTotal) {
						monitor.beginTask(
								Messages.get().ImageLibrary_UpdateImage,
								(int) workTotal);
					}
	 ...
	 * @param fileName
	 */
	protected void uploadNewImage(final String name, final String category,
			final String fileName) {
		new ConsoleJob(Messages.get().ImageLibrary_UploadJob, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(final IProgressMonitor monitor)
					throws Exception {
				...
				session.createImage(image, new ProgressListener() {
					...
					public void setTotalWorkAmount(long workTotal) {
						monitor.beginTask(
								Messages.get().ImageLibrary_UploadImage,
								(int) workTotal);
					}
	...
	private void refreshImages() throws NXCException, IOException {
		new ConsoleJob(Messages.get().ImageLibrary_ReloadJob, this,
				Activator.PLUGIN_ID, null) {
			@Override
			protected void runInternal(IProgressMonitor monitor)
				...
				for (int i = 0; i < imageLibrary.size(); i++) {
					...
					if (!image.isComplete()) {
						try {
							LibraryImage completeImage = session.getImage(image
									.getGuid());
							imageLibrary.set(i, completeImage);
						} catch (Exception e) {
							Activator
									.logError(
											"Exception in ImageLibrary.refreshImages()", e); //$NON-NLS-1$
						}
					}
				}
	...
	private void refreshUI(List<LibraryImage> imageLibrary) {
		...
		for (String category : categories.keySet()) {
			...
			for (final LibraryImage image : categoryImages) {
				final GalleryItem imageItem = new GalleryItem(categoryItem,
						SWT.NONE);
				...
				if (binaryData != null) {
					final ByteArrayInputStream stream = new ByteArrayInputStream(
							binaryData);
					try {
						imageItem.setImage(new Image(getSite().getShell()
								.getDisplay(), stream));
					} catch (SWTException e) {
						Activator.logError(
								"Exception in ImageLibrary.refreshUI()", e); //$NON-NLS-1$
						imageItem.setImage(ImageProvider.getInstance()
								.getImage(null)); // show as "missing"
					}
				} else {
					imageItem.setImage(ImageProvider.getInstance().getImage(
							null)); // show as "missing"
				}
