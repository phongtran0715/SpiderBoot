...
public class ImageProvider {
	...
	private static final Map<UUID, Image> cache = Collections
			.synchronizedMap(new HashMap<UUID, Image>());
	private static final Map<UUID, LibraryImage> libraryIndex = Collections
			.synchronizedMap(new HashMap<UUID, LibraryImage>());
	...
	public Image getImage(final UUID guid) {
		...
		if (cache.containsKey(guid)) {
			...
			if (libraryIndex.containsKey(guid)) {
				new ConsoleJob(Messages.get().ImageProvider_JobName, null,
	...
	private void loadImageFromServer(final UUID guid) {
		...
		try {
			...
			// libraryIndex.put(guid, libraryImage); // replace existing
			// half-loaded object w/o image data
			final ByteArrayInputStream stream = new ByteArrayInputStream(
					libraryImage.getBinaryData());
			try {
