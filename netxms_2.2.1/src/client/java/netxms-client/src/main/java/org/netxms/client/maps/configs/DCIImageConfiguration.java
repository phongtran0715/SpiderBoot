...
public class DCIImageConfiguration {
	 ...
	 */
	public static DCIImageConfiguration createFromXml(final String xml)
			throws Exception {
	...
	/**
	 * Checks is any rule applicable on last value All except line and not like
	 * are compared with help of T.compateTo(T). Like and not like values can be
	 * given with regular expressions so they are always compared as a strings
	 * and with help of Glob.matchIgnoreCase(pattern, string).
	 * 
	...
	public UUID getCorrectImage(DciValue dciValue) {
		...
		if (containRuleList() && dciValue != null) {
			for (int i = 0; i < dciRuleList.length; i++) {
				...
				case OP_LE:
					if (compareValues(dciValue.getValue(),
							dciRuleList[i].getCompareValue(),
							dciValue.getDataType()) < 0)
				...
				case OP_NE:
					...
					break;
				case OP_LIKE:
					if (Glob.matchIgnoreCase(dciRuleList[i].getCompareValue(),
							dciValue.getValue()))
						image = dciRuleList[i].getImage();
					break;
				case OP_NOTLIKE:
	 ...
	 *            new DCI value
	 * @param value2
	 *            value to witch new value is compared
	 * @param dataType
	 *            type to witch both values will be converted
