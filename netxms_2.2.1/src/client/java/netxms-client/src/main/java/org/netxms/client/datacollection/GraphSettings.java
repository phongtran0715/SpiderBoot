...
public class GraphSettings extends ChartConfig implements ObjectAction {
	 ...
	 * Create settings
	 */
	public GraphSettings(long id, long ownerId, int flags,
	...
	public GraphSettings(GraphSettings data, String name) {
		...
		this.accessList = new ArrayList<AccessListElement>(
				data.accessList.size());
	 ...
	 */
	public static GraphSettings createFromXml(final String xml)
			throws Exception {
	 ...
	 */
	static public GraphSettings createGraphSettings(final NXCPMessage msg,
			long baseId) {
		GraphSettings gs;
		try {
			...
			Logger.debug("GraphSettings.CreateGraphSettings",
					"Cannot parse ChartConfig XML", e);
		}
		...
		try {
			gs.filter = ObjectMenuFilter.createFromXml(msg
					.getFieldAsString(baseId + 5));
		} catch (Exception e) {
			Logger.debug("GraphSettings.CreateGraphSettings",
					"Cannot parse ObjectMenuFilter XML: ", e);
	...
	public void fillMessage(NXCPMessage msg) {
		...
		try {
			...
			Logger.debug("GraphSettings.CreateGraphSettings",
					"Cannot convert ChartConfig to XML: ", e);
	 ...
	 * 
	 * @see
	 * org.netxms.client.objecttools.ObjectAction#isApplicableForNode(org.netxms
	 * .client.objects.AbstractNode)
	 */
	@Override
	 ...
	 * 
	 * @see
	 * org.netxms.client.objecttools.ObjectAction#setMenuFilter(org.netxms.client
	 * .ObjectMenuFilter)
	 */
	@Override
