...
public class Interface extends GenericObject implements ZoneMember {
	...
	private static final String[] stateText = { "UNKNOWN", "UP", "DOWN",
			"TESTING", "DORMANT", "NOT PRESENT" };
	private static final String[] paeStateText = { "UNKNOWN", "INITIALIZE",
			"DISCONNECTED", "CONNECTING", "AUTHENTICATING", "AUTHENTICATED",
			"ABORTING", "HELD", "FORCE AUTH", "FORCE UNAUTH", "RESTART" };
	private static final String[] backendStateText = { "UNKNOWN", "REQUEST",
	...
	public Interface(NXCPMessage msg, NXCSession session) {
		...
		peerDiscoveryProtocol = LinkLayerDiscoveryProtocol.getByValue(msg
				.getFieldAsInt32(NXCPCodes.VID_PEER_PROTOCOL));
		...
		dot1xBackendState = msg
				.getFieldAsInt32(NXCPCodes.VID_DOT1X_BACKEND_STATE);
		ifTableSuffix = new SnmpObjectId(
				msg.getFieldAsUInt32Array(NXCPCodes.VID_IFTABLE_SUFFIX));
	 ...
	 * 
	 * @return true if this interface is excluded from network topology
	 *         calculation
	 */
	...
	public InetAddressEx getFirstUnicastAddressEx() {
		for (InetAddressEx a : ipAddressList)
			if (!a.getAddress().isAnyLocalAddress()
					&& !a.getAddress().isLinkLocalAddress()
					&& !a.getAddress().isLoopbackAddress()
					&& !a.getAddress().isMulticastAddress())
				return a;
		return null;
	}
