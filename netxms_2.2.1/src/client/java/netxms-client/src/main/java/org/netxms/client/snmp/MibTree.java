...
public class MibTree {
	private static final byte[] MIB_FILE_MAGIC_NUMBER = { 0x4E, 0x58, 0x4D,
			0x49, 0x42, 0x20 };
	 ...
	 *             if I/O error occurs
	 * @throws NXCException
	 *             if MIB file format errors detected
	 */
	public MibTree(File file) throws IOException, NXCException {
		NXCPDataInputStream in = new NXCPDataInputStream(
		...
		try {
			...
			if ((flags & SMT_COMPRESS_DATA) != 0) {
				...
				in = new NXCPDataInputStream(new InflaterInputStream(
						bufferedInput));
			} else {
