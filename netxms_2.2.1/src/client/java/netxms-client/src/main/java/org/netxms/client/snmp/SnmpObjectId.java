...
public class SnmpObjectId {
	...
	public SnmpObjectId(long[] value) {
		this.value = (value != null) ? Arrays.copyOf(value, value.length)
				: new long[0];
	}
	 ...
	 */
	public static SnmpObjectId parseSnmpObjectId(String s)
			throws SnmpObjectIdFormatException {
		String st = s.trim();
		if (st.isEmpty())
		...
		if (st.charAt(0) != '.')
			throw new SnmpObjectIdFormatException(
					"OID shoud start with . character");
		...
		for (int i = 1; i < parts.length; i++) {
			...
			try {
				...
				throw new SnmpObjectIdFormatException("OID element #" + i
	...
	/**
	 * Compare two object identifiers. The return value is -1 if this object
	 * identifier is less than given object identifier, 0 if they are equal, and
	 * 1 if this object identifier is greater then given object identifier.
	 * 
	 * @param oid
	 ...
	 * @param pos
	 *            position
	 * @return part of object id from given position or -1 if position is
	 *         invalid
	 */
