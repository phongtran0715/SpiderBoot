...
public class ConfigContent {
	...
	public ConfigContent(long id, NXCPMessage response) {
		this.id = id;
		name = response.getFieldAsString(NXCPCodes.VID_NAME) == null ? ""
				: response.getFieldAsString(NXCPCodes.VID_NAME);
		config = response.getFieldAsString(NXCPCodes.VID_CONFIG_FILE) == null ? ""
				: response.getFieldAsString(NXCPCodes.VID_CONFIG_FILE);
		filter = response.getFieldAsString(NXCPCodes.VID_FILTER) == null ? ""
				: response.getFieldAsString(NXCPCodes.VID_FILTER);
		sequenceNumber = response
				.getFieldAsInt64(NXCPCodes.VID_SEQUENCE_NUMBER);
	}
	...
	public void fillMessage(NXCPMessage request) {
		...
		request.setFieldInt32(NXCPCodes.VID_SEQUENCE_NUMBER,
				(int) sequenceNumber);
	}
