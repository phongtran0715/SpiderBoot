...
public final class ServerVariable {
	 ...
	 * @param value
	 *            Variable's value
	 * @param isServerRestartNeeded
	 *            Server restart flag (server has to be restarted after variable
	 *            change if this flag is set)
	 * @param dataType
	 *            Data type for server configuration variable
	 * @param description
	 *            Description
	 * 
	...
	public ServerVariable(String name, String value,
			boolean isServerRestartNeeded, ServerVariableDataType dataType,
			String description) {
		this.name = name;
		this.value = value;
	...
	public ServerVariable(NXCPMessage msg, long baseId) {
		...
		dataType = ServerVariableDataType.getByCode(((code != null) && !code
				.isEmpty()) ? code.charAt(0) : 'S');
	...
	public void addPossibleValue(NXCPMessage msg, long baseId) {
		values.put(msg.getFieldAsString(baseId),
				msg.getFieldAsString(baseId + 1));
	}
