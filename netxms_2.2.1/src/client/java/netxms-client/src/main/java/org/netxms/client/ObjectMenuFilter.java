...
public class ObjectMenuFilter {
	 ...
	 */
	public static ObjectMenuFilter createFromXml(final String xml)
			throws Exception {
	...
	public String createXml() {
		try {
			...
			Logger.warning("ObjectMenuFilter",
					"Exception during object menu filter serialization", e);
			return "";
		}
	}
	...
	public boolean isApplicableForNode(AbstractNode node) {
		...
		if ((flags & REQUIRES_WORKSTATION_OS_MATCH) != 0) {
			...
			for (int i = 0; i < substrings.length; i++) {
				if (Pattern.matches(substrings[i],
		...
		if ((flags & REQUIRES_TEMPLATE_MATCH) != 0) {
			...
			Set<AbstractObject> parents = node
					.getAllParents(AbstractObject.OBJECT_TEMPLATE);
