...
public class DataFormatter {
	 ...
	 *            true if multipliers should be used
	 */
	public DataFormatter(String formatString, int dataType,
			boolean useBinaryMultipliers) {
		...
		for (int i = 0; i < format.length; i++) {
			if (format[i] == '%') {
				...
				if (format[i] == '%') {
					...
					int j;
					for (j = i; (j < format.length)
							&& !Character.isLetter(format[j]); j++)
						;
					...
					final String f = "%"
							+ new String(Arrays.copyOfRange(format, i, j + 1));
					i = j;

					try {
						Value v = getValueForFormat(value, useMultipliers,
								format[j] == 's');
	...
	private static final long[] DECIMAL_MULTIPLIERS = { 1L, 1000L, 1000000L,
			1000000000L, 1000000000000L, 1000000000000000L };
	private static final long[] BINARY_MULTIPLIERS = { 0x400L, 0x100000L,
			0x40000000L, 0x10000000000L, 0x1000000000000000L };
	private static final String[] SUFFIX = { "", " k", " M", " G", " T", " P" };
	private static final String[] FORMATS = { "", ".0", ".00", ".000", ".0000",
			".00000", ".000000" };
	 ...
	 * @return
	 */
	private Value getValueForFormat(String value, boolean useMultipliers,
		...
		try {
			if (useMultipliers) {
				final long[] multipliers = useBinaryMultipliers ? BINARY_MULTIPLIERS
						: DECIMAL_MULTIPLIERS;
	 ...
	 */
	public static String roundDecimalValue(double value, double step,
			int maxPrecision) {
		if (value == 0)
			return "0";
		...
		int precision;
		if (step < 1 || i < 0)
			precision = (calculatePrecision(step) > maxPrecision) ? maxPrecision
					: calculatePrecision(step);
		else
			precision = (calculatePrecision(step / multipliers[i]) > maxPrecision) ? maxPrecision
					: calculatePrecision(step / multipliers[i]);

		DecimalFormat df = new DecimalFormat();
		df.setMaximumFractionDigits(precision);
		return df.format((i < 0 ? value : (value / multipliers[i])))
				+ (i < 0 ? "" : SUFFIX[i]);
		/*
		 * return new DecimalFormat("0" + (precision > FORMATS.length ?
		 * FORMATS[FORMATS.length - 1] : FORMATS[precision])) .format((i < 0 ?
		 * value : (value / multipliers[i]))) + (i < 0 ? "" : SUFFIX[i]);
		 */
	}
