...
public class NetworkMapLink {
	 ...
	 * @param flags
	 */
	public NetworkMapLink(String name, int type, long element1, long element2,
			String connectorName1, String connectorName2,
			SingleDciConfig[] dciList, int flags) {
		config.setDciList(dciList);
		initData(name, type, element1, element2, connectorName1,
				connectorName2, flags);
	}
	 ...
	 */
	public NetworkMapLink(String name, int type, long element1, long element2,
			String connectorName1, String connectorName2, int flags) {
		initData(name, type, element1, element2, connectorName1,
				connectorName2, flags);
	}
	 ...
	 * @param flags
	 */
	public void initData(String name, int type, long element1, long element2,
			String connectorName1, String connectorName2, int flags) {
		this.name = name;
		this.type = type;
	...
	public NetworkMapLink(NXCPMessage msg, long baseId) {
		...
		try {
			config = ((xml != null) && !xml.isEmpty()) ? LinkConfig
					.createFromXml(xml) : new LinkConfig();
		} catch (Exception e) {
			Logger.warning("NetworkMapLink", "Cannot create data from XML ("
					+ xml + ")", e);
	...
	public void fillMessage(NXCPMessage msg, long baseId) {
		...
		try {
			...
			Logger.warning("NetworkMapLink", "Cannot create XML from config ("
					+ config.toString() + ")", e);
		}
	...
	/**
	 * Get label for display. If name is not null and not empty, label will have
	 * form name (connector1 - connector2) otherwise it will have form
	 * connector1 - connector2 If any of connector names is null or empty, it
	 * will be replaced with string "&lt;noname&gt;".
	 * 
	...
	public String getLabel() {
		...
		sb.append(((connectorName1 != null) && !connectorName1.isEmpty()) ? connectorName1
				: "<noname>");
		sb.append(" - ");
		sb.append(((connectorName2 != null) && !connectorName2.isEmpty()) ? connectorName2
				: "<noname>");
	...
	public String toString() {
		return "NetworkMapLink [name=" + name + ", type=" + type
				+ ", element1=" + element1 + ", element2=" + element2
				+ ", connectorName1=" + connectorName1 + ", connectorName2="
				+ connectorName2 + ", color=" + config.getColor()
				+ ", statusObject=" + config.getObjectStatusList()
				+ ", routing=" + config.getRouting() + ", flags=" + flags + "]";
	}
}
