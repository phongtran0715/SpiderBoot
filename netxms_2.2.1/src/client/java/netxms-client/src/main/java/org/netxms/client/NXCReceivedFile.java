...
final class NXCReceivedFile {
	 ...
	 *            true if data is part of compressed data stream
		...
		if (status == OPEN) {
			try {
				if (compressedStream) {
					if (data[0] != 2)
						throw new IOException(
								"Unsupported stream compression method "
										+ (int) data[0]);
					...
					if (decompressor == null) {
						...
						Logger.debug(
								getClass().getName(),
								"Decompressor created for file "
										+ file.getAbsolutePath());
					}
					decompressor.setInput(data, 4, data.length - 4, false);

					int dataLength = (((int) data[2] << 8) & 0xFF00)
							| ((int) data[3] & 0xFF);
				...
				Logger.error(getClass().getName(),
						"Exception during file processing", e);
				try {
					stream.close();
	...
	public IOException getException() {
		return ((exception != null) && (exception instanceof IOException)) ? (IOException) exception
