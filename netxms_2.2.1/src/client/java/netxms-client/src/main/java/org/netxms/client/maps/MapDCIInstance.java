...
/**
 * Class that stores data of map DCI that's last values should be requested from
 * server
 */
...
public class MapDCIInstance {
	 ...
	 *            id of required DCI
	 * @param nodeID
	 *            id of associated node. Is collected to fully fill DciValue
	 *            instances
	 * @param column
	 *            column if DCI is a table DCI
	 * @param instance
	 *            instance if DCI is a table DCI
	 * @param type
	 *            type of DCI
	 */
	public MapDCIInstance(long dciID, long nodeID, String column,
			String instance, int type, String mapId) {
		this.setDciID(dciID);
	...
	/**
	 * This method fills message with data if all required data present. In case
	 * of table DCI if column or/and instance are not present, then this DCI
	 * value will not be requested.
	 * 
	 * @param msg
	 *            Message that should be populated with data
	 * @param base
	...
	public int hashCode() {
		...
		result = prime
				* result
				+ (((column == null) || (type == DataCollectionItem.DCO_TYPE_TABLE)) ? 0
						: column.hashCode());
		result = prime * result + (int) (dciID ^ (dciID >>> 32));
		result = prime
				* result
				+ (((instance == null) || (type == DataCollectionItem.DCO_TYPE_TABLE)) ? 0
						: instance.hashCode());
