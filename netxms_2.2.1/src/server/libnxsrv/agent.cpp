...
*/
UINT32 AgentConnection::createMappingChannel(INT32 id, TCHAR *cHomeId, TCHAR *cMonitorId, INT32 timeSync,
      INT32 statusSync, TCHAR* lastSyncTime, TCHAR* downloadId, TCHAR* renderId, TCHAR* uploadId)
{
   UINT32 dwRqId, dwResult;
   NXCPMessage msg(m_nProtocolVersion);
   if (!m_isConnected)
   ...
   dwRqId = generateRequestId();
   msg.setCode(CMD_CREATE_MAPPING_CHANNEL);
   msg.setId(dwRqId);

   msg.setField(VID_MAPPING_CHANNEL_RECORD_ID, id);
   msg.setField(VID_MAPPING_CHANNEL_HOME_ID, cHomeId);
   msg.setField(VID_MAPPING_CHANNEL_MONITOR_ID, cMonitorId);
   msg.setField(VID_MAPPING_CHANNEL_TIME_SYNC, timeSync);
   msg.setField(VID_MAPPING_CHANNEL_STATUS_SYNC, statusSync);
   msg.setField(VID_MAPPING_CHANNEL_LAST_SYNC_TIME, lastSyncTime);
   msg.setField(VID_MAPPING_CHANNEL_DOWNLOAD_CLUSTER_ID, downloadId);
   msg.setField(VID_MAPPING_CHANNEL_RENDER_CLUSTER_ID, renderId);
   msg.setField(VID_MAPPING_CHANNEL_UPLOAD_CLUSTER_ID, uploadId);
   if (sendMessage(&msg))
   {
      dwResult = waitForRCC(dwRqId, m_dwCommandTimeout);
   }
...
* Create new mapping channel
*/
UINT32 AgentConnection::modifyMappingChannel(INT32 id, TCHAR *cHomeId, TCHAR *cMonitorId, INT32 timeSync,
      INT32 statusSync, TCHAR* lastSyncTime, TCHAR* downloadId, TCHAR* renderId, TCHAR* uploadId)
{
   UINT32 dwRqId, dwResult;
   NXCPMessage msg(m_nProtocolVersion);
   ...
   dwRqId = generateRequestId();
   msg.setCode(CMD_MOD_MAPPING_CHANNEL);
   msg.setId(dwRqId);
...
* Create new mapping channel
*/
UINT32 AgentConnection::deleteMappingChannel(INT32 id)
{
   UINT32 dwRqId, dwResult;
   NXCPMessage msg(m_nProtocolVersion);
   if (!m_isConnected)
   ...
   dwRqId = generateRequestId();
   msg.setCode(CMD_DEL_MAPPING_CHANNEL);
   msg.setId(dwRqId);
